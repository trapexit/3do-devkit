...
Source: [ARM Object Library Format (ALF-1993.txt)](https://www.chiark.greenend.org.uk/~theom/riscos/docs/CodeStds/ALF-1993.txt)

## Introduction

This document defines a file format called **ARM Object Library Format** (ALF),
...
description of the Chunk File Format, see "Chunk File Format" (page 19 in the
original).
...
- **byte**: 8 bits, considered unsigned unless otherwise stated, usually used
  to store flag bits or characters.
...
- **string**: a sequence of bytes terminated by a NUL (`0x00`) byte. The NUL
  byte is part of the string but is not counted in the string's length.
...
chunks), the alignment of words and half-words is defined by the use to which
ALF is being put. For all current ARM-based systems, alignment is strict, as
...
In all chunks, word values are stored with the same byte order as the target
system; strings are stored in ascending address order, which is independent of
target byte order.

### LIB_DIRY

The `LIB_DIRY` chunk contains a directory of the modules in the library, each
...
is determined by the size of the `LIB_DIRY` chunk.
...
header of the corresponding `LIB_DATA` chunk. Conventionally, the first three
chunks of an OFL file are `LIB_DIRY`, `LIB_TIME`, and `LIB_VSRN`, so
`ChunkIndex` is at least 3. A `ChunkIndex` of 0 means the directory entry is
unused.

The corresponding `LIB_DATA` chunk entry gives the offset and size of the
library module in the library file.
...
The format of the time stamp is described in [Time Stamps](#time-stamps). Its
value is an encoded version of the last-modified time of the file from which
the library member was created.
...
A library time stamp is a pair of words encoding:
...
same endian-ness as the containing chunk file.

### LIB_TIME

The `LIB_TIME` chunk contains a 2-word time stamp recording when the library
was last modified. It is therefore 8 bytes long.
...
`LIB_DIRY` chunk. The endian-ness or byte order of this data is, by assumption,
the same as the byte order of the containing library/chunk file.
...
external symbol, and between 1 and 4 bytes of NUL padding:

```
ChunkIndex

EntryLength               the size of this OFL_SYMT chunk (an integral number
                          of words)
...
has the same format as the `LIB_TIME` chunk (see [Time
Stamps](#time-stamps)).
