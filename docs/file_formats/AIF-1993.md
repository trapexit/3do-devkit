...
modified version of the self-move code illustrated in the
"Self-Move and Self-Relocation Code" section, allowing AIF to be easily
tailored to new environments. Using this facility is described in the
`-AIF -Relocatable` section of the User Manual.
...
moment by copying, then modifying, the third word of the AIF header (see
"AIF Header Layout").
...
### Debug Initialisation Instruction
...
the image doesn't need to be moved, the code branches to `RelocateOnly`.
