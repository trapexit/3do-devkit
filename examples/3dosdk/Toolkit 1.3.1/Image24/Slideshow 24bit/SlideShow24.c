...
LoadScreen(char *fileName, int32 whichBuffer)
{
	...
	if ( (loadfile24(imageFileName, gBackPic[whichBuffer],maxImageSize,
	            MEMTYPE_ANY,  &rawVDLPtr,&picHeader[whichBuffer])) < 0 ) 
	{
	...
	if ((oldVDL != defaultVDL[0]) && (oldVDL != defaultVDL[1])) DeleteVDL(oldVDL);
...
CreateScreens(void) 
{
	...
	gScreenCntx.sc_nFrameBufferPages = (kScreenWidth * kScreenHeight * 2
	...
	buffPtr = (ubyte *)ALLOCMEM((int)(2 * gScreenCntx.sc_nFrameBufferPages * GrafBase->gf_VRAMPageSize),
	...
	bitMapBuffs[1] = buffPtr + (gScreenCntx.sc_nFrameBufferPages * GrafBase->gf_VRAMPageSize);
	...
	for ( screenNdx = 0; screenNdx < kScreenCount; screenNdx++ ) 
	{
		...
		else {  defaultVDL[screenNdx]  = errorCode; }  // Save the System default VDL's
		}
