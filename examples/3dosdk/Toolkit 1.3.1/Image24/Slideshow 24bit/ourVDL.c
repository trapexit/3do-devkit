/*
	...
	Contains:	This file contains routines to mess around with video display lists.
	...
	Change History (most recent first):
		 ...
		 <4>	 8/11/93	JAY		remove 2 filler words from DMA control word 'length' field
									because of change in SubmitVDL's verification process. check DMA
									control word of vlds loaded from disk and adjust if length field
									includes filler words
		 <3>	 6/23/93	JAY		intergrated ec's changes for Dragon (4B1)
		 <2>	 3/18/93	JAY		commented out tempLong local variable. It is not used
		 <1>	 3/18/93	JAY		first checked in
				2/17/93		ec		Added VDL diddling routines.
				2/15/93		ec		Add CorrectVDL for Magneto6
				 2/5/93		ec		Change for VDL structure generated by PS export module
...
long ConvertPixel(int32 pixel)
{
	return (Convert8((pixel >> 7) & 0xF8) << 10) | (Convert8((pixel >> 2) & 0xF8) << 5) | Convert8((pixel << 3) & 0xF8);
}
...
InitVDL480(int32 *vdl, Bitmap *destBitmap, int32 bitmapindex)
{
	...
	vdl[4+32] = VDL_DISPCTRL
				| VDL_BACKGROUND|0;
	vdl[4+33] = VDL_DISPCTRL 
				| VDL_VINTEN|VDL_HSUB_ZERO
				| VDL_BLSB_BLUE
 ...
 */
int32
CreateVDL480(Item *vdlItemArray, ScreenContext *sc)
{
	int32	*vdlPtr,
			*vldPtr2;
	...
	vdlPtr = (int32 *)ALLOCMEM((3 * 40 * sizeof(int32)) * 2, 
	...
	vldPtr2 = vdlPtr + (3 * 40 * 4);

	// init the memory we just allocated with the screen specific info it needs
	InitVDL480(vdlPtr, sc->sc_Bitmaps[0], false);
	InitVDL480(vldPtr2, sc->sc_Bitmaps[1], true);
	
	// link the VDLs together
	vdlPtr[83] = (int32)(vldPtr2);
	vdlPtr[123] = (int32)GrafBase->gf_VDLPostDisplay;

	// submit the VDLs to the system for verification
	vdlItemArray[0] = SubmitVDL((VDLEntry *)vdlPtr, 3 * 40, VDLTYPE_FULL);
	if ( vdlItemArray[0] < 0 ) 
	{
		printf("Error in SubmitVDL  (%lx)\n", vdlItemArray[0]);
		PrintfSysErr(vdlItemArray[0]);
		return 0;
	}

	vdlItemArray[1] = SubmitVDL((VDLEntry *)vldPtr2, 3 * 40, VDLTYPE_FULL);
	if ( vdlItemArray[1] < 0 ) 
	{
		printf("Error in SubmitVDL  (%lx)\n", vdlItemArray[1]);
		PrintfSysErr(vdlItemArray[1]);
		return 0;
	}
	
	// done, don't need our local VDL anymore
	FREEMEM(vdlPtr, (3 * 40 * sizeof(int32)) * 2);
...
MergeVDL(VDLEntry *rawVDLPtr, VDLEntry *activeVDLPtr, uint32 vdllines)
{
	...
	for (scanLineEntry = 0; scanLineEntry < vdllines; scanLineEntry++)
	{
		...
		// if this is a sigle VDL color list we need to enable it
		// for all 240 lines  
		if(vdllines == 1) activeVDLRecPtr->controlword =
		...
		if (activeVDLRecPtr->displayControl == 0 )      //  This should not be! 
			...
			if (scanLineEntry == 0 )
				{
				activeVDLRecPtr->displayControl = SetDisplayCtlWord(true, true, false);
				printf(" Warning: Found and changed a NULL VDL displayControlWord to %x\n",activeVDLRecPtr->displayControl );
				}
			else 
				activeVDLRecPtr->displayControl = VDL_NULLVDL;
			}
		if (scanLineEntry != 0 ) activeVDLRecPtr->displayControl = VDL_NULLVDL;
