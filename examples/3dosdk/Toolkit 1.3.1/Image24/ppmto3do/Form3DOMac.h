/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <3>	 7/29/93	Ian 	added xxxx_CHUNKHDR_SIZE
   macros. <2>	  4/2/93	JAY 	replace old definition of RGB555 with
   something more useful in our ARMCC environment <2>	 1/28/93	dsm
   Fixed #define for CHUNK_CRDT <1>	 1/28/93	dsm 	first checked
   in
...
typedef struct PixelChunk
{
  ...
  Int32 chunk_size; /*	size in bytes of chunk including chunk_size */
  ubyte pixels[1];  /*	pixel data (format depends upon description in the
                       imagehdr */
} PixelChunk;
...
{
  UInt32
      controlword; //	VDL display control word (+ number of longwords in this
                   //entry - 4)
