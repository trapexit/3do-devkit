/**************************************************************************************
 *	Project:		DumpAIFF -	An MPW Tool to print formatted
 *output for SSND FORM COMM MARK INST and FVER chunks so it's easy to see what
 *an AIFF/AIFF-C file contains.
 *
 ...
 *						No longer return 0 on error...
 *return 1. Make sure all chunk sizes are rounded up in file position
 *calculations. Fix off by 4 problem when reading container chunk and
 *calculating bytesLeft. Check for unrecognized/unsupported chunkTypes. Check
 *for corrupt/invalid FORM size.  This will detect the invalid AIFC files that
 *an early version of SoundHack was writing. Change command line flag for input
 *file from -if to -i. 8/6/93		RDG		Version 1.1 Add "Ptr"
 *typedefs to match definitions in AIFF.h 7/23/93		RDG
 *Version 1.0 7/23/93		RDG		New today
 *
 *******************************************************************************************/
...
main (int argc, char *argv[])
{
  ...
  if (bytesRead != 12)
    {
      fprintf (stderr, "\nError: Couldn't read a chunk header from %s.\n",
               gInputFileName);
      fprintf (stderr, "       Possibly due to an invalid FORM size or "
                       "file/disk error.\n\n");
      goto error;
    }
  ...
  if ((fcp->formType == ID_AIFF) || (fcp->formType == ID_AIFC))
    {
      ...
      fflush (stdout);
      bytesLeft
          = fcp->ckSize - 4; /* the - 4 accounts for the FormType field */
    }
  else
    {
  ...
  /* Remember which form type this is so we can parse the rest of the chunks
   * properly */
  ...
  while (bytesLeft > 0)
    {
      ...
      /* Get a copy of the chunk size and round it up for file positioning
       * calculations */
      evenChunkSize = ch.ckSize;
      EVENUP (evenChunkSize);

      /* If the number of bytes left in the file (calculated from the FORM
       * size) is less than the size of the current chunk plus it's header,
       * than the FORM size or the chunk size is screwed.  This is not trapped
       * by default so this checks explicitly.
       */
      if (bytesLeft < (evenChunkSize + 8))
        {
          fprintf (stderr, "\nError: FORM size or CHUNK size corrupt! \n");
          fprintf (stderr,
                   "       Current chunk: %.4s chunk, Size (including header) "
                   "= %ld\n",
                   &ch.ckID, (evenChunkSize + 8));
          fprintf (stderr,
                   "       FORM size says only %ld bytes left in file!\n\n",
                   bytesLeft);
          goto error;
        }
      ...
      switch (ch.ckID)
        ...
        case ID_FVER:
          if (formType == ID_AIFF)
            {
              fprintf (stderr,
                       "\nError: %s is supposed to be an AIFF file but "
                       "contains an FVER chunk.\n\n",
                       gInputFileName);
              goto error;
            }
          ...
          if (bytesRead != ch.ckSize)
            {
              fprintf (stderr,
                       "\nError: Couldn't read the Format Version Chunk from "
                       "%s.\n\n",
                       gInputFileName);
              goto error;
            }
        ...
        default:
          ...
          if ((ch.ckID != ID_FORM) && (ch.ckID != ID_AIFF)
              && (ch.ckID != ID_AIFC) && (ch.ckID != ID_FVER)
              && (ch.ckID != ID_SSND) && (ch.ckID != ID_MARK)
              && (ch.ckID != ID_INST) && (ch.ckID != ID_COMM)
              && (ch.ckID != ID_MIDI) && (ch.ckID != ID_APPL)
              && (ch.ckID != ID_COMT) && (ch.ckID != ID_NAME)
              && (ch.ckID != ID_AUTH) && (ch.ckID != ID_COPY)
              && (ch.ckID != ID_ANNO))
            {
      ...
      bytesLeft
          = bytesLeft
            - (evenChunkSize + 8); /* the + 8 accounts for each chunk header */
