/*
	...
	Change History (most recent first):
				10/19/93	jb		Add PROGRAM_VERSION_STRING to usage
		 <1>	  7/20/93	BJB		first checked in
...
fprintf(stderr, "read %d\n", h->length);
	if (h->length > sizeof(buf)) {
		fprintf(stderr, "chunk '%c%c%c%c' (0x%08x) of %d is larger than %d bytes\n",
			(h->type >> 24) & 0xff,
			(h->type >> 16) & 0xff,
			(h->type >> 8) & 0xff,
			h->type & 0xff,
			h->type, h->length, sizeof(buf));
