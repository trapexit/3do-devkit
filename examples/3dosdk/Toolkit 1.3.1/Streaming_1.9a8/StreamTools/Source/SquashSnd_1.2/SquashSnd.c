 ...
 **						Spin cursor.
 **						Change -if and -of to -i and -o.  Change
 *-vb to -v. *	 6/18/93	RDG		version 1.1 *
 *Added call to gestalt manager to check for the existance of an *
 *FPU now that twotoone.c uses inline FPU calls. *	 6/17/93	RDG
 *version 1.0 *	 6/15/93	RDG		version .12
 **  6/15/93	RDG		fixed the compression name string in the comm
 *chunk.  I was *						writing a
 *Str255 instead of a C string.
 ...
 *new AIFC file *	 3/10/93	RDG		made a global of the
 *current commchunk so the compression
 **						  module can know about the ssnd chunk's
 *format *	 3/9/93		RDG		Modified OpenFiles to set the
 *Finder Creator and Type
 ...
 *aiff_tools *	 2/4/93		RDG		Converted to Think C for
 *development *	 2/1/93		RDG		New Today
 **
 **************************************************************************************/
...
static char *fpu[] = { "<none>", "mc68881", "mc68882",
                       "mc68040 built-in" }; /* for gestalt manager call */
...
main (long argc, char *argv[])
{
  ...
  if (!gestaltErr)
  ...
  else
    {
      ...
      printf ("If you don't have an FPU, SquashSnd is about to DIE a horrible "
              "death!\n");
    }
 ...
 *********************************************************************/
static long
OpenFiles (iff_control_ptr in_ptr, iff_control_ptr out_ptr)
{
  long Result = 0;
  long Creator = 'SqSd';
  long Type = 'AIFC';
  ...
  if (gVerbose)
    printf ("Creating %s...  ", gOutputFileName);
 ...
 *********************************************************************/
static void
CloseFiles (iff_control_ptr in_ptr, iff_control_ptr out_ptr)
{
  long Result = 0;
 ...
 *********************************************************************/
static long
VerifyProcessability (iff_control_ptr in_ptr)
{
  long Result = 0;
  uint32 FormType;
  ...
  if (FormType == ID_AIFC)
    {
      ...
      CHECKRESULT (Result, "_VerifyProcessability",
                   "CheckAIFCFormatVersion Failed!");
    }
 ...
 **
 **		On error: prints detailed error messages and returns result
 *code
 *********************************************************************/
static long
CheckFormChunk (iff_control_ptr in_ptr, uint32 *FormType)
{
  long Result = 0;
  ...
  if (Result == 2)
    CHECKRESULT (Result, "_CheckFormChunk",
                 "File damaged or not AIFF/AIFF-C!!!");
 ...
 *********************************************************************/
static long
CheckAIFCFormatVersion (iff_control_ptr in_ptr)
{
  long Result = 0;
  ...
  if (ChunkType != FormatVersionID)
    {
      CHECKRESULT (true, "_CheckAIFCFormatVersion",
                   "AIFF-C file has no Format Version Chunk!");
    }
  else
    {
      Result = iff_read_chunk_data (in_ptr, &myFVC.timestamp, ChunkSize);
      CHECKRESULT (Result, "_CheckSampleFormat",
                   "Couldn't read chunk data!!!");

      if (myFVC.timestamp != AIFCVersion1)
        CHECKRESULT (true, "_CheckAIFCFormatVersion",
                     "AIFF-C file version unsupported!");
    }

  Result = iff_rewind (in_ptr);
  CHECKRESULT (Result, "_CheckAIFCFormatVersion",
               "Couldn't rewind input file!!");
 ...
 *********************************************************************/
static long
CheckSampleFormat (iff_control_ptr in_ptr, uint32 FormType)
{
  long Result = 0;
  uint32 ChunkType, ChunkSize;
  ...
  if ((ChunkSize - 18) >= (sizeof (uint32) * 20)) /* string size < pad size */
    CHECKRESULT (true, "_CheckSampleFormat",
                 "Compression descriptor in Common Chunk too long!!!");
  ...
  if ((theCommChunk.numChannels < 1) || (theCommChunk.numChannels > 2))
    CHECKRESULT (true, "_CheckSampleFormat",
                 "Only Mono and Stereo samples currently supported!!!");

  if (theCommChunk.sampleSize != 16)
    CHECKRESULT (true, "_CheckSampleFormat",
                 "Only 16 bit samples currently supported!!!");

  if ((theCommChunk.compressionType != ID_NONE) && (FormType == ID_AIFC))
    CHECKRESULT (true, "_CheckSampleFormat",
                 "SSND Chunk already compressed!!!");
 ...
 *********************************************************************/
static long
BeginOutputForm (iff_control_ptr out_ptr)
{
  long Result = 0;
 ...
 *********************************************************************/
static long
CompleteOutputForm (iff_control_ptr out_ptr)
{
  long Result = 0;

  Result = iff_end_form (out_ptr);
  CHECKRESULT (Result, "_CompleteOutputForm",
               "Problem writing new form length!!!");
...
/*********************************************************************
 **		Writes a FORM and Format Version chunk and then goes into a
 *loop *		which grabs chunk headers and switches based on the
 *chunk *		type.  All chunks are tossed out except the SSND chunk
...
#define ID_SDX2                                                               \
  MAKE_ID ('S', 'D', 'X',                                                     \
           '2') /* ID for SHayes' Squareroot-Delta-Exact 2:1 compression */
#define AIF_PAD_SIZE 200

static long
ChewChunks (iff_control_ptr in_ptr, iff_control_ptr out_ptr)
{
  long Result = 0;
  uint32 ChunkType, ChunkSize;
  ...
  CHECKRESULT (Result, "_ChewChunks",
               "Couldn't write format version chunk!!!");
  ...
  while (in_ptr->iffc_length > 0)
    {
      ...
      if ((ChunkType != ID_SSND) && (ChunkSize > 0))
        {
          if (ChunkSize > (AIF_PAD_SIZE * sizeof (uint32)))
            CHECKRESULT (true, "_ChewChunks",
                         "Some HUGE chunk (not SSND) is to large to chew!!!");
        }
      ...
      switch (ChunkType)
        ...
        case ID_FVER:
          ...
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not read Format Version Data!!!!");
        ...
        case ID_COMM:
           ...
           */
          Result = iff_read_chunk_data (in_ptr, &myExtCommonChunk.numChannels,
                                        ChunkSize);
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not read COMM chunk data!!!");

          if (gVerbose)
            printf ("  Updating Common chunk for AIFF-C compatibility....\n");
          fflush (stdout);
          ...
          Result = iff_write_chunk (out_ptr, ID_COMM,
                                    &myExtCommonChunk.numChannels,
           ...
           */
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not write new COMM new chunk data!!!");
          break;

        case ID_MARK:
          Result = iff_read_chunk_data (in_ptr, pad, ChunkSize);
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not read Marker chunk data!!!");

          if (gVerbose)
            printf ("  Copying Marker chunk to AIFF-C....\n");
          fflush (stdout);

          Result = iff_write_chunk (out_ptr, ChunkType, pad, ChunkSize);
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not write Marker chunk data!!!");
        ...
        case ID_INST:
          Result = iff_read_chunk_data (in_ptr, pad, ChunkSize);
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not read Instrument chunk data!!!");

          if (gVerbose)
            printf ("  Copying Instrument Chunk to AIFF-C....\n");
          fflush (stdout);

          Result = iff_write_chunk (out_ptr, ChunkType, pad, ChunkSize);
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not write Instrument chunk data!!!");

          break;

        default:
          if (gVerbose)
            printf ("  Tossing out unnecessary chunk....\n");
          fflush (stdout);

          Result = iff_skip_chunk_data (in_ptr, ChunkSize);
          CHECKRESULT (Result, "_ChewChunks",
                       "Could not skip some unnecessary chunk!!!");
          break;
        }
    }
