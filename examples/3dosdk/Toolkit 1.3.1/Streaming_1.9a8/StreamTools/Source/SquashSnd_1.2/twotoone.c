/**************************************************************************************
 **	Project:		SquashSnd - Read AIFF and AIFC files, compress 2:1 with squareroot-delta-exact
 **							encoding and write an AIFC file.  Copy all other chunks
 ...
 **	 6/17/93	RDG		overhaul to use block reads, encoding, and writes
 **	 6/15/93	RDG		version .12
 **	 6/15/93	RDG		include Shayes' fix for the encoder
...
static long DoMonoEncoding (SDX2ContextBlkPtr	ctx)	
{
	...
	for (ix = 0; ix < ctx->framesInBuffer; ++ix)   /* %Q (++ix) why did stever pre-increment counter */
	{
		...
		if (gVerbose)
			...
			if (CurErr > ctx->maxMonoErr) 
				...
				printf("   (New Max Err = %ld...  Curr:%d, Prev:%d, Comp:%d, Deco:%d) \n", ctx->maxMonoErr, 
								CurSamp, ctx->prevMonoSamp, CompSamp,decode((long)CompSamp,(long)ctx->prevMonoSamp));
				}
			/* debug printf for looking at each sample */
			if (0) 
				printf("   DEBUG-- Comp:%d = Curr:%d, Prev:%d .. Deco:%d) \n",CompSamp,
								CurSamp,ctx->prevMonoSamp,decode((long)CompSamp,(long)ctx->prevMonoSamp));
			
			ctx->avgMonoErr += CurErr;
			fflush(stdout);
			}

		ctx->prevMonoSamp = (long) decode((long)CompSamp,(long)ctx->prevMonoSamp);
	}
		...
		*outBufferPtr++ = CompSamp;
		...
		if (CurErr > ctx->maxLeftErr) 
			...
			printf("   (New Max Err (Left) = %ld...  Curr:%d, Prev:%d, Comp:%d, Deco:%d) \n", ctx->maxLeftErr, 
							CurLeftSamp, ctx->prevLeftSamp, CompSamp,decode((long)CompSamp,(long)ctx->prevLeftSamp));
			}
		/* debug printf for looking at each sample */
		if (0) 
			printf("   DEBUG-- Comp:%d = Curr:%d, Prev:%d .. Deco:%d) \n",CompSamp,
							CurLeftSamp,ctx->prevLeftSamp,decode((long)CompSamp,(long)ctx->prevLeftSamp));
		...
		if (gVerbose)
			...
			if (CurErr > ctx->maxRightErr) 
				...
				printf("   (New Max Err (Right) = %ld...  Curr:%d, Prev:%d, Comp:%d, Deco:%d) \n", ctx->maxRightErr, 
								CurRightSamp, ctx->prevRightSamp, CompSamp,decode((long)CompSamp,(long)ctx->prevRightSamp));
				}
			/* debug printf for looking at each sample */
			if (0) 
				printf("   DEBUG-- Comp:%d = Curr:%d, Prev:%d .. Deco:%d) \n",CompSamp,
								CurRightSamp,ctx->prevRightSamp,decode((long)CompSamp,(long)ctx->prevRightSamp));
 ...
 **		On error: returns failure code  
 *********************************************************************/
long CompressSSND(iff_control_ptr in_ptr,iff_control_ptr out_ptr,  ExtCommonChunk CommChunk)	
{
	long 			Result;
	long 			ix;
	SDX2ContextBlk	ctx;
	...
	ctx.bytesPerFrame	=	((long) (CommChunk.sampleSize / 8) * (long) CommChunk.numChannels);
	...
	CHECKRESULT(Result,"_CompressSSND","Problems skipping the Offset and blockSize");

	currFilePos = iff_getpos(in_ptr);						/* keep track of where we are */
	endSSNDChunkPos = currFilePos + ctx.bytesInSample;		/* store end of sample data */
	...
	while (bytesLeft)
	{
		...
		/* If sample is smaller than desired buffersize, only read in "bytesInSample"  number of bytes */
		if (bytesLeft < BUFFERSIZE)
			{
			bytesRead = iff_get_block(in_ptr, (char *) ctx.inBufferPtr, bytesLeft);
				}
		else
			{
			bytesRead = iff_get_block(in_ptr, (char *) ctx.inBufferPtr, BUFFERSIZE);
			}
			
		CHECKRESULT((bytesRead == 0),"_CompressSSND","a block read returned 0 bytes!");
		...
		if (CommChunk.numChannels == 1)
			...
			CHECKRESULT(Result,"_CompressSSND","Problems encoding the SSND chunk");
			} 
		else 
			{
		...
		Result = iff_put_block(out_ptr, (char *) ctx.outBufferPtr, (bytesRead  / 2));
		CHECKRESULT((Result == 0),"_CompressSSND","Couldn't write buffer to output file!");
	...
	CHECKRESULT(Result,"_CompressSSND","Couldn't complete the new SSND chunk");
