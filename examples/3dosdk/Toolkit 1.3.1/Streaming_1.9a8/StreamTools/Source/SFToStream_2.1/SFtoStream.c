 ...
 *						Fix chunkTime to start at 1.  So if you
 *weave multiple audio streams together which start at the same time, all of
 *the headers will be processed together before any data starts playing.  This
 *						was fixed in an earlier version but the
 *change got lost somehow!!! 10/21/93	rdg		Version 2.0 Fix
 *sampleCount field in sampleDescriptor structure.  Was being computed
 *improperly. 10/19/93	jb		Version 1.9 Change command line numeric
 *parsing to accept either decimal or hexadecimal inputs. 9/12/93
 *rdg		version 1.8 Spin cursor. Change input and output file command
 *line flags to -i and -o. 8/25/93		rdg		version 1.7
 *						Alter computation of chunk size to
 *prevent the possibility of yielding a chunk size greater than the one
 *desired.  Rounding is necessary, but a case appeared where the net rounding
 *turned out to be up instead of down. 8/19/93		rdg
 *version 1.6 experimental changes slipped out with v1.5 which caused time
 *stamps to be screwed up.  Reverted affected code to 1.4. 8/07/93 	ian
 *version 1.5 program now exits with zero status for success, non-zero for
 *failure.  on failure, it deletes any partial output file that may have been
 *built.  it also spins the cursor now. 7/16/93 	rdg 	version 1.4 add
 *support for larger I/O buffers to speed processing 6/25/93 	rdg
 *version 1.3 fix aifc compression type check. 6/25/93 	rdg 	version 1.2 add
 *checks for compression type, sample size, sampling frequency, and number of
 *channels.  Exit with polite message if not supported. 6/24/93 	rdg
 *version 1.1 fix default initial pan value check for proper AIFF format
 *printed wrong filename 6/17/93 	rdg 	version 1.0 5/28/93 	njc
 *BytesPTick was not being adjusted for compressionRatio. 5/28/93 	rdg
 *Version .13 Not too pround of the hack to support AIFF and AIFC files
 *directly. Ended up re-writing the main loop. BlockSize is no longer
 *supported. SSND chunks are simply written to the output stream and not
 *grouped into any higher level structure.  ChunkSize is still rounded to the
 *						nearest quadbyte aligned, integral of
 *audio ticks. It also now supports specifing the logical channel number you
 *want on the command line. 5/25/93 	rdg 	Version .12 Fixed compression
 *type specification from command line 5/25/93 	rdg 	Version .11 5/25/93
