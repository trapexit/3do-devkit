 ...
 *
 *	History:
 *	03/15/94	MPH		Changed GetDstStuff() to handle
 centering within any size bitmap buffer. Needed for native PAL
 *	11/23/93	jb		Version 1.2
 *						Added CloseCPakSubscriber() to
 clean up resources allocated by the
 *						InitCPakSubscriber() routine.
 *	10/19/93	jb		Version 1.1
 *						Switch to using UMemory.c routines
 for memory allocation
 *	10/14/93	jb		Add code identified by
 FREE_DATA_WHEN_TIME_HAS_PASSED to optionally
 *						free streamer buffers automatically
 once a buffer's time has expired.
 *						Expiration is determined by buffer
 time+duration being less than
 *						the current stream clock.
 *	8/25/93		jb		Force image width and height to be a
 multiple of 4 in AllocMovieBuff(). *	6/22/93		njc		Changed
 subscriber structure from one msgPort for every channel to *
 a simple linked list with a semaphore to arbitrate access to the list.
 *						In addition, a freeList is
 maintained that contains chunks that are *
 to be returned to the Data Streaming Library. *	5/25/93		njc
 Version 1.0a *	5/19/93		ec		Added reset code to
 FlushCPakChannel. *	4/29/93		njc		New today.
 *
 *******************************************************************************************/
             ...
             Bitmap *bitmap)
{
  ...
  vOff = (((bitmap->bm_Height - height) / 2) & 0xFFFFFFFC)
         * (bitmap->bm_Width * 2);
