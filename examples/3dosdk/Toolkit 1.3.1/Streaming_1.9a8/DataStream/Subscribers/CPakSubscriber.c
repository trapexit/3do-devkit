 ...
 *
 *	History:
 *	03/15/94	MPH		Changed GetDstStuff() to handle centering within any size bitmap buffer.
 						Needed for native PAL
...
void					GetDstStuff(long *baseAddr, long *rowBytes, long width, long height, Bitmap *bitmap);
...
/*******************************************************************************************
 *	Although the frame buffer displays is 320 by 240 (384x288 in PAL) , only about 288 by 216 pixels
 *	are in the visible area. (348x260 in PAL)  All display calculations are therefore based on a 
...
void	GetDstStuff(long *baseAddr, long *rowBytes, long width, long height, Bitmap *bitmap)
{
	...
	hOff = ((bitmap->bm_Width - width)/2)*4;
	vOff = (((bitmap->bm_Height - height)/2) & 0xFFFFFFFC) * (bitmap->bm_Width*2);
