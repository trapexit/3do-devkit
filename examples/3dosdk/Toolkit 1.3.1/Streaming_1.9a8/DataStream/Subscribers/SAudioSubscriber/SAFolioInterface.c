 ...
 *
 *	Contains:		Code to deal with low level folio issues related
 *to streaming.
 *
 ...
 *						Fixed number of parameters to
 *MuteChannel(). 10/14/93	rdg		Version 2.8 -- New Today Split
 *source into separate files. Keep high level subscriber message parsing and
 *logical channel related functionality in SAudioSubscriber.c.  Move AudioFolio
 *stuff here. Move trace implementation to SAudioTrace.c
 */

/* Compile switches for trace buffer */
#define TRACE_BUFFERS 1

/* DON'T TURN ON UNLESS TRACE_BUFFERS IS ON, OR THE COMPILER WILL BARF!!!!! */
#define DUMP_TRACE_ON_BUFFER_COMP_ERR 0
...
/*******************************************************************************************
 * Routine to pass to ForEachFreePoolMember which initalizes and properly
 *inter-relates the AudioFolio samples, attachments, cues and signals needed to
 *do the streaming.
 *******************************************************************************************/
Boolean
InitAudioBuffer (void *initBlockPtr, void *poolEntry)
{
  int32 status;
  ...
  bufferPtr->attachment
      = AttachSample (chanPtr->channelInstrument, bufferPtr->sample, 0);
  CHECKRESULT ("AttachSample", bufferPtr->attachment);

  /* Make sure attachments don't start automatically when StartInstrument() is
   * called */
  ...
  status
      = MonitorAttachment (bufferPtr->attachment, bufferPtr->cue, CUE_AT_END);
  ...
  return true; /* MUST RETURN TRUE IF SUCCESSFUL OR ForEachFreePoolMember()
                  WILL DIE */
}

/*******************************************************************************************
 * Routine to pass to ForEachFreePoolMember which frees up the AudioFolio
 *samples, attachments, cues and signals needed to do the streaming.
 *******************************************************************************************/
Boolean
FreeAudioBuffer (void *initBlockPtr, void *poolEntry)
{
  int32 status;
  SAudioBufferPtr bufferPtr;
  SABufferInitBlockPtr myInitBlockPtr;
...
/*******************************************************************************************
 * Routine to add a buffer onto the tail of the in use queue of buffers sent to
 *the audio folio for the given channel.
 *******************************************************************************************/
bool
AddBufferToTail (SAudioChannelPtr chanPtr, SAudioBufferPtr buffer)
{
  bool fQueueWasEmpty;
...
/*******************************************************************************************
 * Routine to remove a data buffer from the head of the queue of buffers sent
 *to the audio folio for the given channel.
 *******************************************************************************************/
SAudioBufferPtr
GetNextBuffer (SAudioChannelPtr chanPtr)
{
...
/*******************************************************************************************
 * Routine to help prevent concurrent logical channels from getting out of sync
 *with each other.
 *******************************************************************************************/
void
OrphanPlayingBuffers (SAudioContextPtr ctx)
{
  long channelNumber;
  SAudioChannelPtr chanPtr;
  SAudioBufferPtr bufferPtr;
          ...
          if (bufferPtr != NULL)
            {
              ...
              ADD_TRACE (kBufferOrphaned, channelNumber, bufferPtr->signal,
                         (long)bufferPtr);
            }
        }
    }
...
/*******************************************************************************************
 * As long as there are waiting messages and free AudioFolio buffers, move the
 *msgs to the buffer queue.
 *******************************************************************************************/
void
MoveWaitingMsgsToBufferQueue (SAudioContextPtr ctx, long channelNumber)
{
  ...
  /* If channel is active, and as long as there are data messages and free
   * buffers, submit those buffers to the AudioFolio.
   */
  if (IsChanActive (chanPtr))
    {
      ...
      while (
          ...
          && ((bufferPtr = (SAudioBufferPtr)AllocPoolMem (chanPtr->bufferPool))
              != NULL))
        {
          ...
          /* save this message so we can reply to it when the sample finishes
           * playing */
          bufferPtr->pendingMsg = subMsg;

          /* add this signal to the current buffer signals mask for this
           * subscriber context*/
          ctx->allBufferSignals |= bufferPtr->signal;

          /* If the inuseQueue is not empty, link the last buffer submitted to
           * the current one, Since a link to a buffer is made when a following
           * buffer arrives, the final buffer will not be linked and should
           * stop gracefully;  the instrument will begin playing silence until
           * something else happens.
           */
          if (chanPtr->inuseCount != 0)
            {
              LinkAttachments (chanPtr->inuseQueueTail->attachment,
                               bufferPtr->attachment);
            }
          ...
          ADD_TRACE (kMovedBuffer, channelNumber, bufferPtr->signal,
                     (long)bufferPtr);
        }
    }
}
 ...
 *******************************************************************************************/
void
QueueNewAudioBuffer (SAudioContextPtr ctx, SubscriberMsgPtr newSubMsg)
{
  int32 status;
  SAudioDataChunkPtr audioData;
  int32 channelNumber;
  ...
  if (IsChanEnabled (chanPtr))
  ...
  else
    {
      ...
      status = ReplyMsg (newSubMsg->msgItem, kDSNoErr, newSubMsg,
...
/*******************************************************************************************
 * Routine to clean up buffer block data before returning block to the free
 *pool.
 ******************************************************************************************/
void
FreeBufferFromAudioFolio (SAudioBufferPtr bufferPtr)
{
  TagArg Tags[3];
  ...
  ADD_TRACE (kFreedBufferFromFolio, -1, bufferPtr->signal, (long)bufferPtr);
}

/*******************************************************************************************
 * Routine to figure out which buffer completed based on the signalBits
 *returned from the AudioFolio.
 *******************************************************************************************/
void
FindBuffer (SAudioContextPtr ctx, uint32 signalBits,
            SAudioChannelPtr *channelPtr, SAudioBufferPtr *bufferPtr)
{
  ...
  for (channelNumber = 0; channelNumber < SA_SUBS_MAX_CHANNELS;
       channelNumber++)
    {
      ...
      localBuffPtr
          = localChanPtr->inuseQueueHead; /* point to least recent entry in the
                                             in use list */

      /* Mask off all bits that are not possible buffer completion signals for
       * this channel */
      mySigBits = (signalBits & localChanPtr->signalMask);

      if (mySigBits) /* is the signal from this channel? */
        {
          /* Try to determine if a buffer has completed out of order by
           * comparing the signal we got from the AudioFolio with the signal
           * associated with the buffer we found at the head of the in use
           * queue.
           */
          if (mySigBits & localBuffPtr->signal)
            {
              /* One of the completion signal for this channel matches the
               * signal for the buffer at the head of the inUseQueue so we are
               * happy...
               */
              localBuffPtr = GetNextBuffer (localChanPtr);
              ADD_TRACE (kFoundBuffer, channelNumber, localBuffPtr->signal,
                         (long)localBuffPtr);
            }
          else
            {
              /* The completion signal for this channel was not for the buffer
               * at the head of the inUseQueue.  This means that the System or
               * the Folio is sending spurious shit to us.
               */
              ADD_TRACE (kFoundWrongBuffer, channelNumber,
                         localBuffPtr->signal, (long)localBuffPtr);
              localBuffPtr = NULL;
 ...
 * completed.  Get the subscriber message ptr and reply to the message.  Next,
 *check to see if there are any new messages in the queue, if there are, use
 *the same sample resources for the new message.
 *******************************************************************************************/
void
HandleCompletedBuffers (SAudioContextPtr ctx, uint32 signalBits)
{
  int32 status;
  SubscriberMsgPtr subMsg;
  SAudioDataChunkPtr audioData;
  ...
  ADD_TRACE (kBufferCompleted, -1, signalBits, 0);

  /* Mask off anything that isn't a buffer completion signal */
  signalBits &= ctx->allBufferSignals;

  ADD_TRACE (kMaskedSignalBits, -1, signalBits, 0);

  /* Service all completed buffers */
  while (signalBits)
    {
      /* Search all channels in acending order and return least recently
       * submitted buffer on first channel which gets a hit.
       */
      FindBuffer (ctx, signalBits, &chanPtr, &bufferPtr);

      /* If FindBuffer() found the wrong buffer, or no buffer then do nothing
       * else */
      if (bufferPtr == NULL)
        goto error;
      ...
      /* Remove this buffer's signal from the "currently queued" buffer signals
       * mask for this context
       */
      ctx->allBufferSignals &= ~bufferPtr->signal;
      ...
      /* If we had a buffer completion on the clock channel we need to know so
       * we can set the stream time when we are done processing.  We can't set
       * the clock now because we might have gotten more than one buffer
       * completion on the clock channel.
       */
      ...
      *(long *)&audioData->samples = 0xdeadbeef;
      ...
      /* If there are any msgs waiting in the dataQueue, use up any free
       * AudioFolio buffers and move them to the inUseQueue.
       */
      MoveWaitingMsgsToBufferQueue (ctx, channelNumber);

      /* If there is now no data in the inuseQueue or the dataQueue, then this
       * was the last buffer to complete and this channel is in idle.
       */
   ...
   * the head of the inuseQueue for the clock channel.  So we get the time
   * stamp out of the message and set the clock accordingly. If the
   * inuseQueueHead is NULL then this was the last buffer to complete so we
