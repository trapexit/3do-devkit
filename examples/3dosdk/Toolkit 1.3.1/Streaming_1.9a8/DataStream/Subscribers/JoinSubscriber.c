...
int32	FlushJoinSubscriber( JoinContextPtr ctx )
	...
	for (dataPtr = ctx->dataMsgHead; dataPtr != NULL; 
		...
		{
		DoJoinElementDone( ctx, dataPtr );
		RemoveMsgFromJoinQueue( ctx, dataPtr );
		}
...
static int32	DoJoinData( JoinContextPtr ctx, SubscriberMsgPtr subMsg )
	...
	if ( (joMsg = FindJoinElement(ctx, dataPtr->subChunkType, dataPtr->channel)) == NULL )
		{
		...
		// DIAGNOSE(("Alloced JOIN Data -- %d bytes\n", dataPtr->totalDataSize)); 
		joMsg->dataPtr		= NewPtr(dataPtr->totalDataSize, dataPtr->ramType);
		if (joMsg->dataPtr == NULL)
			{
			DIAGNOSE(("DoJoinData -- could not allocate %ld bytes\n", dataPtr->totalDataSize)); 
			}
