 ...
 *
 *    03/18/94	Edgar	Replaced %d with %ld in the DIAGNOSE message.
 *
...
ParseFont (void *fontImage)
{
  ...
  if (fh->fh_bitsPerPixel != 3)
    {
      DIAGNOSE (("This font is %ld bits per pixel; can only handle 3-bpp\n",
                 fh->fh_bitsPerPixel));
      goto ERROR_EXIT;
    }
...
#if DEBUG
  if (dstBPP != 8 || fd->fd_bitsPerPixel != 3)
    {
