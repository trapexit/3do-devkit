 ...
 *the CCB fields related to width/height/bpp. If the cel already has a buffer,
 *and it's big enough, we don't actually alloc a new buffer, we just change the
 *CCB data to reflect the new cel size.  If the cel already has a buffer but it
 *needs a bigger one, we free the existing buffer and alloc a bigger one.
 ****************************************************************************/
