/*
	...
	Change History (most recent first):
		...
		<1+>	  2/8/94	RLC		Add LHCreateCellFromCel() routine to build list cells from a cel
									file's cel.
...
/* LHCreateCellFromCel
 *
 * Create a list cell from the passed cel (using its coords) & positioned at the end of the list.
 *
 */
Err LHCreateCellFromCel(ListHandler	*listP, ListCell **cellP, CCB * ccb)
	...
	if (ccb)
		...
		//printf("cellRect is %ld, %ld, %ld, %ld", cellRect.rect_XLeft, cellRect.rect_YTop,cellRect.rect_XRight,cellRect.rect_YBottom);
