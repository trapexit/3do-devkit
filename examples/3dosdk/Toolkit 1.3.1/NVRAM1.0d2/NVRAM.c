/*
	...
	Contains:	NVRAM Utility Ñ an application for managing NVRAM filesystems
	...
	Change History (most recent first):
				 ...
				 1/26/93	fjs		Start Working on NVRAM utility
				 3/10/94	fjs		change the K figures to %
				 					tone down all of the cels
...
void PrintListCells(ListHandler *list)
	...
	while (LHGetNextCell(list, &cellP))
		kprintf("cellp: %x, next: %x, prev: %x \n", cellP, cellP->lcn_Next, cellP->lcn_Prev);
	}
...
void SetupBackground()
	...
	if (totalCopies > 1)
		...
		// Make enough copies of the cel to fill the screen (minus 1 for 'gTileBackground')
		for (i = 1; i < totalCopies; ++i)
			{
			tmpCel = MakeNewDupCCB(gTileBackground);					// Duplicate the cel exactly
			if (tmpCel == NULL)
				{
			...
			tmpCel->ccb_NextPtr = gTileBackground;					// Put it before 'gTileBackground'
			tmpCel->ccb_Flags &= ~CCB_LAST;					// Mark this as not last
			gTileBackground = tmpCel;									// Make 'gTileBackground' point to first
			}

		// Now we have enough cels to fill the screen in a chain pointed to by 'gTileBackground'
		xPos = yPos = 0;
...
void DrawEverything()
	...
	DrawScreenCels(gScreen.sc_Screens[gScreen.sc_curScreen], gSaverNameCel->tc_CCB);
	DrawScreenCels(gScreen.sc_Screens[gScreen.sc_curScreen], gSaverSpaceCel->tc_CCB);
...
void CreateFileList(void)
	...
	while((result = ReadDirectory(dir, &de)) >= 0)
		...
		SetTextCelColor(tc, 0, MakeRGB15(kNotSoBright, kNotSoBright, kNotSoBright));
...
void CreateSpaceCels()
	{
	gSaverNameCel = CreateTextCel(gFileFont, TC_FORMAT_8BPPCEL, 150, gFontHeight);
	...
	gSaverSpaceCel = CreateTextCel(gFileFont, TC_FORMAT_8BPPCEL, 150, gFontHeight);
	...
	SetTextCelCoords(gSaverSpaceCel, (kListCelX + 5) << 16, (kListCelY + 5 + 16 * 2) << 16);
	}

void UpdateSpaceCels()
	{
