/*
	...
	Change History (most recent first):
				 ...
				 12/7/93	EGH		Added kLHDebug switch debugging code.
				11/12/93	EGH		Began implementation.
				10/20/93	RLC		New file.

	To Do:
	
		¥ Decide if we need so many separate calls to do both convenience things and Get/Set functions
*/
...
#define kAutoScrollH		0x00000002		// automatically scroll selected cells into view
#define kClipCellsToBounds	0x00000004		// only draw cells within the list's bounds
											// NOTE: partial cell clipping not supported!
#define kHiliteOnlyDrawProc	0x00000008		// call optional draw proc for selected cells only
											// after drawing the cell with standard proc
#define kAllCCBsAreLinked	0x00000010		// only call DrawCels for the first CCB encountered
#define kAutoLinkCCBs		0x00000020		// automatically link CCBs as they are set
#define	kSetCCBsSkipFlag	0x00000040		// be nice & set all CCB's skip flags when necessary
...
#define	kHiliteByFrame		0x00000200		// draw hilite by drawing a frame of the list's hilite color
...
typedef Err (*CellDrawProc)
	(struct ListHandler *listP, ListCell *listCell, Item bitMapItem);

// proc that selects the next cell
typedef void (*NextCellProc)
	(struct ListHandler *listP, int32 direction);
...
void	LHSetCellFormat(
			...
			int32				hOffset,
			int32				vOffset);
void	LHGetCellFormat(ListHandler *listP, ListCell *cellP, CellFormat *cellFormatP);
