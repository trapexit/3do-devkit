/*
	...
	Change History (most recent first):
				 ...
				 1/24/94	EGH		Added justification of text in cells.
				 					Should there be separate formatting for
									text & CCBs?
				11/12/93	EGH		Began implementation.
				10/20/93	RLC		File created.
...
Err LHCreateListHandler(
	...
	*listP = NULL;
	tempListP = (ListHandlerPtr)Malloc(sizeof (ListHandler), kDRAMAndClearMemory);
 ...
 */
Err LHDisposeListHandler(
	ListHandler *listP)		// (in) list handler to dispose
	{
	Err err = 0;
...
/* LHPadEvent
 *
 * Process an event. Determine if any directional buttons are down and select the
 * next cell.
 */
bool LHPadEvent(
	ListHandler					*listP,	
...
void LHStandardNextCellProc(
	...
	if ((listP->lh_ListFlags & kAutoScrollV) || (listP->lh_ListFlags & kAutoScrollH))
		{
		int32
			scrollH = 0,
			scrollV = 0;
		...
		if (listP->lh_ListFlags & kAutoScrollV)
			...
			{// horizontal scroll
				...
				if (cellP->lcn_Bounds.rect_XRight > listP->lh_Bounds.rect_XRight)
					{
					scrollH = listP->lh_Bounds.rect_XRight - cellP->lcn_Bounds.rect_XRight;
					}
				else if (cellP->lcn_Bounds.rect_XLeft < listP->lh_Bounds.rect_XLeft)
					{
					scrollH = listP->lh_Bounds.rect_XLeft - cellP->lcn_Bounds.rect_XLeft;
					}
			}
