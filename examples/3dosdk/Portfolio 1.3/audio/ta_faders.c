...
**
**	History:
**	6/14/93		rdg		added display for instrument current
*and max tick count
**	6/14/93		rdg		overhauled to conform to dragon
**						 references to changedir, /remote,
* /dsp, and /aiff removed *	 930602 	PLB 	Round up when
*calculating fdr_increment. *	 940209 	WJB 	Added FaderBlock arg to
*CustomFunc.
**						Fixed printf() type mismatch
*warnings. *						Sorted includes.
********************************************************************/
...
/********************************************************************/
int32
InitFaders (int32 NumFaders, FaderEventFunctionP EventFunc)
...
int32
CustomFaderFunc (int32 kn, int32 FaderValue, FaderBlock *fdbl)
{
  DBUG (("%8ld => %s\n", FaderValue, Name[kn]));
...
/********************************************************************/
int32
SetupKnobFader (Item Knob, Fader *fdr)
{
  int32 Result;
