/* $Id: testaudio.c,v 1.20 1994/02/18 01:54:55 limes Exp $ */
/****************************************************************
**
...
TestPoly (void)
{
  ...
  PRT (
      ("Play %d channels of sine wave in ascending frequency.\n", gNumVoices));
  ...
  for (i = 0; i < gNumVoices; i++)
    {
      TweakKnob (LeftGains[i],
                 ((MAXAMPLITUDE / gNumVoices) * (i)) / gNumVoices);
      TweakKnob (RightGains[i],
                 ((MAXAMPLITUDE / gNumVoices) * (gNumVoices - i))
                     / gNumVoices);
    }
...
TestSteady (void)
{
  ...
  uint32 butn;
  TagArg SamplerTags[] = { { AF_TAG_AMPLITUDE, (int32 *)0x7000 },
                           { AF_TAG_RATE, 0 },
                           { 0, 0 } };
  ...
  Result
      = ConnectInstruments (SamplerIns, "Output", MixerIns, ChannelNames[0]);
  CHECKRESULT (Result, "ConnectInstruments");
  OldMixerChan = 0;
  ...
  while (doit)
    {
      ...
      if (butn & ControlC)
        {
          Result = DisconnectInstruments (SamplerIns, "Output", MixerIns,
                                          ChannelNames[OldMixerChan]);
          CHECKRESULT (Result, "ConnectInstruments");
          OldMixerChan = (OldMixerChan + 1) & 0x7;
          Result = ConnectInstruments (SamplerIns, "Output", MixerIns,
                                       ChannelNames[OldMixerChan]);
          CHECKRESULT (Result, "ConnectInstruments");
          PRT (("MixerChan = %d\n", OldMixerChan));
        }
      ...
      if (butn & ControlUp)
        {
          ...
          if (Amplitude > MAXDSPAMPLITUDE)
            Amplitude = MAXDSPAMPLITUDE;
          PRT (("Amplitude = 0x%x\n", Amplitude));
        }
      if (butn & ControlDown)
        {
      ...
      if ((butn & (ControlRight | ControlLeft | ControlUp | ControlDown))
          && !(butn & JOYFIREB))
        {
          switch (Pan)
            {
