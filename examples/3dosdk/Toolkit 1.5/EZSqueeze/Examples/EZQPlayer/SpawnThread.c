/*
	...
	Change History (most recent first):
		 ...
		 <3>	  8/6/93	JAY		changing CREATETASK_TAG_MAXQ initial value to 0x4000. Before
									Dragon Tail 3 kernel use to see 2 and change it to 0x4000. DT3
									just issues an error (BAD_QUANTA)
		 <2>	 3/18/93	JAY		changing lists.h to list.h for Cardinal build
		 <1>	 3/18/93	JAY		first checked in
				 8/10/92	SH		My First Task (Thread) Demo
...
Item SpawnThread(ThreadProc proc, int stacksz, int pri, char * name) {
  Item myThread;
  char *sb = 0;
  TagArg tags[] = {
	TAG_ITEM_PRI,  (void *)0,
	CREATETASK_TAG_MAXQ,  (void *)0x4000,
	CREATETASK_TAG_PC,  (void *)0,
	TAG_ITEM_NAME,  0,
	CREATETASK_TAG_STACKSIZE,  (void *)0,
	CREATETASK_TAG_SP,  (void *)0,
	TAG_END,  0
	};
	
  myThread = CreateThread(name, pri, proc, stacksz);
  #ifdef dontCompile
	  sb = (char *) ALLOCMEM(stacksz,MEMTYPE_ANY); /* need to make our own stack */
	  if (!sb) {
