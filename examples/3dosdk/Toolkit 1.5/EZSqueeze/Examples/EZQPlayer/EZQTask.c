...
extern int32	InitEZQPlayerFromStreamHeader( PlayerPtr ctx, char* streamFileName );
...
int32 waitBufferFree(PlayerPtr ctx, Frame *frame)
	{
	Signal result;
	do 
		{
...
int32 do_EZQStream(PlayerPtr ctx)
	{
	int32			status;
	Signal signals;
	...
	while ( true )
		{
		if (signals = waitBufferFree(ctx, &EZQ_Task_Parms.frames[EZQ_Task_Parms.frameIndex]))
			{
		...
		else if (!continuousPlayMode) continue;
		
		if ( PollForMsg( ctx->messagePort, NULL, NULL, NULL, &status )) break;
		...
		if ( ctx->ezqContextPtr != nil )
			...
			if( continuousPlayMode )
				{
				validFrameDecoded = DrawEZQToBuffer( ctx->ezqContextPtr, ctx->ezqChannelPtr,
					(Bitmap *)EZQ_Task_Parms.frames[EZQ_Task_Parms.frameIndex].pixMap, 0);
				}
			else
				validFrameDecoded = DrawStepEZQToBuffer(ctx->ezqContextPtr, ctx->ezqChannelPtr,
					(Bitmap *)EZQ_Task_Parms.frames[EZQ_Task_Parms.frameIndex].pixMap);
			if (validFrameDecoded)
				{
				//printf("\n** Decoded **\n");
				EZQ_Task_Parms.frames[EZQ_Task_Parms.frameIndex].valid = 1;
				EZQ_Task_Parms.frameIndex = (EZQ_Task_Parms.frameIndex+1) % EZQ_Task_Parms.frameCount;
				//printf("*");
	...
	/* Unconditionally stop the stream */
			...
			if ( ctx->ezqContextPtr != 0 )		/* Flush anything held by the Cinepak subscriber */
				FlushEZQChannel( ctx->ezqContextPtr, ctx->ezqChannelPtr, 0 );
				
			return(2); /* All done -- return code */
	}

void EZQStreamTask(void)
	{
	int32			status;
	int32			playerResult;
	PlayerPtr		ctx;
	...
	if ( status != 0 )
		...
		/* printf("\nPlayed %d frames, skipped %d frames", playerContext.frameCount,
			playerContext.ezqContextPtr->framesDropped);    */
...
void startEZQTask(char* streamFileName, PlayerFuntionContext *pfc, int32 frameCount,
	char *bufferA, char *bufferB, char *bufferC)
	{
	if (EZQ_Task_Parms.running) killEZQTask();
	//printf("\n** Starting Cpak Task **");
	...
	EZQ_Task_Parms.task_Item = SpawnThread(EZQStreamTask, 8192, CURRENT_TASK_PRIORITY, "EZQ Task");
	//printf("EZQ Task is item %x\n", EZQ_Task_Parms.task_Item);
	EZQ_Task_Parms.result = 0;
	EZQ_Task_Parms.running = 1;
	}
	
void killEZQTask(void)
	{
