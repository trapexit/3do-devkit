 ...
 *
 *	History:
 *	7/6/94			WSD		Change to spawn a separate task for decompression to allow 3 display buffers
...
int32	PlayEZQStream( ScreenContext *screenContextPtr, char* streamFileName,
							PlayEZQUserFn userFn, void *userContext, PlayerFuntionContext *pfc )
	{
	int32 i;
	int32			playerResult;
	...
	while (EZQ_Task_Parms.running)
		...
		if (EZQ_Task_Parms.frames[screen_ID].valid)
			...
			EZQ_Task_Parms.frames[oldScreen_ID].valid = 0;						// allow this one to be used
			oldScreen_ID = screen_ID;
			screen_ID = (screen_ID+1)%EZQ_Task_Parms.frameCount;
			}
		else Yield();
		if ((userFn) && ((playerResult = (*userFn)( (void *) pfc, userContext )) != 0))
				{
				switch (playerResult)
					{
					case kStopMovie:
...
//		printf(" SubscriberType = %x\n", subsPtr->subscriberType );
		switch ( subsPtr->subscriberType )
			...
			case FILM_CHUNK_TYPE:
				...
				status = NewEZQSubscriber( &ctx->ezqContextPtr, 1, subsPtr->deltaPriority );
				...
				status = InitEZQCel( ctx->streamCBPtr,		// Need this for DSGetClock in sub
			...
			default:
				printf( "InitEZQPlayerFromStreamHeader() - unknown subscriber in stream header: '%.4s'\n",
