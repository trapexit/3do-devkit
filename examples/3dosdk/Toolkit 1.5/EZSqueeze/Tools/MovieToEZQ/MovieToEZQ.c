/* MovieToFilm.c
	...
	Contains:		Main control for compressing "raw " format quicktime movies into EZ Squeeze.
	...
	This file is the main control for compressing "raw " quicktime movies into EZ Squeeze format.
	It is an MPW tool.
...
/* Added by WSD 5/xx/94 */
	...
	// Utility Defines
		...
		#define UFix2X(v) (((extended)((unsigned long)(v) >> 16))+(((extended)((v) & 0xFFFF))/32768.0))
	...
	/*  New Timing code by WSD 5/11/94 */
		...
		#define ClockPrecision ((ClockType)10E8) /* Allows error of .0.01035391 frames per hour */
		#define ClockUnitsPerSecond (AudioRate*ClockPrecision)
		#define _3DO_TimeScale (AudioRate/_3DO_AudioTicks)
		#define _3DO_TimeBase ((((ClockType)1.0)/_3DO_TimeScale)*ClockUnitsPerSecond)
	...
	#define ChannelParm(trackNo)	((trackInfo[MovieTrack].channel&0xFFFF)\
	...
	// static and global variables
		...
		short runLiteralThreshold = RunLitThreshold;// Threshold for including literal pixels in runs (squeeze.c)
		
		ClockType timeOffset;
		
		int pixelThreshold = 0;						// minimum difference between pixels for calculating runs
		...
		extern Boolean tablesEveryFrame = true;			// if true, then send a table with each frame -- otherwise, first frame only.
...
/******************************************/
	...
	/* Added (or modified) by WSD 5/xx/94 */
		...
		Boolean		useAlphaChannel		= false;		/* set true if we should use the alpha channel for masking */
		ClockType	gStartTime			= 0;			/* default for a one clip movie */				// lla 12/6/93
		ClockType	totalMovieTime		= 0;			/* if non-0, then the target output time for film */	// WSD 5/16/94
...
/* Utilities from WSD */
	...
	OSErr truncateAndClose(short vRefNum)
		{
		long filePos;
		OSErr oops;
		...
		if ((oops = GetFPos(vRefNum, &filePos)) != 0)
			...
			if ((oops = SetEOF(vRefNum, filePos)) != 0)
				fprintf(stdout, "\nError in setting file EOF = %d", oops);
		return FSClose(vRefNum);
		}

	char *typeToStr(char *str, long type)
		{
	...
	OSErr writeAndUpdate(short file, long size, void *data, int *countPtr)
		{
		OSErr errorCode;
		long writeSize;
		writeSize = size;
		...
		if (writeSize != size) 
			{
			fprintf(stdout, "\tBytes written does not match I/O request (Written = %d, requested = %d)\n",
				writeSize, size);
			return (OSErr)(-1);
			}
		if (countPtr != nil) *countPtr -= writeSize;
		return(noErr);
		}
		
	int adjustOutputBlocking(short file, int size, int *countPtr)
		{
...
long AlignSample(Ptr srcData, Ptr dstData, int width, int height, int imageDepth)
	{
	/* Added by WSD */
	#ifdef UseDPCM
		int bytesCompressed;
		extern void showImage(int width, int height, int depth, long *pixmap, WindowPtr myCWindow);
		extern int compressImage(int width, int height, int depth, char *pixels, char *output, Boolean decimate, DisplayInfo *monitor);
		...
		if (useMonitorWindow && (myCWindow == nil))
			...
			c2pstr(windowName);
			myCWindow = NewCWindow(0, &r, windowName, 0xFF, 0, (WindowPtr)-1, 0xFF, 0);
			}
		...
		bytesCompressed = compressImage(width, height, imageDepth, (char *)srcData, (char *)dstData,
			false, useMonitorWindow?&monitor:nil);
		SetPort(savePort);
 ...
 *	of this data stream.
 */
WriteMarkerChunk (short filmFile, long channel, long *remainingBlockSize, long offset)
{
	...
	if (*remainingBlockSize < sizeof(CtrlMarkerChunk)) {
		fprintf(stdout,"### remainingBlockSize less than CtrlMarkerChunk!!!\n");
		return;
	}
...
OSErr DumpMovie(Movie theMovie, Str255 filmFileName, Str255 keyFileName)
	{ 
	
	#define AbortDumpFile(s) {errorStr = (s); goto DUMP_MOVIE_ABORT;}
	#define MovieTrack 0
	...
	/* Create and open the output file, and initialize counter */
		...
		if (((Oops = Create(filmFileName,0,'CYBD','EZQF')) != noErr) && (Oops != dupFNErr)
	...
	// make sure it doesn't have too many tracks for us to handle
		if ((tracksInMovie = GetMovieTrackCount(theMovie)) >= kMaxTrackCount)
			{
			fprintf(stdout,"#### Can't convert this movie, it has more than %d tracks.  Sorry\n", kMaxTrackCount);
			return -1;
			}
	...
	// First, get information about the movie (track 0 in our tables)
		...
		trackInfo[MovieTrack].length = GetMovieDuration(theMovie);	/* Not exactly correct, but there is no sample count */
		trackInfo[MovieTrack].duration = trackInfo[MovieTrack].timeBase*trackInfo[MovieTrack].length;
	...
	fprintf(stdout, "\t%5d%11s%14s%15d%15.2f%13d%9s\n", MovieTrack, "Movie", "<NA>", 
	...
	for (n = 1; n <= tracksInMovie; n++)
		...
		GetMediaHandlerDescription(trackInfo[n].mediaID, &trackInfo[n].mediaType, originalName, &originalManufacturer);
		trackInfo[n].startTime = GetTrackOffset(track)*trackInfo[MovieTrack].timeBase;
		...
		if (trackInfo[n].mediaType == VideoMediaType)
			{
			trackInfo[n].timeBase = MakeTimeBase(GetMediaTimeScale(trackInfo[n].mediaID));
			VideoMediaCount++;
			GetMediaSampleDescription(trackInfo[n].mediaID,1,(SampleDescriptionHandle)IDesc);
			if ( (*IDesc)->cType != 'raw ' )
				{
				fprintf(stdout,"\t\t#### Track %d type is \"%s\", which is not supported\n",
					n, typeToStr(tempStr, (*IDesc)->cType));
				fprintf(stdout, "\t\t#### Remake the movie in video type \"raw \" and try again.\n");
				return -1;
				}
				
			/*
			fprintf(stdout, "\t*** Duration = %d, timeBase = %6.2g, length = %d, rate = %d***\n",
				GetMediaDuration(trackInfo[n].mediaID), trackInfo[n].timeBase, trackInfo[n].length, 
				(long)(ClockUnitsPerSecond/ ((GetTrackDuration(track)*trackInfo[MovieTrack].timeBase - trackInfo[n].startTime)/trackInfo[n].length)
					));
				*/
			if (defaultFrameRate == 0) 
				defaultFrameRate = ClockUnitsPerSecond/
				((GetTrackDuration(track)*trackInfo[MovieTrack].timeBase - trackInfo[n].startTime)/trackInfo[n].length); /* Choose first video rack rate for default */
			}
		else
			if (trackInfo[n].mediaType == SoundMediaType)
				#ifdef IgnoreSound
					{
				...
				#else
					{
					if (firstAudioTrack == 0) firstAudioTrack = n;
					SoundMediaCount++;
					GetMediaSampleDescription(trackInfo[n].mediaID,1,(SampleDescriptionHandle)SDesc);
					trackInfo[n].timeBase = MakeTimeBase(UFix2X((*SDesc)->sampleRate)); /* Use actual sample rate instead of media scale */
					}
				#endif

		trackInfo[n].duration = trackInfo[n].timeBase*GetMediaDuration(trackInfo[n].mediaID);
		
		fprintf(stdout, "\t%5d%11X%14s%15d%15.2f%13d%9s\n", n, trackInfo[n].mediaID,
			...
			(trackInfo[n].mediaType == VideoMediaType)?typeToStr(tempStr, (*IDesc)->cType):"<NA>");
		}
	
	if (defaultFrameRate == 0)
		{
		fprintf(stdout, "\n##### Default frame could not be set -- assumed to be 20 #####\n\n");
		defaultFrameRate = 20;
		}
	...
	if (totalMovieTime != 0)
		{
		if (frameLimit != 0) // Force output to be exactly totalMovieTime in duration
			{
			timeWarpFactor = totalMovieTime/(frameLimit*output_Frame_TimeBase);
		...
		fprintf(stdout, "\tTime Warp Factor set to %6.2f to make output movie %6.2f seconds long\n",
	...
	if (dilateTimeFlag)
		...
		if (timeWarpFactor == 0)
			...
			fprintf(stdout, "\tFrame rate set to %d to make movie %6.2f seconds long\n",
	...
	/* Now set up the output timing parameters */
		...
		if (frameLimit != 0)
			outputDuration = Min(gStartTime*ClockUnitsPerSecond+frameLimit*output_Frame_TimeBase,
	...
	//	Create the film header -----------------------------------------------------------
		...
		FilmH.time			= 0;
		FilmH.chunkFlags	= 0;
		FilmH.channel		= (gVideoChannel == -1)?ChannelParm(trackNo):gVideoChannel;
		FilmH.version		= 0;
		FilmH.cType			= (*IDesc)->cType;
		FilmH.height		= (Min(MaxDisplayHeight, sourceHeight = (*IDesc)->height)) & -2; // must be even
		FilmH.width			= (Min(MaxDisplayWidth, sourceWidth = (*IDesc)->width)) & -WidthModulus;	// must be multiple of WidthModulus
	...
	// And create a partial film frame header of fields which don't change
		...
		FilmF.channel		= (gVideoChannel == -1)?ChannelParm(trackNo):gVideoChannel;
	...
	// Now print out some progress and diagnostic information
		...
		VFPRINTF(stdout,"\t  Output Duration: %d frames = %5.1f Seconds\n",
			(long)(outputDuration/output_Frame_TimeBase),outputDuration/ClockUnitsPerSecond);
		fflush(stdout);
	...
	//	Write the Film Header to disk -----------------------------------------------------------
		if (Oops = writeAndUpdate(FilmFile, sizeof(FilmHeader), &FilmH, &remainingBlockSize))
	...
	//	Create the Streamed Sound Header -----------------------------------------------------------
			...
			if (SoundMediaCount > 0)
				{
				/* Cycle through each audio track and determine the number of frames in each track's media. */
				...
				SSndH.time			= 0;
				SSndH.channel		= (gVideoChannel == -1)?ChannelParm(trackNo):gVideoChannel;
				...
				SSndH.sampleCount	= GetMediaSampleCount(trackInfo[firstAudioTrack].mediaID);
				
				VFPRINTF(stdout,"\t  %d sound samples at %d khz\n",SSndH.sampleCount,soundScale);

				if (Oops = writeAndUpdate(FilmFile, sizeof(StreamSoundHeader), &SSndH, &remainingBlockSize))
					AbortDumpFile("writeAndUpdate");
	...
	startClock = Min(outputDuration, ((ClockType)gStartTime)*ClockUnitsPerSecond);
	grandTotalBytes = 0;
	...
	while (outputClock < outputDuration)
		...
		// AUDIO LOOP -- Write out 1 second of Audio
				...
				for (trackNo = tracksInMovie; trackNo > 0; trackNo--)
					if ((trackInfo[trackNo].mediaType == SoundMediaType)
						...
						&& ((trackInfo[trackNo].startTime + trackInfo[trackNo].duration) > outputClock))
						{
						...
						audioDuration =
							Min(audioClock, trackInfo[trackNo].startTime + trackInfo[trackNo].duration) - outputClock;

						/* Perform necessary block boundary adjustments for output */
						
							size = (adjustOutputBlocking(FilmFile, kMinSoundChunkSize, &remainingBlockSize) - SoundChunkHdrSize) & -4;
						...
						trackTime = 
							(outputClock - trackInfo[trackNo].startTime)/trackInfo[trackNo].timeBase;
				
						Oops = GetMediaSample(trackInfo[trackNo].mediaID,sounddata,0,&size,trackTime,&sampleTime,
											  0,0,0,sampleCount,&sampleCount,&sampleFlags);
						if (Oops)
							AbortDumpFile("GetMediaSample for sound");
						
						if (audioClock < startClock) continue;
						...
						/* Prepare the sound chunk header */
							...
							SSndC.channel		= 0; // (gVideoChannel == -1)?ChannelParm(trackNo):gVideoChannel;
							SSndC.time			= (trackInfo[trackNo].startTime
													+ sampleTime*trackInfo[trackNo].timeBase)/filmTimeBase;
							/* SSndC.numSamples	= sampleCount; еее might need this someday... */
						...
						/* Write out the Sound Chunk Header and sample Data. */
							...
							if (Oops = writeAndUpdate(FilmFile, SoundChunkHdrSize, &SSndC, &remainingBlockSize))
								AbortDumpFile("Write Sound Chunk Header");
							
							if (Oops = writeAndUpdate(FilmFile, size, *sounddata, &remainingBlockSize))
		...
		//	VIDEO LOOP -- Write out 1 second of video
			...
			if (!gVerbose) fprintf(stdout, "\tSecond #%d ", 
				(long)((outputClock+output_Frame_TimeBase-1+ClockUnitsPerSecond/2)/ClockUnitsPerSecond));
			for (;(outputClock < audioClock) && (outputClock < outputDuration); outputClock += output_Frame_TimeBase)
				{
				SpinCursor(32);
				if (!gVerbose) fprintf(stdout, ".");
					...
					*/
					...
					for (trackNo = tracksInMovie; trackNo > 0; trackNo--)
						if ((trackInfo[trackNo].mediaType == VideoMediaType)
							&& (trackInfo[trackNo].startTime <= outputClock))  break; // take the latest one
				
				trackTime = 
					(outputClock - trackInfo[trackNo].startTime)/trackInfo[trackNo].timeBase;
				
				VFPRINTF(stdout,"\n\tFrame %d at movie time %d, Track time %d; ",
					(long)(outputClock/output_Frame_TimeBase), (long)(outputClock/trackInfo[MovieTrack].timeBase),
					trackTime);
				VFPRINTF(stdout,"remainingBlockSize = %d/%d\n", remainingBlockSize, gPhysBlockSize);
				
				Oops = GetMediaSample(trackInfo[trackNo].mediaID, sampledata, 0, &size,
					trackTime, &sampleTime, &durationPerSample, 0, 0, 1, 0, &sampleFlags);
					
				if (Oops)
					{
					sprintf(tempStr, "GetMediaSample (Track=%d, track time=%d, movie time=%d)",
						x, trackTime, (long)(outputClock/trackInfo[MovieTrack].timeBase));
					errorStr = tempStr;
					goto DUMP_MOVIE_EXIT;		/* Shut down normally */
					}
					
				if ((size = AlignSample(*sampledata, AlignedData, sourceWidth, sourceHeight, imageDepth)) == 0)
					goto DUMP_MOVIE_EXIT;
				
				VFPRINTF(stdout,"\t\tMedia Time %3d, sampleTime %6d, Duration %3d, Size %5d\n",
					trackTime, sampleTime, durationPerSample, size);
				
				/* Create a film frame header */

					FilmF.time			= (timeOffset + trackInfo[trackNo].startTime
											+ sampleTime*trackInfo[trackNo].timeBase - startClock)/filmTimeBase;
					FilmF.duration		= (durationPerSample*trackInfo[trackNo].timeBase)/filmTimeBase;
					FilmF.frameSize		= size;
					FilmF.chunkSize 	= sizeof(FilmFrame) + FilmF.frameSize;
					
					#ifndef UseDPCM
						//lla 1/25/94 -- distinguish between key frames and difference frames
				...
				#ifdef DebugPrint
					VFPRINTF(stdout, "\t\tFilm Time = %d, Channel = %d, duration = %d, frameSize = %d\n\t\tchunkType = %c%c%c%c, chunkSize = %d, chunkFlags = %08X, subChunkType = %c%c%c%c\n",
						FilmF.time, FilmF.channel, FilmF.duration, FilmF.frameSize,
						FilmF.chunkType >> 24, FilmF.chunkType >> 16, FilmF.chunkType >> 8, FilmF.chunkType, 
						FilmF.chunkSize, FilmF.chunkFlags,
						FilmF.subChunkType >> 24, FilmF.subChunkType >> 16, FilmF.subChunkType >> 8, FilmF.subChunkType
						);
					fflush(stdout);	
				#endif
				...
				if ( gEnableMarkerOutput && (sampleFlags != mediaSampleNotSync) )
					...
					if ( (FilmF.time - lastMarkerTime) >= (gMarkerRate * 240) )
						...
						VFPRINTF( stdout,"\t\t\tmarkertime %d, lastMarkerTime <%d>\n", FilmF.time, lastMarkerTime );
						lastMarkerTime = FilmF.time;
						}
					else 
						if ( FilmF.time == 0 && gStartTime != 0)
							{
							fprintf( keyFile, "markertime %d\n", FilmF.time + gStartTime );
							VFPRINTF( stdout,"\t\t\tmarkertime %d, lastMarkerTime <%d>\n", FilmF.time, lastMarkerTime );
							lastMarkerTime = FilmF.time;
				...
				/* Perform necessary block boundary adjustments for output */
					...
					FilmF.chunkSize = adjustOutputBlocking(FilmFile, FilmF.chunkSize, &remainingBlockSize);
				
				/* Write out the Film Frame Header and Data. */
					
					if (Oops = writeAndUpdate(FilmFile, sizeof(FilmFrame), &FilmF, &remainingBlockSize))
						AbortDumpFile("Write Film Frame Header");
					
					if (Oops = writeAndUpdate(FilmFile, FilmF.chunkSize - sizeof(FilmFrame), AlignedData, &remainingBlockSize))
		...
		runNibblesSaved /= 2;
		fprintf(stdout, "\: %d bytes, (audio:video %d:%d), Bytes saved by runs = %d\n",
			audioBytesPerSecond + videoBytesPerSecond, audioBytesPerSecond, videoBytesPerSecond, runNibblesSaved);
		VFPRINTF(stdout, "\tFrame Time = %5.1f, Pass1 time = %5.1f, Pass2 time = %5.1f, , Pass3 time = %5.1f, put Time = %5.1f\n",
			(extended)(pass2_Ticks+pass1_Ticks+pass3_Ticks+putTime)/60.0,
			((extended)pass1_Ticks)/60.0,
			((extended)pass2_Ticks)/60.0,
			((extended)pass3_Ticks)/60.0,
			(extended)putTime/60.0);
	...
	DUMP_MOVIE_ABORT:
		if (Oops != noErr)
			fprintf(stdout,"\n### OS Error #%d in building file %P (Operation: %s)\n",
				Oops, filmFileName, errorStr?errorStr:"Unknown");
...
static void Usage( char *toolName )
	...
	fprintf( stdout,
		"             [-d <outputDirectory]\n");
	fprintf( stdout,
		"             [-e Elapsed Time min:sec.msec]\n");
	fprintf( stdout,
		"             [-f frame rate (fps) {30}]\n");
	fprintf( stdout,
		"             [-g Don't encode black runs\n");
	fprintf( stdout,
		"             [-h Half Pixel Rate (approx 2.3 bits)]\n");
	fprintf( stdout,
		"             [-k Kill Frames {frame skipping on}]\n");
	fprintf( stdout,
		"             [-m marker rate {sec)]\n");
	fprintf( stdout,
	...
	fprintf( stdout,
		"             [-o Time offset {%d}]\n", 0);
	fprintf( stdout,
		"             [-p Pixel Run Threshold {%d}]\n", pixelThreshold);
	fprintf( stdout,
		"             [-q Error Quotient Tables]\n");
	fprintf( stdout,
		"             [-r Run Literal Threshold {%d}]\n", runLiteralThreshold);
	fprintf( stdout,
		"             [-s start time (seconds) {0}]\n");
		
	fprintf( stdout,
		"             [-t Elapsed Movie TIme (seconds) {0}]\n");
		
	fprintf( stdout,
		"             [-u Use Tables First Frame Only]\n");
		
	#ifdef UseDPCM
		fprintf( stdout,
			"             [-w Open Monitor Windows {false}]\n");
		fprintf( stdout,
			"             [-x frame limit {infinity}]\n");
		fprintf( stdout,
			"             [-z R G B Error Thresholds {3 3 3}]\n");
	#endif
...
main(int argc, char *argv[])
{
	...
	for (parms = 1; parms < argc; parms++) 
			...
			if (strlen(argv[parms]) == 2)
				switch (tolower(*(argv[parms]+1)))
					...
					case 'e':	// -e min:sec:msec -- set elapsed time for output
						if ((++parms) >= argc )
							{
							fprintf(stdout,"### %s - Missing elapsed time (min:sec:msec)\n", argv[0]);
							goto INVALID_ARG;
							}
						elapsedMin = elapsedSec = elapsedms = 0;
						switch(sscanf(argv[parms], "%d:%d.%d", &elapsedMin, &elapsedSec, &elapsedms))
							{
							case 0:
							case 1:
								fprintf(stdout,"### %s - Missing or bad format elapsed time (min:sec.msec)\n", argv[0]);
								goto INVALID_ARG;
							}
							
						totalMovieTime = ((ClockType)(elapsedMin*60+elapsedSec))*ClockUnitsPerSecond+(elapsedms*ClockUnitsPerSecond)/(ClockType)1000;
						break; 
					case 'f':
						if (++parms == argc )
							{
					...
					case 'q':
						...
						if (!scanErrorTables(argv[parms]))
							fprintf(stdout, "\tError in scanning error quotients from file %s\n", argv[parms]);
						break; 
					case 'r':
						if (++parms == argc )
							{
							fprintf(stdout,"### %s - invalid Run Literal Threshold.\n", argv[0]);
							goto INVALID_ARG;
							}
					...
					case 't':
						...
						totalMovieTime = strtod(argv[parms], &pEnd)*ClockUnitsPerSecond;
						break; 
					case 'u':
		...
		if (pixelBitRate == 2)
			...
			pixelThreshold = -1;
			VFPRINTF(stdout, "\tPixel Threshold set to %d\n", pixelThreshold);
			fprintf(stdout, "\tPixel Run encoding disabled for 2 Bit per Pixel mode\n");
			}
	// process the files
	if ( files == 0 ) 
		{
		...
		for (parms = 1; parms <= files; parms++) 
			{
				...
				if ( (theMovie = OpenMovie(srcMovieName)) != 0 )
					...
					// Added by WSD to print out some info
						fprintf(stdout, "\tPhysical Block Size = %d, frame Rate = %d, Channel = %d, Start Time = %5.1f\n",
							gPhysBlockSize, gFrameRate, gVideoChannel, gStartTime);
						fprintf(stdout, "\tMarkerOutput %s, MarkerRate = %d, Looping %s, elapsed Time = %6.2f Seconds\n",
							gEnableMarkerOutput?"On":"Off", gMarkerRate, gLoopIt?"On":"Off", totalMovieTime/ClockUnitsPerSecond);
						fprintf(stdout, "\tRun Error Threshold = %d, ###%d Bits per Pixel###\n\n*** Send Table%s Frame ***\n\n",
					...
					grandTotalTime = (grandTotalTime - gStartTime*ClockUnitsPerSecond)/ClockUnitsPerSecond;
					if (grandTotalTime <= 0) grandTotalTime = 10e-20; // prevent divide by 0
					fprintf(stdout, "\nConverted %d frames in %d.%d seconds at an average data rate of %d bytes/second\n",		// WSD
						frameCounter, elapsedTime/60, ((elapsedTime%60)*100)/60, (long)(grandTotalBytes/grandTotalTime));		// WSD
