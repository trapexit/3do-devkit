...
setGWorldAccess(GDHandle theGWorldDevice, GWorldData *GWDPtr)
	...
	if (((*theGWorldDevice)->gdType != directType)
		...
		if (directPixmapAddr == 0) 
			...
			OffsetRect(&directClipRect, -directClipRect.left, -directClipRect.top);
...
#define UseSetPixel
	...
	#define setQPixel(r,x,y,p) {if (directAccess) mySetPixel(r,x,y,p); else xSetQPixel(r,x,y,p);}
#else
	#define setQPixel xSetQPixel
#endif

	#define RGBtoColor32(R,G,B) ((long)(((R&0xFF00L)<<8)|(G&0xFF00L)|(long)(B>>8)))
	static void xSetQPixel(Rect *r, short x, short y, RGBColor *pixel)
		{
		...
		if (directAccess)
				...
				if ((x >= (clipRect = &directClipRect)->right) || (y >= clipRect->bottom)) return;
				if ((x <= clipRect->left) || (y <= clipRect->top)) return;
...
static Boolean sameRect(Rect r1, Rect r2)
	{
	if (
		(r1.left == r2.left)
		&& (r1.right == r2.right)
...
Boolean prepGWorld(WindowPtr window, Rect *rect, GWorldData *GWD)
	{	
	SetPort(window);
...
void showImage(int width, int height, int imageDepth, long *pixmap, WindowPtr myCWindow)
	{
	//char windowName[256];
	//Rect r;
	GrafPtr oldPort;
	...
	if (!monitor.GWD.initialized)
		...
		for (y = 0; y < height; y++)
			...
			for (x = 0, pixelPtr = &((char *)pixmap)[y*width*bytesPerPixel]; x < width; x++, pixelPtr+=bytesPerPixel)
				{
				switch(imageDepth)
					{
					case 16:
