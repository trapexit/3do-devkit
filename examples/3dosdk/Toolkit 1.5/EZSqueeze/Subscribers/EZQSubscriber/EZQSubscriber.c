 ...
 *
 *	History:
 *	7/11/94		dsm		Modified CPakSubscriber to create this EZSqueeze subscriber:
...
/****************************/
Boolean	PollEZQChunk(	EZQContextPtr		ctx,
						EZQRecPtr			ezRecPtr,
...
static int32 	_DrawEZQToBuffer( EZQContextPtr ctx, EZQRecPtr ezRecPtr, Bitmap *bitmap,
...
/*******************************************************************************************
 * Allocate a buffer for the EZSqueeze decompressor. The decompressor will dump data into this
 * buffer in LR form. The Cel Engine can draw this format (on RED systems) to the frame buf.
 *******************************************************************************************/
#define	FORCE_MULTIPLE_OF_4( x )	((x + 3) & ~3)
int32	AllocMovieBuff(ImageDesc *imagePtr, EZSqueezeHeaderPtr pCPHeader)
{
...
/*******************************************************************************************
 * Allocate and initialize a streamed EZSqueeze state record. GetEZQCel will return NULL until
...
/*******************************************************************************************
 *	Return all EZQ msgs to the EZQSubscriber back end. The EZQ subscriber will return one
 ...
 *******************************************************************************************/
Boolean	IsTimeForNextFrame (EZQRecPtr ezRecPtr)
	{
	 ...
	 */
	result =  (curTime > (curFramePtr->time + curFramePtr->duration) ) ? true : false;
	//printf("\nIsTime %d > %d => %d", curTime, curFramePtr->time + curFramePtr->duration, result);
	return result;
	}

Boolean	TimeToCatchUp (EZQRecPtr ezRecPtr)
	{
	EZSqueezeFramePtr		curFramePtr;
	uint32 				curTime;
...
/*******************************************************************************************
 * Send a signal to the EZQ Subscriber telling it to Reply all subMsgs in the freeList
...
/*******************************************************************************************
 * Get the next compressed frame data in the streamed EZSqueeze film. GetEZQCel will return NULL 
...
static CCB*		_GetEZQCel( EZQContextPtr ctx, EZQRecPtr ezRecPtr, 
		...
		if (PollEZQChunk (ctx, ezRecPtr, &pChunkData, &subMsg))
			...
			switch (pChunkData->subChunkType)
				...
				case FHDR_CHUNK_TYPE:	/* EZQ Header Chunk */
					 ...
					 */
					memcpy( &(ezRecPtr->cpHeader), pChunkData, sizeof(EZSqueezeHeader) );
				...
				case DIF_FRME_CHUNK_TYPE:	/* EZQ Frame Chunk */
					...
					PreDecompress(ctx->filmCodec, (void *)(((EZSqueezeFramePtr)pChunkData)->frameData));
					Decompress(ctx->filmCodec, (void *)(((EZSqueezeFramePtr)pChunkData)->frameData),
				...
				default:
					printf("GetEZQCel --- Unknown Chunk Type: '%.4s'", &(pChunkData->subChunkType) );
					break;
				}
			}
		else
	...
	if (DSGetClock( ezRecPtr->streamCBPtr, &curTime ) >= 0 )
		{
		if ( curTime > (ezRecPtr->curFramePtr->time + ezRecPtr->curFramePtr->duration) )
			{
			if (ezRecPtr->curSubMsg)
				{
				DoFreeChunk (ctx, ezRecPtr->curSubMsg);
 ...
 *******************************************************************************************/
int32  DrawEZQToBuffer( EZQContextPtr ctx, EZQRecPtr ezRecPtr, Bitmap *bitmap, long catsup_strategy )
	{
...
/*******************************************************************************************
 * Get the next compressed frame data in the streamed EZSqueeze film. DrawEZQToBuffer will return NULL
 * until the Header and first frame have been received. When the First frame comes in, the
 * current audio time will be read and saved to determine which frame to display each call.
 * 
 * lla 2/14/94
...
static int32 _DrawEZQToBuffer( EZQContextPtr ctx, EZQRecPtr ezRecPtr, Bitmap *bitmap,
	...
	while ( (fStepFrame || IsTimeForNextFrame( ezRecPtr )) && ( framesCaught <10 ) )
		...
		if (PollEZQChunk (ctx, ezRecPtr, &pChunkData, &subMsg))
			...
			switch (pChunkData->subChunkType)
				...
				default:
					printf("_DrawEZQToBuffer --- Unknown Chunk Type: '%.4s'", &(pChunkData->subChunkType) );
					break;
				}	// switch
			framesCaught++;
			}	// if
	...
	if (fGotAFrame)
		...
		GetDstStuff(&baseAddr,&rowBytes,pImageData->width,pImageData->height, bitmap);
		PreDecompress(ctx->filmCodec, (void *)(((EZSqueezeFramePtr)pChunkData)->frameData));
		Decompress(ctx->filmCodec, (void *)(((EZSqueezeFramePtr)pChunkData)->frameData), (char *)baseAddr,rowBytes);
		}
#if FREE_DATA_WHEN_TIME_HAS_PASSED
	/* Check to see if what we are about to return is a "past frame".
	 * If it is, then free the current frame and return NULL.
 ...
 * the EZQ movie.  App developer beware, you can display out of the frame buffer.
 *******************************************************************************************/
...
int32	InitEZQSubscriber( void )
	 ...
	 * contexts.
	 */
	EZGlobals.EZQContextPool = CreateMemPool( CPAK_MAX_SUBSCRIPTIONS, sizeof(EZQContext) );
...
/*******************************************************************************************
 * This thread is started by a call to InitEZQSubscriber(). It reads the subscriber message
