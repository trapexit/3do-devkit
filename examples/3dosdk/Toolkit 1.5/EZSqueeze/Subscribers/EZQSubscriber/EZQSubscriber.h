/*******************************************************************************************
 *	File:			EZQSubscriber.h
 *
 *	Contains:		definitions for EZQSubscriber.c
 *
 ...
 *
 *	History:
 *	7/12/94		WSD		Addeed framesDropped to context
 *structure -- used for keeping track of dropped frames 7/12/94 WSD
 *Addeed firstFrameFlag to context structure -- used for skip key frame scheme
 *	3/14/94		lla		Added SetCpakScreen() to locate the
 *display coordinates of the Cpak movie. 1/24/94		jb
 *Added GetStepEZQCel() and DrawStepEZQToBuffer() stepframe support. 11/23/93
 *jb		Added prototype for CloseEZQSubscriber() 4/28/93
 *njc		New today.
 *
 *******************************************************************************************/
...
#define FILM_CHUNK_TYPE                                                       \
  CHAR4LITERAL ('E', 'Z', 'Q', 'F') /* chunk data type for this subscriber */
...
#define KEY_FRME_CHUNK_TYPE                                                   \
  CHAR4LITERAL ('F', 'R', 'M', 'E') /* chunk data type for this subscriber */
#define DIF_FRME_CHUNK_TYPE                                                   \
  CHAR4LITERAL ('D', 'F', 'R', 'M') /* chunk data type for this subscriber */
...
//
//			SKIP_DIFF_FRAMES			  0		//  skip
//difference frames to catch up 			DECODE_DIFF_FRAMES			  1
////  decode as fast as you can 			BEST_METHOD
//-1		//  something else
//
//		BEST_METHOD has not been determined yet.  However a candidate is
//to keep a running 		average key frame rate.  If the keyframe rate is less than
//or equal to 2 seconds skip 		 diff frames.  However if the keyframe rate is
//greater than 2 seconds decode difference frames.
//
//  lla 2/22/94
//
//------------------------------------------------------------------------------------------------//
...
typedef struct EZQContext
{
  ...
  Boolean
      firstFrameFlag; /* flag: Last chunk received was header frame  -- WSD */

  int32
      framesDropped; /*	count of frames skipped because we are behind -- WSD */
  int32 numChannels;
...
void SetEZQScreen (int32 horizontalOffset, int32 verticalOffset,
                   Boolean centered);

int32 NewEZQSubscriber (EZQContextPtr *pCtx, int32 numChannels,
                        int32 priority);
...
int32 InitEZQCel (DSStreamCBPtr streamCBPtr, EZQContextPtr ctx,
                  EZQRecPtr *pCPRecPtr, int32 channel, Boolean flushOnSync);
...
int32 DrawStepEZQToBuffer (EZQContextPtr ctx, EZQRecPtr ezRecPtr,
                           Bitmap *bitmap);
