 ...
 *
 *	Contains:	Code to demonstrate looping and sequencing of multiple MIDI files
 *
 *	Usage:		LoopMIDIFiles [ <PIMap> <MIDIFile1> <MIDIFile2> <MIDIFile3>]
 *
 *				NOTE: If you specify any command line arguments you must include all 4!	
 *
 * 				This program loads three MIDI files and plays them using Juggler and 
 *				Score toolbox.
 *
 * 				The three files can be played back in varying order using the A, B, 
 *				and C buttons on the Joypad.  
 *
 *				A Button = play sequence1 when current sequence is finished
 *				B Button = play sequence2 when current sequence is finished
 *				C Button = play sequence3 when current sequence is finished
 * 
 *				X Button = stop and exit
 *	
 *				Sequence1 is always first, and will loop until the stop button is 
 *				pressed, or a button is used to change the order of playback.
 *
 *					
 ...
 *	History:
 *
 * 		9/22/94 	rdg 	Added workaround compile switch for music.lib bug.
 * 		9/20/94 	rdg 	Updated for attic release.
 * 		5/06/94 	rdg 	Adapted from PlayMF.c by Phil Burk
 *
 *******************************************************************************************/
...
static	Err 		LoopScores( Collection *Coll1, Collection *Coll2, Collection *Coll3 );
...
usage( void )
	...
	printf("NOTE: If you specify any command line arguments you must include all 4!\n");
	}
...
ScaleSequenceTimes( Sequence* Self, frac16 Scalar )
{
	...
	DBUG(("ScaleSequenceTimes(): objStartTime = %ld, many = %ld, eventPtr = %ld, eventSize = %ld\n",
...
main (int32 argc, char* argv[])
{
	...
	if ( Coll1 == NULL ) goto cleanup;

	Coll2 = LoadScore( scon, midifile2 );
	if ( Coll2 == NULL ) goto cleanup;

	Coll3 = LoadScore( scon, midifile3 );
	if ( Coll3 == NULL ) goto cleanup;
