 ...
 *	07/11/94  Ian	Standarized protective wrapper macro name to FORM3DO_H.
 *					Standardized comment blocks.
 *	07/29/93  Ian 	added xxxx_CHUNKHDR_SIZE macros.
 *	04/02/93  JAY 	replace old definition of RGB555 with something more
 *useful in our ARMCC environment 01/28/93  dsm 	Fixed #define for
 *CHUNK_CRDT 01/28/93  dsm 	first checked in
 *
 *	Implementation notes:
 *
 *	This file format assumes a "big-endian" architecture, that is
 *multi-byte values have their most significant bytes stored at the lowest
 *memory addresses. This is the standard memory organization as the 68000 and
 *is used in the 3DO machine.
 *
 ...
 *						'CCB '  |           CEL header
 *						'PDAT'  |           Pixel Data
 *						'PLUT'  |           PLUT Table (Pixel
 *Lookup Table) 'ANIM'  |           Animation Info 'VDL '  |           VDL list
 *						'CPYR'  |           Copyright
 *Notice 'DESC'  |           Text Description of image 'KWRD'  |           Text
 *Keywords associated with image 'CRDT'              Text credits associated
 *with image
 *
 *	{chunk_size}::= Unsigned 32 bit integer (includes size of chunk body
 *plus size of the chunk header).  Chunk_size is 8 plus size of the chunk_body.
 *					Chunks must be Quad byte alligned.	Chunks
 *will be padded with zeros to fill up the quad word alignment.
 *
 ****************************************************************************/
...
#define CHUNK_3DO CHAR4LITERAL ('3', 'D', 'O', ' ') /* '3DO ' wrapper chunk   \
                                                     */
#define CHUNK_IMAG                                                            \
  CHAR4LITERAL ('I', 'M', 'A', 'G') /* 'IMAG' the image control chunk */
...
#define CHUNK_PLUT                                                            \
  CHAR4LITERAL ('P', 'L', 'U', 'T') /* 'PLUT' pixel lookup table */
#define CHUNK_VDL CHAR4LITERAL ('V', 'D', 'L', ' ')  /* 'VDL ' VDL chunk */
#define CHUNK_CPYR CHAR4LITERAL ('C', 'P', 'Y', 'R') /* 'CPYR' copyright      \
                                                        text*/
#define CHUNK_DESC                                                            \
  CHAR4LITERAL ('D', 'E', 'S', 'C') /* 'DESC' description text */
#define CHUNK_KWRD CHAR4LITERAL ('K', 'W', 'R', 'D') /* 'KWRD' keyword text   \
                                                      */
#define CHUNK_CRDT CHAR4LITERAL ('C', 'R', 'D', 'T') /* 'CRDT' credits text   \
                                                      */
#define CHUNK_XTRA                                                            \
  CHAR4LITERAL ('X', 'T', 'R', 'A') /* 'XTRA' 3DO Animator creates these */
...
typedef struct PixelChunk
{
  ...
  int32 chunk_size; /*	size in bytes of chunk including chunk_size */
  uint8 pixels[1];  /*	pixel data (format depends upon description in the
                       imagehdr */
} PixelChunk;
...
typedef struct CCC
{
  ...
  int32 chunk_size; /* size in bytes of chunk including chunk_size */
  uint32
      ccbversion;   /* version number of the scob data structure.  0 for now */
 ...
 * 0) multi-CCB 	- each frame has its own CCB  and PDAT chunks [and
 * PLUT'S] 1) single-CCB	- there is one CCB	followed by frames
 * containing PDAT chunks [and PLUT'S]
 */
...
{
  uint32
      controlword;       //	VDL display control word (+ number of int32words in
                         //this entry - 4)
 ...
 *
 * CHUNKHDR_SIZE is the size of the common chunk fields (chunk_ID and
 * chunk_size).
 */
