 ...
 *
 *	History:
 *	03/20/94	ian		Moved #includes of stddef.h and stdio.h from conditional
 *						section to top of file so they always get included.  If
 *						folks come to rely on this file including stdio.h when
 *						debugging is on, it's rude to have their code suddenly
 *						break when they compile without debugging.
 *  01/12/94	ian		Added #include "types.h".
 *	12/19/93	ian		Added conditional verbose facility.
 *						Removed direct calls to printf and added calls to
 *						Diagnose1_()/Diagnose2_(), to allow error handling
 *						to be vectored through application-specified hook routine.
 ...
 *	 -	When passing printf-style format and args to one of the documented
 *		macros, a maximum of 10 args are allowed.
 *
 ...
 *	assigned meaning.  Applications can code CVERBOSE() macros which include
 *	a category number, and then turn all verbose messages for a category on
 *	or off using SetVerboseMask().
 *
 ...
 *	supplying a intercept routine via SetDiagnoseHook().  The intercept routine
 *	is supplied with all the information that will be reported; it can do a
 *	variety of things including alternate output handling, suppression of output,
 *	and custom system error messages.  See the Debug3DO.c source code for more
 *	info on writing output intercept routines.
 *
 ...
 *	 CVERBOSE(i,x)
 *
 *		Conditional verbose message: report the current task/module/file,
 *		followed by your message, only if the 'i' category of verbose messages
 *		has been enabled by setting its bit on in a call to SetVerboseMask().
 *		(Initially, all 31 categories of verbose messages are enabled.)
 *		The limitations, caveats, and usage of the 'x' parameter are the 
...
typedef char * (DiagnoseHookFunc)(int32 err, char *module, int32 line, char *sysErrMsg, char *fmt, va_list args);
...
#define	DIAGNOSE_NOSYSMSG	((int32)0x80000000)	// magic value means error not associated with system message
