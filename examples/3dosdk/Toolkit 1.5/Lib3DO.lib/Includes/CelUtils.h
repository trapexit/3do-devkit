 ...
 *					flag, and evaluate to NULL if the flag is set.
 *					Added ParseCel().  (I thought it was here already.)
 *					Added CreateSubrectCel() (new function).
 *	04/07/94  Ian	Added the CrossFadeCels() functions.
 *  03/11/94  Ian	Added #include "mem.h" so that the MEMTYPE_whatever
 *					constants are available to clients for LoadCel() calls.
 *	03/04/94  Ian	Renamed GetCelxRect() to xRectFromCel().
 *					Added IPointIsInxRect() functions.
 *	03/03/94  Ian	More new functions.  Also, fixed order of points in CQuad.
 *	03/02/94  Ian	Continued adding new functions.  Added some comments.
