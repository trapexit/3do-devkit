 ...
 *	File:		CelUtils.h
 *
 *	Contains:	Utilities for working with cels and related datatypes.
 *
 ...
 *	05/11/94  Ian	Corrected the math in the CCB field access macros.
 *	04/09/94  Ian	Modified the CEL_NEXTPTR() macro to check the CCB_LAST
 *					flag, and evaluate to NULL if the flag is
 *set. Added ParseCel().  (I thought it was here already.) Added
 *CreateSubrectCel() (new function).
 *	04/07/94  Ian	Added the CrossFadeCels() functions.
 *  03/11/94  Ian	Added #include "mem.h" so that the MEMTYPE_whatever
 *					constants are available to clients for
 *LoadCel() calls. 03/04/94  Ian	Renamed GetCelxRect() to
 *xRectFromCel(). Added IPointIsInxRect() functions.
 *	03/03/94  Ian	More new functions.  Also, fixed order of points in
 *CQuad. 03/02/94  Ian	Continued adding new functions.  Added some comments.
 *	02/26/94  Ian	New today.
 *
 *	Implementation notes:
 *
 *	Handy mnemonic hints:  parms to all these functions are generally in
 *the order that they appear in the function name.  For example,
 *SRectFromCrect() takes an SRect as the 1st parm, a CRect as the 2nd;
 *CenterCelOverIPoint() takes a cel as the 1st parm, an IPoint as the 2nd, and
 *so on.  Also, the order of parms is generally (destination, source) (like the
 *ANSI string library) when the whole concept of src/dst has any meaning to the
 *function.
 *
 *	For functions that convert between rectangle types, or calc the union
 *or intersection of two rectangles, the destination rect can be the same as
 *	either source rect.	 (Note that for intersections, the dest rect is
 *	modified even when there's no common area.)
 *
 *	Only the intersection functions and the cel creation functions can
 *return a NULL pointer.  The other functions return the same destination
 *pointer as you passed in, to allow inline calling syntax such as:
 *		CenterRectCelOverIPoint(cel, IPointFromIVal(&workpoint, myX,
 *myY)); When the intersection functions return NULL, that indicates that there
 ...
 *	Where flags are defined as 0x bitmapped values, they can be ORed
 *together.
 *--------------------------------------------------------------------------*/
...
#define IS_LASTCEL(ccb)                                                       \
  (((ccb)->ccb_Flags & CCB_LAST) || (ccb)->ccb_NextPtr == NULL)

#define CEL_NEXTPTR(ccb)                                                      \
  (CCB *)(((ccb)->ccb_Flags & CCB_LAST) ? NULL                                \
          : ((ccb)->ccb_Flags & CCB_NPABS)                                    \
              ? (ccb)->ccb_NextPtr                                            \
              : AddToPtr (&((ccb)->ccb_NextPtr),                              \
                          (int32)((ccb)->ccb_NextPtr) + 4))
#define CEL_PLUTPTR(ccb)                                                      \
  (uint16 *)(((ccb)->ccb_Flags & CCB_PPABS)                                   \
                 ? (ccb)->ccb_PLUTPtr                                         \
                 : AddToPtr (&((ccb)->ccb_PLUTPtr),                           \
                             (int32)((ccb)->ccb_PLUTPtr) + 4))
#define CEL_DATAPTR(ccb)                                                      \
  (CelData *)(((ccb)->ccb_Flags & CCB_SPABS)                                  \
                  ? (ccb)->ccb_SourcePtr                                      \
                  : AddToPtr (&((ccb)->ccb_SourcePtr),                        \
                              (int32)((ccb)->ccb_SourcePtr) + 4))

#define CEL_PRE0WORD(ccb)                                                     \
  (((ccb)->ccb_Flags & CCB_CCBPRE) ? (ccb)->ccb_PRE0                          \
                                   : ((uint32 *)CEL_DATAPTR (ccb))[0])
#define CEL_PRE1WORD(ccb)                                                     \
  (((ccb)->ccb_Flags & CCB_CCBPRE) ? (ccb)->ccb_PRE1                          \
                                   : ((uint32 *)CEL_DATAPTR (ccb))[1])
...
extern "C"
{
   ...
   *	All of these quietly cope with NULL pointers in fairly intelligent
   *ways.
   *--------------------------------------------------------------------------*/

  CCB *LastCelInList (CCB *list); // returns -> last cel in list
  CCB *ChainCelsAtTail (CCB *existingCels,
                        CCB *newCels); // returns -> new last cel in list
                                       // (LastCelInList(newcels))
  CCB *
  ChainCelsAtHead (CCB *existingCels,
                   CCB *newCels); // returns -> new first cel in list (newcels)
  ...
  CRect *CRectFromIVal (CRect *dst, int32 tlx, int32 tly, int32 brx,
                        int32 bry);
   ...
   *	including one returned by LoadCel().  UnloadCel() just calls
   *DeleteCel(). DeleteCel() can handle a NULL pointer, and always returns a
   *NULL pointer. DeleteCelList() walks the ccb_NextPtr list, calling
   *DeleteCel() on each.
   *--------------------------------------------------------------------------*/
  ...
  CCB *CreateCel (int32 width, int32 height, int32 bitsPerPixel, int32 options,
                  void *dataBuf);
  CCB *CreateBackdropCel (int32 width, int32 height, int32 color,
                          int32 opacityPercent);
  CCB *CreateLRFormCel (Item screenItem, SRect *subRect);
  ...
  void RenderCelFillRect (CCB *cel, int32 pixel, int32 x, int32 y, int32 w,
                          int32 h);
  void RenderCelOutlineRect (CCB *cel, int32 pixel, int32 x, int32 y, int32 w,
                             int32 h);
  ...
  int32 InitCel (CCB *cel, int32 width, int32 height, int32 bitsPerPixel,
                 int32 options);
