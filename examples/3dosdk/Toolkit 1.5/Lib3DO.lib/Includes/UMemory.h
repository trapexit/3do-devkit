 ...
 *				to the memory allocation scheme available on 3DO. 
 *
 ...
 *	09/09/93  Ian	Added DebugMemoryUsage().  It is the same as ReportMemoryUsage()
 *					except that it disappears when DEBUG compilation flag is set to 0.
 *	08/02/93  Ian	Added handling of page-aligned VRAM allocations.
 *					Added new standardized names for functions, (Malloc/Free)
 *					with macros to support the old names (NewPtr/FreePtr).
 *	05/17/93  akm	Adding rudimentary memory leak detection
 *	05/13/93  akm	Split shared memory code into seperate file Added Phil's
 *					AvailMemory code
 *	05/11/93  akm	Changed shared memory structures to work around possible
 *					compiler bug
 *	05/07/93  akm	Added new implementations for shared memory routines
 *	05/03/93  akm	Adding new headers for NewPtr & FreePtr.  Added stubs for shared
 *					memory utilities.  Moved definition of nil & kZeroedMemType.
 *					Now include mem.h automatically.
 *	5/3/93    akm	First Writing
 *
