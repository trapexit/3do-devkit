 ...
 *						changed; 3DOFontWriter now puts the width of the
 *						widest character here.  This indicated by the 
 *						fh_version field having a value greater than zero.
 *						We retrofit this capability in version-0 files by
 *						scanning for the widest char at parse time.
 *
 *	  06/04/94  Ian		Changed memtypebits for FontDescriptor alloc from
 *						MEMTYPE_ANY to MEMTYPE_DRAM|MEMTYPE_FILL|0.
 *	
...
FontDescriptor * ParseFont(void *fontImage)
{
	...
	if ((fd = (FontDescriptor *)Malloc(sizeof(FontDescriptor), MEMTYPE_DRAM|MEMTYPE_FILL|0)) == NULL) {
