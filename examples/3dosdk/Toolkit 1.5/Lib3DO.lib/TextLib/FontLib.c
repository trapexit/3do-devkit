 ...
 *	File:		FontLib.c
 *
 *	Contains:	Implementation for low-level handling of 3DO font
 *files. This file, along with some assembler code, implements low-level font
 *				handling.  The implementation allows you to load and
 *unload font files, parse a font file that has been pre-loaded into memory by
 *some other means, obtain information about the font file and the characters
 *within it, and unpack/blit individual characters from the font into another
 *				memory location.
 *
 ...
 *
 *	  06/22/94  Ian		The meaning of the fd_charWidth field has
 *						changed; 3DOFontWriter now puts the width
 *of the widest character here.  This indicated by the fh_version field having
 *a value greater than zero. We retrofit this capability in version-0 files by
 *						scanning for the widest char at parse
 *time.
 *
 *	  06/04/94  Ian		Changed memtypebits for FontDescriptor alloc
 *from MEMTYPE_ANY to MEMTYPE_DRAM|MEMTYPE_FILL|0.
 *
 *	  05/08/94	Ian		Added GetFontCharWidest() for finding
 *the widest character from a set of characters you specify. Also added
 *GetFontStringWidth() to get the width of a string as rendered in a given
 *font.
 *
 *	  05/07/94	Ian		Added support for 5bpp TWCOLOR fonts.
 *This new type of font is used for outlined and shadowed font effects.
 *
 *	  03/16/94  Ian		Changed %d to %ld in an error message to clean
 *up a minor compiler warning.
 *
...
ParseFont (void *fontImage)
{
  ...
  if ((fd = (FontDescriptor *)Malloc (sizeof (FontDescriptor),
