 ...
 *	File:		taTextLib.c
 *
 *	Contains:	The TagArgs interface to the TextLib library.
 *
...
static void
init_tags (ParsedTags *pt, int32 init_for)
{
...
static Err
parse_tags (ParsedTags *pt, TagArg *args, int32 parse_for)
{
  Err err = -1;
  ArgValue val;
  ...
  if (IsValid (pt, TCEL_TAG_UPDATE_TEXT_ARGS)
      && !IsValid (pt, TCEL_TAG_UPDATE_TEXT_STRING))
    {
      DIAGNOSE (("TCEL_TAG_UPDATE_TEXT_STRING must be specified when "
                 "TCEL_TAG_UPDATE_TEXT_ARGS is present\n"));
      goto ERROR_EXIT;
    }

  if (IsValid (pt, TCEL_TAG_FORMAT_BUFFER)
      && !IsValid (pt, TCEL_TAG_FORMAT_BUFFER_SIZE)
      && pt->formatBuffer != NULL)
    {
      DIAGNOSE (("TCEL_TAG_FORMAT_BUFFER_SIZE must be specified when "
                 "TCEL_TAG_FORMAT_BUFFER is non-NULL\n"));
      goto ERROR_EXIT;
    }
...
static Err
apply_tags (TextCel *tCel, ParsedTags *pt)
{
  Err err = 0;
  ...
  if (IsValid (pt, TCEL_TAG_FORMAT_BUFFER)
      || IsValid (pt, TCEL_TAG_FORMAT_BUFFER_SIZE))
    {
      if ((err = SetTextCelFormatBuffer (tCel, pt->formatBuffer,
                                         pt->formatBufferSize))
          < 0)
        {
          goto ERROR_EXIT;
  ...
  if (IsValid (pt, TCEL_TAG_UPDATE_TEXT_STRING))
    {
      err = vUpdateTextInCel (tCel, pt->replaceExisting, pt->textFormat,
                              pt->textArgs);
    }
...
taCreateTextCel (TagArg *args)
{
  ...
  if ((tCel = CreateTextCel (pt.font, pt.formatFlags, pt.width, pt.height))
      == NULL)
    {
      err = -1;
      goto ERROR_EXIT;
...
Err
taModifyTextCel (TextCel *tCel, TagArg *args)
{
  Err err;
  ParsedTags pt;
