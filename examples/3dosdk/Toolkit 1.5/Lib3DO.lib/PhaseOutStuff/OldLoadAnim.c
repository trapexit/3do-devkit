...
/* allocated.  If this is exceeded while reading the ANIM then a block with
 * space for */
/* N_FRAMES_PER_CHUNK additional AnimFrames is allocated, the previously
 * computed Animframes */
/* are copied into this new space, and the old full AniFrame buffer space is
 * deallocated */
 ...
 * OldLoadAnim.c
 *
 *	This exists only to help folks who've become wedded to the old
 *LoadAnim() function. You should convert to the new LoadAnim()/UnloadAnim() as
 *soon as you can.
 ***************************************************************************************/
...
ReAllocateAnim (ANIM *pAnim)
{
  ...
  if ((newFrame
       = (AnimFrame *)ALLOCMEM (newn * sizeof (AnimFrame), MEMTYPE_CEL))
      == NULL)
    return NULL;
  for (i = 0; i < oldn; i++)
...
int32
OldLoadAnim (char *name, int32 **buffer, ANIM *pAnim)
{
  int32 result, buffSize, i;
   ...
   */
  if ((aFrame = (AnimFrame *)ALLOCMEM (N_FRAMES_PER_CHUNK * sizeof (AnimFrame),
                                       MEMTYPE_CEL))
      == NULL)
    return NOMEM;
  ...
  while ((pChunk = GetChunk (&chunk_ID, &tempBuf, &tempSize)) != NULL)
    {
      switch (chunk_ID)
        ...
        case CHUNK_CCB:
          ...
          lastCCB->ccb_XPos = lastCCB->ccb_YPos = 0;
          lastCCB->ccb_Flags
