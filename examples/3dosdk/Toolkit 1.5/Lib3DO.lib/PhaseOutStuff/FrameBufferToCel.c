/*****************************************************************************
 *	File:			FrameBufferToCel.c
 *
 *	Contains:		Routine to copy frame buffer contents to cel data
 *buffer.
 *
 ...
 *
 *	This routine uses the ARM chip to actually copy-and-convert the LRForm
 *	data from the screen to the cel's data buffer.  Slow.  You might want
 *	to consider the CreateLRFormCel() function.
 ****************************************************************************/
...
FrameBufferToCel (Item iScreen, CCB *cel)
{
  ...
  /* Screen is actually 640x120 so we need to adjust the top and height to
   * reflect that */
  ...
  pBitMap
      += top * (bitmap->bm_Width); /* We need to advance by top scan lines */
      ...
      while (x < left + w)
        {
          ...
          srcPixel = (*(pBitMap + x) & 0xFFFF0000)
                     | ((*(pBitMap + secondPix) & 0xFFFF0000) >> 16);
