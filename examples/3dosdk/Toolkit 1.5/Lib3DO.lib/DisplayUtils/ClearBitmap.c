 ...
 *					Modified to add an optional Bitmap* parm.  If performance
 *					is an issue when using this function, passing a bitmap ptr
 *					instead of an item number will save a bunch of cycles.
 *					Removed GrafBase references.
 * 	04/06/94  Ian 	New today.
 *
 ...
 *	passed.  For best performance, pass an ioreq (obtained via GetVRAMIOReq())
 *	and a non-NULL Bitmap* parm.
 ****************************************************************************/
...
Err ClearBitmap(Item ioreq, Item screen, Bitmap *bm, int32 value)
{
	...
	if ((err = SetVRAMPages(theIOReq, bm->bm_Buffer, value, numPages, 0xFFFFFFFF)) < 0) {
