 ...
 *	File:			OpenGraphics.c
 *
 *	Contains:		Routine to open the graphics folio and create
 *screens.
 *
 ...
 *					Did a general cleanup of
 *OpenGraphics(). Added CloseGraphics(). <unknown>		New.
 *
 *	Implementation notes:
 *
 *	Ideally, we would have the OS allocate the bitmaps by setting a zero
 *value for CSG_TAG_SPORTBITS and letting the OS make sure we get page-aligned
 ...
 *	what the old code used to do with GETBANKBITS(GrafBase->gf_ZeroPage),
 *so we haven't really changed the functionality at all.  If graphics folio
...
#define kVRAMBANKBITS                                                         \
  (MEMTYPE_VRAM | MEMTYPE_STARTPAGE | MEMTYPE_BANKSELECT | MEMTYPE_BANK1)
...
void
CloseGraphics (ScreenContext *sc)
{
  Item screenGroup;
  Screen *screen;
 ...
 *	hurt if it's already open) and set up bitmaps, screens, and a screen
 *group.
 ****************************************************************************/
...
OpenGraphics (ScreenContext *sc, int nScreens)
{
  ...
  sc->sc_nFrameBufferPages = (width * 2 * height + pageSize - 1) / pageSize;
