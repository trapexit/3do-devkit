 ...
 * File:			CenterPoint.c
 *
 * Contains:		Center points within larger entities.
 *
 * Copyright (c) 1994 The 3DO Company. All Rights Reserved.
 *
 * History:
 *	07/23/94  Ian	Removed references to GrafBase fields.
 * 	02/27/94  Ian 	New today.
 *
 * Implementation notes:
 *
 *	Right now, the display is assumed to be 320x240.  When the new
 *OpenDisplay() library routine comes online it will provide proper display
 *sizes we can refer to.
 *
 *	Even though two static variables show up in this module (to get a
 *slight performance boost when obtaining the screen center point), these
 *functions are still multithreaded-safe.  Even if it should happen that two
 *threads are executing the init code simultaneously, both would be trying to
 *set the static variables to the same values anyway, so there's no problem
 *with them stepping on each others' updates.
 *
