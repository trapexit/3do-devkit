 ...
 *	File:			CreateCel.c
 *
 *	Contains:		Routine to create a cel using the specified
 *attributes. The cel will have a data buffer and PLUT (if needed) attached to
 *it.  If you don't specify a data buffer pointer, an appropriately-sized
 *buffer is allocated for you.
 *
 *					If you need a simple solid-color cel and the
 *pixels in it will never change, the CreateBackdropCel() function will create
 *a cel that uses less memory.
 *
 ...
 *	02/26/94  Ian	The preamble word 1 UNCLSB flag (PRE1_TLLSB_PDC0) is
 *now set for all cels, coded and uncoded.  The manual turns out to be wrong on
 *this: it definitely applies to both types of cels, not just uncoded. 02/25/94
 *Ian	Cels only get a PLUT allocated now if they need it.  If one is needed,
 *it is allocated along with the CCB structure allocation instead of with a
 *separate Malloc call. 02/24/94  Ian 	New today.
 *
...
CreateCel (int32 w, int32 h, int32 bpp, int32 options, void *databuf)
{
  ...
  if (databuf == NULL)
    {
      if ((databuf = Malloc (bufferBytes, MEMTYPE_CEL | MEMTYPE_FILL | 0))
          == NULL)
        {
