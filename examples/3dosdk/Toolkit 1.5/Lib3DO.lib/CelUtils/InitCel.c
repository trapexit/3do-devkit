 ...
 *	File:			InitCel.c
 *
 *	Contains:		Routine to init a cel using the specified
 *attributes. Copyright:	(c) 1994 The 3DO Company. 	All Rights
 *Reserved.
 *
 *	History:
 *	07/26/94  Ian 	New today.
 *
 ...
 *etc.
 *
 *	At the very least you have to attach a cel data buffer to the
 *ccb_SourcePtr field and, if the cel is coded, a PLUT to the ccb_PLUTPtr,
...
int32
InitCel (CCB *cel, int32 w, int32 h, int32 bpp, int32 options)
{
  int32 rowBytes;
  int32 rowWOFFSET;
  int32 wPRE;
  ...
  cel->ccb_Flags = CCB_LAST | CCB_NPABS | CCB_SPABS | CCB_PPABS | CCB_LDSIZE
  ...
  switch (bpp)
    ...
    default:
      DIAGNOSE (
          ("Unsupported bits-per-pixel value %ld; 16-bpp assumed\n", bpp));
      /* fall thru */
    case 16:
