/*****************************************************************************
 *	File:			CreateBackdropCel.c
 *
 *	Contains:		Creates a rectangular cel containing a single
 *color.
 *
 *	Copyright (c) 1993 The 3DO Company. All Rights Reserved.
 *
 *	History:
 *	04/02/94  Ian	We now allow 0% opacity.  This lets you create a sort
 *					of 'virtual' cel that doesn't display anything
 *(the CCB_SKIP flag gets set on for 0% cels), but it can serve as a parent to
 *a list of cels for controlling position.
 *	02/24/94  Ian	Updated to use new CreatCel() and MapRectCel()
 *functions, instead of having cel creation logic duplicated in several
 *					modules.
 *
 *	02/06/94  Ian 	New today.
 *
 ...
 *
 *	The cel is created with a source data size of 1x1 pixel, and the
 *HDX/VDY fields in the CCB are used to project that one pixel to the required
 ...
 *	'nearest' type rounding, but it works well enough and avoids the need
 *for lots of special-case logic for values less than 13% in the code below.)
 ****************************************************************************/
  ...
  if (opacityPct == 0)
  ...
  else
    {
      ...
      if (scaleMul == 8)
        {
          scaledColor = color;
          pCel->ccb_PIXC = 0x1F001F00; // opaque PIXC for 100% opacity
        }
      else
        {
          ...
          pCel->ccb_PIXC = 0x000083C0 | ((scaleMul - 1) << 10);
        }
    }
