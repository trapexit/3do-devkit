 ...
 *	File:			ParseAnim.c
 *
 *	Contains:	Routine to parse an anim file previously loaded into
 *				memory, creating an ANIM structure with attached
 *AnimFrames.
 *
...
static AnimFrame *
ReallocFrames (ANIM *pAnim, int32 newFrameCount, uint32 memTypeBits)
{
  AnimFrame *newFrames;
  int32 oldFrameCount;
  int32 memNeeded;
  ...
  newFrames
      = (AnimFrame *)Malloc (memNeeded, (memTypeBits | MEMTYPE_FILL | 0x00));
...
/*----------------------------------------------------------------------------
 * ParseAnim
 *	Turn a buffer full of anim-ish data into an ANIM and attached
 *AnimFrames. Use UnloadAnim() to release the resources acquired in this
 *process. (UnloadAnim works both after LoadAnim and ParseAnim, with the only
 *	difference being that with ParseAnim you allocated the input buffer,
 *	so you have to free it yourself.)
 *--------------------------------------------------------------------------*/

ANIM *
ParseAnim (void *inBuf, int32 inBufSize, uint32 memTypeBits)
{
  ANIM *pAnim;
  AnimFrame *aFrame;
  int32 allocedFrames;
  ...
  if (inBufSize < sizeof (CCB))
    {
      DIAGNOSE (
          ("Buffer size is less than sizeof(CCB); can't be an Anim file\n"));
      return NULL;
    }
#endif

  if (NULL
      == (pAnim = (ANIM *)Malloc (sizeof (*pAnim),
                                  (memTypeBits | MEMTYPE_FILL | 0x00))))
    {
