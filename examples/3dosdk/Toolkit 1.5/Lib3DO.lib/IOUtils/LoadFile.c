 ...
 *					it and LoadFile() are serviced by a single internal routine.
 ...
 *					as an error is up to the caller; we don't DIAGNOSE() it.
 *	02/18/94  Ian	Reworked error detection logic.  The old way was failing
 *					to return bad status if the open worked but the read failed.
 *	09/06/93  JAY	change free to Free in two error paths in LoadFile.
...
ERROR_EXIT:
	...
	if (ioreq > 0) {				// if we obtained an IOReq, 
		DeleteItem(ioreq);			// delete it.
	}
	
	if (err < 0) {					// if exiting with bad status
		if (internalBuffer) {		// and we acquired a file buffer,
