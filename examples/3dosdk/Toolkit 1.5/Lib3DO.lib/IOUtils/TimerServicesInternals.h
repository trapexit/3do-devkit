/*****************************************************************************
 *	File:			TimerServicesInternals.h
 *
 *	Contains:		Implementation-private header file for timer
 *services.
 *
 ...
 *
 *	History:
 *	06/01/94  Ian	Moved TSFLAG_ constants and TimerControl structure into
 *					this header so that they could be shared by the
 *services thread and the API routines.
 *
...
#ifndef TIMERINTERNALS_H
#define TIMERINTERNALS_H
 ...
 *	Used for internal tracking of timer requests on the service thread
 *side. Also used in read-only fashion from the API side. If you add anything
 *to this structure, go to acquire_timer_control() in the service thread and
 *add code there to zero/init the new field(s). The ownerTask field is used by
 *the service thread to validate requests in pretty much the same way as the OS
 *validates item ownership.  The other ownerXXXX fields are values the service
 *thread remembers as an assist for the API routines on the owner task's side
 *of the world.
 *--------------------------------------------------------------------------*/
...
TimerHandle timer_services_request_ (int32 notify, uint32 seconds,
                                     uint32 useconds, uint32 userdata1,
                                     uint32 userdata2, int32 msgid,
                                     int32 requestdata);
