 ...
 *	File:		SaveFile.c
 *
 * 	Contains:	Routine dump an entire buffer into a file, completely
 *replacing the current contents of the file (if any).  The file is created if
 ...
 *
 *	This currently works only for nvram files, since that's the only
 *writable filesystem (WriteMacFile notwithstanding).
 *
 *	The fileItem returned from CreateFile() cannot be DeleteItem'd until
 *after the file is closed.  This appears to be due to an OS bug; if you delete
 ...
 *	it when the file is closed.  All of this happens only if the
 *CreateFile() call is made; if the file already exists and CreateFile() isn't
 *necessary, then the OS still creates a new file item internally, but it
 *properly releases it when you close the file.  Verra strange mon.
 *
 *	The CMD_ALLOCBLOCKS device command adds the specified number of blocks
 *to the file as it exists already.  The docs lead you to believe that it will
 *	make the file grow to the specified number of blocks, but it really
 *adds the specified number of blocks.
 *******************************************************************************************/
 ...
 *	other writeable filesystems come along this should still work just
 *fine).
 *
 ...
 *	relatively expensive operation, the extrabytes parm lets you grow a
 *file a little at a time while doing the expensive part of the growth only
...
Err
SaveFile (char *filename, void *buffer, int32 bufsize, int32 extrabytes)
{
  Err err;
  Item fileItem;
  BlockFile thefile;
