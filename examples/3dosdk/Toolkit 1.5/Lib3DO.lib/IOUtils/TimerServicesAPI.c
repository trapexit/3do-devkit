 ...
 *
 *	Contains:		Routines to send service requests to TimerServices.
 *
 ...
 *	you specify new timer values.  If the timer values are zero, it effictiely
 *	suspends the timer.
 *--------------------------------------------------------------------------*/

Err	 TimerReset(TimerHandle thandle, uint32 seconds_or_fields, uint32 useconds)
{
	return timer_services_request_(0, seconds_or_fields, useconds, 0, 0, TSREQUEST_MSGID_RESET, thandle);
}
