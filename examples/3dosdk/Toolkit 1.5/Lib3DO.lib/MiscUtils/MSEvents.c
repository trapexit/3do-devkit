 ...
 *	File:			MSEvent.c
 *
 *	Contains:		Routines to wait for and handle multiple messages and
 ...
 *					and when the handler is dispatched the msgItem field
 *					holds a mask of which signal(s) triggered the event.
 *					If this feature is used, a signal handler must handle
 *					all signals found in the msgItem field on a single call.
 *	06/13/94  Ian	Changed setup and cleanup routines so that clients can
 *					pre-allocate their own msgports (indicated by a non-zero
 *					port item in the data array) and we just go with it (and,
 *					of course, we don't try to de-alloc it at cleanup time).
 *	06/08/94  Ian	Fixed a bug in message dispatching: it now loops on a 
 *					message port until all messages are cleared from it 
 *					before moving on to the next signal/port.
...
MSEventHandle SetupMSEvents(MSEventData *events, int32 numEvents, int32 reserved)
{
	...
	if ((msctl = (MSControl *)Malloc(sizeof(*msctl), MEMTYPE_DRAM|MEMTYPE_FILL|0)) == NULL) {
		rv = NOMEM;
