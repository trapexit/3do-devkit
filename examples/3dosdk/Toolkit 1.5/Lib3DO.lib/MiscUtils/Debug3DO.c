 ...
 *					owned by the main parent task even if the first
 *time through this code is run in the context of a thread. (Pity it doesn't
 *work due to SetItemOwner() returning a NOTSUPPORTED error.  ::sigh::)
 *	03/27/94  Ian	Removed the ugly ArgsByValue hack and added vprintf()
 *					prototype.  Turns out vprintf() is present in
 *clib.lib but its prototype is missing from stdio.h.
 *
...
static char *default_diagnose_hook (int32 err, char *module, int32 line,
 ...
 *	The default output intercept routine, when the caller doesn't specify
 *one.
 *
 *	An intercept routine is expected to do these things:
 *	 - 	Return NULL to suppress printf() output by the standard report
 *routine. -	Return non-NULL: a pointer to a character string which appears
 *as the 'title' of the task/module/line output string.  (A pointer to an empty
 *string ("") can be used for 'titleless' unsuppressed output.) -	Supply
 *an alternate 'system error message' string, by placing up to
 *		DIAGNOSE_SYSMSGBUFSIZE characters into the sysErrMsg buffer.
 *(Placing a zero at the start of the buffer suppresses system message output
 *by the standard routines, meaning only the task/module/line id output string
 *and any caller-specified formatted output would appear.) -	Any other
 *things you want to do with the output.  For example, you can selectively
 *suppress output, log messages to a file (with or without suppression of
 *output by the standard report routine), or even force termination of the task
 *if that works for your needs.
 *	 -  If the intercept does not terminate the task, it is expected to
 *		return.  (IE, it shouldn't longjmp() out or anything.)  Also,
 *the intercept routine must not do anything that would cause re-entrance into
 *the diagnose routines.  Both of these limitations are to prevent deadlocks on
 *the semaphore used to sync the phase 1 & 2 processing.
 *
 *	This default intercept routine just supplies a title of ERROR or
 *VERBOSE, keyed off the error code passed in.
 ****************************************************************************/

static char *
default_diagnose_hook (int32 err, char *module, int32 line, char *sysErrMsg,
...
#if 0 // right now, SetItemOwner() returns NOTSUPPORTED.  Maybe the OS guys can
      // fix this.
  {
	Err		err;
	Task *	curTask;
 ...
 *	can write to static variables and ensure that output from multiple
 *threads doesn't get all mixed together.  Save the parameters in the static
 *vars for use by the phase-2 routine.
 ****************************************************************************/

void
Diagnose1_ (int32 err, char *moduleName, int32 moduleLine)
{
...
Diagnose2_ (char *fmt, ...)
{
  ...
  msgTitle = diagnose_hook (last_error, module_name, module_line, sysErrMsg,
                            fmt, args);
  if (msgTitle != NULL)
    {
      printf ("%s %s %s(%ld): ", msgTitle, CURRENTTASK->t.n_Name, module_name,
              module_line);
      vprintf (fmt, args);
      if (sysErrMsg[0] != 0)
        {
...
uint32
SetVerboseMask (uint32 maskBits)
{
  uint32 oldMask;
...
void
SetDiagnoseHook (DiagnoseHookFunc *newDiagnoseHookFunc)
{
