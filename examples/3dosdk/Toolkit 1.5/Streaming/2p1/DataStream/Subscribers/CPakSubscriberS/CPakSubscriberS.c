 ...
 *						Fixed un-terminated comment in IsTimeForNextFrame.
 *						Replaced printf with ERR macro defined in Debug3DO.h.
 *						Fix - Bug 2164: IsTimeForNextFrame fixed to make looping to
 *						beginning of stream work.
 * 	  8/17/94	dtc		Version 2.0.1d3
 *						Fixed IsTimeForNextFrame to delivery the first frame on time.
 * 	  7/29/94	dtc		Version 2.0.1d2
 *						Replaced NULL with "CPakSubsSThread" as thread name when creating
 *						CPakSubscriberS thread in NewCPakSubscriber.
 *						Used DataStream version number of 2.0.1dx to be consistent.
 *	2/16/94		lla		Added catchup strategy parameter:
 *							skip/decode difference frames to make up time
 *  2/2/94		lla		Distinguishes between difference frames and key frames for ketchup
 *	1/24/94		jb		Version 1.3
 *						Added GetStepCPakCel() and DrawStepCPakToBuffer() stepframe support.
 *	11/23/93	jb		Version 1.2
 *						Added CloseCPakSubscriber() to clean up resources allocated by the
 *						InitCPakSubscriber() routine.
 *	10/19/93	jb		Version 1.1
 *						Switch to using UMemory.c routines for memory allocation
 *	10/14/93	jb		Add code identified by FREE_DATA_WHEN_TIME_HAS_PASSED to optionally
 *						free streamer buffers automatically once a buffer's time has expired.
 *						Expiration is determined by buffer time+duration being less than
 *						the current stream clock.
 *	8/25/93		jb		Force image width and height to be a multiple of 4 in AllocMovieBuff().
 *	6/22/93		njc		Changed subscriber structure from one msgPort for every channel to
 *						a simple linked list with a semaphore to arbitrate access to the list.
 *						In addition, a freeList is maintained that contains chunks that are
 *						to be returned to the Data Streaming Library.
...
/****************************/
Boolean	PollCPakChunk(	CPakContextPtr		ctx,
						CPakRecPtr			cpRecPtr,
...
static void		GetDstStuff(long *baseAddr, long *rowBytes, long width, long height, 
						Bitmap *bitmap );
static CCB*		_GetCPakCel( CPakContextPtr ctx, CPakRecPtr cpRecPtr, 
						Boolean fSyncToStreamClock );
static void 	_DrawCPakToBuffer( CPakContextPtr ctx, CPakRecPtr cpRecPtr, Bitmap *bitmap,
						long catsup_strategy, Boolean fSyncToStreamClock );
...
/*******************************************************************************************
 * Allocate a buffer for the Cinepak decompressor. The decompressor will dump data into this
 * buffer in LR form. The Cel Engine can draw this format (on RED systems) to the frame buf.
 *******************************************************************************************/
#define	FORCE_MULTIPLE_OF_4( x )	((x + 3) & ~3)
int32	AllocMovieBuff(ImageDesc *imagePtr, CinePakHeaderPtr pCPHeader)
{
ubyte *	movieFrameBuff;
	...
	movieFrameBuff = (ubyte*) NewPtr(imagePtr->rowBytes * imagePtr->height, MEMTYPE_ANY | (MEMTYPE_FILL | 0x00));
	
	if ( movieFrameBuff == NULL )	
	{
		ERR(( "Failed to allocate movie image buffer.\n"));
		return(-1);
	}
 ...
 *******************************************************************************************/
void SetLRFormCel(ImageDesc *imagePtr, CCB* ccb)
{
	...
	ccb->ccb_Flags		= CCB_LDSIZE		/* Load hdx-y and vdx-y from CCB */
						...
						| CCB_YOXY
						| CCB_LCE			/* Lock the corner engines together */
	...
	ccb->ccb_PRE0	= (((ccb->ccb_Height >> 1)-1)<<PRE0_VCNT_SHIFT) +	/* number of vertical data lines in sprite data -1 */
	 ...
	 */
	ccb->ccb_PRE1	= ((imagePtr->width-2)<<16) +				/* offset (in words) from line start to next line start. */ 
					  (ccb->ccb_Width-1) +						/* number of horizontal pixels to render -1 */
					  PRE1_LRFORM;
...
/*******************************************************************************************
 * Poll for the next msg in this streamed animation and return true if a kStreamOpData 
 * msg is present. Pass a ptr to the chunk back in pChunkDataPtr.
 *******************************************************************************************/
Boolean	PollCPakChunk(	CPakContextPtr		ctx,
						CPakRecPtr			cpRecPtr,
						SubsChunkDataPtr	*pChunkDataPtr,
...
/*******************************************************************************************
 * Allocate and initialize a streamed CinePak state record. GetCPakCel will return NULL until
 * the Header and first frame have been received. When the First frame comes in, the
...
/*******************************************************************************************
 *	Return all CPak msgs to the CPakSubscriber back end. The CPak subscriber will return one
...
/*******************************************************************************************
 * Return true if time for new frame or no frames read yet. Pass back the frame to get.
 *******************************************************************************************/
Boolean	IsTimeForNextFrame (CPakRecPtr cpRecPtr)
	{
	...
	if ( cpRecPtr->cpHeader.time > curTime )
		return false;
	...
	if (cpRecPtr->lastCurTime > curTime)
		{
		cpRecPtr->lastCurTime = curFramePtr->time;
		return true;
		}
	 ...
	 */
	return (curTime > (curFramePtr->time + curFramePtr->duration) ) ? true : false;
	}

Boolean	TimeToCatchUp (CPakRecPtr cpRecPtr)
	{
	CinePakFramePtr		curFramePtr;
	uint32 				curTime;
	 ...
	 * than the current time, then we need a new frame, so return true.
	 */
	return (curTime > (curFramePtr->time + TIME_TO_CATCH_UP * (curFramePtr->duration)) ) ? true : false;
	}


/*******************************************************************************************
 * Send a signal to the CPak Subscriber telling it to Reply all subMsgs in the freeList
 * back to the DSL. This is necessary to release physical blocks for re-use.
 *******************************************************************************************/
...
/*******************************************************************************************
 * Wrapper routine to call the utility version of the code that gets a decoded frame
 * returned as a CCB, while observing stream clock synchronization.
 *******************************************************************************************/
CCB*	GetCPakCel( CPakContextPtr ctx, CPakRecPtr cpRecPtr)
	{
	return _GetCPakCel( ctx, cpRecPtr, false );
	}


/*******************************************************************************************
 * Wrapper routine to call the utility version of the code that gets a decoded frame
 * returned as a CCB, while implementing stepframe behavior (ignores the stream clock
 * and just gets the very next frame).
 *******************************************************************************************/
CCB*	GetStepCPakCel( CPakContextPtr ctx, CPakRecPtr cpRecPtr )
	{
	return _GetCPakCel( ctx, cpRecPtr, true );
	}


/*******************************************************************************************
 * Get the next compressed frame data in the streamed CinePak film. GetCPakCel will return NULL 
...
static CCB*		_GetCPakCel( CPakContextPtr ctx, CPakRecPtr cpRecPtr, 
	 ...
	 */
	while ( fStepFrame || IsTimeForNextFrame( cpRecPtr ) )
		{
		if (PollCPakChunk (ctx, cpRecPtr, &pChunkData, &subMsg))
			...
			switch (pChunkData->subChunkType)
				...
				case FHDR_CHUNK_TYPE:	/* CPak Header Chunk */
					...
					/* Make a copy of the header so we can free up the block that contains it.
					 */
					memcpy( &(cpRecPtr->cpHeader), pChunkData, sizeof(CinePakHeader) );
					
					/* Create the LR Form data buffer that we will fill when decompressing.
					 */
				...
				case KEY_FRME_CHUNK_TYPE:	/* CPak Frame Chunk */
				case DIF_FRME_CHUNK_TYPE:	/* CPak Frame Chunk */
					cpRecPtr->curFramePtr = (CinePakFramePtr)pChunkData;

					if (cpRecPtr->curSubMsg)
						DoFreeChunk (ctx, cpRecPtr->curSubMsg);

					PreDecompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData));
					Decompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData),
				...
				default:
					ERR( ("GetCPakCel --- Unknown Chunk Type: '%.4s'", &(pChunkData->subChunkType) ) );
					break;
				}
			}
		else
	 ...
	 */
	if ( cpRecPtr->curFramePtr != NULL )
		{
		if (DSGetClock( cpRecPtr->streamCBPtr, &curTime ) >= 0 )
			{
			if ( curTime > (cpRecPtr->curFramePtr->time + cpRecPtr->curFramePtr->duration) )
				{
				if (cpRecPtr->curSubMsg)
					{
					DoFreeChunk (ctx, cpRecPtr->curSubMsg);
...
/*******************************************************************************************
 * Wrapper routine to call the utility version of the code that draws a frame directly
 * to a frame buffer, while observing clock synchronization.
 *******************************************************************************************/
void DrawCPakToBuffer( CPakContextPtr ctx, CPakRecPtr cpRecPtr, Bitmap *bitmap, long catsup_strategy )
	{
	_DrawCPakToBuffer( ctx, cpRecPtr, bitmap, catsup_strategy, false );
	}


/*******************************************************************************************
 * Wrapper routine to call the utility version of the code that draws a frame directly
 * to a frame buffer, while implementing stepframe behavior (ignores the stream clock
 * and just gets the very next frame).
 *******************************************************************************************/
void	DrawStepCPakToBuffer	( CPakContextPtr ctx, CPakRecPtr cpRecPtr, Bitmap *bitmap )
	{
	_DrawCPakToBuffer( ctx, cpRecPtr, bitmap, 1, true );
	}


/*******************************************************************************************
 * Get the next compressed frame data in the streamed CinePak film. DrawCPakToBuffer will return NULL
 * until the Header and first frame have been received. When the First frame comes in, the
 * current audio time will be read and saved to determine which frame to display each call.
 * 
 * lla 2/14/94
 *
 * catsup_strategy parameter specifies the method to be used to catch up if the video has fallen
 * behind.  BEST_METHOD is a placeholder in case any one gets smart and figures one out.
 *
...
static void _DrawCPakToBuffer( CPakContextPtr ctx, CPakRecPtr cpRecPtr, Bitmap *bitmap,
	...
	pImageData	= &(pChannel->imageData);
#if 0
if ( fcatsup_strategy == SKIP_DIFF_FRAMES )
	{
	/* Check to see if we need to skip to the next key frame
	 *		Note: I am not sure if a tri-state variable is necessary or useful here??
	 *
	 *	catchUp = 0			just started
	 *	catchUp = 1			video is behind the audio
	 *	catchUp = 2			video is in sync or ahead of the audio
	 */
	if ( TimeToCatchUp( cpRecPtr ) ) catchUp = 1;
	while ( !fStepFrame && ( catchUp == 1 ) )
		{
		if (PollCPakChunk (ctx, cpRecPtr, &pChunkData, &subMsg))
			{
			switch (pChunkData->subChunkType)
				...
				case FHDR_CHUNK_TYPE:	/* CPak Header Chunk */
					...
					/* These are used by GetDstStuff to calc the offset into the Frame Buffer */
				...
				case KEY_FRME_CHUNK_TYPE:	/* CPak Key Frame Chunk */
					
					cpRecPtr->curFramePtr = (CinePakFramePtr)pChunkData;
					if (cpRecPtr->curSubMsg)
						DoFreeChunk (ctx, cpRecPtr->curSubMsg);

					GetDstStuff(&baseAddr,&rowBytes,pImageData->width,pImageData->height, bitmap);
					PreDecompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData));
					Decompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData),
					...
					if ( !TimeToCatchUp( cpRecPtr ) ) catchUp = 2;
					break;
				
				case DIF_FRME_CHUNK_TYPE:	/* CPak Difference Frame Chunk */
					cpRecPtr->curFramePtr = (CinePakFramePtr)pChunkData;
...
//------------------------------------------------------------------------------------------------//
					...
					/*
					 *  skip all difference frames until we have caught up 
					 *	or decode just as fast as our little CPU will permit
					 */	
			 
					if ( catsup_strategy == DECODE_DIFF_FRAMES )
						{
						GetDstStuff(&baseAddr,&rowBytes,pImageData->width,pImageData->height, bitmap);
						PreDecompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData));
						Decompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData),
				...
				default:
					ERR( ("_DrawCPakToBuffer --- Unknown Chunk Type: '%.4s'", &(pChunkData->subChunkType) ) );
					break;
				}	// switch
			}	// if
		else
	 ...
	 */
	framesCaught = 0;
	while ( (fStepFrame || IsTimeForNextFrame( cpRecPtr )) && ( framesCaught <10 ) )
		{
		if (PollCPakChunk (ctx, cpRecPtr, &pChunkData, &subMsg))
			{
			switch (pChunkData->subChunkType)
...
/*******************************************************************************************
 *	Although the frame buffer displays is 320 by 240, only about 288 by 216 pixels
 *	are in the visible area. All display calculations are therefore based on a 
 *	rectangle that is roughly centered in the frame buffer:
 *	VisibleRect(TLBR)= [12,18,12+216,18+288];
 *******************************************************************************************/
static void	GetDstStuff(long *baseAddr, long *rowBytes, long width, long height, Bitmap *bitmap)
{
	long	hOff;
	long	vOff;
...
/*******************************************************************************************
 * Routine to initialize the window within the 320 by 240 frame buffer to display
 * the Cpak movie.  App developer beware, you can display out of the frame buffer.
 *******************************************************************************************/
...
int32	InitCPakSubscriber( void )
	 ...
	 * contexts.
	 */
	CPGlobals.CPakContextPool = CreateMemPool( CPAK_MAX_SUBSCRIPTIONS, sizeof(CPakContext) );
 ...
 * ReplyMsg() to send the empty buffer back to the stream parser.
 *******************************************************************************************/
static int32	DoData( CPakContextPtr ctx, SubscriberMsgPtr subMsg )
	{
...
static int32	_InitializeThread( CPakContextPtr ctx )
	...
	chanPtr = ctx->channel;
	for ( channelNumber = 0; channelNumber < CPAK_MAX_CHANNELS; channelNumber++, chanPtr++ )
		{
		chanPtr->status	= CHAN_ENABLED;
		chanPtr->dataQueue.head	= NULL;
		chanPtr->dataQueue.tail	= NULL;
		chanPtr->fFlushOnSync	= false;
		chanPtr->imageData.baseAddr	= 0;	/* This tells us we have no LR Form Cel yet */

		/* Create a semaphore to manage access to the dataQueue list for this channel
		 */
		semName[0] = 'c';
		semName[1] = 'p';
		semName[2] = 0;
...
/*******************************************************************************************
 * This thread is started by a call to InitCPakSubscriber(). It reads the subscriber message
