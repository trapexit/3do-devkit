 ...
 *						Replaced all success status (0) with
 *kDSNoErr. Fix - Bug 2164: IsTimeForNextFrame fixed to make looping to
 *						beginning of stream work.
 * 	  8/17/94	dtc		Version 2.0.1d3
 *						Fixed IsTimeForNextFrame to delivery the
 *first frame on time. 7/29/94	dtc		Version 2.0.1d2 Replaced NULL
 *with "CPakSubsThread" as thread name when creating CPakSubscriber thread in
 *NewCPakSubscriber. Re-numbered version to 1.3.x to 2.0.1dx to be consistent
 *with DataStream version. 07/14/94	dtc		Version 2.0.1d1
 *						Re-Integrated JAY's changes from
 *Portfolio 1.2.3 (4/11/94): Added two checks for NULL pointers.  One in
 *GetCPakCel.  One in DrawCPakToBuffer. Re-integrated MPH's changes from
 *Portfolio 1.2.3 (3/15/94): Changed GetDstStuff() to handle centering within
 *any size bitmap buffer Needed for native PAL Re-numbered version 2.0.1d1
 *to 1.3. 07/08/94	dtc		Version 1.3 Integrated ccw's Changes:
 *						 Make GetDstStuff static and fix it to
 *process x & y position. GetDstStuff is now called once in DrawCPakToBuffer.
...
/*******************************************************************************************
 *	Although the frame buffer displays is 320 by 240 (384x288 in PAL) ,
 *only about 288 by 216 pixels are in the visible area. (348x260 in PAL)  All
 *display calculations are therefore based on a rectangle that is roughly
 *centered in the frame buffer: VisibleRect(TLBR)= [12,18,12+216,18+288];
 *******************************************************************************************/
static void
GetDstStuff (ImageDesc *pImageData, Bitmap *bitmap)
{
  long hOff;
  long vOff;

  hOff = ((bitmap->bm_Width - (pImageData->width + pImageData->xPos)) / 2) * 4;
  vOff = (((bitmap->bm_Height - (pImageData->height + pImageData->yPos)) / 2)
          & 0xFFFFFFFC)
         * (bitmap->bm_Width * 2);
