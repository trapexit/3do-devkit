 ...
 *						Replaced all success status (0) with kDSNoErr.
 ...
 *						Replaced NULL with "CPakSubsThread" as thread name when creating
 *						CPakSubscriber thread in NewCPakSubscriber.
 *						Re-numbered version to 1.3.x to 2.0.1dx to be consistent with
 *						DataStream version.
 *	07/14/94	dtc		Version 2.0.1d1
 *						Re-Integrated JAY's changes from Portfolio 1.2.3 (4/11/94):
 *						 Added two checks for NULL pointers.  One in GetCPakCel.  One in
 *						 DrawCPakToBuffer.
 *						Re-integrated MPH's changes from Portfolio 1.2.3 (3/15/94):
 *						Changed GetDstStuff() to handle centering within any size bitmap
 *						 buffer Needed for native PAL
 *						Re-numbered version 2.0.1d1 to 1.3.
 *	07/08/94	dtc		Version 1.3
 *						Integrated ccw's Changes:
 *						 Make GetDstStuff static and fix it to process x & y position.
 *						 GetDstStuff is now called once in DrawCPakToBuffer.
...
static void				GetDstStuff(ImageDesc *pImageData, Bitmap *bitmap);
...
ubyte *	movieFrameBuff;
	...
	{
		ERR(( "AllocMovieBuff - Failed to allocate movie image buffer.\n")) ;
		return(-1);
	}
...
int32	InitCPakCel(	DSStreamCBPtr	streamCBPtr,
	...
	*pCPRecPtr				= cpRecPtr;
	...
	memset ( &cpRecPtr->cpHeader, 0, sizeof(CinePakHeader) );
...
Boolean	IsTimeForNextFrame (CPakRecPtr cpRecPtr)
	...
	if (cpRecPtr->lastCurTime > curTime )
		{
		/* Continue playing till we hit the end of stream.  The streamclock may
		 * have been reset due to Alrm chunk for syncing before this subscriber
		 * had time to finish playing its data till the end of stream.
		 */
...
CCB*	GetCPakCel( CPakContextPtr ctx, CPakRecPtr cpRecPtr)
	 ...
	 */
	if ((DSGetClock( cpRecPtr->streamCBPtr, &curTime ) >= 0) && cpRecPtr->curFramePtr )
		{
		...
		if (PollCPakChunk (ctx, cpRecPtr, &pChunkData, &subMsg))
			...
			switch (pChunkData->subChunkType)
				...
				case FHDR_CHUNK_TYPE:	/* CPak Header Chunk */
					...
					/* Set up destination buffer once during header processing
					 *
					 */
					GetDstStuff(pImageData, bitmap);
				...
				case FRME_CHUNK_TYPE:	/* CPak Frame Chunk */
					...
					PreDecompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData));
					Decompress(ctx->filmCodec, (void *)(((CinePakFramePtr)pChunkData)->frameData),
																	(char *)pImageData->baseAddrFrame,pImageData->rowBytes);
#endif
					/* Remember the frame subMsg so this frame
					 * can be released back to the subscriber later.
					 */
...
static void	GetDstStuff(ImageDesc *pImageData, Bitmap *bitmap)
{
	...
	hOff = ((bitmap->bm_Width - (pImageData->width+pImageData->xPos))/2)*4;
	vOff = (((bitmap->bm_Height - (pImageData->height+pImageData->yPos))/2) & 0xFFFFFFFC) *
		(bitmap->bm_Width*2);
	
	pImageData->baseAddrFrame = (long)bitmap->bm_Buffer + hOff + vOff;
	pImageData->rowBytes = bitmap->bm_Width * 2;
}
...
BAILOUT:
	...
	if ( (ctx->creatorStatus < kDSNoErr) || (status < kDSNoErr) )
		return -1;
	else
		return kDSNoErr;
	}
...
void	CPakSubscriberThread( int32 notUsed, CPakContextPtr ctx )
	 ...
	 */
	if ( (status = _InitializeThread( ctx )) != kDSNoErr )
		exit(0);
