 ...
 *allocating subscriber contexts.  A context is allocated and a new thread
 *started with calls to NewProtoSubscriber().
 *******************************************************************************************/
long
InitProtoSubscriber (void)
{
   ...
   * contexts.
   */
  ProtoGlobals.contextPool
      = CreateMemPool (PR_SUBS_MAX_SUBSCRIPTIONS, sizeof (ProtoContext));
...
/*******************************************************************************************
 * Routine to instantiate a new subscriber. Creates the subscriber thread and
 *passes the new context (allocated from the global pool) to it. Creates the
 *message port through which all subsequent communications between the
 *subscriber and the streamer take place, as well as any other necessary
 *per-context resources.
 *******************************************************************************************/
long
NewProtoSubscriber (ProtoContextPtr *pCtx, DSStreamCBPtr streamCBPtr,
                    long deltaPriority)
{
  long status;
  ProtoContextPtr ctx;
 ...
 *******************************************************************************************/
long
DisposeProtoSubscriber (ProtoContextPtr ctx)
{
  if (ctx != NULL)
    {
 ...
 *******************************************************************************************/
static long
DoData (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long status;
  ...
  switch (protoData->subChunkType)
    ...
    case PHLT_CHUNK_TYPE: /* HALT chunk arrived */
      ADD_PROTO_TRACE_L1 (ProtoTraceBufPtr, kHaltChunkArrived,
                          protoHalt->channel, protoHalt->haltDuration, 0);

      /* Make an Audio Timer Cue */
      aTimerCue = CreateCue (NULL);

      /* Sleep for the duration specified in the HALT chunk */
      status = SleepUntilTime (aTimerCue,
                               GetAudioTime () + protoHalt->haltDuration);

      DeleteCue (aTimerCue);

      /* As with header chunks, we have to reply to the header chunk message
       * here.
       */
      status
          = ReplyMsg (subMsg->msgItem, status, subMsg, sizeof (SubscriberMsg));
 ...
 *******************************************************************************************/
static long
DoSetChan (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long status;
  long channelNumber;
 ...
 *******************************************************************************************/
static long
DoGetChan (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long status;
  long channelNumber;
 ...
 *******************************************************************************************/
static long
DoControl (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long status;
  long userWhatToDo;
 ...
 *******************************************************************************************/
static long
DoOpening (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  return kDSNoErr;
}
 ...
 *******************************************************************************************/
static long
DoClosing (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long channelNumber;
 ...
 *******************************************************************************************/
static long
DoStart (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long channelNumber;
 ...
 *******************************************************************************************/
static long
DoStop (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long channelNumber;
  ...
  for (channelNumber = 0; channelNumber < PR_SUBS_MAX_CHANNELS;
       channelNumber++)
    {
      if (subMsg->msg.stop.options & SOPT_FLUSH)
        FlushProtoChannel (
            ctx, channelNumber); /* FlushProtoChannel() has an implicit Stop */
      else
...
/*******************************************************************************************
 * Routine flush all data waiting and/or queued under the assumption that we
 *have just arrived at a branch point and should be ready to deal with all new
 *data from an entirely different part of the stream.
 *******************************************************************************************/
static long
DoSync (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long channelNumber;
 ...
 *******************************************************************************************/
static long
DoEOF (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  return kDSNoErr;
}
 ...
 *everything. Should return all channels to pre-initalized state.
 *******************************************************************************************/
static long
DoAbort (ProtoContextPtr ctx, SubscriberMsgPtr subMsg)
{
  long channelNumber;
...
static long
InitializeThread (ProtoContextPtr ctx)
{
  long status;
  long k;
...
/*******************************************************************************************
 * This thread is started by a call to InitProtoSubscriber(). It reads the
 ...
 *******************************************************************************************/
static void
ProtoSubscriberThread (long notUsed, ProtoContextPtr ctx)
{
  long status;
  ...
  while (fKeepRunning)
    {
      ...
      ADD_PROTO_TRACE_L1 (ProtoTraceBufPtr, kTraceWaitingOnSignal, -1,
                          ctx->requestPortSignal, 0);

      signalBits = WaitSignal (anySignal);

      ADD_PROTO_TRACE_L1 (ProtoTraceBufPtr, kTraceGotSignal, -1, signalBits,
                          0);
                             ...
                             &status))
            {
              switch (subMsg->whatToDo)
                ...
                case kStreamOpControl: /* perform subscriber defined function
                                        */
                  ADD_PROTO_TRACE_L1 (ProtoTraceBufPtr, kTraceControlMsg, -1,
                                      0, subMsg);
                  status = DoControl (ctx, subMsg);
                  break;

                case kStreamOpSync: /* clock stream resynched the clock */
                  ADD_PROTO_TRACE_L1 (ProtoTraceBufPtr, kTraceSyncMsg, -1, 0,
                                      subMsg);
                ...
                case kStreamOpOpening: /* one time initialization call from DSH
                                        */
                  ADD_PROTO_TRACE_L1 (ProtoTraceBufPtr, kTraceOpeningMsg, -1,
                                      0, subMsg);
                  status = DoOpening (ctx, subMsg);
                  break;

                case kStreamOpClosing: /* stream is being closed */
                  ADD_PROTO_TRACE_L1 (ProtoTraceBufPtr, kTraceClosingMsg, -1,
                                      0, subMsg);
