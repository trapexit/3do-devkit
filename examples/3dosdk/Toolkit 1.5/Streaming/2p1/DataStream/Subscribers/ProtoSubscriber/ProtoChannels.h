 ...
 *
 * Channel Enabled 		- means that a channel is prepared to queue data
 *msgs. Channel 0 defaults to enabled, but all other channels must be
 *explicitly set with a DoSetChan() call.
 *
 * Channel Active  		- means that a "stream started" msg has been
 *received.
 *
 * Channel Initalized 	- means that a header chunk has arrived for this
 *channel.
 */
...
extern "C"
{
  ...
  long InitProtoChannel (struct ProtoContext *ctx,
  ...
  long ProcessNewProtoDataChunk (struct ProtoContext *ctx,
