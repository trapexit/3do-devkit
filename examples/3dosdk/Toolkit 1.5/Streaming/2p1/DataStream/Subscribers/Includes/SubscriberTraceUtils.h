 ...
 *
 *	History:
 *	6/23/94		rdg		Version 1.5
 *						change typedef enum to enum to avoid compler complaints
 *						Fixed cut and paste bug: the subscriber trace error codes 
 *						were named "ProtoErrorCode" and collided with ProtoSubscriber.
 ...
 *						every subscriber have it's own buffer.  The
 *						routines prototyped here now take a pointer to
 *						a trace buffer and a file name to write the
 *						output to, instead of using globals.
 *	1/12/93		rdg		Version 1.2 
 *						Update for use with multiple subscribers.
...
void	AddTrace( TraceBufferPtr traceBuffer, long event, long chan, long value, void* ptr );
