 ...
 *
 *	History:
 *	07/08/94	dtc		Version 2.0.1d1
 *						Integrated header support for ccw's
 *changes. 4/27/94		fyp		Version 2.0 Changed the
 *definitions of DS_MSG_HEADER and SUBS_CHUNK_COMMON to require semicolon when
 *used in context.  (For readibility and compilation using ObjectMaster).
...
#define JF_RELEASERESOURCESONLY                                               \
  0x0001 /* release everything but the data block */
...
typedef struct JoinContext
{
  ...
  Boolean
      bRequestSignal; /* Boolean indicating a request needs to be processed */
