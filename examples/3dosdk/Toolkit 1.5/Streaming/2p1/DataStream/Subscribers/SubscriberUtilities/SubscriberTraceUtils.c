 ...
 *						Create a "TraceBuffer" struct which contains the index and the data.
 *						Change the "ec" trace code prefix to "tc".
 *	3/25/94		rdg		Version 1.3 
 *						Last implementation was fucked.  This time, let
 *						every subscriber have it's own traceBuffer.  The
 *						routines implemented here now take a pointer to
 *						any valid trace buffer and a file name to write the
 *						output to, instead of using globals. Should be better.
 *	1/12/94		rdg		Version 1.2 
 *						Make useable by all subscribers.
 *						Write trace output to a file and not do printf().
 *	12/24/93	rdg		Version 1.1  
 *						Generalize for use with other subscribers.
 *	10/14/93	rdg		Version 1.0  -  New Today
 *
 *
 */ 
...
void	AddTrace( TraceBufferPtr traceBuffer, long event,
					long chan, long value, void* ptr )
	{
...
static	long	FindCompletion( TraceBufferPtr traceBuffer, long startEventIndex, 
	...
	TraceEntryPtr 	startEventEntry;	// ptr to the trace entry where we start searching
	...
	while ( tmpIndex != startEventIndex )
		 ...
		 * this is it...
		 */
		if ( (curTraceEntry->what == completionEventCode)  &&		/* is at a completion */
			 (curTraceEntry->aValue == startEventEntry->aValue) &&	/* do the values match */
 ...
 * after you queued it.
 */
long	DumpTraceCompletionStats ( TraceBufferPtr traceBuffer, long startEventCode, 
	...
	sprintf( str, "\nStart Index\tCompletion Index\tChannel Num\tSignalBits/Value\tSubmission Time\tCompletion Time\tDur Since Submission\n\n" );
	fputs( str, LogFile );
	...
	while ( tmpIndex != traceBuffer->index )
		...
		if ( curTraceEntry->what == startEventCode )
			{
			completionIndex = FindCompletion( traceBuffer, tmpIndex, completionEventCode );
			
			/* -1 means couldn't find completion */
			if ( completionIndex < 0 )
				{
				sprintf( str,"\nCouldn't find completion event for event start at index %ld...\n",tmpIndex ) ;
				fputs( str, LogFile );
				}
			else
				{
