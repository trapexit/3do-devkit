 ...
 *
 *	History:
 *	10/03/94	dtc		Fix - Bug #3402: added code to arbitrate access to the
 *						dataqueue in DoSAnimData.
 *						Fix - Bug 2164: IsTimeForNextFrame fixed to make looping to
 *						beginning of stream work.
 *	 09/20/94	dtc		Fixed un-terminated comment in IsTimeForNextFrame.
 *						Replaced printf with ERR macro defined in Debug3DO.h.
 *	 08/22/94	dtc		Version 2.0.1d4
 *						Fixed memory leak.  _InitalizeSAnimThread() was zeroing the ctx's
 *						stackBlock field so DisposeSAnimSubscriber() was never able to free
 *						the stackBlock.
 * 	  8/17/94	dtc		Version 2.0.1d3
 *						Fixed IsTimeForNextFrame to delivery the first frame on time.
 * 	  7/29/94	dtc		Version 2.0.1d2
 *						Replaced NULL with "SAnimSubsThread" as thread name when creating
 *						SAnimSubscriber thread in NewSAnimSubscriber.
 *  07/19/94	DLD		Fixed a low-mem-abort bug that I introduced on 7/17.
 *	07/17/94	DLD		Added AMAP chunks.
 *	07/17/94	DLD		Fixed a bug in GetSAnimCel where individual channels would start
 *						playing their frames before it was time for them.  This code is
 *						dovetailed in with the FREE_DATA_WHEN_TIME_HAS_PASSED code.
 *	06/29/94	rdg		removed include of CPakSubscriber.h....
 *	05/24/94	DLD		Changed CCB and POS chunk definitions!  Now the X.Y positions 
 *						are assumed to be fixed point values, not integers.
 *						If your animations all come out at 0,0, then
 *						you must rebuild with SHIFT_CCB_POSITIONS defined to get the
 *						old behavior.
 *	05/23/94	fyp		Fixed a low-memory access problem caused by a nil curFramePtr
 *						at the end of GetSAnimCel. 
 *	11/23/93	jb		Version 1.2
 *						Added CloseSAnimSubscriber(), a routine to deallocate resources 
 *						allocated by InitSAnimSubscriber().
 *	10/19/93	jb		Version 1.1
 *						Switch to using UMemory.c routines for memory allocation.
 *						Add code identified by FREE_DATA_WHEN_TIME_HAS_PASSED to optionally
 ...
 *	8/18/93		jb		Changed curTime in IsTimeForNextFrame() to a SIGNED value to
 *						make comparisons work correctly.
 ...
 *	4/23/93		njc		All sorts of new routines to prepare for first testing.
...
/****************************/
Boolean	PollSAnimChunk(	SAnimContextPtr		ctx,
 ...
 * GetSAnimCel will return NULL until the Header, CCB, and first frame have been received.
 *******************************************************************************************/
...
/*******************************************************************************************
 *	Return all sanim msgs to the SAnimSubscriber back end. The sanim subscriber will return one
 ...
 *******************************************************************************************/
CCB*	SetupNextSAFrame (CCB* ccb, StreamAnimFramePtr pSAFrame)
	{
	char*		pChunk;
	char*		tempBuf;
	int32		tempSize;
	...
	while ( (pChunk = GetChunk( &chunk_ID, &tempBuf, &tempSize )) != NULL)
		...
		switch (chunk_ID)
			...
			default:
				ERR( ("SetupNextSAFrame --- Unexpected chunk ID: '%4.s'", &chunk_ID) );
				return NULL;
			}
		}
 ...
 *******************************************************************************************/
Boolean	IsTimeForNextFrame (SAnimContextPtr ctx, SAnimRecPtr saRecPtr)
	{
...
/*******************************************************************************************
 * Send a signal to the SAnim Subscriber telling it to Reply all subMsgs in the freeList
...
/*******************************************************************************************
 * Return the duration of the sanim (in 240ths), or -1 of the sanim frame hasn't been read 
 * yet.  NOTE: this function returns the time of the BEGINING of the last frame
 *******************************************************************************************/
int32	SAnimDuration (SAnimRecPtr saRecPtr)
	{
	int32						frameRateAudioTime;
	
	frameRateAudioTime = 240 / saRecPtr->saHeader.frameRate;
	return (saRecPtr->saHeader.numFrames * frameRateAudioTime) - frameRateAudioTime;

	}

/*******************************************************************************************
 * Return the current time of the sanim (in 240ths), or -1 of the sanim frame hasn't been 
 *  read yet.
 *******************************************************************************************/
int32	SAnimCurrTime (SAnimRecPtr saRecPtr)
	{
...
/*******************************************************************************************
 * Get the next Cel in the streamed animation. GetSAnimCel will return NULL until the
 * Header, CCB, and first frame have been received. When the First frame comes in, the
...
CCB*	GetSAnimCel( SAnimContextPtr ctx, SAnimRecPtr saRecPtr )
		...
		if (PollSAnimChunk (ctx, saRecPtr, &pChunkData, &subMsg))
			...
			switch (pChunkData->subChunkType)
				...
				case AHDR_CHUNK_TYPE:	/* SAnim Header Chunk */
					memcpy( &(saRecPtr->saHeader), pChunkData, sizeof(StreamAnimHeader) );
					saRecPtr->curFramePtr = NULL;
				...
				case FRME_CHUNK_TYPE:	/* SAnim Frame Chunk */
					...
					/* Fill the CCB fields for this frame and remember the frame subMsg
					 * so this frame can be released back to the subscriber later.
					 */
	...
	if ( DSGetClock( saRecPtr->streamCBPtr, &curTime ) >= 0 )
		{
		if ( saRecPtr->curFramePtr != NULL ) {
			if ( curTime > (saRecPtr->curFramePtr->time + saRecPtr->curFramePtr->duration) )
				{
				if ( saRecPtr->curSubMsg )
					{
					DoFreeChunk( ctx, saRecPtr->curSubMsg );
...
int32	InitSAnimSubscriber( void )
	 ...
	 * contexts.
	 */
	SAGlobals.SAnimContextPool = CreateMemPool( SANM_MAX_SUBSCRIPTIONS, sizeof(SAnimContext) );
	 ...
	 * state records.
	 */
	SAGlobals.SAnimRecPool = CreateMemPool( SANM_MAX_CHANNELS, sizeof(SAnimRec) );
...
/*******************************************************************************************
 * Routine to queue arriving data chunks. Each chunk will be added to the end of its
 * channel's chunk list. When the App requests a chunk by sending a request msg, the first
 * chunk in the channel queue will be removed and sent with the reply. Later, the App will
 * send a msg telling the subscriber to reply a chunk back to the DSL. This will tell the
 * DSL to free the chunk.
 *******************************************************************************************/
static int32	DoSAnimData( SAnimContextPtr ctx, SubscriberMsgPtr subMsg )
	{
	int32				status;
...
/*******************************************************************************************
 * Routine to add a chunk to the free list. These chunks will be replied back to the DSL
 * later as a result of a signal sent by the client. (see DoReplyChunksToDSL)
 *******************************************************************************************/		
static int32	DoFreeChunk( SAnimContextPtr ctx, SubscriberMsgPtr subMsg )
	{
...
int32	_InitializeSAnimThread( SAnimContextPtr ctx )
	...
	chanPtr = ctx->channel;
	for ( channelNumber = 0; channelNumber < SANM_MAX_CHANNELS; channelNumber++, chanPtr++ )
		{
		chanPtr->status			= CHAN_ENABLED;
		chanPtr->dataQueue.head	= NULL;
		chanPtr->dataQueue.tail	= NULL;
...
/*******************************************************************************************
 * This thread is started by a call to InitSAnimSubscriber(). It reads the subscriber message
