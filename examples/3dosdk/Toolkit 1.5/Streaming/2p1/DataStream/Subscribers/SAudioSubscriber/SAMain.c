 ...
 *						Re-numbered version number to 2.0.1dx to be consistent with
 *						DataStream release version???
 *	7/27/94		rdg		Version 2.9.9
 *						Add support for Pause and Unpause
 *	7/07/94		rdg		Version 2.9.8
 *						Add support for ADPCM and 22K uncompressed instruments.
 *	6/23/94		rdg		Version 2.9.7
 *						Renamed TRACE_MAIN to SAUDIO_TRACE_MAIN and DUMP_TRACE_XXX
 *						to SAUDIO_DUMP_TRACE_XXXX; required for uniquenes
 *	6/8/94		fyp		Version 2.9.6
 *						Yanked trace compile switches to the make file level.
 *	5/31/94		rdg		Version 2.9.5
 *						Make "channel" function names unique by adding SAudio to the name
 *	3/26/94		rdg		Version 2.9.4
 *						Modify to use new trace code technology, yea.
 ...
 *						Make DoAbort() call CloseSAudioChannel() for all channels instead 
 *						of FlushSAudioChannel().
 *						DoSync() now calls FlushSAudioChannel() directly.
 ...
 *						Make CloseSAudioChannel() reset all of the channel's variables.
 ...
 *						Add check for stream version.  InitSAudioChannel() will now fail if the
 *						version field in the saudio header is greater than COMPATIBLE_AUDIO_STREAM_VERSION.
 *						Modify SetSAudioChannelAmplitude() to ramp quickly when going to 0 or only 
 ...
 *						Also remove the DUMP_SAMPLE code from FlushSAudioChannel().
 *						New routines MuteSAudioChannel() and UnMuteSAudioChannel() to encapsulate all amplitude
 *						change and testing functionality inside of these routines as well as 
 *						SetSAudioChannelAmplitude() and SetSAudioChannelPan().
 *						If you set amplitude or pan while a channel is muted, it will remember
 *						what you did and restore that when UnMuteSAudioChannel() is called.
 ...
 *						Fix redundancy bugs in CloseSAudioChannel()... was doing all sorts of things that 
 *						that had already been done by StopSAudioChannel().  
 *						CloseSAudioChannel() now disables the channel so it can't receive any more
 ...
 *						to use it again.
 *						Fix bug in InitSAudioChannel()... was setting status EQUAL to CHAN_ENABLED instead
 ...
 *						to be used in Start/StopSAudioChannel().  When off, Start/StopAttachment()
 ...
 *						InitalizeThread(), InitSAudioChannel(), DoStart(), DoStop(), and DoAbort() were 
 ...
 *						Add code to StopSAudioChannel() and CloseSAudioChannel() so they now check to see if 
 ...
 *						Moved ramping code after StopInstrument() call in StopSAudioChannel() and CloseSAudioChannel() 
 ...
 *							Modify InitSAudioChannel() BeginSAudioPlaybackIfAppropriate() and HandleCompletedBuffers() to
 *							deal with amplitude differently.  Now InitSAudioChannel() saves the proper amplitude in *						the outputPtr->savedAmp.  This is used in  BeginSAudioPlaybackIfAppropriate() to set the
 ...
 *							Modified SetSAudioChannelAmplitude() and SetSAudioChannelPan() to ramp volume.  This seems
 ...
 *							Modified CloseSAudioChannel() to Ramp down volume on mixer and do free instruments 
 ...
 *						arrives, InitSAudioChannel() will ignore the initialAmplitude value in the SSND header.
 ...
 *						Modified StopSAudioChannel() and FlushSAudioChannel() to return status.
 ...
 *	6/28/93		rdg		Fix bug in InitSAudioChannel(): SetSAudioChannelPan() was being called on
 *						initalization of a stereo stream. This causes an error in SetSAudioChannelPan().
 ...
 *						If an error occurs in InitSAudioChannel(), DoData() for now just disables the 
 *						channel and returns 0 anyway.
 *						Clean up error codes in InitSAudioChannel() to return meaningful codes.
 *						Fix bug with channelOutput(s) in all routines that references them.  
 *						Fix bug in InitSAudioChannel(): i was not setting chanOutputPtr->stereo based 
 ...
 *						to QueueNewAudioBuffer() and BeginSAudioPlaybackIfAppropriate().  QueueNewAudioBuffer() 
 ...
 *						BeginSAudioPlaybackIfAppropriate() can be called to find out if conditions are right 
 ...
 *						and can be set only by BeginSAudioPlaybackIfAppropriate(), HandleCompletedBuffers() and
 *						StopSAudioChannel().
 *						HandleCompletedBuffers() sets it to false when it leaves the inuseQueue and 
 *						the dataQueue empty.  BeginSAudioPlaybackIfAppropriate() sets it to true when it first
 *						starts the instrument, and when it restarts an attachment after things have been
 *						stopped or idle.  StopSAudioChannel() sets it to false when it calls StopAttachment().
 ...
 *						new buffers arrive, when buffers complete from the AudioFolio and when StartSAudioChannel()
 ...
 *						Modify InitSAudioChannel() to use new channelOutput structure fields.
 ...
 *						Rename OpenChannel() to InitSAudioChannel().  InitSAudioChannel() now does
 *						NOT enable the channel, it only initalizes the channel.
 *						Modify InitalizeChannel() to default channel 0 to enabled and all 
 *						others to disabled.
 *						Fix bug in StopSAudioChannel(): it wasn't checking to make sure data
 ...
 *						StopSAudioChannel() now has guts that do something.
 *						StartSAudioChannel() added from scratch.
 ...
 *						Changed the name of InitSAudioChannel to OpenChannel.
 ...
 *	4/21/93		njc		FlushSAudioChannel was caught in an infinite loop calling
...
long	DisposeSAudioSubscriber( SAudioContextPtr ctx )
	...
	if ( ctx != NULL )
		 ...
		 */
		if ( ctx->decodeADPCMIns > 0 )	UnloadInstrument( ctx->decodeADPCMIns );
...
static long	DoData( SAudioContextPtr ctx, SubscriberMsgPtr subMsg )
{
	...
	switch (audioData->subChunkType)
		...
		case SSMP_CHUNK_TYPE:	/* Sample data */
			...
			ADD_TRACE_L2( SATraceBufPtr, kNewDataMsgArrived, channelNumber, 0, subMsg );
		...
		case SHDR_CHUNK_TYPE:	/* Sample header structure */
			...
			ADD_TRACE_L2( SATraceBufPtr, kNewHeaderMsgArrived, channelNumber, 0, subMsg );
...
static long	DoControl( SAudioContextPtr ctx, SubscriberMsgPtr subMsg )
	...
	switch ( userWhatToDo )
		...
		case kSAudioCtlOpLoadTemplates:
			status = LoadTemplates( ctx, ctlBlockPtr->loadTemplates.tagListPtr, kMaxTemplateCount );
			break;

		case kSAudioCtlOpSetVol:
			if ( ctlBlockPtr->amplitude.channelNumber < SA_SUBS_MAX_CHANNELS )
				{
				status = SetSAudioChannelAmplitude( ctx, ctlBlockPtr->amplitude.channelNumber, 
		...
		case kSAudioCtlOpGetVol:
			if ( ctlBlockPtr->amplitude.channelNumber < SA_SUBS_MAX_CHANNELS )
				{
				status = GetSAudioChannelAmplitude( ctx, ctlBlockPtr->amplitude.channelNumber, 
								&ctlBlockPtr->amplitude.value);
				}
			else
				status = SAudioErrChanOutOfRange;
		...
		case kSAudioCtlOpMute:
			if ( ctlBlockPtr->mute.channelNumber < SA_SUBS_MAX_CHANNELS )
				{
				status = MuteSAudioChannel( ctx, ctlBlockPtr->mute.channelNumber, USER_MUTE );
				}
			else
				status = SAudioErrChanOutOfRange;
			break;
		...
		case kSAudioCtlOpUnMute:
			if ( ctlBlockPtr->unMute.channelNumber < SA_SUBS_MAX_CHANNELS )
				{
				status = UnMuteSAudioChannel( ctx, ctlBlockPtr->unMute.channelNumber, USER_UNMUTE );
				}
			else
				status = SAudioErrChanOutOfRange;
			break;
		...
		case kSAudioCtlOpCloseChannel:
			status = CloseSAudioChannel( ctx, ctlBlockPtr->CloseSAudioChannel.channelNumber );
			break;

		case kSAudioCtlOpFlushChannel:
			status = FlushSAudioChannel( ctx, ctlBlockPtr->FlushSAudioChannel.channelNumber );
			break;
 ...
 *		to track buffer completions signals which come from the AudioFolio
 *		and MPEG driver.  You don't necessarily have to use signals here
 *		but this feature is maintined for compatibility and becase a
 *		the AudioSubscriber already exists as a example of how to do it.
 *
			...
			while( PollForMsg( ctx->requestPort, NULL, NULL, (void **) &subMsg, &status ) )
				...
				switch ( subMsg->whatToDo )
					...
					case kStreamOpGetChan:			/* get logical channel status */
						ADD_TRACE_L1( SATraceBufPtr, kTraceGetChanMsg, subMsg->msg.channel.number, 0, subMsg );
						status = DoGetChan( ctx, subMsg );		
						break;
		
		
					case kStreamOpSetChan:			/* set logical channel status */
						ADD_TRACE_L1( SATraceBufPtr, kTraceSetChanMsg, subMsg->msg.channel.number, subMsg->msg.channel.status, subMsg );
						status = DoSetChan( ctx, subMsg );		
						break;
		
		
					case kStreamOpControl:			/* perform subscriber defined function */
						ADD_TRACE_L1( SATraceBufPtr, kTraceControlMsg, -1, 0, subMsg );
				 ...
				 ************************************************************************/
					...
					case kStreamOpOpening:			/* one time initialization call from DSH */
						ADD_TRACE_L1( SATraceBufPtr, kTraceOpeningMsg, -1, 0, subMsg );
						status = DoOpening( ctx, subMsg );		
						break;
		
		
					case kStreamOpClosing:			/* stream is being closed */
						ADD_TRACE_L1( SATraceBufPtr, kStreamOpClosing, -1, 0, subMsg );
