 ...
 *	7/07/94		rdg		Version 2.9.5
 *						Modify to support ADPCM... GetTemplateItem()'s parameters changed.
 *	6/23/94		rdg		Version 2.9.4
 *						Renamed TRACE_CHANNELS to SAUDIO_TRACE_CHANNELS; required for uniquenes
 *	6/8/94		fyp		Version 2.9.3
...
static long		InternalStopSAudioChannel( SAudioContextPtr ctx, long channelNumber );
static void		InternalPauseSAudioChannel( SAudioContextPtr ctx, long channelNumber );
 ...
 *******************************************************************************************/
void		StartSAudioChannel( SAudioContextPtr ctx, long channelNumber, Boolean doFlush )
	{
	SAudioChannelPtr	chanPtr;
	...
	if ( IsChanEnabled(chanPtr) && (!IsChanActive(chanPtr)) )
		...
		if ( doFlush )
			...
			if ( chanPtr->instPaused == true )
				{
				ADD_TRACE_L2( SATraceBufPtr, kStartWFlushWhilePaused, channelNumber, 0, 0 );
				
				/* Prevent any buffers from crossing the link to the next queued buffer.
				 */
				OrphanPlayingBuffers( ctx );	
	
				/* ReStart the instrument exactly where it left off.  The
				 * Channel was muted by the pause so we don't care about amplitude...
				 */
...
static void		InternalPauseSAudioChannel( SAudioContextPtr ctx, long channelNumber )
	...
	ADD_TRACE_L2( SATraceBufPtr, kTraceInternalChannelPause, channelNumber, chanPtr->pauseTime, 0 );
	...
	/* Leave a trace so we can tell which buffer was playing when pause was called */
	ADD_TRACE_L3( SATraceBufPtr, kCurrentBufferOnPause, channelNumber, 
					bufferPtr->signal, bufferPtr );

	/* Check to see if that buffer completed before we could get to PauseInstrument() 
	 * and if it did, pull it out of the inUseQueue, call WaitSignal() to clear the bit.  
	 * Then free up the buffer for re-use.
	 */
	if ( (GetCurrentSignals() & bufferPtr->signal) )
		{
		ADD_TRACE_L2( SATraceBufPtr, kAttachCrossedWhilePausing, channelNumber, 0, 0 );
...
static long		InternalStopSAudioChannel( SAudioContextPtr ctx, long channelNumber )
	...
	if ((bufferPtr = GetNextBuffer( chanPtr )) != NULL)		
		...
		if ( (GetCurrentSignals() & bufferPtr->signal) )
			{
			ADD_TRACE_L2( SATraceBufPtr, kAttachCrossedWhileStopping, channelNumber, 0, 0 );
			WaitSignal(bufferPtr->signal);
			}
 ...
 *******************************************************************************************/
long		StopSAudioChannel( SAudioContextPtr ctx, long channelNumber, Boolean doFlush )
	{
	long				status = 0;
	SAudioChannelPtr	chanPtr;
	...
	if ( IsChanEnabled(chanPtr) && (IsChanActive(chanPtr)) )
		...
		if ( doFlush )
			...
			if ( chanPtr->instPaused == true )
				{
				ADD_TRACE_L2( SATraceBufPtr, kStopWFlushWhilePaused, channelNumber, 0, 0 );
...
void		BeginSAudioPlaybackIfAppropriate( SAudioContextPtr ctx, long channelNumber )
	...
	if 	( (IsChanEnabled(chanPtr)) && (chanPtr->inuseCount != 0) )
		...
		if ( chanPtr->attachmentsRunning == false ) 
			 ...
			 */
			ADD_TRACE_L2( SATraceBufPtr, kStartAttachment, channelNumber, audioData->time, 0 );
		...
		/* Otherwise see if we are paused and if so, resume from where we were */
			...
			ADD_TRACE_L2( SATraceBufPtr, kResumeInstrument, channelNumber, chanPtr->pauseTime, 0 );
			ResumeInstrument( chanPtr->channelInstrument );
			chanPtr->instPaused = false;	
		
			/* Restore gain on the mixer after ResumeInstrument() has been called.
			 * This should guarantee no more pops.
			 */
			UnMuteSAudioChannel( ctx, channelNumber, INTERNAL_UNMUTE );	
			}
