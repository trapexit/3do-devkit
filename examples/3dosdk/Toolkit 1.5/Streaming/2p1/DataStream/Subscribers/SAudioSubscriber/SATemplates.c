 ...
 *						Add support for ADPCM4 and 22K
 *uncompressed instruments. 6/23/94		rdg		Version 2.8.2
 *						Renamed TRACE_TEMPLATES to
 *SAUDIO_TRACE_TEMPLATES 6/8/94		fyp		Version 2.8.1 Yanked
 *trace compile switches to the make file level. 10/14/93	rdg
 *Version 2.8 -- New Today Split source into separate files.
 */
 ...
 *******************************************************************************************/
Item
GetTemplateItem (SAudioContextPtr ctx, long templateTag, long templateCount)
{
  long k;
  TemplateRecPtr tp;
      ...
      if (tp->templateTag == templateTag)
        {
          ...
          /* If we're going to use an ADPCM instrument, the support code must
           * be loaded */
          if (((templateTag == SA_44K_16B_M_ADPCM4)
               || (templateTag == SA_22K_16B_M_ADPCM4))
 ...
 *******************************************************************************************/
long
LoadTemplates (SAudioContextPtr ctx, long *tagPtr, long templateCount)
{
  long status;
  TemplateRecPtr tp;
