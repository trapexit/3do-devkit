 ...
 *
 *	Contains:		Implementation of routines to manage instrument templates.
 *
 ...
 *						Add support for ADPCM4 and 22K uncompressed instruments.
 *	6/23/94		rdg		Version 2.8.2
 *						Renamed TRACE_TEMPLATES to SAUDIO_TRACE_TEMPLATES
 *	6/8/94		fyp		Version 2.8.1
...
/* The following table describes all the AudioFolio instrument templates that we know about.
 * These will be dynamically or pre-use loaded. The user is able to cause them to
 * be preloaded by sending us a control message that tells us which of these
 * is desired. Each is specified by its tag value. The Item associated with
 * the entity is cached so that if we are asked for the same thing again later,
 * we will already have loaded it from disk. A copy of this table is made for
 * each instantiation of an audio subscriber. This is done because each 
...
TemplateRec gInitialTemplates[] = {
	...
	{ SA_22K_16B_M,			0,	"halfmonosample.dsp"	},
	{ SA_22K_16B_S,			0,	"halfstereosample.dsp"	},
	...
	{ SA_44K_16B_M_ADPCM4,	0,	"adpcmmono.dsp"			},
	{ SA_22K_16B_M_ADPCM4,	0,	"adpcmhalfmono.dsp"		}
...
long	GetTemplateTag( SAudioSampleDescriptorPtr descPtr )    
	...
	if ( descPtr->numChannels == 1 )
		...
		if ( descPtr->compressionType == ID_ADP4 )
			{
			if ( descPtr->sampleRate == 22050 )
				return SA_22K_16B_M_ADPCM4;
			else
				return SA_44K_16B_M_ADPCM4;			
			}
 ...
 *******************************************************************************************/
Item	GetTemplateItem( SAudioContextPtr ctx, long templateTag, long templateCount )
	{
	long				k;
	TemplateRecPtr 		tp;
	
	/* Point to the array of instrument templates */
	tp = ctx->templateArray;
	...
	for ( k = 0; k < templateCount; k++, tp++ )
		...
		if ( tp->templateTag == templateTag ) 
			...
			/* If we're going to use an ADPCM instrument, the support code must be loaded */
			if ( ( (templateTag == SA_44K_16B_M_ADPCM4) || (templateTag == SA_22K_16B_M_ADPCM4) ) 
				&& (ctx->decodeADPCMIns == 0) )
				{
				 ctx->decodeADPCMIns = LoadInstrument( "decodeadpcm.dsp", 0, 100 ); 
				}
 ...
 *******************************************************************************************/
long	LoadTemplates( SAudioContextPtr ctx, long* tagPtr, long templateCount )
	{
	long				status;
	TemplateRecPtr 		tp;
