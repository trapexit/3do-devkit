 ...
 *
 *	History:
 *	 09/07/94	dtc		Version 2.1a2
 *						Replaced printf with ERR macro defined in
 *Debug3DO.h. 6/23/94		rdg		Version 2.9.4 Renamed
 *TRACE_BUFFERS to SAUDIO_TRACE_BUFFERS and DUMP_TRACE_ON_BUFFER_COMP_ERR to
 *SAUDIO_DUMP_TRACE_ON_BUFFER_COMP_ERR 6/8/94		fyp
 *Version 2.9.3 Yanked trace compile switches to the make file level. 5/31/94
 *rdg		Version 2.9.2 Make "channel" function names unique 10/26/93
 *rdg		Version 2.9 Fixed number of parameters to MuteSAudioChannel().
 *	10/14/93	rdg		Version 2.8 -- New Today
 *						Split source into separate
 *files. Keep high level subscriber message parsing and logical channel related
 *						functionality in SAudioSubscriber.c.
 *Move AudioFolio stuff here. Move trace implementation to SAudioTrace.c
 */
...
FreeBufferFromAudioFolio (SAudioBufferPtr bufferPtr)
{
  ...
  ADD_TRACE_L3 (SATraceBufPtr, kFreedBufferFromFolio, -1, bufferPtr->signal,
                bufferPtr);
}
...
#if SAUDIO_DUMP_TRACE_ON_BUFFER_COMP_ERR
