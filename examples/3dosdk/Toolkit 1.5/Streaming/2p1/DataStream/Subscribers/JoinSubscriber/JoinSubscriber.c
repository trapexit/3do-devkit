 ...
 *
 *					This subscriber accepts chunks of type 'JOIN' and reassembles the data
 *					into local RAM. Each chunk is separated by subChunkType and, within the
 *					subChunkType, channel. So, if you wished to read in 4 ANIMs, each ANIM
 *					would have a different channel number {0,1,2,3} but the same subChunkType.
 *
 ...
 *						Replaced NULL with "JoinSubsThread" as thread name when creating
 *						JoinSubscriber thread in NewJoinSubscriber.
 *	07/11/94	dtc		Version 2.0.1d1
 *						We now break out of request poll loop when no memory is
 *						available.
 *	07/08/94	dtc		Integrated ccw's changes for memory allocation, flush,
 *						and resource management in DoJoinData, FlushJoinSubscriber,
 *						JoinSubscriberThread, ReleaseJoinElementResources, and
 *						FreeJoinData.
 *	12/1/93		jb		Version 1.4
 *						Switch to using UMemory.h NewPtr() and FreePtr() routines.
 *	11/29/93	jb		Version 1.3
 *						Integrated changes from Neil Cormia's version of the code.
 *	11/23/93	jb		Version 1.2
 *						Added CloseJoinSubscriber() to deallocate resources allocated 
 *						by the InitJoinSubscriber() routine.
 *	11/18/93	njc		Renamed _FlushJoin to FlushJoinSubscriber and made it global in scope.
 *						Added a RemoveMsgFromQueue to its loop to remove joMsgs as it flushes.
 *	10/19/93	jb		Version 1.1
 *						Switch to using UMemory.c routines for memory allocation
 *	9/21/93		njc		Was not calling ReturnPoolMem on the joMsg if the JOIN data had
 *						no join port allocated. This is true for the MRKR tables in twisted
 *						streams. Also, the joMsg-link was not being cleared to NULL. This
 *						could result in a joMsg pointing to itself.
 *	9/5/93		njc		In DoGetData(), if no port exists for a data element, Free it up.
 *	6/27/93		njc		Added semaphore protection to port list. Made InitJoinPort accept
 *						a client defined joinPort so clients can make a port containing
 *						a signal to be triggered on receipt of a join msg.
...
static JoinElementMsgPtr FindJoinElement( JoinContextPtr ctx, int32 elemType, int32 channel );
static void RemoveMsgFromJoinQueue( JoinContextPtr ctx, JoinElementMsgPtr joMsg );
...
int32	InitJoinSubscriber( void )
	 ...
	 * contexts.
	 */
	DGlobals.JoinContextPool = CreateMemPool( DATA_MAX_SUBSCRIPTIONS, sizeof(JoinContext) );
...
/*******************************************************************************************
 * Routine to deallocate resources allocated by the InitJoinSubscriber() routine
 *******************************************************************************************/
...
int32	FlushJoinSubscriber( JoinContextPtr ctx )
	...
	{
		ReplyMsg( ctx->psubMsgWaiting->msgItem, kDSNoErr, ctx->psubMsgWaiting, 0 );
		ctx->psubMsgWaiting = NULL;
	}
...
static int32	DoJoinData( JoinContextPtr ctx, SubscriberMsgPtr subMsg )
	...
	if (ctx->JoinElemMsgWaitingPtr != NULL)
	{
	...
	else if ( (joMsg = FindJoinElement(ctx, dataPtr->subChunkType, dataPtr->channel)) == NULL )
		{
		/* Check to make sure this is a first chunk */
		if (dataPtr->joinChunkType != JOIN_HEADER_SUBTYPE)
			{
			DIAGNOSE(("Got a  JOIN_DATA_SUBTYPE -- expected a JOIN_HEADER_SUBTYPE\n")); 
			status =  kDSAbortErr;
			goto EXIT;
			}
		...
		if ((joMsg->dataPtr = NewPtr(dataPtr->totalDataSize, dataPtr->ramType)) == NULL)
			{
		...
		if (dataPtr->joinChunkType != JOIN_DATA_SUBTYPE)
			{
			DIAGNOSE(("Got a  JOIN_HEADER_SUBTYPE -- expected a JOIN_DATA_SUBTYPE\n")); 
			status =  kDSAbortErr;
			goto EXIT;
			}
	...
	if ( (joMsg->dataOffset + dataSize) > joMsg->dataSize)
		CHECKRESULT("JoinSubscriber -- data past end of element. Offset = ", joMsg->dataOffset);
	...
	if (joMsg->dataOffset == joMsg->dataSize)
		...
		*/
		...
		if (foundPort)
			status = SendMsg( ctx->dataPort[curPortIndex], joMsg->msgItem, joMsg, sizeof(JoinElementMsg) );
		else
			{
...
static int32	_InitializeJoinThread( JoinContextPtr ctx )
	...
	ctx->joinElemMsgPool = CreateMemPool( DATA_MAX_ELEMENTS, sizeof(JoinElementMsg) );
...
void	JoinSubscriberThread( int32 notUsed, JoinContextPtr ctx )
	...
	while ( fKeepRunning )
		...
		if ( signalBits & ctx->replyPortSignal )
			{
			while( PollForMsg( ctx->replyPort, NULL, NULL, (void **) &joMsg, &status ) )
				{	
		...
		if (ctx->JoinElemMsgWaitingPtr != NULL)
		{
			...
			if ((ctx->JoinElemMsgWaitingPtr->dataPtr = NewPtr(dataPtr->totalDataSize, dataPtr->ramType)) != NULL)
		...
		if (ctx->JoinElemMsgWaitingPtr == NULL)
		{
			if  (( signalBits & ctx->requestPortSignal)  || ctx->bRequestSignal)
				...
				while( PollForMsg( ctx->requestPort, NULL, NULL, (void **) &subMsg, &status ) )
					...
					/* subMsg->whatToDo == kStreamOpData and no memory break out of the poll loop */
