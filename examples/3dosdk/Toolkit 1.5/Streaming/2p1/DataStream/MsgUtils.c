 ...
 *Debug3DO.h. 08/22/94	dtc		Version 2.0.1 Replaced printf with
 *printf. 6/1/93		jb		Version 1.2.3 Remove all
 *variable initializers in local variable declarations to avoid register
 *bashing by the ARM C compiler. 5/21/93		jb		Add
 *paranoia check in WaitForMsg(). See the code demarked by the
 *CHECK_FOR_GETTING_UNCLEAN_MESSAGES symbol.
 *	5/9/93		jb		Fix possible uninitialized use of
 *'msgPtr' and make exit code common to PollForMsg() in WaitForMsg(). 4/18/93
 *jb		Install WAITPORT_HACK to tiptoe around the fact that WaitPort()
 ...
 *jb		Fix PollForMsg() so that it can optionally return any of its
 *result args (not conditionalized on any of them needing to be set in order to
 *get the others).
 *	4/5/93		jb		Add 'signalMaskPtr' arg to
 *CreateMsgPort()
