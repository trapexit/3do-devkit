 ...
 *	History:
 *
 *	10/10/94	dtc		Version 2.1b2
 *						Removed if-else RELATIVE_BRANCHING at end HandleDataAcqReply 
 *						that enclosed a check for unexpected replies from DataAcq.
 *						Check should be the same whether or not RELATIVE_BRANCHING
 *						is defined.
 *	09/19/94	dtc		Version 2.1b2
 *						Fix: Bug #3315. Ignore new data from DataAcq until Data Acq
 *						replies to go-marker request.
 *	09/16/94	nmd		added support for portfolio style error codes.
 *	09/07/94	dtc		Version 2.1a1
 *						Replaced printf with ERR and
 *						fixed HALT_ENABLE code.  The previous version had a race condition
 *						when resuming delivery of data chunks from the current buffer
 *						but the buffer was already freed by _ReleaseChunk.
 *						Redesigned HALT_ENABLE code and added two new functions:
 *						  - _CreateHaltChunkMsg to create the new message, message
 *							port and message item,
 *						  - DeliverHaltCHunk to deliver the halt chunk on a separate
 *							message port and wait for the reply before resuming data
 *							delivery.
 *						Fixed _InitializeThread ABORTOPEN code: check the
 *						streamCBPtr->creatorStatus flag for error code and call
 *						_TearDownStreamCB if an error occurred during initialization.
 *						Removed DISCOVER_STUPID_OS_MSG_BUG compile switch in DeliverData.
 * 	 7/29/94	dtc		Version 2.0.1d2
 *						Replaced NULL with "DataStreamThread" as thread name when creating
 *						DataStream thread in NewDataStream.
 *	  7/1/94	dtc		Version 2.0.1d1
 *						Bug #1622: Fixed _DeleteSubscriber to correctly
 *						"slide down" the entries in subscriber table.
 *  6/20/94		dld		Added HALT_ENABLE feature.  When a halt chunk is encountered
 *						the chunk embeded within the HALT chunk is sent to it's subscriber
 *						and then the data streamer immediatly halts.  Data acquisition
 *						continues, but no further data messages are sent to subscribers
 *						(The STOP chunk does not do this).  The streamer hangs out in
 *						halt mode until there is a reply to the message embeded in the
 *						HALT chunk, and the processing continues.  This is the only way
 *						out of the HALT state.
 *	5/19/94		fyp		Version 2.0
 *						integrated lla and nmd as condition RELATIVE_BRANCHING
 *	3/9/94		lla		Moved the DoGoMarker into HandleDSRequest (DSSetSkipMode)
 *	3/6/94		lla		Sync'd all subscribers using DSClockSync ()
 *	2/28/94		nmd		Added searching for key frame in DeliverData.
 *	12/14/93	jb		Version 1.9.1
 *						Moved setting of the "branch in progress" flag and the translated
 *						branch destination value to an external routine callable by
 *						data acquisition called DSSetBrancDest(). Code was moved from 
 *						the DoGoMarker() routine to set the flag and store the target 
 *						value.
 *	12/2/93		jb		Version 1.9
 ...
 *						Add the CloseDataStreaming() routine to allow for deallocation
 *						of context block pool. Connect message to data acquisition
 *						now includes a pointer to the stream control block of the
 *						connecting stream.
 *	11/22/93	jb		Version 1.7
 *						Add check for impossible chunk sizes, zero or negative, in
 *						the DeliverData() routine.
 *	10/21/93	rdg		Version 1.6
 *						Modified DoStopStream() to always send a stop message even 
 *						if the stream is already stopped so if a flush option is 
 *						present, it will always be passed on to the subscribers.
 ...
 *	8/18/93		jb		Version 1.3.4
 *						Initialize repliesPending to zero in HandleDSRequest() before
 *						dispatching to a request handler. Also, reply to requests
 *						immediately in that routine if processing is completed and
 *						no replies from dataacq or subscribers are required.
 *	8/16/93		jb		Version 1.3.2
 *						Add call to OpenAudioFolio() in _InitializeThread() if the
 *						compile switch STARTSTREAM_SETS_THE_CLOCK is set.
 *						Add a range check on the whatToDo field HandleDSRequest()
 *						to prevent queueing of totally invalid request messages.
 *	8/5/93		jb		Version 1.3.1
 *						Add EXIT_ONLY_AFTER_CLOSE_REPLIES related code to cause the
 *						streamer only to exit after all replies to subscriber close
 *						messages have been received.
 *						Fix bug in DoSetChannel() that would always return an error status.
 *	8/3/93		jb		Set privatePtr = NULL in DoConnect() for the disconnect message
 *						sent to previous data acquisition thread.
 *						Get rid of semaphore control of message pool since procedure
 *						level access to the library routines can't use the messages
 *						due to resource ownership problems anyway.
 ...
 *						Added HandleSubscriberReply() routine.
 *						Added _AllocSubsMsg() routine to allocate subscriber messages under
 *						control of a semaphore.
 *						Get rid of _CallDataAcquisition() routine as all communications with
 *						data acquisition are now asynchronous.
 *	6/25/93		jb		Version 1.2.7
 *						Change check in _ReleaseChunk() to == 0 from <= 0 so that
 *						if someone replies chaotically to messages we don't get confused
 *						about the buffer.
 *						Move processing of DSClockSync() to a procedure in the 
 *						DataStreamLib.c file so that sync can be sent immediately
 *						instead of suffering propagation delay by handling it as a
 *						streamer request message.
 *						Export _ForEachSubscriber() for use by DataStreamLib.c
 *	6/25/93		jb		Substract some from end of buffer test in the parser to take into
 *						account FILL chunks that are smaller than the smallest legitimate
 *						size.
 *	6/24/93		jb		Version 1.2.6
 *						Add options to GoMarker messages to allow for relative branching
 ...
 *						started or stopped.
 *						Add an 'exemptStream' arg to _ForEachSubscriber() to allow the
 *						prevention of deadlock when syncrhonously calling subscribers.
 *						This is currently only used to handle DoClockSync() where it
 *						is the normal case that a subscriber is the originator of the
 *						DSClockSync() call.
 *	6/18/93		jb		Make DoStartStream() send a 'kStreamOpStart' message to all
 *						subscribers when a stream is started. This will allow pause/resume
 *						to be implemented in subscribers by providing a way to stop the
 *						data flow without flushing buffered data.
 *	6/15/93		jb		Make DoGoMarker() set STRM_GO_INPROGRESS flag and remember branch
 *						destination if data acquisition accepts the request.
 *						Make STRM_CLOCK_VALID transition when stream is started, stopped
 *						or aborted. This gets set only in DSSetClock().
 ...
 *	5/20/93		jb		Add test for quadbyte alignment of chunk sizes. Also, add the
 *						NOT_QUADBYTE_ALIGNED() macro to clean up same test for buffer list.
 *	5/20/93		jb		Add DoConnect() and the general connect/disconnect facility.
 *						Prevent queuing data unless a data port is defined.
 *	5/19/93		jb		Add additional data request messages when instantiating a new
 *						stream to cover extras needed if trying to send to data acq when
 *						all buffers are in use.
 *	5/12/93		jb		Get rid of STRM_OPEN now that we are thread based.
 *	5/12/93		jb		Move _FillPoolWithMsgItems() to library interface module and
 *						rename as FillPoolWithMsgItems().
 *	5/11/93		jb		Major, significant overhaul. Entire stream parser is now a thread.
 *						All "library" routines now are message handlers. An external
 *						interface library builds request messages and sends them to 
 *						the thread. This mess was necessitated because it is otherwise
 *						impossible for subscribers to call any stream control routines,
 *						such as "go marker", or "stop". This is because doing so requires
 *						sending messages, and the messages owned by a stream were *really*
 *						owned by the stream's initializer.
 ...
 *	5/10/93		jb		Clear the EOF flag in the stream control block when a DSGoMarker()
 *						call is received.
 *	5/10/93		jb		Removed 'DSHTime' data type
 *	5/10/93		jb		Remove DSHGetStreamTime(), remove 'streamTime' from stream 
 *						control block.
 *	5/9/93		jb		Ensure that 'status' is set properly in DSHOpenStream() when
 *						branching to the 'ABORTOPEN' label.
 *	5/9/93		jb		Added DSGoMarker(), handle EOF replies from data client.
 *	5/9/93		jb		Remove all "seeking" related code from the parser. All seeking
 *						actions are handled by calls to DSGoMarker(), and subsequently
 *						by the data acquisition client. 
 *	5/9/93		jb		Switch to using new iterator function in MemPool.c for initing
 *						message pools with message items in _FillPoolWithMsgItems().
 *	4/25/93		njc		Added _TryToSeekStream to DSHRunStream. Looks for a seek chunk
 *						and, if a seek chunk is found, flushes the filled buffer queue
 *						and sends a DoGoMarker msg to the Data Acquisition module.
 *	4/22/93		njc		Test messagePtr->msg_Result in DSHRunStream for Read errors.
 *						On error, stop stream and add buffer back to free list.
 *	4/21/93		njc		Was not incrementing the useCount for buffers in DSHRunStream.
 *	4/19/93		jb		Remove 'acquireContext' since it is implied by the acquisition 
 *						message port now.
 *	4/19/93		jb		Get our ports straight when communicating with data acquisition.
 *						Change dataPort to acqReplyPort, subsPort to subsReplyPort
 *	4/19/93		jb		Handle the case where no subscriber claims any of the chunks in 
 *						a filled data buffer by placing the buffer back into the free pool.
 *	4/19/93		jb		Fix use of 'privatePtr' field in data acq messages.
 *	4/19/93		jb		Make sure free buffers get filled in DSHRunStream()
 *	4/16/93		jb		Do not attempt to maintain stream time. Timestamp info must be
 *						imbedded in time-critical data chunk streams so that individual
 *						subscribers can output their data at rates appropriate to the data.
 *						Get rid of 'currTime' field in message header, only allow calls
 *						to DSClockSync() to change the stream time, and pass the new
 *						clock to all subscribers as a result of that call.
 *	4/16/93		jb		Switch to using SendMsg() from PutMsg() in 3B1
 *	4/13/93		jb		Switch to using NewMsgPort()
 *	4/4/93		jb		Get rid of _GetTime() and use direct reference to stream control
 *						block's time field. This assumes that a timer thread has been
 *						created that will periodically update the time for the stream,
 *						which is done when a stream is opened.
 *	4/3/93		jb		Add checks for open/running stream at start of each routine
 *	4/1/93		jb		Mostly done with 1st cut of message based version
 ...
 *	3/26/93		jb		Add 'opening' call to data acquisition, and pass stream context
 *						in args to open and closing so that data acquisition can be
 *						written to support multiple streams.
 *	3/24/93		jb		Wrote rest of "run stream" and buffer management.
 *	3/20/93		jb		Rethink the data acquisition interface to include markers
 *						and non sequential buffer returns.
 *	3/18/93		jb		Add convenience functions; start on buffer management
 *	3/16/93		jb		New today.
 *
 *******************************************************************************************/

/* Settomg the following switch causes the streamer thread to defer exiting the system
 * until all subscribers have replied to their close messages. Otherwise, the streamer
 * will exit immediately upon receipt of having sent the close messages.
 */
...
/* The following compile switch causes the stream clock to be paused and resumed
 * when stop stream and start stream are called. It depends on the lastValidClock field 
 * and STRM_CLOCK_WAS_VALID bit to do its job. When stop is called, the bit is tested
 * to see if the clock was ever set. If so, the current relative time is remembered so
 * that start can just set the clock to that value.
 */
...
struct DataStreamGlobals	{
	...
	Item			ErrorTextTable;			/* Item containing portfolio extended error texts */
...
/************************************/
static int32		_GetSubscriberGivenType( DSStreamCBPtr streamCBPtr, DSDataType streamType, 
...
static int32		_ReleaseChunk( DSStreamCBPtr streamCBPtr, int32 subscriberStatus, 
								DSDataBufPtr bp );
static Err			_InstallDSErrors(void);

#if HALT_ENABLE
static int32		_CreateHaltChunkMsg( DSStreamCBPtr streamCBPtr );
#endif
...
static int32	DoPreRollStream( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg );
...
static int32 	DoRegisterEndOfStream( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg );

	/* High-level utilities local to the data streaming thread */

static void		HandleSubscriberReply( DSStreamCBPtr streamCBPtr, SubscriberMsgPtr subMsg,
									Message* messagePtr );
static void		HandleDataAcqReply( DSStreamCBPtr streamCBPtr, DataAcqMsgPtr acqMsg,
									Message* messagePtr );
static int32	DeliverData( DSStreamCBPtr streamCBPtr );
static int32	DeliverDataChunks( DSStreamCBPtr streamCBPtr, DSDataBufPtr bp, 
									StreamChunkPtr cp);
#if HALT_ENABLE
static int32	DeliverHaltChunk( DSStreamCBPtr streamCBPtr, DSDataBufPtr bp, 
									StreamChunkPtr cp);
#endif
static int32	HandleDSRequest( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg,
...
/* THIS MUST BE KEPT IN SYNCH WITH DATASTREAM.H */
/************************************************/

static char *DSErrorText[] = 
{
	/* kDSNoErr */					"no error",
	/* kDSWasFlushedErr */			"Buffer associated with this request was flushed",
	/* kDSNotRunningErr */			"Stream was not running",
	/* kDSWasRunningErr */			"Stream was already running",
	/* kDSNoPortErr */				"couldn't allocate a message port for stream", 
	/* kDSNoMsgErr */				"couldn't allocate a message item for data buffer",
	...
	/* kDSBadBufAlignErr */			"buffer list passed contains a non QUADBYTE aligned buffer",
	/* kDSBadChunkSizeErr */		"chunk size in stream is a non-QUADBYTE multiple",
	/* kDSInitErr */				"the streamer has fallen, and it can't get up (internal initialization failed)",
	...
	/* kDSEOSRegistrationErr */		"End Of Stream registrant replaced by new registrant",
	/* kDSRangeErr */				"parameter out of range",
	/* kDSBranchNotDefined */		"branch destination not defined"
};

#define MAX_ERROR_LEN 77

static TagArg ErrorTags[] =
{
    TAG_ITEM_NAME,	(void *)"DataStream",
    ERRTEXT_TAG_OBJID,	(void *)((ER_LINKLIB<<ERR_IDSIZE)|(ER_DATASTREAM)),
    ERRTEXT_TAG_MAXERR,	(void *)(sizeof(DSErrorText)/sizeof(char *)),
...
/*******************************************************************************************
 * This routine links the DataStreamers private error codes into the system error handler
 *******************************************************************************************/
static Err _InstallDSErrors(void)
{
 ...
 * returned, some subscribers may not get called.
 *******************************************************************************************/
static int32	_ForEachSubscriber( DSStreamCBPtr streamCBPtr, SubscriberMsgPtr subMsg )
	{
	int32				status;
	long				index;
	DSSubscriberPtr		sp;
	...
	if ( streamCBPtr->subsMsgPool->numFreeInPool < streamCBPtr->numSubscribers )
		{
		ERR (("_ForEachSubscriber - insufficient number of subscriber messages\n"));
		return kDSNoMsgErr;
		}
	...
	for ( index = 0; index < streamCBPtr->numSubscribers; index++ )
		...
		if ( copySubMsg != NULL )
			...
			*copySubMsg = *subMsg;					/* ...now copy ALL fields from template */
			...
			if ( status < 0 )
				{
				CHECKRESULT( "DataStreamThread - _ForEachSubscriber - SendMsg() ", status );
				break;
				}
			else
...
/*******************************************************************************************
 * Routine to find the subscriber descriptor given the data type. Sets up pSubscriberPtr
 * with a pointer to the subscriber if found, else sets it to NULL.
 *******************************************************************************************/
static int32	_GetSubscriberGivenType( DSStreamCBPtr streamCBPtr, DSDataType streamType,
							DSSubscriberPtr* pSubscriberPtr )
	{
...
/*******************************************************************************************
 * Utility routine to tear down a stream control block. Here, we only dispose of the 
 * non-system resources that were created when the stream was created. The system is
 * supposed to be responsible for removing all system resources we created, so we
 * let it do its own cleanup.
 *******************************************************************************************/
static void _TearDownStreamCB( DSStreamCBPtr streamCBPtr )
	{
    ...
    /* Remove all message ports */
	if ( streamCBPtr->acquirePort )			RemoveMsgPort( streamCBPtr->acquirePort  );
	if ( streamCBPtr->acqReplyPort )		RemoveMsgPort( streamCBPtr->acqReplyPort  );
	if ( streamCBPtr->requestPort )			RemoveMsgPort( streamCBPtr->requestPort );
	if ( streamCBPtr->subsReplyPort )		RemoveMsgPort( streamCBPtr->subsReplyPort  );

#if HALT_ENABLE
	if ( streamCBPtr->halted_msg )			FreePtr( streamCBPtr->halted_msg );
	if ( streamCBPtr->haltChunkReplyPort )	RemoveMsgPort( streamCBPtr->haltChunkReplyPort );
#endif
	}
...
static void		_FreeDataBuf( DSStreamCBPtr streamCBPtr, DSDataBufPtr bp )
	...
	if ( (streamCBPtr->endOfStreamMsg != NULL)
		...
		&& (streamCBPtr->currentFreeBufferCount == streamCBPtr->totalBufferCount) )
		{
...
/*******************************************************************************************
 * Routine to return a pointer to a free data buffer. Called by the buffer filling
 * code that will schedule disk reads into these buffers.
 *******************************************************************************************/
static DSDataBufPtr _GetFreeDataBuf( DSStreamCBPtr streamCBPtr )
	{
	DSDataBufPtr		bp;
...
/*******************************************************************************************
 * Routine to return a pointer to a filled data buffer. Called by the buffer delivery
 * code that will dispatch chunks from these buffers. Returns NULL if there are no
 * more filled data buffers in the list.
 *******************************************************************************************/
static DSDataBufPtr	_GetFilledDataBuf( DSStreamCBPtr streamCBPtr )
	{
	DSDataBufPtr		bp;
...
/*******************************************************************************************
 * Utility routine to fill an empty data buffer with data. Routine either sends a 
 * request to the data acquisition client, or queues the free buffer onto the stream's
 * free buffer list. The latter happens when the stream is stopping, at EOF, or
 * has had some other problem.
 *******************************************************************************************/
static int32	_FillDataBuf( DSStreamCBPtr streamCBPtr, DSDataBufPtr bp )
	{
	...
	if ( (streamCBPtr->acquirePort != 0)
		...
		if ( acqMsg != NULL )
			{
			/* Inform the data acquisition module that a stream is opening. Send a
			 * message to the acquisition port and await its reply.
			 */
...
/*******************************************************************************************
 * Utility routine to record the end of use of a chunk in a buffer. This routine is
 * called by DSHRunStream() when subscribers reply to their "data arriving" messages.
 * A count is kept for each physical I/O block of the number of outstanding owners
 * of chunks in that block. When the count is decremented to zero, the block is 
 * sent to data acquisition to be refilled.
 *******************************************************************************************/
static int32	_ReleaseChunk( DSStreamCBPtr streamCBPtr, int32 subscriberStatus, 
								DSDataBufPtr bp )
	{
...
/*******************************************************************************************
 * Utility routine to add a streamer request message onto the tail of the streamer's 
 * request queue.
 *******************************************************************************************/
static void		_AddDSRequestToTail( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
	reqMsg->link = NULL;
...
/*******************************************************************************************
 * Utility routine to remove the first entry from the streamer request queue and return
 * a point to it. If the queue is empty, returns NULL.
 *******************************************************************************************/
static DSRequestMsgPtr	_GetNextDSRequest( DSStreamCBPtr streamCBPtr )
	{
...
/*******************************************************************************************
 * Routine to get channel status bits. Note that this call is routed through DSH
 * such that runtime binding is available for commonly used subscriber functions.
 * This will permit the creation of easily replacable subscriber components.
 * The subscriber proc gets called with whatToDo == kStreamOpGetChan
 *******************************************************************************************/
static int32	DoGetChannel( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
	int32				status;
	 ...
	 */
	status = _GetSubscriberGivenType( streamCBPtr, reqMsg->msg.getChannel.streamType, &sp );
	if ( status != kDSNoErr )
		return status;

	/* Allocate a message and send the request to the subscriber */
	if ( (subMsg = ((SubscriberMsgPtr) AllocPoolMem( streamCBPtr->subsMsgPool ))) != NULL )
		{
		...
		status = SendMsg( sp->subscriberPort, subMsg->msgItem, subMsg, sizeof(SubscriberMsg) );
...
static int32	DoSetChannel( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	...
	/* Find the subscriber descriptor given the data type 
	 */
	status = _GetSubscriberGivenType( streamCBPtr, reqMsg->msg.setChannel.streamType, &sp );
	if ( status != kDSNoErr )
		return status;
...
static int32	DoControl( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	...
	/* Find the subscriber descriptor given the data type 
	 */
	status = _GetSubscriberGivenType( streamCBPtr, reqMsg->msg.control.streamType, &sp );
	if ( status != kDSNoErr )
		return status;
...
/*******************************************************************************************
 * Routine to discontinue the use of an open stream. Passes the "closing" message
 * to all subscribers.
 *******************************************************************************************/
static int32	DoCloseStream( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
...
/*******************************************************************************************
 * Routine to add a data subscriber to the subscriber list for a given stream. Afterwards,
 * all data designated as belonging to this subscriber (by dataType) will be delivered
 * to the subscriber's message port.
 *******************************************************************************************/
static int32	DoSubscribe( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
	int32				status;
	 ...
	 */
	status = _GetSubscriberGivenType( streamCBPtr, reqMsg->msg.subscribe.dataType, &sp );
...
/*******************************************************************************************
 * Routine to cause empty buffers to be prefilled with data before starting a stream.
 *******************************************************************************************/
static int32	DoPreRollStream( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
	int32			status;
	...
	while ( (bp = _GetFreeDataBuf( streamCBPtr )) != NULL )
		...
		if ( acqMsg != NULL )
			...
			status = SendMsg( streamCBPtr->acquirePort, acqMsg->msgItem, acqMsg, sizeof(DataAcqMsg) );
			if ( status < 0 )
				{
...
/*******************************************************************************************
 * Routine to cause streaming to stop. This causes the "stop" message to be propagated
 * to all subscribers and to the data acquisition proc.
 *******************************************************************************************/
static int32	DoStopStream( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
	int32			status;
...
/*******************************************************************************************
 * Routine to propagate a positioning operation through the system. This consists of 
 * flushing any queued, completed data buffers and sending a positioning message to the data 
 * acquisition client.
 *******************************************************************************************/
static int32	DoGoMarker( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
...
/*******************************************************************************************
 * Routine to connect a new data acquisition to the stream. First, if we are currently
 * connected we send a disconnect message to the current data acquisition client. Then,
 * we switch the aquireport in the stream control block to the new port and send a 
 * connect message to the new data supplier. The disconnect message should cause the
 * first data supplier to cancel any outstanding I/O's and send them back to us as
 * "flushed", which we will simply ignore and place back into the free buffer pool.
 * We take care of flushing any data we currently have queued.
 *******************************************************************************************/
static int32	DoConnect( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg )
	{
	int32			status;
...
/*==========================================================================================
  ==========================================================================================
							Data Streamer Init and Instantiation
  ==========================================================================================
  ==========================================================================================*/


/*******************************************************************************************
 * Routine to initialize all data acquisition activity. Called once at program init
 * time to allow allocation of context pool memory.
 *******************************************************************************************/
int32	InitDataStreaming( long maxNumberOfStreams )
	 ...
	 * contexts.
	 */
	DSGlobals.streamContextPool = CreateMemPool( maxNumberOfStreams, sizeof(DSStreamCB) );
	if ( DSGlobals.streamContextPool == NULL )
		return kDSNoMemErr;

	DSGlobals.ErrorTextTable = _InstallDSErrors ();
...
/*******************************************************************************************
 * Routine to clean up global data streaming resources, called after all data streaming
 * had been shut down and is no longer needed. Currently, just deallocates context 
 * block pool.
 *******************************************************************************************/
int32	CloseDataStreaming( void )
	...
	DeleteItem (DSGlobals.ErrorTextTable);
...
/*******************************************************************************************
 * Routine to instantiate a new data acquirer. Creates all necessary resoures and
 * starts the data acquisition thread running. Called to create a stream interface
 * for a data streaming file.
 *******************************************************************************************/
int32	NewDataStream( DSStreamCBPtr *pCtx, void* bufferListPtr, long bufferSize, 
	...
	/* Create the data stream execution thread, passing it a pointer
...
/*******************************************************************************************
 * Routine to format a "close stream" message and send it to the data stream thread.
 * Then we return the stream context back to the free pool.
 *******************************************************************************************/
int32	DisposeDataStream( Item msgItem, DSStreamCBPtr streamCBPtr )
	...
	status = _SendRequestToDSThread( msgItem, 
	...
	if ( streamCBPtr->threadItem > 0 )		DisposeThread( streamCBPtr->threadItem );
	if ( streamCBPtr->threadStackBlock )	FreePtr( streamCBPtr->threadStackBlock );
...
static void		HandleSubscriberReply( DSStreamCBPtr streamCBPtr, SubscriberMsgPtr subMsg,
	 ...
	 */
	if ( (subMsg->privatePtr != NULL) && (subMsg->privatePtr == streamCBPtr->requestMsgHead) )
		{
		...
		if ( --streamCBPtr->repliesPending == 0 )
			...
			if ( subMsg->whatToDo == kStreamOpGetChan )
				*(reqMsg->msg.getChannel.channelStatusPtr) = messagePtr->msg_Result;
			...
			status = ReplyMsg( reqMsg->msgItem, messagePtr->msg_Result, reqMsg, 
								sizeof(DSRequestMsg) );
			CHECKRESULT( "DataStreamThread - HandleSubscriberReply - ReplyMsg() ", status );
			}
		}
		...
		_ReleaseChunk( streamCBPtr, messagePtr->msg_Result, (DSDataBufPtr) subMsg->privatePtr );
		}
...
#else
	else if ( (subMsg->whatToDo != kStreamOpAbort) && (subMsg->whatToDo != kStreamOpEOF) )
#endif
...
/*******************************************************************************************
 * This routine is called repeatedly with the replies of requests we have sent to the
 * data acquisition client. Some of these requests are simply requests to fill additional
 * empty data buffers, while others implement stream positioning, etc. Some replies will
 * cause the completion of streamer requests, those that are implemented by data 
 * acquisition (as opposed to those implemented by subscribers).
 *******************************************************************************************/
static void		HandleDataAcqReply( DSStreamCBPtr streamCBPtr, DataAcqMsgPtr acqMsg,
									Message* messagePtr )
	{
	...
	/* If DataAcq replied to a go-marker request (in response to either a client
	 * request or a streamer request for shuttling), clear the fDiscardRcvdData
	 * flag to resume paying attention to data buffers received from DataAcq. */
	if ( acqMsg->whatToDo == kAcqOpGoMarker )
		{
		streamCBPtr->fDiscardRcvdData = FALSE;
		}
	 ...
	 * queue, permitting the next request to begin processing.
	 */
	if ( (acqMsg->privatePtr != NULL) && (acqMsg->privatePtr == streamCBPtr->requestMsgHead) )
		{
		...
		if ( --streamCBPtr->repliesPending == 0 )
			...
			status = ReplyMsg( reqMsg->msgItem, messagePtr->msg_Result, reqMsg, 
								sizeof(DSRequestMsg) );
			CHECKRESULT( "DataStreamThread - HandleDataAcqReply - ReplyMsg() ", status );
			}
		else
			/* Since there is one and only one data acquisition client, we should NEVER
			 * ever have a repliesPending > 1. The only streamer requests that are
			 * actually processed by data acquisition currently are:
			 *
			 ...
			 */
			ERR (("DataStreamThread - HandleDataAcqReply - streamCBPtr->repliesPending wrong = %ld\n",
				streamCBPtr->repliesPending));
		}
	 ...
	 */
		...
		if ( messagePtr->msg_Result == 0 &&
			...
			} /* ( messagePtr->msg_Result == 0  && !streamCBPtr->fDiscardRcvdData ) */
			 ...
			 * We may be here also due to fDiscardRcvdData (i.e., stale buffers) or
			 * if this data reply is not from the current DataAcq.
			 * Always return these buffers to the free pool for eventual re-use.
			 */
			_FreeDataBuf( streamCBPtr, bp );
		
			/* If this data is stale, then discount its contents and its msg_Result. */
			if ( streamCBPtr->fDiscardRcvdData )
				messagePtr->msg_Result = kDSNoErr;
			}
		...
		switch ( messagePtr->msg_Result )
			...
			case kDSNoErr:
				...
				default:
					ERR (("DataStreamThread - HandleDataAcqReply - kAcqOpGetData failed with msg_Result = %ld\n", 
	 ...
	 * so dump an error message if we get one?!
	 */
	else if ( (acqMsg->whatToDo != kAcqOpDisconnect) && (acqMsg->whatToDo != kAcqOpGoMarker) )
		{
		ERR (("DataStreamThread - HandleDataAcqReply - unexpected dataacq reply, whatToDo = %ld\n", 
				acqMsg->whatToDo));
		}
...
/*******************************************************************************************
 * Routine to deliver all data currently awaiting delivery, if any.
 *******************************************************************************************/
static int32	DeliverData( DSStreamCBPtr streamCBPtr )
	{
	DSDataBufPtr		bp;
	...
	while ( (bp = _GetFilledDataBuf( streamCBPtr )) != NULL )
		...
		/* Deliver each data chunk in the buffer to it's subscriber.
		 */
		status = DeliverDataChunks( streamCBPtr, bp, cp );
		if ( status != kDSNoErr ) return status;
...
/*******************************************************************************************
 * Routine to parse the current buffer and deliver all data chunks left in a buffer, if any
 * to the appropriate subscribers. This routine is the guts of DeliverData.
 *******************************************************************************************/
static int32	DeliverDataChunks( DSStreamCBPtr streamCBPtr, DSDataBufPtr bp, StreamChunkPtr cp)
	{
	SubscriberMsgPtr	subMsg;
	int32				status;
	DSSubscriberPtr		sp;
	...
	EndOfBufferPtr = ((char *) (((char *)bp->streamData) 
						+ streamCBPtr->bufDataSize 
						- sizeof(StreamChunk) 
						- sizeof(cp->streamChunkData) ));
	
	while ( ((char *) cp) < EndOfBufferPtr )
		{

#if RELATIVE_BRANCHING

		if ( streamCBPtr->streamFlags & STRM_KEYCHUNK_SEARCH )
			{
			/* determine if we've encountered a key chunk */
			if (  (DSGetChunkFlag (cp)) & kKeyChunkFlag ) 
				streamCBPtr->streamFlags &= ~STRM_KEYCHUNK_SEARCH;
			else
				{
		...
		/* When a halt chunk is encountered, the data streamer is
		...
		if ( cp->streamChunkType == STREAM_CHUNK_TYPE ) 
			{
			StreamControlChunk* scp = (StreamControlChunk*) cp;
			
			if ( scp->subChunkType == HALT_CHUNK_TYPE )
				{
				/* Bump the cp to point to the message embeded in the HALT chunk
				 *   and process it as usual.
				 */
				 cp = &scp->subscriberChunk;
				status = DeliverHaltChunk( streamCBPtr, bp, cp );
		
				} /* if HALT_CHUNK_TYPE */

			} /* if STREAM_CHUNK_TYPE */
		else
#endif /* HALT_ENABLE */
			...
			if ( NOT_QUADBYTE_ALIGNED( cp->streamChunkSize ) 
				 ...
				 */
				status = _GetSubscriberGivenType( streamCBPtr, cp->streamChunkType, &sp );
				...
				if ( status == kDSNoErr )
					{
					subMsg = ((SubscriberMsgPtr) AllocPoolMem( streamCBPtr->subsMsgPool ));
					if ( subMsg != NULL )
						{
...
/*******************************************************************************************
 * Routine to deliver halt chunk in a buffer.  This routine waits for a replay from the
 * subscriber before resuming data delivery.
 * [TBD] We may want to revisit this design later.  Halting DataStreamer for a long period
 * of time may cause resources (i.e. buffers ) to not be freed which may be a bad thing.
 *******************************************************************************************/
#if HALT_ENABLE
static int32	DeliverHaltChunk( DSStreamCBPtr streamCBPtr, DSDataBufPtr bp, StreamChunkPtr cp)
	{
	...
	/* If there is a subscriber for this data, pass a pointer
	 * to the data to its subscriber.
	 */
	if ( status == kDSNoErr )
		{
		if ( subMsg != NULL )
			{
			/* whatToDo is kStreamOpData (initialized in _CreateHaltChunkMsg) */
			...
			status = SendMsg( sp->subscriberPort, subMsg->msgItem, subMsg, 
				...
				if ( status == kDSNoErr )
					{
#if DEBUG
					PRT( ( "DeliverHaltChunk() - Sending halt chunk.\n" ) );
#endif
					/* Wait for completion reply */
					msgItem = WaitPort( streamCBPtr->haltChunkReplyPort, subMsg->msgItem );
...
#if DEBUG
					if ( status == kDSNoErr )
						PRT( ( "DeliverHaltChunk() - Reply to halt chunk received.\n" ) );
#endif
					} /* status == kDSNoErr */
...
/*******************************************************************************************
 * This routine is called repeatedly with request messages for the data stream thread.
 * It acts as an overall dispatcher to the routines which actually implement the
 * actions requested by callers of the DataStream Library. Each message is a single
 * request, and each must be replied to once processed so that the request's sender
 * may know when the request has been processed. This allows the sender to operate
 * either synchronously or asynchronously with the DataStream Library.
 *******************************************************************************************/
static int32	HandleDSRequest( DSStreamCBPtr streamCBPtr, DSRequestMsgPtr reqMsg,
									Boolean* pFKeepRunning )
	{
	 ...
	 *
	 * NOTE:	We may queue a new entry, but then initiate whatever is currently
...
#if RELATIVE_BRANCHING
	if ( (reqMsg->whatToDo < 0) || (reqMsg->whatToDo > kDSOpLastOp) )
#else
...
#if RELATIVE_BRANCHING
		case kDSOpSetSkipMode:
			DSGetClock( streamCBPtr, &streamTime );
			status = DSClockSync( streamCBPtr, streamCBPtr->subsMsgPool, streamTime );
			reqMsg->whatToDo = kDSOpGoMarker;
			status = DoGoMarker( streamCBPtr, reqMsg );
			streamCBPtr->repliesPending = 0;	/* NJC - can't accept a reply {lla - so he says}*/
			streamCBPtr->streamFlags |= STRM_KEYCHUNK_SEARCH;
			break;
#endif
...
#if RELATIVE_BRANCHING
			MsgItemPtr = (Message*) LookupItem( reqMsg->msgItem );
			if (MsgItemPtr)
				{
				status = ReplyMsg( reqMsg->msgItem, status, reqMsg, sizeof(DSRequestMsg) );
				CHECKRESULT( "DataStreamThread - HandleDSRequest - ReplyMsg() ", status );
				}
#else
			status = ReplyMsg( reqMsg->msgItem, status, reqMsg, sizeof(DSRequestMsg) );
			CHECKRESULT( "DataStreamThread - HandleDSRequest - ReplyMsg() ", status );
...
static int32	_CreateHaltChunkMsg( DSStreamCBPtr streamCBPtr )
	...
	/* Create a halt message and initialize it. */
	streamCBPtr->halted_msg = (SubscriberMsgPtr) NewPtr( sizeof(SubscriberMsg), MEMTYPE_ANY );
	if ( streamCBPtr->halted_msg != NULL )
		{
		streamCBPtr->halted_msg->whatToDo 	= kStreamOpData;

		/* Create the message item */
		streamCBPtr->halted_msg->msgItem = CreateMsgItem( streamCBPtr->haltChunkReplyPort );
		if ( streamCBPtr->halted_msg->msgItem <= 0 )
			return kDSNoMsgErr;		/* Resources will be released by _TearDownStreamCB */
		}
	else
		return kDSNoMemErr;			/* Resources will be released by _TearDownStreamCB */
...
/*******************************************************************************************
 * Utility routine to perform thread startup initialization. Handles all resource
 * setup and sets the context appropriately for communications with our creator.
 *******************************************************************************************/
static int32	_InitializeThread( DSStreamCBPtr streamCBPtr )
	{
	int32				status;
	...
	streamCBPtr->creatorStatus 			= kDSInitErr;	/* assume initialization will fail */
...
#if HALT_ENABLE
	if( ( status = _CreateHaltChunkMsg( streamCBPtr ) ) < kDSNoErr )
		goto ABORTOPEN;
#endif
	...
	streamCBPtr->subsMsgPool = CreateMemPool( streamCBPtr->numSubsMsgs, sizeof(SubscriberMsg) );
	 ...
	 * to "talk" to data acquisition at a time when all buffers are in progress.
	 * If we don't have some extra, we run out and die. There needs to be one
	 * additional message for each type of async request we may need to send.
	 * For now, these are: kAcqOpGoMarker, kAcqOpConnect, kAcqOpDisconnect.
	 * We fudge and add a couple in case of back to back requests...
	 */
#define	NUMBER_OF_ADDITIONAL_DATA_MSGS	(3 * 2)

	streamCBPtr->dataMsgPool = CreateMemPool( NUMBER_OF_ADDITIONAL_DATA_MSGS
												+ bufferCount, sizeof(DataAcqMsg) );
	...
	if ( ! FillPoolWithMsgItems( streamCBPtr->subsMsgPool, streamCBPtr->subsReplyPort ) )
		goto ABORTOPEN;

	/* Initialize each data message with the necessary
	 * system resources.
	 */
	if ( ! FillPoolWithMsgItems( streamCBPtr->dataMsgPool, streamCBPtr->acqReplyPort ) )
		goto ABORTOPEN;

	/* Looks like everything initialized just fine.
	 */
	streamCBPtr->creatorStatus = kDSNoErr;

ABORTOPEN:
	/* ASSUMES: streamCBPtr->creatorStatus now holds kDSNoErr (success) or a negative
	 * error code.
	 ...
	 */
	if ( streamCBPtr->creatorStatus < 0 ) 
		_TearDownStreamCB( streamCBPtr );
...
/*******************************************************************************************
 * Main thread for running a data stream. Waits for new service requests, data completion
 * messages, and subscriber completion messages. Delivers any received data to subscribers
 * and repeats the entire thing forever. Process exits when a "close" request is received
 * or if some catestrophic error occurrs.
 *******************************************************************************************/
static void		DataStreamThread( long notUsed, DSStreamCBPtr streamCBPtr )
	{
	int32				status;
	...
	while ( true )
		...
		if ( signalBits & streamCBPtr->requestPortSignal )
			{
			while( PollForMsg( streamCBPtr->requestPort, NULL, NULL, (void **) &reqMsg, &status ) )
				{
				status = HandleDSRequest( streamCBPtr, reqMsg, &fKeepRunning );
#if 1
				if ( status != 0 )
					ERR(( "reqMsg->whatToDo = %ld\n", reqMsg->whatToDo ));
#endif
				CHECKRESULT( "DataStreamThread - HandleDSRequest() #1", status );

#if !EXIT_ONLY_AFTER_CLOSE_REPLIES
				/* If we're supposed to exit now as a result of a request, do so...
				 */
...
#if EXIT_ONLY_AFTER_CLOSE_REPLIES
				/* If a close stream request was previously received, and the number
				 * of pending replies just got decremented to zero, then we
				 * are now ready to exit because all subscribers have replied to
				 * the close broadcast.
				 */
				if ( (fKeepRunning == false) && (streamCBPtr->repliesPending == 0) )
					goto THREAD_EXIT;
#endif
				}
			}
		...
		if ( signalBits & streamCBPtr->acqReplyPortSignal )
			{
			while( PollForMsg( (Item) streamCBPtr->acqReplyPort, &msgItem, &messagePtr,  
							(void **) &acqMsg, &status ) )
				{
