 ...
 *	File:			MakeName.c
 *
 *	Contains:		routine to generate a unique name for use with kernel services
 *
...
#define	MAX_NAME_LENGTH		32		/* max length of name we'll create from user prefix */
 ...
 * unique 32-bit value. Typically, the uniqueID value is an address of something that
 * is guaranteed to not be ureused until the object associated with the name is deleted,
 * for example, a pointer that stores an Item named with such a name. Returns a pointer
 * to the output buffer, name, or NULL if it can't fit the postfix into the buffer
 * because it it too small.
 *******************************************************************************************/
char*	MakeName( char* name, int32 maxNameLen, char* baseNameString, int32 uniqueID )
	{
	int32	index;
	char*	destPtr;
