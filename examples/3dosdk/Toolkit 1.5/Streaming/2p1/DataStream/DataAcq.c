 ...
 *	File:			DataAcq.c
 *
 *	Contains:		DataStream data acquisition module for disk file data
 *
 ...
 *						Replaced kprintf with printf.
 *	08/04/94	dtc		Version 2.0.1d3
 *						Fixed code in DoDisconnect to not un-subscribe if
 *						already disconnected.
 * 	  7/29/94	dtc		Version 2.0.1d2
 *						Replaced NULL with "DataAcqThread" as thread name when creating
 *						DataAcq thread in NewDataAcq.
 * 	  7/1/94	dtc		Version 2.0.1d1
 * 						Bug #2166: Don't initialize threadItem to zero in
 * 						_InitializeThread.  ctx->threadItem
 * 						was already created and has a valid item number.
 *	5/19/94		fyp		Version 2.0
 *  5/19/94		fyp		Integrated dlg's changes.  Fixed bug in marker table length (in sizeof) plus
 *						some minor fixes.
 *	5/10/94		fyp		Integrated ccw's changes.  Added index to the marker table, GOMARKER_NUMBER.
 *				fyp		Integrated lla and nmd's changes as condition RELATIVE_BRANCHING.
 *	31/3/94		lla		_MapMarkerToOffset was changed to cope with branching off the
 *						end of the stream for the GOMARKER_FWD_POS_RELATIVE case.
 * 	3/4/94		lla		GOMARKER_FWD_POS_RELATIVE & GOMARKER_BWD_POS_RELATIVE replaced 
 *						GOMARKER_POS_OFFSET for relative move forward and backward
 *	2/28/94		nmd		Added GOMARKER_POS_OFFSET support for relative position based
 *						branching.
 *	12/14/93	jb		Version 1.9.1
 *						Add a call to DSSetBranchDest() to tell the streamer the
 *						translated stream time for the branch destination we calculate.
 *						This enables the CTRL/SYNC mechanism to detect branch destinations
 *						correctly.
 *	11/23/93	jb		Version 1.9
 *						Added CloseDataAcq() routine to clean up things allocated by 
 *						the InitDataAcq() routine.
 *	11/22/93	jb		Version 1.8
 *						Add circular buffer trace of branch destinations. See code
 *						conditionalized by TRACE_TIME_BASED_BRANCHING.
 *	10/19/93	jb		Version 1.7
 *						Switch to using UMemory.c routines for memory allocation
 *	10/5/93		jb		Version 1.6
 *						Moved DeleteItemPool() to thread exit point to fix memory leak.
 *	9/13/93		rdg		Version 1.5
 *						Fixed memory leak.  _InitalizeThread() was zeroing the ctx's stackBlock 
 *						field so DisposeDataAcq() was never able to free the stackBlock.
 *	8/19/93		jb		Version 1.4
 *						Took FAKE_ABORT_IO out. Kept the internal abortQueue 
 ...
 *						Initialize ctx->abortQueue now that we append request to onto
 *						this queue instead of just moving the request queue onto it
 *						(and perhaps blowing a previous queue of aborting requests).
 *	8/3/93		jb		Version 1.2.4
 *						Fix bug in _FlushPendingIO() when called while the abort
 *						queue was non-empty. Now, appends request queue onto the
 *						abort queue instead of always replacing the abort queue
 ...
 *						More kitchen sink code added to FAKE_ABORT_IO hack. Now
 *						we prevent the thread exiting if any I/O's are pending. We wait
 *						around until they complete so that the silly system doesn't crash
 *						itself trying to abort the pending requests when we are requested
 *						to exit via receipt of a 'kAcqOpExit' message.
 *	5/30/93		jb		Substantial change to DisposeDataAcq(). Now it sends an "exit"
 *						message to the thread to get it to clean up anything it created.
 *						Add handling of 'kAcqOpExit' message to the thread.
 *						Set 'threadItem' to zero so that dispose won't try to delete
 *						the thread if the "exit" message has already been sent.
 ...
 *	5/20/93		jb		Change to using connect/disconnect model of data acquisition.
 *						Requires multiple instantiations of data acquisition thread,
 *						one per open file, but prevents opening files "on the fly".
 *	5/19/93		jb		Add FAKE_ABORT_IO hack.
 *	5/16/93		ec		Add call to CloseBlockFile() to DoClosing(), was leaving file open.
 *	5/15/93		ec		Split code to open block file and initialize IORequest items
 *							out of _InitializeThread(), placed in new function 
 *							OpenStreamFile().  Add 'kAcqOpChangeFile' case to DataAcqThread()
 *							Wrote new function DoChangeStreamFile().
 *	5/12/93		jb		Change DoClosing() to flush pending I/O, and get rid of
 *						args to DoOpening() since we don't really use them right now.
 ...
 *						past the EOF.
 *	5/9/93		jb		Added _MapMarkerToOffset() to break out the mapping of markers to
 *						file position offsets. This is the place where anyone modifying
 *						this code should consider adding a higher level mapping function
 *						for mapping markers to file positions. For now, markers *are*
 *						file positions. At a minimum, these should be times instead.
 *	5/9/93		jb		Change 'ResetDataAcq()' to _FlushPendingIO(), & make routine static
 *						instead of external. All I/O flushing is now controlled from within
 *						the thread by DoGoMarker().
 ...
 *	4/25/93		njc		DoGoMarker now removes buffers from the pending queue and calls AbortIO.
 *						Changed call to NewThread to create the thread 2 levels above the main level.
 *	4/20/93		jb		If DoGetData() returns an error, reply to the work request
 *						immediately (do not wait for an I/O completion message that
 *						will never come).
 *	4/20/93		jb		Be sure to send the data message pointer back to the requestor
 *						in our call to ReplyMsg() when completing an async I/O.
 *	4/20/93		jb		Fixed linked list deletion in _FindRequest()
 *	4/20/93		jb		Use ioreqItemPtr from the data message instead of looking up the
 *						item on every I/O completion.
 *	4/19/93		jb		Context is no longer passed to us in any messages, rather, it is
 *						implied by the message port that gets the messages.
 ...
 *	4/17/93		jb		Completely rework using New() & Dispose() model like TestSubscriber.c
 *						Move all "open" initializations to the New() routine, create the
 *						thread and pass the context pointer to the thread. Get rid of all
 *						global data except the context allocation pool. Export the context
 *						block definitions to the header file.
 *	4/16/93		jb		Change to WaitSignal() from Wait() in 3B1
 *	4/13/93		jb		Switch to using NewMsgPort()
 *	4/5/93		jb		Masssive code injection to do message & thread based implementation.
...
#if ALLOW_REQUEST_OVERLOADING
/*******************************************************************************************
 * Utility routine to add a data message onto the tail of the queue of data messages
 * waiting for available IOReq Items.
 *******************************************************************************************/
static bool		AddDataAcqMsgToTail( AcqContextPtr ctx, DataAcqMsgPtr dataMsg )
	{
	bool	fQueueWasEmpty;
...
/*******************************************************************************************
 * Utility routine to locate a data buffer given an I/O request Item and a queue in which to
 * search for the Item. This is called to find the data message that matches a completing
 * I/O request.
 *******************************************************************************************/
static DataAcqMsgPtr	_FindRequest( AcqContextPtr ctx, DataAcqMsgPtr* queueHead, 
										Item ioReqItem )
	 ...
	 * belongs to.
	 */
	for ( acqMsg = *queueHead; acqMsg != NULL; acqMsg = (DataAcqMsgPtr) acqMsg->link )
		{
...
/*******************************************************************************************
 * Flush pending I/O requests. This is called when a positioning operation is taking
 * place to cancel any queued requests since there is no point in continuing to read
 * data from a place in the stream that is no longer of use.
 *******************************************************************************************/
static void		_FlushPendingIO( AcqContextPtr ctx )
	{
	DataAcqMsgPtr		acqMsg;
	 ...
	 */
	for ( acqMsg = ctx->requestQueue; acqMsg != NULL; acqMsg = (DataAcqMsgPtr) acqMsg->link )
		{
...
/*******************************************************************************************
 * Routine to get rid of all data acquisition resources we created. We send an "exit"
 * message to the data acquisition thread. It cleans up all the resources it created.
 * Mainly, this is memory since thread exit should delete all system resources other
 * than memory that the thread created. All that is left should be the thread's stack
 * memory which we delete here. Note that this should be called after any "disconnect"
 * actions since the streamer will get errors if it trys to communicate with the data
 * acquisition thread if a disconnect does not preceed this call.
 *******************************************************************************************/
void	DisposeDataAcq( AcqContextPtr ctx )
	...
	if ( ctx )
		...
		if ( dataReplyPort < 0 )
			{
			printf( "DisposeDataAcq - NewMsgPort() failed\n" );
			return;
			}
		...
		if ( acqMsg.msgItem < 0 )
			{
			printf( "DisposeDataAcq - CreateMsgItem() failed\n" );
			return;
			}
		 ...
		 * acquisition thread.
		 */
		status = SendMsg( ctx->requestPort, acqMsg.msgItem, &acqMsg, sizeof(DataAcqMsg) );
		if ( status >= 0 )
			{
			status = WaitForMsg( dataReplyPort, NULL, &msgPtr, NULL, acqMsg.msgItem );
			}
		...
		if ( ctx->markerChunk )			FreePtr( ctx->markerChunk );
#endif
		}
	}
...
/*******************************************************************************************
 * Routine to perform any initialization at the time when a connection to a stream happens.
 * Gets called as a result of a call to DSConnect(). Note that the message buffer and
 * message item used to post the asynchronous subscription request to the streamer are
 * disposed of when the streamer responds to the request.
 *******************************************************************************************/
static int32	DoConnect( AcqContextPtr ctx, DataAcqMsgPtr dataMsg )
	{
#if TIME_BASED_BRANCHING
...
/*******************************************************************************************
 * Routine to handle disconnection to a stream. Flush any pending I/O operations.
 *******************************************************************************************/
static int32	DoDisconnect( AcqContextPtr ctx )
	{
#if TIME_BASED_BRANCHING
	...
	if (ctx->streamCBPtr == NULL)
		goto BAILOUT;	/* don't try to un-subscribe if already disconnected */
	...
	/* Un-subscribe to the time to position table data type. This will cause
	 * translation table data to be delivered to us when it appears in the
	 * stream.
	 */
	status = DSSubscribe( 
...
BAILOUT:
	 ...
	 */
	if ( dsReqMsgPtr )		FreePtr( dsReqMsgPtr );
	if ( messageItem )		RemoveMsgItem( messageItem );
...
/*******************************************************************************************
 * Routine to validate a marker value, and convert the marker to a file position that
 * can be used in subsequent read operations. This routine should map the marker to a
 * place in the stream where all logical streams within the physical data stream have
 * been temporally synchronized by the stream layout tool to insure that, when data
 * begins to flow again, all data belonging to time T and beyond arrive *after* the
 * physical location returned by this routine.
 *******************************************************************************************/
static int32	_MapMarkerToOffset( AcqContextPtr ctx, unsigned long markerValue, 
									unsigned long markerOptions, long* pOffset )
	{
	...
	/*  Absolute marker number  */
	
	if ( markerOptions == GOMARKER_NUMBER )
	{
		if (ctx->markerChunk == NULL)
			return kDSAbortErr;		// wait for the ack to load the marker table
		
		markerTable = (MarkerRecPtr) ( ((char *) ctx->markerChunk) + sizeof(MarkerChunk) );

#if 0
		marker = markerTable;
		 
		while ( marker < (MarkerRecPtr) ( (char *)markerTable + ctx->markerChunk->chunkSize ))
		{
			printf("marker %d: time %d offset %d\n", marker, marker->markerTime,  marker->markerOffset);
		...
		marker = markerTable+markerValue;
	...
	if ( markerOptions == GOMARKER_ABSOLUTE ) 
		...
		(*pOffset) = markerValue;
//		DSSetBranchType( ctx->streamCBPtr );
		return kDSNoErr;
		}

#if RELATIVE_BRANCHING
	
	/* [NMD] Position Relative branching is always allowed, even if we are not
	 * currently connected to a stream.  This takes the current file position and adds a user
	 * provided value to determine the new file read position...
	 */
	if ( markerOptions == GOMARKER_FWD_POS_RELATIVE )
		{
		markerValue = ctx->offset + markerValue;

		//	lla 3/24/94.  permits ffwd off the end of a stream.   

		if ( markerValue >= GetBlockFileSize( &ctx->blockFile ) )
				(*pOffset) = ctx->offset;
		else	(*pOffset) = markerValue;		

		return kDSNoErr;
		}

	if ( markerOptions == GOMARKER_BWD_POS_RELATIVE )
		{
		if ( ctx->offset >= markerValue )
			markerValue = ctx->offset - markerValue;
		else
			markerValue = 0;
	 ...
	 */
	marker = markerTable;
	while ( (marker < endOfMarkerTable) && (streamClockTarget > marker->markerTime) )
		marker++;
	 ...
	 */
	if ( (marker < markerTable) || (marker >= endOfMarkerTable) )		// changed > to >= 
	return kDSRangeErr;
	...
	/* Just return the input value. Later on, there will be some kind of translation
	 * function that will map a marker number or time to a file position. This may
	 * require some kind of lookup function to find a mapping in a data structure
	 * obtained during stream initialization, for example. Any mapping is possible,
	 * and for now we map 1:1 ==> marker:file position.
	 */
	(*pOffset) = markerValue;
	return kDSNoErr;
#endif
...
/*******************************************************************************************
 * Routine to position subsequent data fetches to a different point in the data stream.
 * NOTE: This functionality is OPTIONAL for data acquisition. For some implementations,
 * such as cable TV streams, seeking may make no sense.
 *******************************************************************************************/
static int32	DoGoMarker( AcqContextPtr ctx, DataAcqMsgPtr dataMsg )
	{
	int32			status;
...
static int32	DoGetData( AcqContextPtr ctx, DataAcqMsgPtr dataMsg )
	...
	if ( bp->ioreqItem == 0 )
		...
		return kDSNoErr;
#else
		printf( "DoGetData - DataAcq.c - AllocPoolItem( ctx->ioReqItemPoolPtr ) failed !!!" );
		return kDSNoMemErr;
#endif
		}
	...
	status = AsynchReadBlockFile( &ctx->blockFile, 	/* stream's block file */
	...
	if ( status >= 0 )
		 ...
		 *		It may be the case that completing I/O does NOT complete
		 *		in the order it was requested. This can be solved here by
...
static int32	_InitializeThread( AcqContextPtr ctx )
	...
	/* Bug #2166: Don't initialize threadItem to zero.  ctx->threadItem
	 * was already created and has a valid item number.
	 */
	ctx->abortQueue			= 0;
	ctx->requestPort		= 0;
...
/*******************************************************************************************
 * This proc is called by CreateItemPool to create an ioReqItem for each pool entry.
 *******************************************************************************************/
	static Item _createIOReqItem ( AcqContextPtr ctx )
		{
...
static int32	OpenStreamFile( AcqContextPtr ctx )
	 ...
	 */
	ctx->ioReqItemPoolPtr = CreateItemPool( NUM_IOREQ_ITEMS, 
								(CreateProcPtr)_createIOReqItem, (void *)ctx );
 ...
 *	Data Acquisition
 *	----------------
 *	This thread performs data acquisition services for the data stream parser.
 *	The parser expects to be supplied with a stream of data buffers in response to a
 *	set of messages to this code. This code is responsible for asynchronously posting 
 *	reads to obtain the data, and for reporting back to the stream parser when each
 *	buffer is filled. The interface to this code from the stream parser is opcode
 *	driven in that each message specifies what action to take via the 'whatToDo' field
 *	of the 'DataAcqMsg' message struct defined in DataStream.h.
 *
 *	Opcodes and what they do:
 *	-------------------------
 *	kAcqOpConnect	This call is make as a result of calling DSConnect(). The
 ...
 *
 *	kAcqOpDisconnect This call is made as a result of calling DSHStreamClose() and is
 ...
 *
 *	kAcqOpGetData	This is the data delivery call. The stream parser is requesting
 ...
 *
 *	kAcqOpGoMarker	No assumption is made by the parser about the addressability of
 ...
 *
 *	kAcqOpExit		Tells the thread to clean up and exit. Sent as a result of a call
 *					to DisposeDataAcq().
 *
 *******************************************************************************************/
static void		DataAcqThread( long notUsed, AcqContextPtr ctx )
	...
	while ( fKeepRunning )
		 ...
		 * freeing up so that additional I/O can be queued (keep the data flowing).
		 */
		if ( signalBits & ctx->ioDoneReplyPortSignal )
			{
			while( PollForMsg( ctx->ioDoneReplyPort, NULL, NULL, (void **) &ioReqItem, &status ) )
				{
				...
				if ( dataMsg != NULL )
					{
					ReplyMsg( dataMsg->msgItem, status, dataMsg, sizeof(DataAcqMsg));

					/* Free the ioReqItem for this dataBuffer back to the item pool */
					ReturnPoolItem( ctx->ioReqItemPoolPtr, ioReqItem );

#if ALLOW_REQUEST_OVERLOADING
					/* Since we know there's now a free IOReqItem available, we check
					 * to see if there's any pending data requests that we can
					 * start up at this point. If there is one, start one and just
					 * one, because that's all the free IOReq items that we know
					 * exist at this point.
					 */
					...
					printf( "DataAcqThread got unknown i/o reply message!\n" );
				}
			}
		...
		if ( signalBits & ctx->requestPortSignal )
			{
			while( PollForMsg( ctx->requestPort, NULL, NULL, (void **) &dataMsg, &status ) )
				{
				switch ( dataMsg->whatToDo )
					...
					case kAcqOpGoMarker:	
						...
						if (status < 0)
							ReplyMsg( dataMsg->msgItem, status, dataMsg, sizeof(DataAcqMsg));		// ccw
					...
					case kAcqOpExit:
#if WAIT_FOR_IO_COMPLETION_BEFORE_EXIT
						/* In the FAKE_ABORT_IO world, we can't really exit now unless
						 * all pending I/O is quiescent. Instead, we set a "we want to exit"
						 * flag and allow the I/O to finish up before actually exiting.
						 * Later, we check for empty in-progress and going-to-abort
						 * queues and exit the thread ONLY when these are both empty.
						 * We must wait until then to ReplyMsg() to the exit request.
						 */
						fWantToExitButCant = true;

						/* Make sure we don't get more than one exit request because
						 * our FAKE_ABORT_IO logic is not fancy enough to handle these.
						 * Only one to a customer...
						 */
						if ( exitRequestMsg == NULL )
							exitRequestMsg = dataMsg;
						else
							printf( "DataAcqThread got multiple kAcqOpExit messages!\n" );
#else
						fKeepRunning = false;
#endif
						break;
		...
		if ( signalBits & ctx->dsReqReplyPortSignal )
			...
			while( PollForMsg( ctx->dsReqReplyPort, &reqCompleteMsgItem, NULL, 
				 ...
				 * Instead, because these happen very infrequently, we dynamically
				 * allocate and free the message items and buffers needed. If 
				 * this proves to cause fragmentation or other problems, we
				 * can always switch to using a pool approach...
				 */
		...
		if ( signalBits & ctx->subscriberPortSignal )
			{
			while( PollForMsg( ctx->subscriberPort, NULL, NULL, (void **) &subMsg, &status ) )
				{	
				if ( subMsg->whatToDo == kStreamOpData )
					...
					if ( markerChunkPtr->subChunkType == MARKER_TABLE_SUBTYPE )
						...
						ctx->markerChunk = (MarkerChunkPtr) NewPtr( markerChunkPtr->chunkSize, 
																	MEMTYPE_ANY );
				 ...
				 */
				ReplyMsg( subMsg->msgItem, kDSNoErr, subMsg, sizeof(SubscriberMsg) );
				}
			}
#endif

#if WAIT_FOR_IO_COMPLETION_BEFORE_EXIT
		/* If we are having to implement the odious fake AbortIO() functionality, we
		 * check for a pending exit request and the absence of either valid pending
		 * I/O requests (found in ctx->requestQueue) or fake-aborted I/O operations
		 * (found in ctx->abortQueue). If both queues are empty AND we have a pending
		 * exit request, we can *really* exit. Otherwise, the stupid system crashes
		 * because its _OWN_ attempt to abort pending operations will cause a crash. 
		 * At this point, we can actually reply to the original exit request message...
		 */
		if ( fWantToExitButCant && (ctx->abortQueue == NULL) && (ctx->requestQueue == NULL) )
			{
			ReplyMsg( exitRequestMsg->msgItem, kDSNoErr, exitRequestMsg, sizeof(DataAcqMsg));
			fKeepRunning = false;
			}
#endif
		}
