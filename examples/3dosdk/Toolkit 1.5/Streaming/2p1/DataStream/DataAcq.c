 ...
 *						Fixed code in DoDisconnect to not
 *un-subscribe if already disconnected. 7/29/94	dtc		Version 2.0.1d2
 *						Replaced NULL with "DataAcqThread" as
 *thread name when creating DataAcq thread in NewDataAcq. 7/1/94	dtc
 *Version 2.0.1d1 Bug #2166: Don't initialize threadItem to zero in
 * 						_InitializeThread.
 *ctx->threadItem was already created and has a valid item number. 5/19/94
 *fyp		Version 2.0
 *  5/19/94		fyp		Integrated dlg's changes.  Fixed bug in
 *marker table length (in sizeof) plus some minor fixes. 5/10/94
 *fyp		Integrated ccw's changes.  Added index to the marker table,
 *GOMARKER_NUMBER. fyp		Integrated lla and nmd's changes as condition
 *RELATIVE_BRANCHING.
 *	31/3/94		lla		_MapMarkerToOffset was changed to cope
 *with branching off the end of the stream for the GOMARKER_FWD_POS_RELATIVE
 *case. 3/4/94		lla		GOMARKER_FWD_POS_RELATIVE &
 *GOMARKER_BWD_POS_RELATIVE replaced GOMARKER_POS_OFFSET for relative move
 *forward and backward
 *	2/28/94		nmd		Added GOMARKER_POS_OFFSET support for
 *relative position based branching. 12/14/93	jb		Version 1.9.1
                    ...
                    unsigned long markerOptions, long *pOffset)
{
   ...
   * currently connected to a stream.  This takes the current file position and
   * adds a user provided value to determine the new file read position...
   */
   ...
   */
  if ((marker < markerTable)
      || (marker >= endOfMarkerTable)) // changed > to >=
    return kDSRangeErr;
  ...
  if (bp->ioreqItem == 0)
    {
      ...
      return kDSNoErr;
#else
      printf ("DoGetData - DataAcq.c - AllocPoolItem( ctx->ioReqItemPoolPtr ) "
              "failed !!!");
      return kDSNoMemErr;
#endif
    }
