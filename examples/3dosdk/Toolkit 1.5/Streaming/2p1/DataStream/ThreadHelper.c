 ...
 *	File:			ThreadHelper.c
 *
 *	Contains:		wrapper routines for kernel thread related services
 *
 *	Copyright © 1992-3 The 3DO Company. All Rights Reserved.
 *
 *	History:
 * 	  7/29/94	dtc		Version 2.0.1d2
 *						Fixed NewThread to make the thread name provided, unique.
 *	10/19/93	jb		Version 1.5
 *						Switch to using UMemory.c routines for memory allocation
 *	8/9/93		jb		Version 1.4
 *						Changed DEFAULT_MAXQ to 0x4000 for Dragontail3 release as required.
 *	7/10/93		jb		Version 1.3
 *						Switch to using MakeName() routine and get rid of non-reentrant
 *						hack that we had before for generating unique names. Now, when we
 *						create a thread with a unique name, we use the thread's stack
 *						memory address as a postfix to the string "Thread" to name it.
 ...
 *	4/18/93		jb		Now that thread names do not appear to be optional, create
 *						an unspeakable hack to generate thread names so the system
 *						won't have to strain itself doing this for us...
 *	4/18/93		jb		Make thread name optional (omit from tag argument list)
 *						if specified name string pointer is NULL.
...
/*******************************************************************************************
 * Routine to create a new thread of execution given the proc pointer, stack size,
 * thread priority, and thread name. Returns the thread's Item or error.
 *******************************************************************************************/
Item	NewThread( void* threadProcPtr, int32 stackSize, int32 threadPriority, 
	 ...
	 */
	*stackMemoryPtr = 0;
	stackMemory = (char *) NewPtr( alignedStackSize, MEMTYPE_ANY );	/* allocate stack space */
	if ( stackMemory == NULL )
		return (Item) 0;
	...
	*stackMemoryPtr = stackMemory;
	...
	if ( threadName == NULL )
		 ...
		 */
		tags[7].ta_Arg = (void *) MakeName( nameBuf, sizeof(nameBuf), 
										"Thread", (int32) stackMemory );
	else
		tags[7].ta_Arg = (void *) MakeName( nameBuf, sizeof(nameBuf), 
										threadName, (int32) stackMemory );
