 ...
 *						Removed typedef for SubsChunkHeader if Relative branching.
 *	05/19/94	fyp		Version 2.0
 *  					Integrated dlg's changes.  Fixed absolute seek by flushing upon
 *						reading a marker.  Before the fix, seek and then check time to see if 
 *						correct marker and then flush, but since absolute seek has no time 
 *						dependency, it would not flush. 
 *	3/14/94		lla		Replaced IsKeyChunk with a DSGetChunkFlag to enable the retrieval of both chunkFlags
 *						and chunk cues.
 ...
 *	3/4/94		lla		Added function IsKeyChunk to determine if a chunkHeader->chunkFlags has the KeyChunk bit set
 *	3/3/94		lla		increased DSControl search to include kDSOpSetSkipModein _SendRequestToDSThread
 *						GOMARKER_POS_RELATIVE forces offset to be measured in block sizes
...
int32	DSIsMarker( DSStreamCBPtr streamCBPtr, uint32 markerValue, Boolean* pfIsMarker )
	...
	if ((streamCBPtr->streamFlags & STRM_GO_INPROGRESS)	&& (streamCBPtr->streamFlags & STRM_GO_ABSOLUTE)) {
...
/*******************************************************************************************
 * Routine to set the flags of the stream to tell it that it is making an absolute seek,
 * so that on an absolute seek to a marker... the marker will be read as a true marker
 * and everything will be flushed
 *******************************************************************************************/
...
#if RELATIVE_BRANCHING
	if ( options == GOMARKER_FWD_POS_RELATIVE || options == GOMARKER_BWD_POS_RELATIVE )
...
/*************************************************************************************
 * This is a shell to keep the DataStream Thread from having to know about subscriber
 * fields.
 *************************************************************************************/
