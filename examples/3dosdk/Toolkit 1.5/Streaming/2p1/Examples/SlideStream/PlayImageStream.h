 ...
 *
 *	History:
 *	11/30/93		jb		Modified into a Join/Image stream player
...
#define	IMAGE_DATA_TYPE		((unsigned long) 'IMAG')

typedef long (*PlayerUserFn)( void *ctx, void *userContext );
...
typedef struct Player {
	...
	PlayerUserFn		userFn;				/* user callback function pointer */
	...
	Item				VRAMIOReq;			/* ioreq for calling CopyVRAMPages() */
	...
	JoinContextPtr		joinContextPtr;		/* Join subscriber context ptr */
	Item				joinPort;			/* join subscription port */
