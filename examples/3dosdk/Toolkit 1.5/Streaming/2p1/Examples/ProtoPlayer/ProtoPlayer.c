 ...
 *	File:			ProtoPlayer.c
 *
 *	Contains:		Simple example of playing test data through the Prototype Subscriber.
 *
 ...
 *
 *	History:
 *  09/01/94	dtc		Replaced kprintf with printf and changed version number to 1.1.
 *	4/04/94		rdg		New today.. cut and past from many sources.
 *
 *******************************************************************************************/
...
int main (int argc, char **argv)
	...
	status = NewProtoSubscriber( &protoCtxPtr, streamCBPtr, SUBSCRIBER_DELTA_PRI );
	...
	status = NewCtrlSubscriber( &controlCtxPtr, streamCBPtr, CTRL_SUBSCRIBER_DELTA_PRI );
	 ...
	 *******************************************************/
	status = DSConnect( messageItem, NULL, streamCBPtr, dataAcqCtxPtr->requestPort );
