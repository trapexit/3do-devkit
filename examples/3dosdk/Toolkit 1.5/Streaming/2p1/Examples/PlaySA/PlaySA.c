 ...
 *
 *	Contains:		example of using high level stream playback routines for audio
 *
 *	Usage:			TestSA <streamFile>
 *					"A" button:		Toggles stream stop/start
 *					"B" button:		Advances current logical channel number (0 - 3).
 ...
 *							Fixed documentation on usage of "B" button.
 *							Added include <stdio.h>
 *	08/18/94		dtc		Version 1.2
 *							Call KillJoyPad to free resources allocated by GetJoyPad and
 *							replace kprintf with printf.
 *							Fixed documetation on usage of "B" button.
 *	10/31/93		rdg		v1.1  Modify to use logical channel 3
 *	10/26/93		rdg		Modify to use more complete joypad implementation
 *							Add button combinations for muting channels
 *	10/25/93		rdg		New today
 *
 *******************************************************************************************/
...
Usage( char* programName )
{
	...
	printf ("\t\"B\" button:	Toggles current logical channel number between 0 and 3.\n");
...
long DoUserInterface( PlayerPtr ctx )
	...
	/* For readability, get the user interface context block from the player context */
	...
	if ( UICtx->jpState.cBtn && !UICtx->jpState.leftShift && !UICtx->jpState.rightShift )
		{
		...
		for ( UICtx->curChannel = 0; UICtx->curChannel < 4; UICtx->curChannel++)
			...
			status = DSControl( ctx->messageItem, NULL, ctx->streamCBPtr, SNDS_CHUNK_TYPE,
									 kSAudioCtlOpCloseChannel, &UICtx->ctlBlock );
			CHECK_DS_RESULT( "DSControl - closing channel", status );

			/* ReEnable audio channels so they will receive data */
			status = DSSetChannel( ctx->messageItem, NULL, ctx->streamCBPtr, SNDS_CHUNK_TYPE, 
										UICtx->curChannel, CHAN_ENABLED );
...
int	 main( int argc, char **argv )
	...
	status = PlaySSNDStream( argv[1], (PlaySSNDUserFn) DoUserInterface, (void*) &UICtx, 
