 ...
 *	11/23/93	jb		Call all "close" routines to get rid of one-time initialization
 *						allocations. Fixes memory leaks.
 *	10/21/93	rdg		Make sure that the "C" button always calls stopstream with
 *						the flush option so that any queued data will be thrown
 *						out before we rewind and start again.
 *	10/19/93	jb		Switch to using OpenAudioFolio() instead of OpenAudio() to
 *						get rid of the "pop" when the application starts up.
 *	10/13/93	jb		Remove call to SendFreeCPakSignal()
 *	10/4/93		jb		Add end of stream test code. NOTE: THIS ENTIRE FILE NEEDS TO 
 *						BE TRASHED AND REWRITTEN. PLEASE, PLEASE, PLEASE DO NOT 
 *						CUT AND PASTE FROM THIS CODE IF YOU WANT A CLEAN EXAMPLE!!!
...
#define	PROGRAM_VERSION_STRING		"2.4"
...
static int32	InitStreaming( DSStreamCBPtr* streamCBPtrPtr, DSDataBufPtr bufferList, 
...
static int32	InitStreaming( DSStreamCBPtr* streamCBPtrPtr, DSDataBufPtr bufferList, 
	...
	status = NewSAnimSubscriber(&gSaAnimCntxPtr, NUMBER_OF_STREAMS, SUBSCRIBER_DELTA_PRI);
	...
	status = NewCPakSubscriber(&gCnaPakCntxPtr, NUMBER_OF_STREAMS, SUBSCRIBER_DELTA_PRI);
	...
	status = NewSAudioSubscriber(&gSAudioCntxPtr, *streamCBPtrPtr, AUDIO_SUBSCRIBER_DELTA_PRI);
	...
	status = NewCtrlSubscriber(&gCntrlCntxPtr, *streamCBPtrPtr, CTRL_SUBSCRIBER_DELTA_PRI);
...
Usage( char* programName )
{
	...
	printf ("\t\t-c 	cinepak stream \n");
	printf ("\t\t-a 	anim stream\n");
	printf ("\t\t-s 	sound only stream\n");
	printf ("\t\"A\" button:	Toggles stream stop/start\n");
	printf ("\t\"B\" button:	Toggles stream timing\n");
	printf ("\t\"C\" button:	Switches to playing the next stream\n");
	printf ("\t				in the command line list. Rewinds\n");
	printf ("\t				the stream in between switching streams.\n");
}
 ...
 */
void
PutNumXY(int32 callsPerSec, int32 secCount, uint32 streamClock, int32 x, int32 y)
{
...
int main (int argc, char **argv)
	...
	// check to make sure we have a file type flag for each file name (remember that the
	//  first param is the program name)
	if ( argc % 2 == 0 )
	{
		printf("### %s - invalid parameter list, need a stream type specified for each stream file name.\n", argv[0], argv[parmNdx]);
		Usage(argv[0]);
		exit(0);
	}
	...
	for ( parmNdx = 1; parmNdx < argc; parmNdx += 2 ) 
	{
		...
		// figure out which stream type the is and grab a pointer to the file name
		streamFileType[fileCount] = StreamFileType(*(argv[parmNdx]+1));
		if ( streamFileType[fileCount] == kNotDefined )
		{
			printf("### %s - \"%c\" is not a valid file type.\n", argv[0], *(argv[parmNdx]+1));
			Usage(argv[0]);
			exit(0);
		}
	...
	for ( acqNdx = 0; acqNdx < fileCount; acqNdx++ )
	{
		...
		status = NewDataAcq( tempAcqPtrPtr, streamFileName[acqNdx], DATA_ACQ_DELTA_PRI );
		CHECK_DS_RESULT( "NewDataAcq 1", status );
	}
	
	status = InitStreaming( &streamCBPtr, dataBufferList, DATA_BUFFER_SIZE,	messageItem);
	 ...
	 * movie, if present.
	 */
	status = DSSetChannel( messageItem, NULL, streamCBPtr, SNDS_CHUNK_TYPE, 1, CHAN_ENABLED );
	CHECK_DS_RESULT( "DSSetChannel", status );

	/* Attach the initial data acquisition to the stream
	 */
	status = DSConnect( messageItem, NULL, streamCBPtr, dataContextPtr[0]->requestPort );
	...
	status = DSWaitEndOfStream( EndOfStreamMessageItem, &EOSMessage, streamCBPtr );
	...
	while ( true )
		 ...
		 */
		endOfStreamFlag = PollForMsg( messagePort, NULL, NULL, (void**) &msgPtr, &status );
		if ( endOfStreamFlag )
			{
			/* Register for end of stream notification */
			status = DSWaitEndOfStream( EndOfStreamMessageItem, &EOSMessage, streamCBPtr );
		...
		if ( joybits & JOYFIREC )
		{
			...
			status = DSSetChannel( messageItem, NULL, streamCBPtr, SNDS_CHUNK_TYPE, 
									1, CHAN_ENABLED );
			CHECK_DS_RESULT( "DSSetChannel", status );
#endif
			...
			switch ( currStreamType )
			{
				...
				case kCinePakStream:
					...
					status = DSControl( messageItem, NULL, streamCBPtr, SNDS_CHUNK_TYPE,
											 kSAudioCtlOpSetClockChan, &ctlBlock );
					CHECK_DS_RESULT( "DSControl - setting audio clock chan = 0", status );
					break;
			}
			...
			status = DSGoMarker( messageItem, NULL, streamCBPtr, (unsigned long)0,
									GOMARKER_ABSOLUTE );
		...
		// call the appropriate subscriber.  the audio subscriber, of course, doesn't
		...
		switch ( currStreamType )
		{
			case kAnimStream:
			{
				...
				/* Tell the SAnimSubscriber to reply all freed chunks back to the DSL */
			...
			case kCinePakStream:
				...
				WaitVBL( VBLIOReq, 1 );
				DrawCPakToBuffer(gCnaPakCntxPtr, gCnaRecChanPtr, 
	...
	for ( acqNdx = 0; acqNdx < fileCount; acqNdx++ )
		...
		printf("Disposing data acquisition #%ld\n", acqNdx );
