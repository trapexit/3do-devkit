 ...
 *
 *	History:
 *  08/22/94		dtc		Replaced printf with printf and included
 *							filefunctions.h
 *	12/10/93		lla		Used Greg's menu screen and hilite cel
...
long RemoveTitle ( ScreenContext *gScreenContextPtr )
	{
	Item 			VRAMIOReq;
	long			status;

	VRAMIOReq = GetVRAMIOReq ();
	status = CopyVRAMPages ( VRAMIOReq,
						gScreenContextPtr->sc_Bitmaps[menuScreen]->bm_Buffer,
						menuImagePtr,
						gScreenContextPtr->sc_nFrameBufferPages, -1 );
	if ( status )
		{
		printf ("Unable to display VRAM pages");
		return ( status );
		}
	status = DisplayScreen (gScreenContextPtr->sc_Screens[menuScreen], 0);
	if ( status )
		{
		printf ("Unable to display screen");
		return ( status );
		}
	return (0);
	}
...
long Initialize_Menu_Stuff ()
	...
	while ( i < N_TITLES )
		...
		if ( temp_item < 0 ) break;
		status = CloseDiskFile ( temp_item );
		if ( status ) printf ( "Error - CloseDiskFile: %ld\n", status );
		
		if ((title[i] = LoadCel( "$boot/Player_Screens/Hilite.cel" /*cel_name ()*/, MEMTYPE_CEL)) == NULL)
			break;
	...
	n_titles = i - 1;
	if ( n_titles == 0 ) return -1;
	else return (n_titles);
	}
...
long ShowTitle ( long selection, ScreenContext *gScreenContextPtr )
	{
	long				status;

	status = DrawCels (gScreenContextPtr->sc_BitmapItems[menuScreen], title[selection]);
	if ( status )
		{
		printf ("Unable to draw cels on screen");
		return (status);
		}
...
//-----------------------------------------------------------------------------
//
//		Tracks cursor movement through the menu and returns the item selected
//
//-----------------------------------------------------------------------------

long	SelectFilm(long last_selection, ScreenContext *gScreenContextPtr)
	{
	...
	while ( true )
		...
		if ( joypadStatePtr->aBtn )
			...
			{			
				...
				RemoveTitle ( gScreenContextPtr );
				WaitAudioClicks (25);
				if ( current_selection == 0) current_selection = n_titles;
				else current_selection = (current_selection - 1)%(n_titles + 1);
				ShowTitle ( current_selection, gScreenContextPtr );
				WaitAudioClicks (50);
			}
		else if ( joypadStatePtr->cBtn ) ShowHelp (gScreenContextPtr);
		}
	}
...
long GetSelection (ScreenContext *gScreenContextPtr, long	LastSelection)
	{
	JoyPadState 	joypadState, *joypadStatePtr = &joypadState;
	
	gScreenContextPtr->sc_curScreen = menuScreen;
	DisplayScreen(gScreenContextPtr->sc_Screens[gScreenContextPtr->sc_curScreen], 0);
...
//------------------------------------------------------------------------------------
	...
	if ( LastSelection > N_STREAMS ) 	LastSelection = 0;
