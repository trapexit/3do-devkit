...
int32	PlayCPakStream( ScreenContext *screenContextPtr, char* streamFileName,
							PlayCPakUserFn userFn, void *userContext, PlayerFuntionContext *pfc )
	{
	int32			status;
	int32			playerResult;
	PlayerPtr		ctx;
	...
	while ( true )
		...
		if ( ctx->userFn )
			{
			playerResult = (*ctx->userFn)( (void *) pfc /*ctx*/, ctx->userContext );

			if ( playerResult )
				{
					if ( playerResult == kDSStepFrame ) continuousPlayMode = false;
					else if ( playerResult == kDSContinuousFrame ) continuousPlayMode = true;
					else if (playerResult == kDSDrawSingleFrame ) continuousPlayMode = false;
					else break;
				}
			}
...
//------------------------------------------------------------------------------------------------//
				...
				DrawCPakToBuffer( ctx->cpakContextPtr, ctx->cpakChannelPtr, bitmap, SKIP_DIFF_FRAMES );
			...
			/* Free any streamer chunks back to the streamer (we're done with them now) */
				...
				{
				DrawStepCPakToBuffer( ctx->cpakContextPtr, ctx->cpakChannelPtr, bitmap );
...
static int32	InitCPakPlayerFromStreamHeader( PlayerPtr ctx, char* streamFileName )
	...
	for ( subscriberIndex = 0; ctx->hdr.subscriberList[ subscriberIndex ].subscriberType != 0;
		...
		switch ( subsPtr->subscriberType )
			...
			default:
				printf( "InitCPakPlayerFromStreamHeader() - unknown subscriber in stream header: '%.4s'\n",
