                ...
                PlayerFuntionContext *pfc)
{
  ...
  while (true)
    {
      ...
      if (ctx->userFn)
        {
          playerResult
              = (*ctx->userFn) ((void *)pfc /*ctx*/, ctx->userContext);

          if (playerResult)
            {
              if (playerResult == kDSStepFrame)
                continuousPlayMode = false;
              else if (playerResult == kDSContinuousFrame)
                continuousPlayMode = true;
              else if (playerResult == kDSDrawSingleFrame)
                continuousPlayMode = false;
              else
                break;
            }
      ...
      if (PollForMsg (ctx->messagePort, NULL, NULL, NULL, &status))
        {
          /* Let calling routine know the stream is all over
           */
          playerResult = 2;
          break;
        }
          ...
          if (continuousPlayMode)
            {
              ...
              //
              //			SKIP_DIFF_FRAMES
              //0		//  skip difference frames to catch up
              //			DECODE_DIFF_FRAMES
              //1		//  decode as fast as you can 			BEST_METHOD
              //-1		//  something else
              //
              //		BEST_METHOD has not been determined yet.
              //However a candidate is to keep a running 		average key frame
              //rate.  If the keyframe rate is less than or equal to 2 seconds
              //skip 		 diff frames.  However if the keyframe rate is greater than
              //2 seconds decode difference frames.
              //
              //  lla 2/22/94
              //
              //------------------------------------------------------------------------------------------------//

              DrawCPakToBuffer (ctx->cpakContextPtr, ctx->cpakChannelPtr,
                                bitmap, SKIP_DIFF_FRAMES);
            ...
            {
              DrawStepCPakToBuffer (ctx->cpakContextPtr, ctx->cpakChannelPtr,
                                    bitmap);
  ...
  CHECK_DS_RESULT ("DSStopStream: PlayCPakStream", status);
