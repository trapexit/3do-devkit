 ...
 *printf. 12/13/93		lla		Fix order of args to
 *CHECK_DS_RESULT()
 *	12/01/93		lla		Made over into a player with menus
 *etc. 10/20/93		jb		New today
 *
 *******************************************************************************************/
...
extern long GetSelection (ScreenContext *gScreenContextPtr,
                          long LastSelection);
...
extern long InitPlayerFunction (PlayerFuntionContextPtr pfc,
                                long numAsyncMsgs);
extern void ClosePlayerFunction (PlayerFuntionContextPtr pfc);
#endif
extern void SetCpakScreen (int32 horizontalOffset, int32 verticalOffset,
                           Boolean centered);
...
void
WaitAudioClicks (AudioTime clicks)
{
  AudioTime endClicks;
      ...
      while (status == kMovieOK)
        {
          ...
          status
              = PlayCPakStream (&gScreenContext, movie_name (streamFile),
