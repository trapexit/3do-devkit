 ...
 *							Replaced printf with
 *printf. 3/7/94			lla		Added
 *GOMARKER_FWD_POS_RELATIVE & GOMARKER_BWD_POS_RELATIVE for fast fwd & bwd
 *	3/2/94			lla		Added block skipping for fast
 *fwd & bwd 12/10/93		lla		Added fast forward and reverse
...
//------------------------------------------------------------------------------------
//
//		These functions are called each time through the player's main
//loop and can be used to 		implement any user interface actions necessary.
//Returning a zero causes playback to 		continue. Any other value causes playback
//to cease and immediately return from 		the PlayCPakStream() function.
//
//------------------------------------------------------------------------------------
//************************************************************************************
//
//			The "PlayerFunction" is the user controls for the main
//movie player 			"X"			- Back to menu
...
long
PlayerFunction (PlayerFuntionContextPtr pfc)
{
  long status;
  ...
  if (joypadStatePtr->xBtn)
    {
      //------------------------------------------------------------------------------------
      //			The "stop button" and "right shift" or "left
      //shift" rewinds movie
      //------------------------------------------------------------------------------------
      if (SHIFT_KEY)
        {
          if (!fRunStream)
            {
              //				status = DSStartStream(
              //ctx->messageItem, NULL, ctx->streamCBPtr, 0 ); 				CHECK_DS_RESULT(
              //"DSStartStream", status );
      ...
      //------------------------------------------------------------------------------------
      else
        {
          ...
          status = DSStopStream (ctx->messageItem, false, ctx->streamCBPtr,
                                 SOPT_FLUSH);
      ...
      if (fRunStream)
        {
          ctlBlockPtr->mute.channelNumber = 0;
          status = DSControl (ctx->messageItem, NULL, ctx->streamCBPtr,
                              SNDS_CHUNK_TYPE, kSAudioCtlOpMute, ctlBlockPtr);
          status = kDSStepFrame;
        }
      else
        {
          ...
          status
              = DSControl (ctx->messageItem, NULL, ctx->streamCBPtr,
                           SNDS_CHUNK_TYPE, kSAudioCtlOpUnMute, ctlBlockPtr);
          status = kDSContinuousFrame;
        }
  ...
  //------------------------------------------------------------------------------------
    ...
    {
      //------------------------------------------------------------------------------------
      //			The "Right arrow" fast forwards at least one
      //second at a time
      //------------------------------------------------------------------------------------
      ...
      if (joypadStatePtr->rightArrow)
        {
          if (SHIFT_KEY)
            skipBlocks = 5 * gFwdBlockSkip;
          else
            skipBlocks = gFwdBlockSkip;
          ...
          if (reqMsg)
            {
              status
                  = DSSetSkipMode (reqMsg->msgItem, reqMsg, ctx->streamCBPtr,
                                   skipBlocks, GOMARKER_FWD_POS_RELATIVE);
      ...
      //------------------------------------------------------------------------------------
      //			The "Down & Left arrow" fast reverses at least
      ...
      //------------------------------------------------------------------------------------
        ...
        {
          if (SHIFT_KEY)
            skipBlocks = 5 * gBwdBlockSkip;
          else
            skipBlocks = gBwdBlockSkip;
          ...
          if (reqMsg)
            {
              status
                  = DSSetSkipMode (reqMsg->msgItem, reqMsg, ctx->streamCBPtr,
                                   skipBlocks, GOMARKER_BWD_POS_RELATIVE);
  ...
  //------------------------------------------------------------------------------------
  //			Poll for any completion messages and return them to the
  //pool
  //------------------------------------------------------------------------------------

  while ((temp = PollForMsg (pfc->replyPort, NULL, NULL, (void **)&reqMsg,
                             &poll_status))
         != false)
    {
...
//		Allocates a pool of streamer request message buffers, a reply
//port, 		and message items to correspond with the request message buffers. 		These
//are dynamically allocated by the player function as needed 		(when there is
//user interaction with the joypad), and are returned 		to the pool by polling
//the reply port in the callback routine. 		The corresponding cleanup routine,
//ClosePlayerFunction(), should 		be called upon completion of use of the player
//function.
//
//************************************************************************************

long
InitPlayerFunction (PlayerFuntionContextPtr pfc, long numAsyncMsgs)
{
  long status;
...
void
ClosePlayerFunction (PlayerFuntionContextPtr pfc)
{
  DSRequestMsgPtr reqMsg;
