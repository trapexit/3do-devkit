 ...
 *
 *	History:
 *	05/24/94	DLD		Added feature to use a default stream
 *header if there doesn't seem to be one in the input stream.
 ...
 *********************************************************************************************/
static DSDataBufPtr
CreateBufferList (long numBuffers, long bufferSize, SAnimPlayerPtr ctx)
{
 ...
 *********************************************************************************************/
static int32
InitSAnimPlayerFromStreamHeader (SAnimPlayerPtr ctx, char *streamFileName)
{
  int32 status;
  long channelNum;
  ...
  for (subscriberIndex = 0;
       ...
       subscriberIndex++)
    {
      ...
      switch (subsPtr->subscriberType)
        ...
        default:
          printf ("InitSAnimPlayerFromStreamHeader() - unknown subscriber in "
                  "stream header: '%.4s'\n",
...
SetupSAnimSubscriber (SAnimPlayerPtr ctx, DSHeaderSubsPtr subsPtr)
{
  ...
  status = NewSAnimSubscriber (&ctx->sanimContextPtr, NUMBER_OF_STREAMS,
  ...
  for (channelNum = 0; channelNum < NUM_DIRECTOR_CHANNELS; channelNum++)
    {
      status
          = InitSAnimCel (ctx->streamCBPtr, ctx->sanimContextPtr,
                          &ctx->sanimChannelPtr[channelNum], channelNum, true);
      if (status != 0)
        goto CLEANUP;
    }
 ...
 *******************************************************************************************/
void
DismantlePlayer (SAnimPlayerPtr ctx)
{
  long channelNum;
  ...
  if (ctx->sanimChannelPtr)
    {
      for (channelNum = 0; channelNum < NUM_DIRECTOR_CHANNELS; channelNum++)
        {
          DestroySAnimCel (ctx->sanimContextPtr,
                           ctx->sanimChannelPtr[channelNum],
                           ctx->sanimChannelPtr[channelNum]->channel);
        }
    }
  ...
  if (ctx->rawBufferList)
    free (ctx->rawBufferList);
  if (ctx->messageItem)
    RemoveMsgItem (ctx->messageItem);
  // if ( ctx->endOfStreamMessageItem )	RemoveMsgItem(
 ...
 */
void
ShowStreamTiming (SAnimPlayerPtr ctx)
{
...
int32
PlaySAnimStream (ScreenContext *screenContextPtr, char *streamFileName)
{
  int32 status;
  int32 playerResult;
  ...
  status
      = InitSAnimPlayerFromStreamHeader (&sanimPlayerContext, streamFileName);
  if (status != 0)
    return status;
  ...
  while (true)
    {
      ...
      if (joybits & JOYFIREA)
        {
          ...
          if (fRunStream)
          ...
          else
            {
               ...
               */
              status
                  = DSStopStream (ctx->messageItem, NULL, ctx->streamCBPtr, 0);
      ...
      if (joybits & JOYFIREC)
        {
          ...
          for (channelNum = 0; channelNum < NUM_DIRECTOR_CHANNELS;
               channelNum++)
            FlushSAnimChannel (ctx->sanimContextPtr,
                               ctx->sanimChannelPtr[channelNum], 0);
          ...
          if (fRunStream)
            {
              status
                  = DSPreRollStream (ctx->messageItem, NULL, ctx->streamCBPtr);
              CHECK_SA_RESULT ("DSPreRollStream", status);
      ...
      if (joybits & JOYUP)
        {
          ...
          status = DSControl (ctx->messageItem, NULL, ctx->streamCBPtr,
                              SNDS_CHUNK_TYPE, kSAudioCtlOpGetVol, &ctlBlock);
          if (status < 0)
            {
          ...
          if (volume < 0x7A00)
            {
              ...
              status
                  = DSControl (ctx->messageItem, NULL, ctx->streamCBPtr,
                               SNDS_CHUNK_TYPE, kSAudioCtlOpSetVol, &ctlBlock);
              if (status < 0)
                {
                  printf ("\n error sending control message!\n");
                  printf (" status = %ld.\n", status);
      ...
      if (joybits & JOYLEFT)
        {
          ...
          status = DSControl (ctx->messageItem, NULL, ctx->streamCBPtr,
                              SNDS_CHUNK_TYPE, kSAudioCtlOpGetPan, &ctlBlock);
          if (status < 0)
            {
              printf ("\n error sending control message!\n");
              printf (" status = %ld.\n", status);
          ...
          if (pan > 0x600)
            {
              ...
              status
                  = DSControl (ctx->messageItem, NULL, ctx->streamCBPtr,
                               SNDS_CHUNK_TYPE, kSAudioCtlOpSetPan, &ctlBlock);
              if (status < 0)
                {
                  printf ("\n error sending control message!\n");
                  printf (" status = %ld.\n", status);
      ...
      for (channelNum = 0; channelNum < NUM_DIRECTOR_CHANNELS; channelNum++)
        {
          /* Get the next frame */
          ccbPtr = GetSAnimCel (ctx->sanimContextPtr,
                                ctx->sanimChannelPtr[channelNum]);
          if (ccbPtr != NULL)
            {
  ...
  for (channelNum = 0; channelNum < NUM_DIRECTOR_CHANNELS; channelNum++)
    FlushSAnimChannel (ctx->sanimContextPtr, ctx->sanimChannelPtr[channelNum],
                       0);
