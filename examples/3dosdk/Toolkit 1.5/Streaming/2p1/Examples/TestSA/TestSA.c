 ...
 *					"B" button:		Advances current channel number (0 - 4).
 *					"C" button:		Rewinds the stream
 *
 ...
 *						Fix Bug #3336: Fixed usage documentation on "B" button.
 *						Added include <stdio.h>
 *	12/2/93		jb		Add addition new arg to NewDataStream()
 *	10/11/93	rdg		Make sure CLOSE_AUDIO_ON_SWITCH re-enables channels after
 *						they are closed.
 *	9/30/93		rdg		Change CLOSE_AUDIO_ON_SWITCH to send close messages to
 *						channels 0-4 on rewind.
 *	9/29/93		rdg		Enable chans 1-4, don't init volume to zero.
 *	9/28/93		rdg		Fix volume control... was impossible to get full max and min.
 *	9/27/93		rdg		Add template tag for 16Bit, 22K, Stereo, 2:1 SDX2
 *	9/20/93		rdg		Enable channel 2 
 *						Support up to 3 concurrent logical channels
 *	9/17/93		rdg		Modified the code which parses the "C" button so that it
 *						will always call StopWithFlush.  This way, if you are 
 *						playing a stream and you...
 *							Stop the stream with the "A" button. 
 *							Rewind with the "C" button.
 *							Start again with the "A" button.
 *						data will be flushed by the subscribers as it should be.
 *						Before this mod you would hear the last audio buffer
 *						queued before the rewind when you re-started the stream. 
...
Usage( char* programName )
{
	...
	printf ("\t\"B\" button:	Toggles current channel number between 0 - 4.\n");
...
#define	DATA_ACQ_DELTA_PRI	8
	...
	status = DSSetChannel( messageItem, NULL, streamCBPtr, SNDS_CHUNK_TYPE, 2, CHAN_ENABLED );
	CHECK_DS_RESULT( "DSSetChannel", status );

	/* Enable audio channel #3 */
	status = DSSetChannel( messageItem, NULL, streamCBPtr, SNDS_CHUNK_TYPE, 3, CHAN_ENABLED );
	CHECK_DS_RESULT( "DSSetChannel", status );

	/* Enable audio channel #4 */
	status = DSSetChannel( messageItem, NULL, streamCBPtr, SNDS_CHUNK_TYPE, 4, CHAN_ENABLED );
	CHECK_DS_RESULT( "DSSetChannel", status );

	/* Attach the initial data acquisition to the stream
	 */
	status = DSConnect( messageItem, NULL, streamCBPtr, dataContextPtr->requestPort );
