 ...
 *
 *	History:
 *  06/30/94	DLD		Added fWriteHaltChunk and HaltChunkList to WeaveParams structure, 
 *						to support the new weaver command "WriteHaltChunk"
 *  04/25/94	dtc		Version 2.2
 *						Changed definition of InStream to allow unlimited number of input
 *						streams. It currently handles 20 filenames.
 *						Added the definition of ControlChunk struct to handle output
 *						of stop and goto chunk if requested by the user.
 *	12/2/93		jb		Add numSubsMsgs to weave parameter block
 *	11/9/93		jb		Move definition of MarkerRec to MarkerChunk.h
 *	10/25/93	jb		Add fWriteMarkerTable flag; increase MAX_INPUT_STREAMS to 20;
 *						Add maxMarkers field to param block
 *	10/13/93	jb		Add fields to WeaveParams for stream header output
 *	6/27/93		jb		Add ioBufferSize to parameter block
 *	6/17/93		jb		Add CHAR4LITERAL macro and switch to using it in place of character
 *						literals for portability's sake.
 *	5/18/93		jb		Switch to using hexadecimal const for FILLER_DATA_TYPE to keep
 *						compiler from being confused about scalar size.
...
/* The following compile switch determines whether or not to allow unlimited number of
 * input streams through USE_MAC_IO. If you don't have to weave more than 15 files, 
 * portability will be maintained by setting USE_MAC_IO to 0.  USE_MAC_IO is defined in
 * Weaver.make.
 */
#if USE_MAC_IO
#include <files.h>
...
#define	CONTROL_CHUNK_TYPE		CHAR4LITERAL('C','T','R','L')	/* Chunk type for this subscriber */

#define	ALRM_CHUNK_SUBTYPE		CHAR4LITERAL('A','L','R','M')	/* ALRM chunk sub-type            */
#define	CONTROL_SYNC_SUBTYPE	CHAR4LITERAL('S','Y','N','C')	/* SYNC chunk sub-type            */
...
typedef struct InStream {
		...
		IOParam				iopb;			/* Need to use a PBOpen call to get read/only for input files */
#else
