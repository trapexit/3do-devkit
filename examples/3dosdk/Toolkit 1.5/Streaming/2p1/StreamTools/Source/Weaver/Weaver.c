 ...
 *	History:
 *
 *	10/07/94	dtc		Version 2.3d3
 *						Fix - Bug#3433: Make sure that at least one file name was specified
 *	10/03/94			Bug #3306: Range check IOBufferSize, max range should not exceed
 *						65535.
 *	 7/12/94	dtc		Version 2.3d2
 *						added support for ADPCM and 22K uncompressed DSP instruments
 *	 7/12/94	dtc		Version 2.3d1
 *						Updated PROGRAM_VERSION_STRING to 2.3.
 *	 7/12/94	dtc		Version 2.3
 *						Changed subspart and haltpart to subscriberChunk
 *						and streamerChunk.
 *  06/30/94	DLD		Added command WriteHaltChunk.  
 *						Syntax: WriteHaltChunk <streamtime> <SubscriberCode>
 *						This will output a Halt chunk keyed to the specified
 *						subscriber after all the chunks for a given stream time.
 *						The chunk sent to the subscriber at halt time will have
 *						the subtype HALT.
 *  06/30/94	DLD		Added code to zero the chunkFlags when RELATIVE_BRANCHING is 
 *						enabled in the StopChunk and GotoChunk writing code.
 ...
 *						Integrated Donn's fix to allow quoted instream filenames.
 ...
 *						Fix ParseCommandLine so that a boolen false is returned if
 *						there's no command to parse.
 *						Fixed the handling of commentline so that everything after
 *						the '#' is treated as comment line (before you must follow the
 *						'#' with a space).
...
long		gIOBufferSize			= (32 * 1024L);	/* Max range is 65535, since size_t is
													 * defined as integer */
...
long main( int argc, char* argv[] )
	...
	gWSParamBlk.fWriteHaltChunk		= gWriteHaltChunk;
...
BAILOUT:
	...
	for ( ; gWSParamBlk.HaltChunkList != nil; )
		{
		HaltChunksPtr				HPtr;
...
typedef enum {
	...
	kHaltChunk,			/* expecting HALT control chunk */
	kHaltSubscriber		/* expecting HALT subscriber code */
	} ParserState;
...
static Boolean ParseDirectives( char* pWord )
	...
	static HaltChunksPtr	CurHaltPtr;		/* This holds the pointer to the current
											 * halt chunk structure.
											 */
	...
	switch ( gParseState )
		...
		case kNewCommand:
			...
			else if ( WordCmp( pWord, "writehaltchunk" ) == 0 )
				{
				gParseState = kHaltChunk;
		...
		case kStopChunk:
			...
			StopChunksPtr->DataChunk.chunkFlags		= 0;
#endif
...
#if RELATIVE_BRANCHING
			GotoChunksPtr->DataChunk.chunkFlags		= 0;
#endif
		...
		case kHaltChunk:
			...
			CurHaltPtr = (HaltChunksPtr) malloc ( sizeof(HaltChunk) );
	 		if ( CurHaltPtr == nil )
	 			{
				fprintf( stderr, "Error allocating memory for Halt Chunk object.\n" );
				gParseError = true;
				return false;
				}

			/* Fill in the entry */
			CurHaltPtr->DataChunk.streamerChunk.chunkType		= STREAM_CHUNK_TYPE;
			CurHaltPtr->DataChunk.streamerChunk.subChunkType	= HALT_CHUNK_TYPE;
			CurHaltPtr->DataChunk.streamerChunk.chunkSize		= sizeof(HaltChunk);
#if RELATIVE_BRANCHING
			CurHaltPtr->DataChunk.streamerChunk.chunkFlags	= 0;
#endif
			CurHaltPtr->DataChunk.streamerChunk.channel		= 0;
			sscanf( pWord, "%li", &CurHaltPtr->DataChunk.streamerChunk.time );
			
			gParseState = kHaltSubscriber;
	 		break;
			
		case kHaltSubscriber:
			 ...
			 */
			{
				...
				CurHaltPtr->DataChunk.subscriberChunk.chunkType	= subscriberTag;
				CurHaltPtr->DataChunk.subscriberChunk.subChunkType	= HALT_CHUNK_TYPE;
				CurHaltPtr->DataChunk.subscriberChunk.chunkSize	= sizeof(SimpleChunk);
#if RELATIVE_BRANCHING
				CurHaltPtr->DataChunk.subscriberChunk.chunkFlags	= 0;
#endif
				CurHaltPtr->DataChunk.subscriberChunk.channel		= 0;
				CurHaltPtr->DataChunk.subscriberChunk.time 		= CurHaltPtr->DataChunk.streamerChunk.time;
				CurHaltPtr->Next							= nil;
	
				/* insert this chunk into the linked list of Halt chunks */
				InsertCtrlChunks( (CtrlChunks**) &gWSParamBlk.HaltChunkList, 
								  (CtrlChunksPtr) CurHaltPtr );
			}
			gParseState = kNewCommand;
			break;
...
static void	AddPreloadInstrument( WeaveParamsPtr pb, char* pWord )
	...
	if ( gNextPreloadInstIndex < (DS_HDR_MAX_PRELOADINST - 1) )
		...
		else if ( WordCmp( pWord, "sa_22k_16b_s" ) == 0 )
			instrumentTag = SA_22K_16B_S;

		else if ( WordCmp( pWord, "sa_22k_16b_m" ) == 0 )
			instrumentTag = SA_22K_16B_M;
		...
		else if ( WordCmp( pWord, "sa_44k_16b_m_adpcm4" ) == 0 )
			instrumentTag = SA_44K_16B_M_ADPCM4;

		else if ( WordCmp( pWord, "sa_22k_16b_m_adpcm4" ) == 0 )
			instrumentTag = SA_22K_16B_M_ADPCM4;
...
static Boolean ParseCommandLine ( int argc, char **argv )
	...
	while ( argc > 0 )
		 ...
		 */
			...
			/* Range check gIOBufferSize 0 < gIOBufferSize < 65536 */
			if ( gIOBufferSize > 0 || gIOBufferSize < 65536 )
				{
				fprintf( stderr, "I/O buffersize must be between 1 and 65,535.\n\n");
				return false;
				} /* if ( gIOBufferSize <= 0 || gIOBufferSize > 65535 ) */
			}
...
static void Usage( char* commandNamePtr )
	...
	fprintf( stderr, "		%s <size>	I/O buffer size (0<size<65536, Def: 32k)\n", IOBUFSIZE_FLAG_STRING );
