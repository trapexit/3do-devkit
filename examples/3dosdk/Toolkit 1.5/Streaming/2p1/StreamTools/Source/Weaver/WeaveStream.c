 ...
 *						Removed a definition that is now on the command
 *						line.  Changed a reference to HaltPart to its new
 *						name: streamerChunk.
 *						
 *	07/12/94	dtc		Changed subspart and haltpart to subscriberChunk
 *						and streamerChunk.
 *  06/30/94	DLD		Added WriteHaltChunk() and code to call it, 
 *						to support the new weaver command "WriteHaltChunk"
 *	06/09/94	dtc		Moved the following compile switches to Weaver.make:
 *							FORCE_FIRST_DATA_ONTO_STREAMBLOCK_BOUNDARY
 *							USE_MAC_IO
 *							USE_BIGGER_BUFFERS
 *						Fixed to pass in the correct number of bytes remaining in the first
 *						block of the stream before calling WriteMarkerTable the second time
 *						to write in the table with all entries filled in.
 *						Fixed to writeMarkerTable only if fWriteMarkerTable flag is set.
 *
...
static Boolean WriteHaltChunk
	 ...
	 */
	 if ( ( isp->buf.weaveChunkTime > pb->HaltChunkList->DataChunk.streamerChunk.time ) ||
		 ( isp->eof ) )
		{
...
#if USE_MAC_IO
		if ( noErr != FSWrite( pb->outputStreamFd, &bytesToWrite, &pb->HaltChunkList->DataChunk ) )
#else
