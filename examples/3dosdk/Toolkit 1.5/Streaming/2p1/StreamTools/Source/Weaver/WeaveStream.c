 ...
 *	History:
 *
 *	 7/12/94	dtc		Version 2.3
 *						Removed a definition that is now on the
 *command line.  Changed a reference to HaltPart to its new name:
 *streamerChunk.
 *
 *	07/12/94	dtc		Changed subspart and haltpart to
 *subscriberChunk and streamerChunk.
 *  06/30/94	DLD		Added WriteHaltChunk() and code to call it,
 *						to support the new weaver command
 *"WriteHaltChunk" 06/09/94	dtc		Moved the following compile
 *switches to Weaver.make: FORCE_FIRST_DATA_ONTO_STREAMBLOCK_BOUNDARY
 *							USE_MAC_IO
 *							USE_BIGGER_BUFFERS
 *						Fixed to pass in the correct number of
 *bytes remaining in the first block of the stream before calling
 *WriteMarkerTable the second time to write in the table with all entries
 *filled in. Fixed to writeMarkerTable only if fWriteMarkerTable flag is set.
 *
 *  04/25/94	dtc		Version 2.2
 *						Allow unlimited number of input streams
 *through USE_MAC_IO. The limit was set to 20 files using standard I/O. Added
 *functions WriteAlrmChunk WriteGotoChunk, WriteStopChunk, WriteFillerChunk to
 *process commands to accept stop and goto control chunks: writegotochunk
 *<streamtime> <destinationtime> writestopchunk <streamtime> Replaced
 *SortStreamFiles with InsertStreamFiles to handle sorting of linked list of
 *input streams in ascending priority order. SortStreamFiles sorted the fixed
 *size array of input streams. Moved the definition of ControlChunk struct to
 *WeaveStream.h. 12/2/93		jb		Propagate numSubsMsgs
 *from weave param block into header data 10/25/93	jb		Change
 *fopen() mode to "wb+"; add code to write the marker table into the stream.
 *Force stream data onto block #1 of output file if either stream header or
 *marker table are present in the output file.
 *	10/20/93	jb		Make output of stream header chunk
 *optional. Added some very questionable code in ReadChunkHeader() to handle
 *input streawms with partially formed FILL chunks. This is to allow woven
 *streams as input files. This should be fixed in the future by requiring woven
 *streams to contain a stream header. This way, we can use the stream block
 *size value in the input stream to determine start of block addresses much the
 *same way it is done in the DumpStream tool. 10/19/93	jb		Added
 *WriteHeaderChunk() routine which outputs a descriptive header data chunk to
 *make generalized playback functions possible. 10/11/93	jb
 *Check chunk size <= stream block size in ReadChunkHeader().
 *	9/12/93		rdg		Spin cursor.
 *	6/27/93		jb		Allocate large I/O buffers to make
 *things go faster
 *	6/21/93		jb		Change Control Subscriber main chunk
 *type to 'CTRL' to match everything else in the world. 6/17/93 jb
 *Output Control Subscriber 'SYNC' chunks at the beginning of every marker
 *block to allow the implementation of a "conditional flush". Added
 *WriteSyncChunk() routine.
...
/****************************/
static InStreamPtr GetEarliestBelowNextMarker (WeaveParamsPtr pb,
                                               long sizeRemaining);
static InStreamPtr GetBestFitBelowNextMarker (WeaveParamsPtr pb,
                                              long sizeRemaining);
...
static Boolean WriteChunk (WeaveParamsPtr pb, InStreamPtr isp,
                           long *bytesRemaining);
static Boolean WriteSyncChunk (WeaveParamsPtr pb, long *bytesRemaining,
                               long timeValue);
...
static Boolean WriteAlrmChunk (WeaveParamsPtr pb, long *bytesRemaining,
                               long timeValue);
static Boolean WriteFillerChunk (WeaveParamsPtr pb, WeaveChunkPtr pFillerBlk,
                                 long *bytesRemaining);
static Boolean WriteGotoChunk (WeaveParamsPtr pb, InStreamPtr isp,
                               WeaveChunkPtr pFillerBlk, long *bytesRemaining);
static Boolean WriteStopChunk (WeaveParamsPtr pb, InStreamPtr isp,
                               WeaveChunkPtr pFillerBlk, long *bytesRemaining);
static Boolean WriteHaltChunk (WeaveParamsPtr pb, InStreamPtr isp,
                               WeaveChunkPtr pFillerBlk, long *bytesRemaining);
...
SortMarkers (WeaveParamsPtr pb)
{
  ...
  if (pb->numMarkers < (pb->maxMarkers - 1))
    ...
    {
      fprintf (stderr,
               "SortMarkers() - unable to add infinity marker to table\n");
      return;
    }
...
/*******************************************************************************************
 * Routine to search the input streams and determine if any data is left to
 *output.
 *******************************************************************************************/
static Boolean
InputDataRemaining (WeaveParamsPtr pb)
{
...
/*******************************************************************************************
 * Routine to search the input streams and find the stream whose chunk is
 *temporally "next" in time (least value numerically), and whose time is before
 *the next marker time in the stream.
 *******************************************************************************************/
static InStreamPtr
GetEarliestBelowNextMarker (WeaveParamsPtr pb, long sizeRemaining)
{
...
/*******************************************************************************************
 * Routine to find the next block in the stream which is both below the next
 *marker temporally, and is a best fit for the 'size' given.
 *******************************************************************************************/
static InStreamPtr
GetBestFitBelowNextMarker (WeaveParamsPtr pb, long sizeRemaining)
{
  InStreamPtr winner = nil;
  InStreamPtr isp = pb->InStreamList;
  long sizeToBeat = -1;
...
/*******************************************************************************************
 * Routine to read the next chunk header for a given input stream. Sets any
 *necessary flags and outputs diagnostic messages if there's any error.
 *******************************************************************************************/
static Boolean
ReadChunkHeader (WeaveParamsPtr pb, InStreamPtr isp)
{
  long fillChunkSize;
      ...
      if (isp->buf.weaveChunkType == FILLER_DATA_TYPE)
        {
...
#if USE_MAC_IO
          if (noErr
              == SetFPos (isp->fd, fsFromMark,
                          fillChunkSize - sizeof (WeaveChunk)))
            continue;
#else
          if (0
              == fseek (isp->fd, fillChunkSize - sizeof (WeaveChunk),
                        SEEK_CUR))
            continue;
#endif
          ...
          fprintf (
              stderr,
              "ReadChunkHeader() - error seeking past filler in file: %s\n",
              isp->fileName);
      ...
      if (isp->buf.weaveChunkSize > pb->streamBlockSize)
        {
          fprintf (
              stderr,
              "ReadChunkHeader() - chunk size > streamblocksize in file: %s\n",
              isp->fileName);
          return false;
        }
  ...
  if (status == eofErr)
  ...
  else
    {
      fprintf (
          stderr,
          "ReadChunkHeader() - error reading next chunk header from: %s\n",
          isp->fileName);
      return false;
    }
#else
 ...
 *******************************************************************************************/
static Boolean
WriteChunk (WeaveParamsPtr pb, InStreamPtr isp, long *bytesRemaining)
{
   ...
   *			boundary is approaching, not that any buffer is actually
   *being consumed. This allow the algorithm to write out blocks that will
   *match the stream reader's input buffers.
   */
 ...
 *******************************************************************************************/
static Boolean
WriteHeaderChunk (WeaveParamsPtr pb, long *bytesRemaining)
{
  ...
  for (index = 0;
       index < sizeof (pb->preloadInstList) / sizeof (pb->preloadInstList[0]);
       index++)
    header.preloadInstList[index] = pb->preloadInstList[index];

  /* Copy the subscriber tag table */
  for (index = 0;
       index < sizeof (pb->subscriberList) / sizeof (pb->subscriberList[0]);
       index++)
 ...
 *******************************************************************************************/
static Boolean
WriteMarkerTable (WeaveParamsPtr pb, long *bytesRemaining)
{
  ...
  if (markerChunk.chunkSize > (*bytesRemaining))
    {
      fprintf (stderr, "WriteMarkerTable() - marker table will not into first "
                       "stream block\n");
      return false;
    }
  ...
  if (1 != fwrite (&markerChunk, sizeof (MarkerChunk), 1, pb->outputStreamFd))
    ...
    {
      fprintf (stderr,
               "WriteMarkerTable() - error writing marker table header\n");
      return false;
    }
  ...
  if (1 != fwrite (pb->marker, sizeOfMarkerData, 1, pb->outputStreamFd))
    ...
    {
      fprintf (stderr,
               "WriteMarkerTable() - error writing marker table data\n");
      return false;
    }
...
/*******************************************************************************************
 * Routine to write out a sync chunk for the Control Subscriber. This is done
 *at the beginning of every marker block to allow the Control Subscriber to
 *conditionally send out a "clock sync" call at runtime. The conditional part
 *comes from not wanting to cause buffer flushing in subscribers unless the
 *receipt of the data is as a result of a branching operation (normal
 *playthrough should not cause flushing and clock sync should not be called).
 *******************************************************************************************/
static Boolean
WriteSyncChunk (WeaveParamsPtr pb, long *bytesRemaining, long timeValue)
{
...
/*******************************************************************************************
 * Routine to write out a alrm chunk for the Control Subscriber. This is done
 *after the a goto chunk.
 *******************************************************************************************/
static Boolean
WriteAlrmChunk (WeaveParamsPtr pb, long *bytesRemaining, long timeValue)
{
 ...
 *******************************************************************************************/
static Boolean
WriteFillerChunk (WeaveParamsPtr pb, WeaveChunkPtr pFillerBlk,
                  long *bytesRemaining)
 ...
 *******************************************************************************************/
static Boolean
WriteGotoChunk (WeaveParamsPtr pb, InStreamPtr isp, WeaveChunkPtr pFillerBlk,
                long *bytesRemaining)
{ /* Begin WriteGotoChunk */
  ...
  if ((isp->buf.weaveChunkTime > pb->GotoChunkList->DataChunk.time)
      || (isp->eof))
    {
...
#if USE_MAC_IO
      if (noErr
          != FSWrite (pb->outputStreamFd, &bytesToWrite,
                      &pb->GotoChunkList->DataChunk))
#else
      if (1
          != fwrite (&pb->GotoChunkList->DataChunk, bytesToWrite, 1,
                     pb->outputStreamFd))
#endif
       ...
       */
      if (!WriteAlrmChunk (pb, bytesRemaining,
                           pb->GotoChunkList->DataChunk.time))
        {
...
/*******************************************************************************************
 * Routine to write out a stop chunk for the Control Subscriber.
 *******************************************************************************************/
static Boolean
WriteStopChunk (WeaveParamsPtr pb, InStreamPtr isp, WeaveChunkPtr pFillerBlk,
                long *bytesRemaining)
{ /* Begin WriteStopChunk */
  ...
  if ((isp->buf.weaveChunkTime > pb->StopChunkList->DataChunk.time)
      || (isp->eof))
    {
      ...
      if (noErr
          != FSWrite (pb->outputStreamFd, &bytesToWrite,
                      &pb->StopChunkList->DataChunk))
#else
      if (1
          != fwrite (&pb->StopChunkList->DataChunk, bytesToWrite, 1,
                     pb->outputStreamFd))
#endif
 ...
 *******************************************************************************************/
static Boolean
WriteHaltChunk (WeaveParamsPtr pb, InStreamPtr isp, WeaveChunkPtr pFillerBlk,
                long *bytesRemaining)
{ /* Begin WriteHaltChunk */
   ...
   */
  if ((isp->buf.weaveChunkTime
       > pb->HaltChunkList->DataChunk.streamerChunk.time)
      || (isp->eof))
    {
      ...
      if (noErr
          != FSWrite (pb->outputStreamFd, &bytesToWrite,
                      &pb->HaltChunkList->DataChunk))
#else
      if (1
          != fwrite (&pb->HaltChunkList->DataChunk, bytesToWrite, 1,
                     pb->outputStreamFd))
#endif
 ...
 *******************************************************************************************/
Boolean
OpenDataStreams (WeaveParamsPtr pb)
{
  ...
  if (status != noErr)
    {
      fprintf (stderr,
               "error %d, unable to create output data stream file: %s\n",
  ...
  if (status != noErr)
    {
      fprintf (stderr,
               "error %d, unable to open output data stream file: %s\n",
               status, pb->outputStreamName);
      return false;
    }
  ...
  if (setvbuf (pb->outputStreamFd, GetIOBuffer (pb->ioBufferSize), _IOFBF,
               pb->ioBufferSize))
    {
  ...
  for (; isp != nil; isp = isp->Next)
    {
      ...
      if (status != noErr)
        {
          fprintf (stderr,
                   "error %d, unable to open input data stream file: %s\n",
                   status, isp->fileName);
          return false;
        }
...
#if USE_BIGGER_BUFFERS
      if (setvbuf (isp->fd, GetIOBuffer (pb->ioBufferSize), _IOFBF,
                   pb->ioBufferSize))
        {
          fprintf (stderr, "setvbuf() failed for file: %s\n", isp->fileName);
          return false;
 ...
 *******************************************************************************************/
void
CloseDataStreams (WeaveParamsPtr pb)
{
  InStreamPtr isp = pb->InStreamList;
...
/*******************************************************************************************
 * Routine to merge all input data streams into a single output data stream.
 *The parameter block contains all the info we need to do this already set up
 *for us.
 *
 *	Rules for output chunk selection:
 *	---------------------------------
 *	1.	write the lowest TIME chunk that will fit into the current
 *block whose time is below the next MARKER
 *
 ...
 *				&& the chunk's TIME is below the next MARKER
 *value write the selected chunk else if there's another chunk in the input
 *collection that will fit into the current block
 *				&& the chunk's TIME is below the next MARKER
 *value write that block else write a FILLER block
 *
 *******************************************************************************************/
long
WeaveStreams (WeaveParamsPtr pb)
{
  InStreamPtr isp;
  ...
  /* Force input stream data onto the next stream block boundary so that
   * branching to time zero can be done meaningfully. If options dictate that
   * neither a header nor a marker table have been written to the output
   * stream, then skip writing a filler chunk.
   */
...
#if USE_MAC_IO
      if (noErr
          != FSWrite (pb->outputStreamFd, &remainingInCurrentBlock,
                      pFillerBlk))
#else
      if (1
          != fwrite (pFillerBlk, remainingInCurrentBlock, 1,
                     pb->outputStreamFd))
#endif
        {
          return -402;
  ...
  while (InputDataRemaining (pb))
    {
      ...
      else if ((isp = GetEarliestBelowNextMarker (pb, remainingInCurrentBlock))
               != nil)
        {
          ...
          if (pb->fWriteGotoChunk)
            {
              if (!WriteGotoChunk (pb, isp, pFillerBlk,
          ...
          if (pb->fWriteStopChunk)
            {
              if (!WriteStopChunk (pb, isp, pFillerBlk,
                                   &remainingInCurrentBlock))
                {
                  status = -15;
                  goto BAILOUT;
          ...
          if (pb->fWriteHaltChunk)
            {
              if (!WriteHaltChunk (pb, isp, pFillerBlk,
                                   &remainingInCurrentBlock))
                {
                  status = -15;
                  goto BAILOUT;
      ...
      else if ((isp = GetBestFitBelowNextMarker (pb, remainingInCurrentBlock))
               != nil)
        {
          if (!WriteChunk (pb, isp, &remainingInCurrentBlock))
            {
      ...
      if ((pb->fWriteMarkerTable) && (pb->fUpdateMarkerTable))
        {
...
#if USE_MAC_IO
          GetFPos (pb->outputStreamFd,
                   &pb->marker[pb->currMarkerIndex].markerOffset);
#else
          pb->marker[pb->currMarkerIndex].markerOffset
              = ftell (pb->outputStreamFd);
#endif
          ...
          if (pb->currMarkerIndex >= pb->numMarkers)
            {
              fprintf (stderr,
                       "WeaveStreams() - generating too many markers!\n");
              return -10;
            }
   ...
   */
    ...
    {
      if (!WriteHaltChunk (pb, isp, pFillerBlk, &remainingInCurrentBlock))
        {
          status = -15;
          goto BAILOUT;
        }
  ...
  if (remainingInCurrentBlock != pb->streamBlockSize)
    ...
    /* Write out the marker table, this time the table will have all its
     * entries filled in. First, seek back to the place where it was first
     * written, and then OVERWRITE it in place.
     */
#if USE_MAC_IO
  ...
  /* Fixed the following to pass in the correct bytesremaining in the first
   * block.
   *
   */
