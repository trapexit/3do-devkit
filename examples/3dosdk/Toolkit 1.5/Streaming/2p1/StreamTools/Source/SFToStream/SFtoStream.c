 ...
 *
 *	Notes:			This tool will convert AIFF and AIFC soundfiles into chunk format
 *					Does NOT write fully qualified stream blocks, it only writes SSND chunks.
 *
 *	History:
 *
 *	10/10/94	dtc		Version 2.3d3
 *						Fixed "drop frame" approach to audio clock tick error.
 *	 7/19/94	rdg		Version 2.3d2
 *						Fix quadbyte data alignment problem revealed by ADPCM playback.
 *						Implement a "drop frame" like approch to audio clock tick error.
 *	 7/12/94	rdg		Version 2.3d1
 *						Add support for ADPCM 4:1 compression
 *	 7/11/94	dtc		Version 2.3
 *						Integrated Eric's fix to write out the last chunk
 *						with the correct size.
 *						Modified integer arithmatic to real arithmatic
 *						for better acurracy.
 *	5/12/94		dtc		Version 2.2
 *						Changed the definitions of DS_MSG_HEADER and SUBS_CHUNK_COMMON 
 *						to require semicolon when used in context.  (For readibility
 *						and compilation using ObjectMaster).
...
int main( int argc, char* argv[] )
	...
	FramesPTick		=	(myCommChunk.sampleRate / AUDIO_TICK_RATE) + 0.5;	/* round up */

	BytesPTick		=	((BytesPFrame * FramesPTick) / CompressionRatio); 
	
	/* Bug# 2077: Incorrect calculation of how many samples fit into a single SNDS chunk to make
	 * its duration a whole number of audio ticks.
	 * was: SampleDataSize	=	(gPropChunkSize - ( (gPropChunkSize - SSMP_CHUNK_HDR_SIZE) % BytesPTick) );
	 * is:	SampleDataSize	=	( (gPropChunkSize - SSMP_CHUNK_HDR_SIZE) / BytesPTick) * BytesPTick;
	 */		

	/* Round up to accomodate the AudioFolio's fractional audio ticks per second */
	SampleDataSize	=	( ( (gPropChunkSize - SSMP_CHUNK_HDR_SIZE) / BytesPTick) * BytesPTick);
	
	/* Now we have to find a value for SampleDataSize which is still an even number of
	 * audio ticks per second, but is also quadbyte multiple.  If not, DMA will cause
	 * problems (because it always moves 32 bits at a time)  i.e. there will be random
	 * bytes at the end of the chunk which are read by DMA even though we didn't ask for
	 * them.
	 */
	...
	printf(" If this size is unacceptable, you must rechunkify with a different\n");
	...
	while (currFilePos < endSSNDChunkPos)
		...
		/* This is my implementation of a psudeo-SMPTE style "drop frame" approach
		 * to time correction.  I keep track of accumulated audio clock
