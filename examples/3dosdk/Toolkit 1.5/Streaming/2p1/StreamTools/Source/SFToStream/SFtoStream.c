 ...
 *	File:			SFtoStream.c
 *
 *	Contains:		MPW tool to turn an AIFF or AIFC soundfile into a
 *stream file.
 *
 ...
 *	History:
 *
 *	10/10/94	dtc		Version 2.3d3
 *						Fixed "drop frame" approach to audio
 *clock tick error. 7/19/94	rdg		Version 2.3d2 Fix quadbyte data
 *alignment problem revealed by ADPCM playback. Implement a "drop frame" like
 *approch to audio clock tick error. 7/12/94	rdg		Version 2.3d1
 *						Add support for ADPCM 4:1
 *compression 7/11/94	dtc		Version 2.3 Integrated Eric's fix to
 *write out the last chunk with the correct size. Modified integer arithmatic
 *to real arithmatic for better acurracy. 5/12/94		dtc
 *Version 2.2 Changed the definitions of DS_MSG_HEADER and SUBS_CHUNK_COMMON to
 *require semicolon when used in context.  (For readibility and compilation
 *using ObjectMaster). 10/31/93	rdg		Version 2.1 Fix chunkTime to
 *start at 1.  So if you weave multiple audio streams together which start at
 *the same time, all of the headers will be processed together before any data
 *starts playing.  This was fixed in an earlier version but the change got lost
 *somehow!!! 10/21/93	rdg		Version 2.0 Fix sampleCount field in
 *sampleDescriptor structure.  Was being computed improperly. 10/19/93	jb
 *Version 1.9 Change command line numeric parsing to accept either decimal or
 *hexadecimal inputs. 9/12/93		rdg		version 1.8 Spin
 *cursor. Change input and output file command line flags to -i and -o. 8/25/93
 *rdg		version 1.7 Alter computation of chunk size to prevent the
 *possibility of yielding a chunk size greater than the one desired.  Rounding
 *is necessary, but a case appeared where the net rounding turned out to be up
 *instead of down. 8/19/93		rdg		version 1.6
 *						experimental changes slipped out with
 *v1.5 which caused time stamps to be screwed up.  Reverted affected code
 *to 1.4. 8/07/93 	ian 	version 1.5 program now exits with zero status
 *for success, non-zero for failure.  on failure, it deletes any partial output
 *file that may have been built.  it also spins the cursor now. 7/16/93
 *rdg 	version 1.4 add support for larger I/O buffers to speed processing
 *	6/25/93 	rdg 	version 1.3
 *						fix aifc compression type
 *check. 6/25/93 	rdg 	version 1.2 add checks for compression type,
 *sample size, sampling frequency, and number of channels.  Exit with polite
 *message if not supported. 6/24/93 	rdg 	version 1.1 fix default initial
 *pan value check for proper AIFF format printed wrong filename 6/17/93
 *rdg 	version 1.0 5/28/93 	njc 	BytesPTick was not being adjusted for
 *compressionRatio. 5/28/93 	rdg 	Version .13 Not too pround of the hack
 *to support AIFF and AIFC files directly. Ended up re-writing the main loop.
 *BlockSize is no longer supported. SSND chunks are simply written to the
 *output stream and not grouped into any higher level structure.  ChunkSize is
 *still rounded to the nearest quadbyte aligned, integral of audio ticks. It
 *also now supports specifing the logical channel number you want on the
 *command line. 5/25/93 	rdg 	Version .12 Fixed compression type
 *specification from command line 5/25/93 	rdg 	Version .11 5/25/93
 *rdg 	Add actualSampleSize to the SSMPchunkHeader and longword align the
 *chunk. 5/20/93 	rdg 	Version .10 Substantially altered data
 *structures; now writes new timed chunks with header and data sub chunks.
 *	5/8/93		rdg 	Converted to read headerless sound files and
 *parse into stream format.
 *	4/17/93 	jb		Add command line arg parsing and usage()
 *routine. 4/17/93 	jb		New today
 *
 *******************************************************************************************/
...
/* This value represents the difference between the theoretical number
 * of audio ticks per second (240), and the actual number of
 * ticks per second (239.67391304).  We use this to keep track of accumulated
 * error and adjust for it when computing a data chunk's time stamp.
 */
#define AUDIO_TICK_RATE_ERROR .32608696
...
long gNumBuffers
    = 8; /* max # of buffers that can be queued to the AudioFolio	*/
...
{
  printf (
      "Summary: Reads AIFF or AIFF-C soundfiles and writes audio streams\n");
...
main (int argc, char *argv[])
{
  ...
  BytesLeft
      = AIFFormHeader[1] - 4; /* subtract the length of the formType field */
  ...
  while (BytesLeft > 0)
    {
      ...
      if (AIFChunkHeader[0] != CommonID)
        {
          EVENUP (AIFChunkHeader[1]); /* round up chunk size to and even number
                                         per aif spec */
          result = fseek (inputFile, AIFChunkHeader[1],
                          SEEK_CUR);      /* point to next chunk header */
          BytesLeft -= AIFChunkHeader[1]; /* update size */
        }
      else
        {
          /* read in the COMM chunk data and leave the loop */
          BytesRead = fread ((char *)&myCommChunk.numChannels, 1,
                             sizeof (ExtCommonChunk) - 8, inputFile);
          break;
        }
    }
  ...
  if ((AIFFormHeader[2] == AIFCID)
      && (myCommChunk.compressionType != COMPRESSION_TYPE_SDX2)
      && (myCommChunk.compressionType != COMPRESSION_TYPE_ADP4)
      && (myCommChunk.compressionType != COMPRESSION_TYPE_NONE))
    {
      printf (
          "error: Unrecognized compression type or invalid AIFC header.\n");
      goto ERROR_EXIT;
    }
  else
    /* Figure out compression type and ratio from AIF COMM chunk info */
    ...
    else if (myCommChunk.compressionType == COMPRESSION_TYPE_ADP4)
      {
  ...
  if (((long)myCommChunk.sampleRate != 44100)
      && ((long)myCommChunk.sampleRate != 22050))
    {
      printf ("error: Sample rate must be 44100 or 22050.\n");
      goto ERROR_EXIT;
    }

  if (((long)myCommChunk.numChannels != 1)
      && ((long)myCommChunk.numChannels != 2))
    {
      printf ("error: Sample must be mono or stereo.\n");
      goto ERROR_EXIT;
    }

  /* Figure out actual stream chunk size and audio ticks per chunk from AIF
   * COMM chunk info */
  BytesPFrame = ((myCommChunk.sampleSize / 8) * myCommChunk.numChannels);

  FramesPTick
      = (myCommChunk.sampleRate / AUDIO_TICK_RATE) + 0.5; /* round up */

  BytesPTick = ((BytesPFrame * FramesPTick) / CompressionRatio);

  /* Bug# 2077: Incorrect calculation of how many samples fit into a single
   * SNDS chunk to make its duration a whole number of audio ticks. was:
   * SampleDataSize	=	(gPropChunkSize - ( (gPropChunkSize -
   * SSMP_CHUNK_HDR_SIZE) % BytesPTick) ); is:	SampleDataSize	=	(
   * (gPropChunkSize - SSMP_CHUNK_HDR_SIZE) / BytesPTick) * BytesPTick;
   */

  /* Round up to accomodate the AudioFolio's fractional audio ticks per second
   */
  SampleDataSize
      = (((gPropChunkSize - SSMP_CHUNK_HDR_SIZE) / BytesPTick) * BytesPTick);

  /* Now we have to find a value for SampleDataSize which is still an even
   * number of audio ticks per second, but is also quadbyte multiple.  If not,
   * DMA will cause problems (because it always moves 32 bits at a time)  i.e.
   * there will be random bytes at the end of the chunk which are read by DMA
   * even though we didn't ask for them.
   */
  while ((SampleDataSize % 4) != 0)
    SampleDataSize = SampleDataSize - BytesPTick;

  TicksPChunk = SampleDataSize / BytesPTick;

  /* Compute the full chunk size including header and adjusted data size */
  ChunkSize = SSMP_CHUNK_HDR_SIZE + SampleDataSize;

  printf (
      "\nThe chunk size closest to the one you asked for which satisfies\n");
  printf (" quadbyte data and audio tick alignment requirements is %ld.\n",
          ChunkSize);
  printf (
      " If this size is unacceptable, you must rechunkify with a different\n");
  printf (" (i.e. probably larger) chunk size value on the command line.\n");

  /* Now Search for the SSND chunk */
  result = fseek (inputFile, 12,
                  SEEK_SET); /* point back to first chunk after FORM chunk */
  BytesLeft = AIFFormHeader[1]
              - 4; /* reset size since we're pointing back to the beginning */
  ...
  while (BytesLeft > 0)
    {
      ...
      if (AIFChunkHeader[0] != SoundDataID)
      ...
      else
        {
          result = fseek (
              inputFile, 8,
              SEEK_CUR); /* skip to the beginning of the sample data */
          SSNDChunkSize = AIFChunkHeader[1]
                          - 8; /* account for offset and blocksize fields */
          BytesLeft = SSNDChunkSize; /* set up BytesLeft for reading just the
                                        sample data */
          break;
        }
    }
  ...
  /*******************************************************************************************
   * Write a header chunk into the output stream file using our AIFF/C header
   *data
   ******************************************************************************************/
  ...
  hcp->numBuffers = gNumBuffers; /* max # of buffers that can be queued to the
                                    AudioFolio	*/
  ...
  hcp->sampleDesc.sampleSize
      = myCommChunk.sampleSize; /* bits per sample, typically 16 */
  hcp->sampleDesc.sampleRate
      = (long)myCommChunk.sampleRate; /* 44100 or 22050 */
  hcp->sampleDesc.numChannels
      = myCommChunk.numChannels; /* channels per frame, 1 = mono, 2=stereo */
   ...
   * divided by number of bytes per sample i.e. ((myCommChunk.sampleSize /
   * CompressionRatio) / 8)
   */
  hcp->sampleDesc.sampleCount
      = (long)((float)SSNDChunkSize
               / (((float)myCommChunk.sampleSize / (float)CompressionRatio)
                  / 8));
  ...
  while (currFilePos < endSSNDChunkPos)
    {
      ...
      /* This is my implementation of a psudeo-SMPTE style "drop frame"
       * approach to time correction.  I keep track of accumulated audio clock
       * error and when it exceeds 1 audio tick, I simply adjust the chunk
       * time downward to match the actual audio clock rate.
       */
      ClockError = ((ChunkTime / 240) * AUDIO_TICK_RATE_ERROR);

      if (ClockError > 1)
        /* Put the error corrected chunk time into the header struct */
        dcp->time = ChunkTime - (int)ClockError;
      else
        dcp->time = ChunkTime;
