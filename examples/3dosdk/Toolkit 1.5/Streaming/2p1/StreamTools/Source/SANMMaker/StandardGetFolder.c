 ...
 *	History: *
 *                                                                              *
 *	08/17/94	dtc		Replaced sysqu.h with LowMem.h,
 *CurDirStore with       * LMGetCurDirStore , and SFSaveDisk with
 *LMGetSFSaveDisk *
 *******************************************************************************/
...
pascal short MyCustomGetDirectoryDlogHook (short item, DialogPtr theDialog,
                                           Ptr myDataPtr);
...
void
StandardGetFolder (Point where, Str255 message, StandardFileReply *mySFReply)

{
  ...
  +-------------------------------------------------------------------------*/
  CustomGetFile ((FileFilterYDProcPtr)MyCustomGetDirectoryFileFilter, numTypes,
                 theTypeList, mySFReply, rGetFolderDialog, where,
                 (DlgHookYDProcPtr)MyCustomGetDirectoryDlogHook, NULL,
                 (short *)NULL, NULL, (void *)(mySFReply));
...
pascal short
MyCustomGetDirectoryDlogHook (short item, DialogPtr theDialog, Ptr myDataPtr)
{
 ...
 *******************************************************************************/
void
SetButtonTitle (Handle ButtonHdl, Str255 name, Rect *ButtonRect)
{
  short resultCode;
  short width;
  char TmpStr[256];
  ...
  width = (ButtonRect->right - ButtonRect->left)
          - (StringWidth ((StringPtr) "\pSelect \"\"") + CharWidth ('J'));
