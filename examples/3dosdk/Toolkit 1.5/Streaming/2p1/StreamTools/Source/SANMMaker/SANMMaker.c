 ...
 *	File: SANMMaker.c
 *
 *	Contains:	Think C source file for program which converts a folder of 3DO cels to a file
 *				containing a streamed anim.
 *
 *				NOTE:  Assumes that only cel files are in the folder.
 *					   Assumes that all cels in an anim share the same CCB.
 *
 ...
 *	
 *	History:
 *	08/17/94	dtc		Fixed to pass the address of gd.gray  to FillRect;
 *						replaced sysqu.h with LowMem.h; and used GetMBarHeight to
 *						access MBarHeight.
 *	10/19/93	jb		Add gLoopOption and code to handle new checkbox item in the
 *						dialog filter proc. Optionally output a looping chunk in the
 *						output stream ONLY if this option is selected.
 *	10/8/93		jb		Fix the way we get the arrow cursor by referencing &arrow,
 *						the global.
 *	5/21/93		JML		Combined AA and Regular versions into one
 *						AA version now has just one PLUT for the AA cels
 *						Added status dialog and error checking
 *	4/23/93		JML		New Today.
 *
 *******************************************************************************************/
...
void centerWindow(theResType, theResID)
	ResType		theResType;
	short		theResID;
...
{	Rect			**theResource;
	...
	if ((theResType != 'DLOG') && (theResType != 'ALRT') && (theResType != 'WIND'))
		return;
	...
	if ((theResType == 'WIND') && ((**theResource).top != 0))	/* allow windows to have fixed locations */
		return;
	...
	(**theResource).left = screenRect.left + (((screenRect.right - screenRect.left) - resourceWidth) / 2);
	(**theResource).right = (**theResource).left + resourceWidth;

	(**theResource).top = screenRect.top + (((screenRect.bottom - screenRect.top) - resourceHeight) / 3);
	(**theResource).top = ((**theResource).top <= (GetMBarHeight() + 4)) ? (GetMBarHeight() + 4) : (**theResource).top;
	(**theResource).bottom = (**theResource).top + resourceHeight;
}


 /***************************************************************************************
 *	ROUTINE:	FrameButton - draws outline around a button to indicate default button
 *	INPUT:		Pointer to the dialog and the item number
 *	OUTPUT:		None
 *
 ***************************************************************************************/
 
 void FrameButton(DialogPtr thedialog, int itemNum)
...
void ErrorAlert(int MessageID, OSErr error, Boolean deleteOutFile, Boolean closeInFile) {
	DialogPtr	AlertDLOG;
	short		ItemHit;
	...
	if (deleteOutFile) {  // Close and Delete the file we created for SANM
		FSClose(gOutRefNum);
 ...
 *	OUTPUT:		None
 *	NOTES:		This routine puts a message on the screen either in the counter display
 *				box or in an error box, if an error message is specified.
 *
 ***************************************************************************************/
 
 void DisplayMessage(long total, long current)

{
	...
	if (total != 0) {
		...
		updaterect.top += 1;
		updaterect.bottom -= 1;
		updaterect.right = updaterect.left + ((updaterect.right - 1 - updaterect.left)*current)/total;
		FillRect(&updaterect, (ConstPatternParam)&qd.gray);
	}
 ...
 *	INPUT:		None
 *	OUTPUT:		None
 *	NOTES:		This routine de-allocates the space for the counter in case we forget
 *				to call ClearCounter. *
 ***************************************************************************************/
...
pascal short myDlg(short Item, DialogPtr theDialog) {
	...
	if (Item == putSave) {
		...
		if ( (gChannel < 0) || (gChannel > MAX_CHANNELS-1) || (gFrames < 1) || (gFrames > 60) ) {
			SysBeep(0);
			Item = 100;
		}
	}
...
short SaveDialog(void) {
	...
	SFPPutFile (where, prompt, "\p", (DlgHookProcPtr)myDlg, &gSFReply, PUTFILE_DIALOG_ID, NIL );
	if ( !gSFReply.good )
		return 0;
	...
	(void) FSDelete(gSFReply.fName, gSFReply.vRefNum);
	error = Create(gSFReply.fName, gSFReply.vRefNum, CREATOR_3DO, STREAM_FILE_TYPE);
...
//	err= PBHGetFInfo((HParmBlkPtr) &gMyCPB, false);
	err= PBGetCatInfo((CInfoPBPtr) &gMyCPB, false);
	if (gMyCPB.ioFlFndrInfo.fdType != CEL_FILE_TYPE) return -1;
 ...
 ********************************************************************/
Boolean	TrapAvailable(short tNumber, long tType)
{
...
long main(void) {
	...
	gOutRefNum = SaveDialog();								// ask the user to give a file name to create the SANM file
	if (gOutRefNum <= 0)
		ErrorAlert(kCreateErrID,gOutRefNum, false, false);	// user cancelled or couldn't create file
	...
	gNumFiles = GetNumFilesInDirectory(dirID,vRefNum);	// how many cel files are in this directory?
		...
		if (SetupCPBForIndexedFile(dirID, vRefNum, i) == 0) {
			...
			if (error == noErr) {
				if (i == 1) {
					...
					if (error != noErr) ErrorAlert(kReadErrID,error, true, true);
					memcpy(&sanmCCB.ccb_Flags,&CCBHdr.scb_Flags,sizeof(spritehdr)-12);
					...
					if (error != noErr) ErrorAlert(kWriteErrID,error, true, true);
					blockSize += numBytes;
				}
				...
				if (gAntiAlias && (i == 2)) {
					...
					// read the AA cels' plut, we'll use the same plut for all frames
					sanmCCB.chunkType = SANM_CHUNK_TYPE;
					sanmCCB.channel = gChannel + 1;
					sanmCCB.time = 0;
					...
					if (error == eofErr) ErrorAlert(kBadAACelID, error, true, true);
					if (error != noErr) ErrorAlert(kReadErrID,error, true, true);
					sanmCCB.chunkSize = GetPtrSize(PLUTBuffer)+sizeof(StreamAnimPLUT);
				...
				if (gAntiAlias && ((i%2) == 0)) {	// always check the AA cel for a plut
					PLUTBuffer = NIL;
					error = ReadAChunk(inrefnum,tPLUT,&PLUTBuffer);
					if (error != noErr) ErrorAlert(kBadAACelID, error, true, true);
					DisposPtr(PLUTBuffer);
					PLUTBuffer = NIL;
				}
				else {		// each cel in a non-AA anim can have its own plut
					PLUTBuffer = NIL;
					error = ReadAChunk(inrefnum,tPLUT,&PLUTBuffer);
				...
				if (blockSize+numFRMEBytes > MAX_BLOCKSIZE) {		// we must fill in the 32K block
					numBytes = MAX_BLOCKSIZE-blockSize;
					fillBuffer = NewPtrClear(numBytes);
					if (fillBuffer == NIL) ErrorAlert(kOutOfMemErrID,error, true, true);
	...
	if ( gLoopOption )
		...
		if (error != noErr) ErrorAlert(kWriteErrID,error, true, false);
		blockSize += numBytes;
		}
	...
	numBytes = MAX_BLOCKSIZE-blockSize;
	fillBuffer = NewPtrClear(numBytes);
	if (fillBuffer == NIL) ErrorAlert(kOutOfMemErrID,error, true, false);
	*(long *)fillBuffer = FILLER_CHUNK_TYPE;
	*(long *)((char *)fillBuffer+4) = numBytes;
	error = FSWrite(gOutRefNum, (long*) &numBytes, fillBuffer);
	DisposePtr(fillBuffer);
...
END:
	...
	if (gNumFiles > 0)
		RemoveMessage();
	SetCursor(&qd.arrow);		// set the arrow cursor
	if (!userCancelled && (gProcessedFiles != gNumFiles))		// We found files in the folder that were not cels
