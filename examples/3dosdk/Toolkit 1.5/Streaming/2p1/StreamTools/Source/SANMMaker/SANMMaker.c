 ...
 *FillRect; replaced sysqu.h with LowMem.h; and used GetMBarHeight to access
 *MBarHeight.
...

{
  ...
  (**theResource).top = ((**theResource).top <= (GetMBarHeight () + 4))
                            ? (GetMBarHeight () + 4)
                            : (**theResource).top;
...
SaveDialog (void)
{
  ...
  SFPPutFile (where, prompt, "\p", (DlgHookProcPtr)myDlg, &gSFReply,
