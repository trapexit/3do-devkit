 ...
 *
 *	History:
 *	7/7/94		rdg		v1.1 modified to support creation of HALT chunks
 *	4/4/94		rdg		adapted from original test data tool
 *
 *******************************************************************************************/
...
unsigned long	gSubsChunkType		= PRTO_CHUNK_TYPE;	/* default subscriber chunk type */
unsigned long	gHeaderChunkType	= PHDR_CHUNK_TYPE;	/* default header chunk type  */
...
static	void	usage( void )
	...
	printf( "	%s\t<num>\t\t1\t\t\tnumber of chunks to write to the output file\n", NUM_CHUNKS_STRING );
...
int	main( int argc, char* argv[] )
	...
	if ( setvbuf( outputFile, GetIOBuffer( IO_BUFFER_SIZE ), _IOFBF, IO_BUFFER_SIZE ) )
		{
	...
	if ( gWriteHaltChunk ) 
		{
		/* This chunk header is for the Streamer... it then delivers the encapsulated
		 * chunk described below to the appropriate subscriber.
		 */
		...
		aHaltChunk.haltHeader.subscriberPart.chunkSize 		=	( sizeof( ProtoHaltChunk ) 
																	- sizeof( SimpleChunk ) );
	...
	for ( chunkNum = 1; chunkNum < gNumChunks; chunkNum++ )
		...
		if ( (gWriteHaltChunk) && (dcp->time >= gHaltChunkTime) )
			...
			if (1 != fwrite( &aHaltChunk, aHaltChunk.haltHeader.streamerPart.chunkSize, 
								1, outputFile ) )
				{
				printf("error: write failed!\n");
				return 0;
