 ...
 **	 2/16/93	RDG		ported to back to MPW 3.2.3 and release first version .10
 **  2/15/93	RDG		added iff_begin_snd and iff_end_snd to more easily handle SSND
 **						 chunk size... also modified the iff_control struct by adding
 **						 member iffc_ssnd_pos to hold file position of the ssnd data
 **	 2/7/93		RDG		added uint,uint32,int32 typedefs
...
/******************************************************/
long iff_create_file (iff_control *iffc, char *filename, long creator, long fileType)
{
...
/******************************************************/
long iff_write_chunk (iff_control *iffc, uint32 type, void *data, int32 numbytes)
{
	uint32	pad[2];
	long	Result = 0;
...
{
	long	Result = 0;
	uint32	pad[2] = {0,0};	/*  these represent offset and blockSize which are unused  */
...
/* calculate size of compressed SSND chunk */
	...
	ssnd_size = (8 + (cur_pos - iffc->iffc_ssnd_pos));   /*offset and blockSize + raw snd data */
