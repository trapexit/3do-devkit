 ...
 *
 *	History:
 *	10/10/94	dtc		Fix - Print a blank line between blocks that are on block
 *						boundary (and has no fillers).
 *  9/1/94		dsm		Changed to get StreamBlockSize from StreamHeader
 *						if there was no blocksize specified on the command line
 *						Fixed usage to mention -bs the blocksize option
 *						Fixed -xf option to work properly
 *						Changed Chunksize to hex output if file positions are hex
 *	9/12/93		rdg		Version 1.5
 *						Spin cursor.
 *	7/29/93		jb		Version 1.4
 *						Add output of channel number and subchunk type
 *	7/14/93		jb		Version 1.3
 *						Add BANDWIDTH_FLAG_STRING option to allow data bandwidth
 *						problem detection (not foolproof, but better than nothing).
 *						Add FILLBLOCK_FLAG_STRING option to allow output of filler
 *						block size to stdout (permit analysis of stream with other
 *						tools, such as spreadsheets).
 *	6/25/93		jb		Version 1.2
 *						Make sure we don't try to dump data when the chunk size is
 *						less than a reasonable minimum (should only happen with
 *						filler chunks)
 *	6/3/93		jb		Version 1.1
 *						Add statistics calcs and output.
 *	5/20/93		jb		Version 1.0
 *	5/20/93		jb		Add check for non-quadbyte aligned chunk sizes. Add program
 *						version string output in Usage() routine. Add general purpose
 *						warnings switch to enable future data checking. 
 *	5/18/93		jb		Output blank lines between stream blocks (demarked by filler chunk)
 *	5/17/93		jb		New today.
 *
 *******************************************************************************************/

#define	PROGRAM_VERSION_STRING	"1.6"
...
long		gUserSpecifiedBlockSize = false;
/********************************************************/
...
static Boolean ParseCommandLine ( int argc, char **argv )
	...
	while ( argc > 0 )
		 ...
		 */
			...
			argv++;
			argc--;
			gVerboseHexFlag = ~gVerboseHexFlag;
			}
...
static void Usage( char* commandNamePtr )
	...
	fprintf (stderr, "\t%s\t\t\t\tverbose output with hex dump\n", VERBOSEHEX_FLAG_STRING);
	fprintf (stderr, "\t%s\t\t\thex file position output\n", HEX_FPOS_FLAG_STRING);
	fprintf (stderr, "\t%s\t\t\tstream block size\n", BLOCK_SIZE_FLAG);
...
static void		DumpStreamFile( char* fileName, FILE* fd, long startTime, 
	...
	if ((sizeof(WeaveChunk)+sizeof(StreamBlockXtra)) > bytesToRead) 
		bytesToRead = (sizeof(WeaveChunk)+sizeof(StreamBlockXtra));
	
	if ((sizeof(WeaveChunk)+HEX_DUMP_SIZE) > bytesToRead) 
		bytesToRead = (sizeof(WeaveChunk)+HEX_DUMP_SIZE);
	
	while ( 1 == fread( buffer, bytesToRead, 1, fd ) )
		{
		SpinCursor(32);
...
#define	ALT_FILLER_DATA_TYPE		(('F'<<24)|('I'<<16)|('L'<<8)|'L')
		...
		*/
		if ( wcp->weaveChunkType == HEADER_CHUNK_TYPE) 
			{
			hdrp = (DSHeaderChunkPtr) wcp;
			if( hdrp->headerVersion == DS_STREAM_VERSION)
				{
				if (hdrp->streamBlockSize )
					{
					if ( gUserSpecifiedBlockSize==false )
						gBlockSize = hdrp->streamBlockSize;
					else if ( gBlockSize != hdrp->streamBlockSize )
						{	
						...
						gBlockSize = hdrp->streamBlockSize;
						if ( !verbose ) 
							printf("\n  Assuming Stream Block size of %d (0x%X)\n  as specified in Stream header\n\n",gBlockSize,gBlockSize);
						}
		...
		if ( (! fFillerChunk) && gBandwidthCheck )
			...
			if ( wcp->weaveChunkTime != lastStreamTime )
				...
				if ( (wcp->weaveChunkTime * (gBandwidthMax / AUDIO_TICKS_PER_SECOND) )
						< filePosition )
					{
		...
		if ( fDumping )
			...
			if ( !gStatsFlag )
				...
				if ( verbose || verboseHex )
					{
					data = (char*) &wcp->weaveChunkTime + sizeof(wcp->weaveChunkTime);
					...
					if ( (bytesToDump <= 0) && (! fFillerChunk) )
						printf( "*** data chunk size less than normal minimum ***" );
						...
						printf( " --" );
						printf( "%5ld%10.4s", sbx->channel, &sbx->subChunkType );
							
						} /* if ( verbose ) */
					} /* if ( verboseHex || verboseHex ) */
		...
		/* Output a blank line to separate blocks in the stream.
		 */
		if ( !fFillerChunk && ( filePosition % gBlockSize == 0 ) )
			printf( "\n" );
		...
		if ( (filePosition > 0) && (! gFillerBlockSizesFlag) )
			...
			if ( totalFillerChunks > 0 )
				...
				printf( "%c%s%c contains %ld%% filler\n", 
						QUOTE, fileName, QUOTE, percentFiller );
...
void main( int argc, char* argv[] )
	...
	DumpStreamFile( gInputStreamName, fd, gDumpStartTime, gVerboseFlag, gVerboseHexFlag );
