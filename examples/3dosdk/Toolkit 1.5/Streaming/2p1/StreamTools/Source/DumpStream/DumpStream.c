 ...
 *
 *	History:
 *	10/10/94	dtc		Fix - Print a blank line between blocks
 *that are on block boundary (and has no fillers). 9/1/94		dsm
 *Changed to get StreamBlockSize from StreamHeader if there was no blocksize
 *specified on the command line Fixed usage to mention -bs the blocksize option
 *						Fixed -xf option to work
 *properly Changed Chunksize to hex output if file positions are hex 9/12/93
 *rdg		Version 1.5 Spin cursor. 7/29/93		jb
 *Version 1.4 Add output of channel number and subchunk type
 *	7/14/93		jb		Version 1.3
 *						Add BANDWIDTH_FLAG_STRING option to allow
 *data bandwidth problem detection (not foolproof, but better than nothing).
 *						Add FILLBLOCK_FLAG_STRING option to allow
 *output of filler block size to stdout (permit analysis of stream with other
 *						tools, such as spreadsheets).
 *	6/25/93		jb		Version 1.2
 *						Make sure we don't try to dump data when
 *the chunk size is less than a reasonable minimum (should only happen with
 *						filler chunks)
 *	6/3/93		jb		Version 1.1
 *						Add statistics calcs and
 *output. 5/20/93		jb		Version 1.0
              ...
              if (hdrp->streamBlockSize)
                {
                  if (gUserSpecifiedBlockSize == false)
                    ...
                    {
                      /* Command line blocksize doesn't match the blocksize in
                       * streamheader. */
                      printf ("\nCommand line blocksize doesn't match the "
                              "blocksize in streamheader.");
                      printf ("\nCommand line blocksize is : %d\nStream "
                              "blocksize is : %d\n",
                              gBlockSize, hdrp->streamBlockSize);
                      gBlockSize = hdrp->streamBlockSize;
                      if (!verbose)
                        printf ("\n  Assuming Stream Block size of %d "
      ...
      if ((firstTime) && (!gStatsFlag))
        {
          ...
          if (verbose)
            {
              printf ("  File Pos   Type        Size       Time    Chan    "
                      "SubType\n");
              printf ("  --------  ------       ----       ----    ----    "
                      "-------\n");
            }
              ...
              if (gHexFposOutputFlag)
                {
                  ...
                  printf ("'%c%c%c%c' %10lX %10ld", type[0], type[1], type[2],
              ...
              if (verbose || verboseHex)
                {
                  ...
                  if ((bytesToDump <= 0) && (!fFillerChunk))
                    printf (
                        "*** data chunk size less than normal minimum ***");
