...
HISTORY:
	...
	10/05/94	dtc		Version 2.0b1
						...
						in DumpMovie.
	10/02/94			Bug #2661: Check if QuickTime is installed.
	...
	05/13/94	dtc		Version 1.8
						...
						filename:
							-o <outputfilename>
						Changed usage printout: print an option per line.
						Integrated the following Lynn Ackler's fix:
							Added key chunk flag to the chunkFlags, although
							redundant with the subChunk type, it does allow the
							flexibility to have other chunks types be "key"
							Added chunkFlags to the chunk header struct
							Added key frame and diff frame subchunk types
							Add -s option to set the start time for marker offsets.
							Necessary when conjoining numerous clips.
...
//dtc			FilmF.time			= CvtTo3DOTime( sampleTime, (Fixed)(mediaTimeScale << 16) );
			FilmF.time			= CvtTo3DOTime( sampleTime, (Fixed)(mediaTimeScale << 16) ) + gStartTime;
