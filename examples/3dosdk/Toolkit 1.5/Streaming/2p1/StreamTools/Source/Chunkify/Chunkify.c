 ...
 *	File:			Chunkify.c
 *
 *	Contains:		MPW tool to take a binary file and turn it into chunks that can be
 ...
 *						Spin cursor.
 *						Use bigger I/O buffers for speed.
 *						Fix bug in initial chunk size computation.  It was not
 *						quadbyte aligning the proper variable.
 *						Fix another bug in chunk size computation.  Since the
 *						first chunk written uses a special larger header than
 *						the subsequent data chunks, the data size needs to be
 *						increased for all non-first-chunks to make up for the
 *						difference in size between the headers.  YUK!
 *						Initalize variables in main().
 *						Return 1 on error instead of 0 and delete partial output.
 *	6/16/93		jb		Update to use Neil's join subscriber definitions.
 *	6/16/93		jb		New today from WriteTestStream source
 *
 *******************************************************************************************/
...
#define	CHUNK_SUBTYPE_STRING	"-type"			/* switch for specifying data type we're joining */
...
#define	COMPRESSION_TYPE_STRING	"-comp"			/* switch for specifying compression type */
...
void usage( void )
	...
	printf( "	%s <size>			size of a data chunk in BYTES (16384)\n", CHUNK_SIZE_STRING );
...
int	main( int argc, char* argv[] )
	...
	subChunkType = (gSubchunkTypeString[0]<<24)
				...
				| (gSubchunkTypeString[2]<<8)
				| gSubchunkTypeString[3];
	...
	while ( ! feof(inputFile) )
		...
		bytesRead = fread( buffer + chunkHeaderSize, 1, chunkDataSize, inputFile );
		if ( bytesRead < 0 )
			{
		...
		if ( 1 != fwrite( buffer, alignedBytesRead + chunkHeaderSize, 1, outputFile ) )
			{
		...
		if ( alignedBytesRead != bytesRead )
			{
			if ( 1 != fwrite( &fourZeroes, alignedBytesRead - bytesRead, 1, outputFile ) )
				{
				printf("error: fwrite() failed!\n");
				goto error;
				}
