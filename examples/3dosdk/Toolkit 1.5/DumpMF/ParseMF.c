 ...
 *	File:			ParseMF.c
 *
 *	Contains:		Interface to a module which parses Standard
 *MIDIFiles and callsback for every event in the file.
 *
 ...
 *ThinkC Tighten track parsing logic Replaced raw hex values with #defines
 *whereever possible for increased readability
 *	06/01/94	rdg		Make sure MetaEvent() doesn't try to
 *allocate memory for a message of size zero.
 *	05/31/94	rdg		Fix logic in ParseTrack() to properly
...
static void
MFError (MIDIFileParserPtr mfp, char *string, Err code)
{
...
static long
ReadByte (MIDIFileParserPtr mfp, uchar *byte)
{
  long result;
...
static long
ReadData (MIDIFileParserPtr mfp, void *buff, long count)
{
  long result;
...
static long
Read32Bits (MIDIFileParserPtr mfp, long *val)
{
  long result;
  uchar c[4];
...
/* Suck 32 bits from the MIDIFile */
static long
Read16Bits (MIDIFileParserPtr mfp, short *val)
{
  long result;
  uchar c[2];
 ...
 */
static long
ParseVLN (MIDIFileParserPtr mfp, long *val)
{
  long result = 0;
  uchar datum = 0;
  long byteCount = 0;
...
static long
To32Bit (uchar c1, uchar c2, uchar c3, uchar c4)
{
  long value = 0;
...
static Err
ReadTypeString (MIDIFileParserPtr mfp, char *str)
{
  long type, num;
  long IDExpected;
...
static Err
ReadHeader (MIDIFileParserPtr mfp)
{
  long result;
  long size;
  char pad[16];
...
static void
ChanMessage (MIDIFileParserPtr mfp)
{
...
static Err
HandleSysex (MIDIFileParserPtr mfp, uchar byteCode)
{
  ...
  if (msgLength > 0)
    {
      ...
      if (mfp->genMsg.msg == NULL)
        {
          sprintf (errBuff, "Not enough memory for Sysex msg!; size = %d.",
                   msgLength);
...
static Err
MetaEvent (MIDIFileParserPtr mfp)
{
  long result;
  long msgLength;
  ...
  if (msgLength > 0)
    {
      ...
      if (mfp->genMsg.msg == NULL)
        {
          sprintf (errBuff,
                   "Not enough memory for Meta event msg!; size = %d.",
                   msgLength);
          MFError (mfp, errBuff, PMFErrInsufficientMemory);
          return PMFErrInsufficientMemory;
        }
...
static Err
HandleSpecialMsgs (MIDIFileParserPtr mfp, uchar aByte)
{
  Err result;
 ...
 */
static Err
ParseTrack (MIDIFileParserPtr mfp)
{
...
Err
InitMIDIFileParser (MIDIFileParserPtr mfp)
{
  Err result = 0;
  ...
  if (mfp->cb_ReadFile == NULL)
    {
      printf (
          "InitMidiFileParser() called without providing a ReadFile callback");
...
Err
ParseMIDIFile (MIDIFileParserPtr mfp)
{
  Err result;
  long trackCount;
...
Err
TermMIDIFileParser (MIDIFileParserPtr mfp)
{
  return 0;
}
