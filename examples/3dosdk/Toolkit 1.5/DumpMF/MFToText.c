 ...
 *	File:			MFToText.c
 *
 *	Contains:		This module sets up callback for the MIDIFileParser which
 *					print out formatted text for each event that the parser
 *					supports.  It serves as an example of how to use the parser.
 *
 ...
 *	History:
 *
 *	06/03/94	rdg		Fix sign extension problem in KeySignature
 *	06/02/94	rdg		Port back to MPW after debugging in ThinkC
 *						Add callbacks for MIDI Real Time Msgs
 *						Re Name lots of things
 *	06/01/94	rdg		re-format printf() statements for better readablilty; lots of
 *						gnarly re-casting, yuk.. isn't there a better way?
 *	06/01/94	rdg		fixed missing parameter to printf() in txt_tempo()
 *	05/23/94	rdg		new today
 *
 *******************************************************************************************/
...
void	txt_Sysex( long evTime, uchar code, GenMsgPtr genMsg )
{
	printf( "\nSysex: evTime = %ld; Type = %lx, Length = %ld\n", 
				evTime, (ulong) code, genMsg->length );
	fflush(stdout);
}
...
void	txt_MEv_SeqSpecific( long evTime, GenMsgPtr genMsg )
{
	printf( "\nMeta event - Sequencer Specific: evTime = %d; Type = 0x%02lx Length = %ld\n",
				evTime, (ulong)genMsg->type, genMsg->length );
	fflush(stdout);
}
...
void	txt_MEv_SMPTE( long evTime, GenMsgPtr genMsg )
{
	...
	printf( "\nSMPTE: Hour = %lu; Minute = %lu; Second = %lu; Frame = %lu; Fract-Frame = %lu\n",
		hr, mn, se, fr, ff );
	fflush(stdout);
}
