 ...
 *
 *	History:
 *	9/29/94		crm		Added operator ! to detect if file opened.
...
private:
	enum AccessPermission { READ_PERMISSION, WRITE_PERMISSION };

							TChunkFile (const TChunkFile&)	{}
			void			operator = (const TChunkFile&)	{}

			void			InternalOpen (const char* name, AccessPermission permission, long blockingFactor = 0);
