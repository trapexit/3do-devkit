 ...
 *	File:			Options.h
 *
 *	Contains:		Options object represents program options specified on the
 *					shell command line.
 *
 ...
 *
 *	History:
 *	9/26/94		crm		Reduced stream I/O expression causing Apple C++ compiler to fail
...
public:
	...
	enum Operation { NO_OPERATION, EXTRACT, EXTRACT_GROUPS, STREAM_INFO };
...
private:
	...
	static const char*	SEPARATE_FLAG;			// separate cels that normally would be written together
	static const char*	EXTRACT_COMMAND;		// extraction operation
	static const char*	EXTRACT_GROUPS_COMMAND;	// extraction operation (group items together)
	static const char*	STREAM_INFO_COMMAND;	// stream analysis operation


						TOptions (const TOptions&) {}
			void		operator = (const TOptions&) {}
