/*
	...
	Change History (most recent first):
		 ...
		 <3>	 3/18/93	JAY		changing Wait to WaitSignal and Signal to SendSignal. Wait and
									Signal where changed
		 <2>	 3/18/93	JAY		chaging lists.h to list.h for Cardinal build
...
void Thread(void) {
  ...
  while (1) {
    ++threadMod;
    SendSignal(myParent, parentSignal);	/* Tell parent task that data is now ready */
...
int main (int argc, char ** argv) {
  ...
  for (ix = 0; ix < 100; ix++)
    ...
    if ((ix%10)==0) kprintf("\n");
    kprintf(" %d",my);		/* 'my' could differ from 'threadMod' at this point */
    }
