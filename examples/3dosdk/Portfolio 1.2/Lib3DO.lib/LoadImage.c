 ...
 *
 *	History:
 *	8/6/93		jml	Updated LoadImage with new SetVRamPages call
 *for new OS 7/25/93 	ian Now uses new faster LoadFile() block I/O instead of
 *					filestream I/O. Also, it now makes fewer
 *assumptions about the validity and content of the data.
 *
...
LoadImage (char *name, ubyte *dest, VdlChunk **rawVDLPtr, ScreenContext *sc)
{
  ...
  if (dest == NULL)
    {
      dest = (ubyte *)Malloc (destbufsize, GETBANKBITS (GrafBase->gf_ZeroPage)
                                               | MEMTYPE_STARTPAGE
