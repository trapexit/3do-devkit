/*
	...
	Change History (most recent first):
		 ...
		 <8>	 7/30/93	JAY		added IO parameter block for SetVRAMPages() and CopyVRAMPages().
									Set IOInfo blocks to zero before assigning values. Modified
									MySportIO.
		 <5>	 6/25/93	dsm		Changed Hilighting mechanism to positional since I couldn't get
									the font_CCB control to work.
		 <4>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
		 <4>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
		 <3>	  4/5/93	JAY		more work on JoyPad for Cardinal and added
									ChangeInitialDirectory to support running from /CD-ROM or
									/remote
		 <2>	  4/2/93	JAY		rehosting menu to run with the new eventBroker under Cardinal1
									and real red
		 <1>	  4/1/93	JAY		first checked in
...
TagArg ScreenTags[] =
{
	...
	/* Kludgy code below presumes that SPORTBITS is first */ 
	/* Kludgy code below presumes that VDLTYPE is second */ 
	CSG_TAG_SPORTBITS,   (void *)0,
	CSG_TAG_BITMAPBUF_ARRAY, (void *)(&BitmapBufPtr),
...
main (int32 argc, char **argv)
{
	...
	FOREVER
		...
		if ( *nextline && (*nextline != ';') )
			...
			line++;
			MenuLineCount++;
			if ( line >= MENU_LINE_COUNT ) goto MENU_NAMES_GOTTEN;
			}

		while ( *nextline++ && (nextline <= lastchar) ) ;
		if ( nextline > lastchar ) goto MENU_NAMES_GOTTEN;
		}
MENU_NAMES_GOTTEN:
	...
	FOREVER
		{
		if ( OpenMenuDisplay() < 0 ) goto DONE;
		RenderMenu( 1 );
...
LaunchProgram( int32 program )
{
	...
	if (launchedItem < 0) 
		...
		{
		kprintf( "Launched %s as item 0x%x\n", 
				ProgramLines[program], launchedItem );
		while (1)
			{
			if ( (t = (Task *)CheckItem( launchedItem,
...
GetMenuSelection()
{
	...
	FOREVER
		...
		//newbits = GetJoystick();
 		...
 		newmenu = 0;
		if ( newbits & JOYFIREA ) return( MenuSelect );
		if ( newbits & JOYFIREB ) return( MenuSelect );
		if ( newbits & JOYFIREC ) return( MenuSelect );
		if ( newbits & JOYSTART ) return( -1 );
		if ( newbits & JOYUP ) 
			{
			upcount++;
			if (( (oldbits & JOYUP) == 0 ) 
...
OpenMenuDisplay()
{
	...
	/* Set up so we can use the graphics folio's page of zero memory to 
	 * clear our screens' buffers
	 */
 ...
 */
{
	...
	if ( maxred <= 0 ) maxred = MAX_RED;
	if ( maxgreen <= 0 ) maxgreen = MAX_GREEN;
	if ( maxblue <= 0 ) maxblue = MAX_BLUE;
	red = Random( maxred + 1 );
...
/* Uses the SPORT to clear to zero the two bitmaps */
{
	...
	SendIO( mySPORTIO[0], &inf[0] );
	DoIO( mySPORTIO[1], &inf[1] );
}
 ...
 */
{
	...
	for ( i = 1;  i >= 0; i-- )
		if ( (mySPORTIO[i] = CreateItem (MKNODEID (KERNELNODE,
							 IOREQNODE),
					      targs)) < 0 )
			{
...
RenderMenu( int32 fillrect )
{
	...
	if ( MenuLineCount > 19 ) lineheight = 8;
	else if ( MenuLineCount > 17 ) lineheight = 9;
	else if ( MenuLineCount > 14 ) lineheight = 10;
	else lineheight = 11;
	...
	for ( i = 0; i < MenuLineCount; i++ )
		...
		if ( i != MenuSelect ) gcon.gc_PenX +=  8;
