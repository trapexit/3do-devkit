 ...
 *NewDataStream() 12/1/93			jb		Switch to using
 *memcpy() from CopyVRAMPages() since the latter requires page aligned VRAM
 *sources *only*.
 *	11/30/93		jb		Added CloseXXXX() routines in
 *DismantlePlayer() routine. 11/29/93		jb		New today from
 *PlayCPakStream.c
 *
 *******************************************************************************************/
...
/************************************/
static int32 InitImagePlayerFromStreamHeader (PlayerPtr ctx,
                                              char *streamFileName);
void DismantlePlayer (PlayerPtr ctx);

/*******************************************************************************************
 * Routine to play an chunified IMAG stream file. Stream may or may not contain
 *audio data. Audio channel selection is based upon info found in the stream
 *header.
 *******************************************************************************************/
int32
PlayImageStream (ScreenContext *screenContextPtr, char *streamFileName,
      ...
      if ((jp = GetJoinElement (ctx->joinContextPtr, ctx->joinPort)) != NULL)
        {
...
#else
          memcpy (ctx->screenContextPtr
                      ->sc_Bitmaps[ctx->screenContextPtr->sc_curScreen]
                      ->bm_Buffer,
          ...
          playerResult = DisplayScreen (
              ctx->screenContextPtr
                  ->sc_Screens[ctx->screenContextPtr->sc_curScreen],
              0);
          if (playerResult < 0)
            {
          ...
          /* Make the other screen buffer the offscreen buffer that we will
           * copy the next image into before displaying it.
           */
          ctx->screenContextPtr->sc_curScreen
              = 1 - ctx->screenContextPtr->sc_curScreen;
 ...
 *******************************************************************************************/
static int32
InitImagePlayerFromStreamHeader (PlayerPtr ctx, char *streamFileName)
{
  int32 status;
       ...
       subscriberIndex++)
    {
      ...
      switch (subsPtr->subscriberType)
        ...
        case JOIN_CHUNK_TYPE:
          ...
          status = NewJoinSubscriber (&ctx->joinContextPtr,
                                      subsPtr->deltaPriority);
           ...
           */
          ctx->joinPort = 0;
          status = InitJoinPort (ctx->joinContextPtr, &ctx->joinPort,
                                 IMAGE_DATA_TYPE);
        ...
        default:
          printf ("InitImagePlayerFromStreamHeader() - unknown subscriber in "
                  "stream header: '%.4s'\n",
