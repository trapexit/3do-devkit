 ...
 *
 *	History:
 *	12/2/93		jb		Propagate numSubsMsgs from weave param
 *block into header data 10/25/93	jb		Change fopen() mode to
 *"wb+"; add code to write the marker table into the stream. Force stream data
 *onto block #1 of output file if either stream header or marker table are
 ...
 *******************************************************************************************/
static void
SortStreamFiles (WeaveParamsPtr pb)
{
  long k;
  long n;
  InStream temp;
...
#if USE_BIGGER_BUFFERS
      if (setvbuf (pb->inStream[k].fd, GetIOBuffer (pb->ioBufferSize), _IOFBF,
                   pb->ioBufferSize))
        {
          fprintf (stderr, "setvbuf() failed for file: %s\n",
                   pb->inStream[k].fileName);
