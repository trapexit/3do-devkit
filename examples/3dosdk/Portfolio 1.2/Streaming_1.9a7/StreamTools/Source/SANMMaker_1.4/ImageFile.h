...
:: = { chunk } | { file } { chunk } { chunk_header }
:: = { chunk_ID } { chunk_size } { chunk }
:: = { chunk_header } { chunk body } { chunk_ID }
:: = 'IMAG' | /* Image header  */
     'SPRH' | /* Sprite header */
     'PDAT' | /* Pixel Data	 */
     'PIPT' | /* PIP Table     */
...
:: = Unsigned 32 bit integer (
         includes size of chunk body plus size of the chunk header)
         .Chunk_size is 8 plus size of the
             chunk_body.Chunks must be Quad byte alligned
         .Chunks will be padded with zeros to fill up the quad word alignment.
/*

ISSUES:
        Should the chunk_size include the 8 bytes needed for the Chunk ID and
the chunk size? If we don't include these 8 bytes then just after you have read
the size you can skip that many bytes and find the next Chunk_ID  (or the EOF).
        Alignment:  I suspect we will be better off by making all chunks 4 byte
aligned. The Arm processor is much better at loading 32 bit quantities from
quad word addresses. Since not all chunks will be a multiple of 4 bytes in
lenght we are forced to put the actual chunk length in the chunk_size field and
then round up to find the next chunk boundary.
...
32 bit fields in the files structure?  The Arm Processor provides more
efficient access to 32 bit fields, but the 16 bit fields save space.  To access
16 bit fields the Arm processor actually performs two byte access and then
shifts and adds to assemble the 16 bit value.
*/
#endif
...
struct SPRITEHDR
{
  ...
  Int32 chunk_size; /*  size in bytes of chunk including chunk_size */
  ulong
      scobversion; /* version number of the scob data structure.  0 for now */
