 ...
 *						Add dumping for APOS and AMAP chunks; add
 *command line switches to control dumping of all chunks.
 *	9/16/93		jb		Version 1.0d2
 *						Add capability to dump old fashioned cel
 *files (non-stream). Decode the AV Value in the PIXC word.
 *	9/14/93		jb		Version 1.0d1
 *						Started with source code of DumpStream.c
 *tool. Does extensive decoding of the CCB flags, PIXC, and Preamble control
 *words.
 *	=======================================================================================
...
DumpSAMAP (long filePosition, StreamAnimMapPtr samap)
{
  ...
  for (cornerNumber = 0; cornerNumber < 4; cornerNumber++)
    {
      printf ("%s%s:\t%ld, %ld\n", gCCBDetailsIndent,
              gQuadCornerName[cornerNumber], samap->quad[cornerNumber].x,
