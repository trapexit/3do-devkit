 ...
 *						Added CloseCtrlSubscriber() to deallocate
 *resources allocated by the InitCtrlSubscriber() routine. 10/19/93	jb
 *Version 1.4 Switch to using UMemory.c routines for memory allocation 8/18/93
 *jb		Version 1.3 Check the calculated delay time in DoAlarm() and if
 *the time to perform the alarm action has already passed, trigger the action
 *immediately. 8/16/93		jb		Version 1.2 Change API to match
 *the actual async API. Allocate a memory pool of streamer request messages
 *(msg items and buffers) for making async requests. 8/10/93		jb
 *Version 1.1 Clean up routine prototypes to make everything private except for
 *things we want explicitly exported.
 *	8/9/93		jb		Add dsReqMsgItemPool, a pool of message
 *items for communication of streamer requests (necessitated by async
 *operation). Add dsReqReplyPort, dsReqReplyPortSignal, to go with this. 8/6/93
 *jb		Version 1.0.1 Add a subscriber message pool used for broadcast
 *operations as required by DSClockSync(), which uses the
 *_SubscriberBroadcast() routine internally. This may be handy for other
 *subscriber broadcast purposes in the future. 7/9/93		jb
 *Version 1.0 Switch to using all async calls back into streamer. 6/28/93
 *jb		Version 0.6 Add hack for setting stream clock based on an ALRM
 *subchunk to make the SAnim subscriber not be required to drive the stream
 *clock itself. 6/22/93		jb		Version 0.5 Add argument to
 *DSClockSync() call to avoid deadlock when the streamer synchronously calls
 **all* subscribers. 6/15/93		jb		Version 0.4 Create a
 *reply port and message item for communicating with the streamer at thread
 *init time instead of creating and deleting same every time a data message
 *arrives. Add code in DoData() to implement the conditional call to the
 *DSClockSync() routine. Remove all extraneous references to timers and audio
 *cue items and audio cue signals. Remove all extraneous include file
 *references. Set chanPtr->msgQueue.tail = NULL at end of flush loop to prevent
 *spurious queuing later. 6/2/93		jb		Version 0.3
 *						Switch to using Debug3DO.h for
 *CHECKRESULT() macro Remove all variable initializers in local variable
 *declarations to avoid register bashing by the ARM C compiler. 5/30/93
 *jb		Version 0.2
 *	5/30/93		jb		Set 'threadItem' to zero so that
 *dispose won't try to delete the thread if close has already been called.
...
/************************************/
static void DoAlarm (CtrlContextPtr ctx, uint32 time, uint32 newTime,
                     uint32 options);
static void DoPause (CtrlContextPtr ctx, uint32 time, uint32 amount,
                     uint32 options);
 ...
 *******************************************************************************************/
int32
InitCtrlSubscriber (void)
{
  /* Create the memory pool for allocating data stream
   * contexts.
   */
  CSGlobals.CtrlContextPool
      = CreateMemPool (CTRL_MAX_SUBSCRIPTIONS, sizeof (CtrlContext));
 ...
 *******************************************************************************************/
int32
NewCtrlSubscriber (CtrlContextPtr *pCtx, DSStreamCBPtr streamCBPtr,
                   int32 priority)
{
  int32 status;
  CtrlContextPtr ctx;
 ...
 *******************************************************************************************/
int32
DisposeCtrlSubscriber (CtrlContextPtr ctx)
{
  if (ctx != NULL)
    {
      if (ctx->threadItem > 0)
...
/*==========================================================================================
  ==========================================================================================
                                                Routines to handle subchunk
  actions
  ==========================================================================================
  ==========================================================================================*/

/*******************************************************************************************
 * This routine schedules an action that will wait for the stream clock to
 *advance to whenTime, and then set the stream clock to newTime. For now,
 *options are ignored.
 *******************************************************************************************/
static void
DoAlarm (CtrlContextPtr ctx, uint32 whenTime, uint32 newTime, uint32 options)
{
  int32 status;
  uint32 streamClock;
  int32 amountToDelay;
   ...
   *			Audio Folio for our timing. This is the best timing
   *source because it syncs directly with our other data, but there is
   *			currently no way to cancel a cue timer once you start
   *one.
   */
 ...
 *******************************************************************************************/
static void
DoPause (CtrlContextPtr ctx, uint32 time, uint32 amount, uint32 options)
{
  int32 status;
  uint32 streamClock;
  AudioTime whenToWakeUp;
  ...
  if (reqMsg != NULL)
    {
      ...
      CHECK_DS_RESULT ("ControlSubscriber >> DoPause() >> DSStopStream() = ",
                       status);
...
/*******************************************************************************************
 * Routine to handle the timer going off. This may be set by either an ALRM
 *subchunk's processing or a PAUS subchunk's processing.
 *******************************************************************************************/
static void
DoTimerExpired (CtrlContextPtr ctx)
{
  DSRequestMsgPtr reqMsg;
 ...
 *******************************************************************************************/
static void
_StopCtrlChannel (CtrlContextPtr ctx, long channelNumber)
{
 ...
 *******************************************************************************************/
static void
_FlushCtrlChannel (CtrlContextPtr ctx, long channelNumber)
{
  SubsChannelPtr chanPtr;
...
/*******************************************************************************************
 * Routine to queue arriving data chunks. We determine the type of control
 *message and call back into the streamer to handle it.
 *******************************************************************************************/
static int32
DoData (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  int32 status;
  ControlChunkPtr ccp;
  ...
  switch (ccp->subChunkType)
    ...
    case SYNC_CHUNK_SUBTYPE:
       ...
       * arriving at a "marker" due to our playing through the marker, or due
       * to having executed a branch to the marker. It is assumed that
       * subscribers will flush any queued data when receiving one of these
       * messages.
       */
 ...
 *******************************************************************************************/
static int32
DoGetChan (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  int32 status;
  long channelNumber;
 ...
 *******************************************************************************************/
static int32
DoSetChan (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  long channelNumber;
 ...
 *******************************************************************************************/
static int32
DoControl (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
#if 0
	int32			status;
 ...
 *******************************************************************************************/
static int32
DoOpening (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  return kDSNoErr;
}
 ...
 *******************************************************************************************/
static int32
DoClosing (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  return kDSNoErr;
}
 ...
 *******************************************************************************************/
static int32
DoStop (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  long channelNumber;
...
/*******************************************************************************************
 * Routine to (normally) flush any queued data we might have. Since we are
 *(usually) the originator of this message, we simply ignore it.
 *******************************************************************************************/
static int32
DoSync (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  return kDSNoErr;
}
 ...
 *******************************************************************************************/
static int32
DoEOF (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  return kDSNoErr;
}
 ...
 *******************************************************************************************/
static int32
DoAbort (SubscriberMsgPtr subMsg, CtrlContextPtr ctx)
{
  long channelNumber;
...
/*******************************************************************************************
 * Routine to to perform one-time initialization for the thread prior to its
 *execution. Allocates all system resources, and communicates with the thread's
 *creator if the thread and all its resources cannot be created.
 *******************************************************************************************/
static int32
_InitializeCtrlThread (CtrlContextPtr ctx)
{
  int32 status;
  long channelNumber;
  ...
  ctx->dsReqMsgPool
      = CreateMemPool (CTRL_NUM_DS_REQS_MSGS, sizeof (DSRequestMsg));
  ...
  ctx->subsMsgPool
      = CreateMemPool (CTRL_MAX_SUBS_MESSAGES, sizeof (SubscriberMsg));
...
/*******************************************************************************************
 * This thread is started by a call to InitCtrlSubscriber(). It reads the
 ...
 *******************************************************************************************/
void
CtrlSubscriberThread (int32 notUsed, CtrlContextPtr ctx)
{
  int32 status;
   ...
   */
  anySignal = ctx->cueSignal | ctx->requestPortSignal
              | ctx->subsReplyPortSignal | ctx->dsReqReplyPortSignal;
  ...
  while (fKeepRunning)
    {
      ...
      if (signalBits & ctx->subsReplyPortSignal)
        {
          while (PollForMsg (ctx->subsReplyPort, NULL, NULL, (void **)&subMsg,
                             &status))
      ...
      if (signalBits & ctx->dsReqReplyPortSignal)
        {
          while (PollForMsg (ctx->dsReqReplyPort, NULL, NULL, (void **)&reqMsg,
                             &status))
                             ...
                             &status))
            {
              switch (subMsg->whatToDo)
                ...
                case kStreamOpSync: /* clock stream resynched the clock */
                   ...
                   *arguments and only may use the whatToDo field in the
                   *message.
                   ************************************************************************/
               ...
               */
              ReplyMsg (subMsg->msgItem, status, subMsg,
                        sizeof (SubscriberMsgPtr));
            }
        }
    }
