 ...
 *******************************************************************************************/
long
InitChannel (SAudioContextPtr ctx, SAudioHeaderChunkPtr headerPtr)
{
  long status;
   ...
   * with CloseChannel() -, figure out which one to make based on the header
  ...
  if (chanPtr->channelInstrument == 0)
    {
      ...
      templateItem = GetTemplateItem (ctx->templateArray, templateTag,
                                      kMaxTemplateCount);
      if (templateItem < 0)
       ...
       * by BeginPlaybackIfAppropriate().  This is accomplished by calling
       * MuteChannel() before SetChannelAmplitude().  This will cause
       ...
       * amplitude". Since we've called MuteChannel(), the current amplitude
       * setting has been saved in savedAmp.  We need to check that now to find
       * out weather we've received an amplitude change control message or not.
       * This is admittedly a bit ugly, but it's the only place where we need
       * to worry about such stuff - so it gets left this way.
       *
       * So, if we didn't get a control message, use the initialAmplitude field
       * from the .saudio header - otherwise MuteChannel() captured whatever
       * the user set with the control call and UnMuteChannel() will restore it
       * when appropriate.  Clear as mud?
       */
      if (chanOutputPtr->savedAmp == -1)
        status = SetChannelAmplitude (ctx, headerPtr->channel,
                                      headerPtr->initialAmplitude);

      if (status < 0)
        return status;

      /* Only try to set channel pan if this is a mono stream; SetChannelPan()
       * will return an error if called on stereo data.
       */
      if (chanOutputPtr->numChannels == 1)
        {
          if (chanOutputPtr->currentPan == 0)
            status = SetChannelPan (ctx, headerPtr->channel,
 ...
 *******************************************************************************************/
void
StartChannel (SAudioContextPtr ctx, long channelNumber)
{
  SAudioChannelPtr chanPtr;
 ...
 *******************************************************************************************/
long
StopChannel (SAudioContextPtr ctx, long channelNumber)
{
  long status;
  SAudioChannelPtr chanPtr;
      ...
      if ((bufferPtr = GetNextBuffer (chanPtr)) != NULL)
        {
          ...
          /* Leave a trace so we can tell which buffer was playing when stop
           * was called */
          ADD_TRACE (kCurrentBufferOnStop, channelNumber, bufferPtr->signal,
                     (long)bufferPtr);

          /* Check to see if this buffer completed before we could get to
           * StopInstrument() and if it did, call WaitSignal() to clear the
           * bit.
           */
 ...
 *******************************************************************************************/
long
FlushChannel (SAudioContextPtr ctx, long channelNumber)
{
  long status;
  SAudioChannelPtr chanPtr;
  ...
  if (IsChanEnabled (chanPtr))
    {
      ...
      while ((bufferPtr = GetNextBuffer (chanPtr)) != NULL)
        {
          /* Reply to the message */
          ADD_TRACE (kFlushedBuffer, channelNumber, bufferPtr->signal,
                     (long)bufferPtr);
          status = ReplyMsg (bufferPtr->pendingMsg->msgItem, kDSNoErr,
                             bufferPtr->pendingMsg, sizeof (SubscriberMsg));
 ...
 *******************************************************************************************/
long
CloseChannel (SAudioContextPtr ctx, long channelNumber)
{
  long status;
  SABufferInitBlock buffInitBlock;
  SAudioChannelPtr chanPtr;
 ...
 *******************************************************************************************/
long
SetChannelAmplitude (SAudioContextPtr ctx, long channelNumber, long newAmp)
{
  SAudioChannelPtr chanPtr;
  SAudioOutputPtr chanOutputPtr;
  long leftAmp;
   ...
   * amplitude values were not changed; but, we want GetChannelAmplitude() to
   * return the new setting AND we want UnMuteChannel() to restore to the
   * proper value.
   */
 ...
 *******************************************************************************************/
long
SetChannelPan (SAudioContextPtr ctx, long channelNumber, long newPan)
{
  SAudioChannelPtr chanPtr;
  SAudioOutputPtr chanOutputPtr;
  long leftAmp;
 ...
 *******************************************************************************************/
long
GetChannelAmplitude (SAudioContextPtr ctx, long channelNumber, long *Amp)
{
  SAudioChannelPtr chanPtr;
  SAudioOutputPtr chanOutputPtr;
 ...
 *******************************************************************************************/
long
GetChannelPan (SAudioContextPtr ctx, long channelNumber, long *Pan)
{
  SAudioChannelPtr chanPtr;
  SAudioOutputPtr chanOutputPtr;
 ...
 *******************************************************************************************/
long
MuteChannel (SAudioContextPtr ctx, long channelNumber, Boolean external)
{
  long status;
  SAudioChannelPtr chanPtr;
  SAudioOutputPtr chanOutputPtr;
 ...
 *******************************************************************************************/
long
UnMuteChannel (SAudioContextPtr ctx, long channelNumber, Boolean external)
{
  long status;
  SAudioChannelPtr chanPtr;
  SAudioOutputPtr chanOutputPtr;
  ...
  if (chanOutputPtr->muted)
    {
      ...
      if ((external && chanOutputPtr->externalMute)
          || (!external && !chanOutputPtr->externalMute))
        {
          ...
          status = SetChannelAmplitude (ctx, channelNumber,
                                        chanOutputPtr->savedAmp);
          ADD_TRACE (kChannelUnMuted, channelNumber, chanOutputPtr->currentAmp,
                     0);
 ...
 *******************************************************************************************/
void
BeginPlaybackIfAppropriate (SAudioContextPtr ctx, long channelNumber)
{
  SAudioBufferPtr bufferPtr;
