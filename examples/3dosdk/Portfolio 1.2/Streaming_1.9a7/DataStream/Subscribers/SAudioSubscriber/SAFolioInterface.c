 ...
 *						Fixed number of parameters to MuteChannel().
	...
	for ( channelNumber = 0; channelNumber < SA_SUBS_MAX_CHANNELS; channelNumber++ )
		...
		if ( IsChanEnabled(chanPtr) && IsChanActive(chanPtr) )
			...
			if ( bufferPtr != NULL )
				...
				ADD_TRACE( kBufferOrphaned, channelNumber, bufferPtr->signal, (long) bufferPtr );
				}
			}
		}
	}
...
void		MoveWaitingMsgsToBufferQueue( SAudioContextPtr ctx, long channelNumber )
	...
	if ( IsChanActive(chanPtr) )
		...
		while ( (chanPtr->dataQueue.head != NULL) 
			...
			ADD_TRACE( kMovedBuffer, channelNumber, bufferPtr->signal, (long)bufferPtr );
			}
		}
	}
	...
	for (channelNumber=0; channelNumber < SA_SUBS_MAX_CHANNELS; channelNumber++)
		...
		if ( mySigBits )	/* is the signal from this channel? */
			...
			if ( mySigBits & localBuffPtr->signal )
				...
				ADD_TRACE( kFoundBuffer, channelNumber, localBuffPtr->signal, (long)localBuffPtr );
				}
			else
				{
				 ...
				 */
				ADD_TRACE( kFoundWrongBuffer, channelNumber, localBuffPtr->signal, (long)localBuffPtr );
				localBuffPtr = NULL;
