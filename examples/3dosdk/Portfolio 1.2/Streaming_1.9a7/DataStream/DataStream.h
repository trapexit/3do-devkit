 ...
 *						Added argument to NewDataStream() to
 *specify the number of subscriber messages that should be allocated upon
 *stream creation. Removed the definition of DS_MAX_SUBS_MESSAGES, which can
 *now be specified dynamically because of the aforementioned modification. Add
 *numSubsMsgs field to stream control block. 11/23/93	jb
 *Version 1.6 Added CloseDataStreaming() routine. 11/9/93		jb
 *Version 1.5 Added 'connect' union member to DataAcqMsg message to data
 *acquirers. The connect message now carries a pointer to the stream control
 *block with it so that data acquirers can communicate with the streamer.
