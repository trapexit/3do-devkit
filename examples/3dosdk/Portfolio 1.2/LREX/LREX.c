/*
	...
	Change History (most recent first):
		 ...
		 <4>	 7/30/93	JAY		add IO Parameter to CopyVRAMPages() and SetVRAMPages() for
									DragonTail release
		 <3>	 6/24/93	JAY		change FindMathFolio to OpenMathFolio. Change ccb offset name to
									Dragon1 names.
		 <2>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal
 ...
 */
{
    ...
    if ( FlagIsSet( KernelBase->kb_CPUFlags, KB_RED )  )
		...
		{
		kprintf(" %s requires Red Silicon or later to run. It uses LRForm Cels, sorry. \n",progname);
		exit(0);
		}
	if (argc <= 1)
	{
	...
	lrccb.ccb_PRE0	= ((lrccb.ccb_Height-1)<<6) +	/* number of vertical data lines in sprite data -1 */
					  PRE0_LINEAR +					/* use PIN for IPN (0x10) */ 
					  PRE0_BPP_16;					/* set bits/pixel to 16 (0x6) see HDWR spec 3.6.3 */

	lrccb.ccb_PRE1	= ((640/2-2)<<16) +				/* offset (in words) from line start to next line start. */ 
    ...
    kprintf(" Arrow keys move cel around using the underlying image data in the cel  \n");
...
/* Returns non-FALSE if user didn't press START, else returns FALSE to quit */
{
	...
	if ( joybits & (JOYUP|JOYDOWN|JOYLEFT|JOYRIGHT) )
		...
		if ( joybits & JOYUP )
			...
			{
			++gYPos;
			if (++gYPos > (DISPLAY_HEIGHT-ccb->ccb_Height)) gYPos = DISPLAY_HEIGHT-ccb->ccb_Height;
			}
		if ( joybits & JOYLEFT )
			{
			gReverse = FALSE;
			...
			gReverse = TRUE;
			++gXPos;
			if (++gXPos > (DISPLAY_WIDTH-ccb->ccb_Width)) gXPos = DISPLAY_WIDTH-ccb->ccb_Width;
			}
