/*
	...
	Change History (most recent first):
		...
		<8+>	 8/10/93	JAY		changing to new LoadAnim interface
		 <8>	 7/28/93	JAY		modify WaitVBL(), SetVRAMPages(), CopyVRAMPages() to conform to
									Dragon Tail release which requires a IO Parameter be added to
									the calls.
		 <7>	 6/24/93	JAY		change FindMathFolio to OpenMathFolio
		 <6>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
		 <5>	  4/5/93	JAY		Trying to figure our ReadControlPad(). Final conclustion is that
									this program is not interactive enough and the eventBroker is
									not robust enough. Removed /remote/ in filenames (i.e. pathname
									are now relative to initial working directory. Added
									ChangeInitialDirectory() to assist with relative pathnames.
		 <4>	  4/2/93	JAY		add KillEventUtility() to exit the EventBroker gracefully
		 <3>	 3/23/93	JAY		add FindMathFolio() to startup for Cardinal
		 <2>	 3/17/93	JAY		¥'lists.h' to 'list.h' for Cardinal build
		 <1>	 3/17/93	JAY		This is the 2B1 version being check in for Cardinal build
 ...
 */
{
	...
	gBackPic = (ubyte *)ALLOCMEM( (int)(sc->sc_nFrameBufferPages*GrafBase->gf_VRAMPageSize),
...
long sinv(long angle, long value)
	{
	return ((MulSF16(SinF16(angle),(value << 16)))+0x8000) >> 16;
	}

long cosv(long angle, long value)
	{
...
show3DO(ANIM *anim1, ANIM *anim2, ANIM *anim3)
	...
	while (true)
		...
		for (i = 0; i < NumObjs; i++)
			...
			DrawAnimCel(obj[i].anim, sc->sc_BitmapItems[ ScreenSelect ], x, y+obj[i].vAdjust, 0, CenterHotSpot);
			}
		...
		ScreenSelect = 1 - ScreenSelect;
			if ((stopping >= 0) && (--downCounter < 0))
				...
				if (stopping >= NumObjs)
					{
					if ((!obj[NumObjs-1].alive) && (stopping++ > NumObjs)) break;
					}
				else
					{
					if (obj[stopping].waitingToDie)
						...
						if (obj[stopping].angle < inc/2)
							...
							if (stopping == 0)
								...
								{
								obj[stopping].r = obj[stopping-1].r + (obj[stopping-1].height + obj[stopping].height)/4;
								obj[stopping].v = 
									obj[stopping-1].v - (obj[stopping].r - obj[stopping-1].r);
								}
							}
						}
					}
			...
			for (i = 0; i < NumObjs; i++)
				...
				if (obj[i].angle < 0) obj[i].angle += circle;
				}
			
			if (inc < MaxSpeed) inc = MulSF16(inc, Accelerator);
			else if (stopping < 0) stopping = 0;
			}
	}
...
static ANIM* animVec[NumObjs];	/* these are huge! We don't want these on the stack! */
...
int main (int argc, char **argv)
{
  ...
  int i;
	...
	if (opened & GRAPHICSMASK) DBUG(("Graphics Foio Opened \n"));
	if (opened & AUDIOMASK) DBUG(("Audio Foio Opened \n"));
	if (opened & SPORTIOMASK) DBUG(("Sportio  Opened \n"));
	if (opened & MACLINKMASK) DBUG(("MACLINK Opened \n"));
	
	if (!InitOrbitSound()) printf("\rCould not initialize sound");
	...
	for (i = 0; i < NumObjs; i++)
		{
		if (i < (argc-2)) strcpy(animname, argv[i+1]);
		else strcpy(animname, defaultAnimNames[i]);
		...
		else kprintf("\rLoaded %s\r", animname);
			
		if (animVec[i]->num_Frames > 0)
			kprintf("%d cels were found in %s\n",animVec[i]->num_Frames,animname);
		else 
			{
	...
	while (!click())
		...
		PlayAnnounce();
#endif	
//		for (i = 0; i < 60; i++) WaitVBL(); // while (!click());
		WaitVBL( VBLIOReq, 60 );
		}
