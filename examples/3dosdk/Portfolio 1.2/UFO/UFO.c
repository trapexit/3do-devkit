/*
	...
	Change History (most recent first):
		 ...
		 <8>	 8/15/93	JAY		change LoadAnim interface
		 <7>	 7/30/93	JAY		add new parameters to WaitVBL() for Dragon Tail
		 <6>	 6/24/93	JAY		change path for PlayBgndMusic.task from remote to UFOSound
		 <5>	 6/23/93	JAY		change ccb offset names for Dragon (4B1)
		 <4>	  4/5/93	JAY		removing /remote/ from filename (i.e. making pathname relative
									to initial working directory
		 <3>	 3/29/93	JAY		change AF_TAG_LOUDNESS to AF_TAG_AMPLITUDE for Cardinal1
		 <2>	 3/18/93	JAY		changed TAG_FREQ to TAG_RATE and changed number of arguments in
									StopInstrument
...
unsigned int randbits(int shift)
	{
	shift = 31 - shift;
	return (rand() >> shift);
	}
...
bool UpdateDashboard()
	...
	if (nLaserCount)
		{
		--nLaserCount;
		DrawScreenCels(TheScreen.sc_Screens[TheScreen.sc_curScreen], pcCockpitRed);
		}
		
	if (bHoverMode)
		DrawScreenCels(TheScreen.sc_Screens[TheScreen.sc_curScreen], pcCockpitGreen);
	
	if (!randbits(8))
		DrawScreenCels(TheScreen.sc_Screens[TheScreen.sc_curScreen], pcCockpitFire);
...
bool Zap(CCB* pcTarget, Coord X, Coord Y)
	...
	if (	X >= 0
		&&	Y >= 0
		&&	(Y < ((pcTarget->ccb_Height<< 8) / iDistance))
...
bool PlayerAction(void)
	...
	if ( lEvent & JOYSTART )
		...
		if ( lEvent & JOYFIREA && !nLaserCount)
			...
			if (Zap(pcUFO, (pcCrosshairs->ccb_XPos>>16)+(pcCrosshairs->ccb_Width>>1), (pcCrosshairs->ccb_YPos>>16)+(pcCrosshairs->ccb_Height>>1)))
				{
				Balance = ((pcCrosshairs->ccb_XPos + (pcCrosshairs->ccb_Width >> 1)) >> 2) / 160;
				...
				pcExplosion->ccb_XPos = pcCrosshairs->ccb_XPos + (pcCrosshairs->ccb_Width<<15) - (pcExplosion->ccb_Width * pcUFO->ccb_VDY >> 1);
				pcExplosion->ccb_YPos = pcCrosshairs->ccb_YPos + (pcCrosshairs->ccb_Height<<15) - (pcExplosion->ccb_Height * pcUFO->ccb_VDY >> 1);
		...
		if (lEvent & JOYRIGHT && pcCrosshairs->ccb_XPos < ((320-pcCrosshairs->ccb_Width)<<16))
			pcCrosshairs->ccb_XPos += lJoyMove;
			
		if (lEvent & JOYLEFT && pcCrosshairs->ccb_XPos > 0)
			pcCrosshairs->ccb_XPos -= lJoyMove;
			
		if (lEvent & JOYDOWN && pcCrosshairs->ccb_YPos < ((240-pcCrosshairs->ccb_Height)<<16))
...
void TargetAction(CCB* pcTarget)
	...
	iTest = (pcTarget->ccb_YPos >> 16) + ((pcTarget->ccb_Height << 8)/ iDistance);
	...
	iTest = (pcTarget->ccb_XPos >> 16) + ((pcTarget->ccb_Width << 8) / iDistance);
	...
	if (nBoomCount)
		...
		if (nBoomCount)
			...
			DrawScreenCels(TheScreen.sc_Screens[TheScreen.sc_curScreen], pcExplosion);
			}
		else
			{
...
bool InitGame(void)
	...
	if ((pbBackground = LoadImage("UFOArt/Sky.IMG", NULL, NULL, &TheScreen)) == NULL)
		{
	...
	(void)SetItemPri(iSoundHandler, KernelBase->kb_CurrentTask->t.n_Priority+1);
	...
	ALERT("Loading green...");
	if ((pcCockpitGreen = LoadCel("UFOArt/CockpitGreen.CEL", MEMTYPE_CEL)) == NULL)
		{
	...
	ALERT("Loading fire...");
	if ((pcCockpitFire = LoadCel("UFOArt/CockpitFire.CEL", MEMTYPE_CEL)) == NULL)
		{
	...
	for (i = FIRST_RED; i <= LAST_RED; i++)
		p[i] >>= 5;
	// The above is a crude hack to take the red band of the color-cycling palette and turn it green.
	// I'm not proud of it.
	...
	//Signal(iSoundHandler, 0x10000000);	// Just pick a random signal, the task waits for them all.
	//ALERT("Back from Signal, music can start now.\n");
...
int main(int argc, char** argv)
	...
	if (InitGame())
		...
		while (PlayerAction())
			...
			DrawImage(TheScreen.sc_Screens[TheScreen.sc_curScreen], pbBackground, &TheScreen);
			TargetAction(pcUFO);
			if (!UpdateDashboard())
				break;
			DisplayScreen(TheScreen.sc_Screens[TheScreen.sc_curScreen], 0);
