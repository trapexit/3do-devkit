/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <2>	 1/3/94		CDE		Derived from
   SlideShow.c; Replaced direct joypad access with event broker interface;
                                                                        Cleaned
   up includes, prototypes, changed some function types; Removed -r, -p options
   from Usage() since they were unimplemented; Added ec's fix for bug in
   ParseScriptFile which prevented the last file from being loaded if the line
   didn't end in a return.
*/
  ...
  for (aScreenIndex = 0; aScreenIndex < kBufferCount; aScreenIndex++)
    {
      ...
      errorCode = SetVDL (gScreenContext.sc_Screens[aScreenIndex],
                          gVDLItems[aScreenIndex]);
      if (errorCode < 0)
        {
          printf ("Error: SetVDL() failed with error code %d\n", errorCode);
