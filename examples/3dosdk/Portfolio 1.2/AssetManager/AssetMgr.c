 ...
 *	File:			AssetMgr.c
 *
 *	Contains:		simple read-only asset management database for 3DO applications
 *
 ...
 *******************************************************************************************/
int32	LoadAssetFile( char *name, AssetFileDescPtr af )
	{
	int32			status;
	BlockFile		assetFile;
	...
	fileBufSize = ((fileSizeInBytes + fileBlocksize - 1) / fileBlocksize) * fileBlocksize;
	...
	status = AsynchReadBlockFile( &assetFile, ioItem, af->assetBuffer, fileBufSize, 0 );
	if ( status != 0 )
		return status;
	...
	if ( (hdr->chunkType != ASSET_CHUNK_TYPE) || (hdr->subChunkType != ASSET_HEAD_TYPE) )
		{
		UnloadAssetFile( af );
	...
	if ( (afd->chunkType != ASSET_CHUNK_TYPE) || (afd->subChunkType != ASSET_DATA_TYPE) )
		{
		UnloadAssetFile( af );
		return kAMBadFileForm;
		}
...
/*******************************************************************************************
 * Routine to close an asset file. Free's the data buffer associated with the file
 * and actually closes the file.
 *******************************************************************************************/
void	UnloadAssetFile( AssetFileDescPtr afd )
	{
 ...
 * NOTE: THIS VERSION ONLY SUPPORTS MEMORY RESIDENT ASSET FILES!!!
 *******************************************************************************************/
int32	GetAssetInfo( AssetFileDescPtr afd, AssetType type, AssetID id, 
...
/*******************************************************************************************
 * Routine to get a pointer to asset data stored in an asset file. Loads the data from
 * disk if necessary.
 * NOTE: THIS VERSION ONLY SUPPORTS MEMORY RESIDENT ASSET FILES!!!
 *******************************************************************************************/
int32	GetAsset( AssetFileDescPtr afd, AssetType type, AssetID id, void** pAssetData )
	{
	int32		status;
	AssetKeyPtr	keyPtr;
