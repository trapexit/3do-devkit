/*
	...
	Change History (most recent first):
		...
		<2+>	 8/10/93	JAY		change calling sequence to LoadAnim for new Lib3DO.lib
		 <2>	 7/31/93	JAY		add IO parameters to SetVRAMPages() and CopyVRAMPages() for
									Dragon Tail release
		 <1>	 6/25/93	JAY		first checked in
 			6/1/93		ec		Updated for dragon.
 			4/1/93		ec		New today
 ...
 *
 *					"A" button:		and up or down arrow changes animation speed
 *
 *					"B" button:		and arrows changes cel width and height
 *
...
StartUp( void )
{
	...
	// initialize the various managers.  The only one we really NEED is the graphics folio
...
Usage(char *progName, Boolean errorCond)
{
	...
	kprintf("C button enables or disables drawing of the alpha channel cel.\n\n");
 ...
 */
{
	...
	ANIM*	animRec;
	int32	errorVal,
			waitCount = gPauseBeforeNextCell;
	...
	while (TRUE)
	{
		...
		{
			/* Grab the next cel in the animation.  An 'edge-o-matic' file has the
			 *  image cel first, followed by the edge cel - grab one of each
			 */
		...
		ShowCelTiming();
		retvalue = DrawCels (gScreenContext.sc_BitmapItems[gScreenContext.sc_curScreen], alphaCCB);
		if ( retvalue < 0 ) 
