/*
	...
	Change History (most recent first):
		 ...
		 <4>	 7/30/93	JAY		add IO Parameter for SetVRAMPages() and CopyVRAMPages for Dragon
									Tail
		 <3>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
		 <2>	  4/5/93	JAY		remove /remote/ from filenames (i.e. make pathname relative to
									initial directory). Add ChangeInitialDirectory() to assist in
									relative pathname
		 <1>	  4/3/93	JAY		first checked in
				  4/3/93	DSM		There is no such thing as water, only melted ice.
...
/* if it doesn't work, so what? */
  ...
  spr = LoadSprite("perfpics/testoOpaque.cel", Convert32_F16(1), Convert32_F16(1), 0x00001000, Convert32_F16(1)); 
	spr = LoadSprite("perfpics/testoOpaque.cel", Convert32_F16(1),Convert32_F16(2), 0x00002000, Convert32_F16(2)); 
	spr = LoadSprite("perfpics/testoOpaque.cel", Convert32_F16(2),Convert32_F16(2), 0x00002800, Convert32_F16(3));  
