...
int numlit, numpack, numtrans, tokensize, wcount, literalflag, bgndflag,
    pipsize, opffFlag;
...
long bpptable[] = {
  NULL,       PRE0_BPP_1, PRE0_BPP_2, NULL, PRE0_BPP_4,  NULL,
  PRE0_BPP_6, NULL,       PRE0_BPP_8, NULL, NULL,        NULL,
  NULL,       NULL,       NULL,       NULL, PRE0_BPP_16,
};
...
printccb (CCC *ccb)
{
  ...
  if (ccb->ccb_Flags & SCB_SCBPRE)
    {
      if ((ccb->ccb_PRE0 & PRE0_BPP_MASK) == 0)
        printf (" Bits per pixel is 0\n");
      if ((ccb->ccb_PRE0 & PRE0_BPP_MASK) == 1)
        printf (" Bits per pixel =  1\n");
      if ((ccb->ccb_PRE0 & PRE0_BPP_MASK) == 2)
        printf (" Bits per pixel =  2\n");
      if ((ccb->ccb_PRE0 & PRE0_BPP_MASK) == 3)
        printf (" Bits per pixel =  4\n");
      if ((ccb->ccb_PRE0 & PRE0_BPP_MASK) == 4)
        printf (" Bits per pixel =  6\n");
      if ((ccb->ccb_PRE0 & PRE0_BPP_MASK) == 5)
        printf (" Bits per pixel =  8\n");
      if ((ccb->ccb_PRE0 & PRE0_BPP_MASK) == 0)
...
char *argv[];
{
  ...
  if (codedSelected == FALSE)
    {
      fprintf (stderr, "Error - one of \"-c\" (coded) or \"-uc\" (uncoded) "
                       "must be specified\n");
      exit (1);
    }
      ...
      if ((tokensize != 8) && (tokensize != 16))
        {
          fprintf (
              stderr,
              "Error - Uncoded cels must be either 8- or 16-bits per pixel\n");
          exit (1);
        }
    }
  ...
  if (woffset < 0)
    woffset = 0;
  pre1bits |= woffset << ((tokensize < 8) ? PRE1_WOFFSET8_SHFT
                                          : PRE1_WOFFSET10_SHFT);
  ...
  if (opffFlag == TRUE)
    {
      ...
      celHdr.ccb_Flags
          = SCB_LDSIZE   /* Load hdx-y and vdx-y from SCoB */
          ...
          if (opffFlag == TRUE)
            {
              ...
              writeword (outfile,
                         pipsize * 2 + 12); /* pipsize is in shorts: PIP
                                               entries are 1555 = 16bits */
              writeword (outfile,
                         pipsize); /* number of entries in PIP Table */
            }
          writepip ();
        }
...
void readword (file, word) FILE *file;
char *word;
{
  int c, i;
...
void writeword (file, word) FILE *file;
long word;
{
...
long t;
{
  ...
  if (t < 0)
    return t;
  if ((t == transparenttoken) && bgndflag && !literalflag)
    return 0x7fffffff;
  if (pre0bits & PRE0_LINEAR)
    {
      if (tokensize == 16)
        { /* if 16-bit linear sprite, return token unaltered */
          return t;
        }
      else
        { /* if 8-bit linear sprite, return token reduced to 8 bits */
          return ((t >> 7) & (0x7 << 5)) + ((t >> 5) & (0x7 << 2))
                 + ((t >> 3) & 0x3);
        }
    }
  else
  ...
  if (wcount--)
    {
      ...
      t = (long)i * 256 + j;
      if ((literalflag || !bgndflag) && (t == transparenttoken))
        t = 0;
      return t;
    }
  else
  ...
  for (j = 0; j < height; j++)
    {
      ...
      while ((newtoken = gettoken ()) >= 0)
        {
          if ((newtoken == transparenttoken) && bgndflag && !literalflag)
            continue;
          ...
          if (k == i)
            {
              if (i < pipsize)
              ...
              else
                {
                  fprintf (
                      stderr,
                      "Error - more than %d different colors in input file\n",
                      pipsize);
                  exit (1);
                }
            }
...
void addliteral (token) long token;
{
  int i;
  string *j;
...
void addtrans (token) long token;
{
  int i;
  string *j;
...
void addpacked (token) long token;
{
  int i;
  string *j;
...
void initstring (str) string **str;
{
  if (!*str)
    {
      if (!(*str = (string *)malloc (sizeof (string))))
        {
          fprintf (stderr,
                   "Unable to allocate memory for string( %xH bytes)\n",
                   sizeof (string));
          exit (1);
        }
    }
...
void copystring (str1, str2) string *str1, *str2;
{
  int i;
...
void addbits (outarray, i, numbits, cnt, bitcnt) ulong outarray[];
ulong i;
int numbits;
int *cnt, *bitcnt;
{
      ...
      if (!infile)
        {
          ...
          if (imgChunk.chunk_ID == imagetype)
            ...
            fseek (infile, 0L,
                   0); /* rewind to top of infile if not OPFF format */
      ...
      switch (tolower (option[1]))
        ...
        case 'b':
          ...
          if (tokensize < 0 || tokensize > 16 || !bpptable[tokensize])
            {
              fprintf (stderr,
                       "Error - illegal number of bits per pixel: %s\n",
                       option);
              usage ();
            }
          break;
        case 't':
          ...
          if (transparenttoken > 65535)
            {
              fprintf (stderr,
                       "Error - illegal value for transparent color: %s\n",
                       option);
              usage ();
            }
          bgndflag = FALSE;
...
usage ()
{
  ...
  fprintf (stderr,
           "makecel reads a non-LR-form 3DO image from the file specified\n");
  fprintf (
      stderr,
      "by \"imagefile\" on the command line, and writes a 3DO cel to the\n");
  fprintf (
      stderr,
      "file specified as \"celfile\" on the command line.  If the output\n");
  fprintf (
      stderr,
      "format is other than an 8- or 16-bit uncoded cel then a PLUT will\n");
  fprintf (stderr, "also be written to the file \"celfile\"\n");
  fprintf (stderr, "\n");
  fprintf (stderr, "Valid options are:\n");
  fprintf (stderr, "\t-b<n>\tspecify number of bits per pixel for output cel "
                   "(default: 16)\n");
  fprintf (stderr, "\t-f\tthis is a foreground cel (clears BGND bit); black "
                   "is transparent\n");
  fprintf (stderr, "\t-t<n>\tspecify value (hex 555-RGB) of transparent color "
                   "in input (clears BGND bit)\n");
  ...
  fprintf (stderr,
           "\t-rc\tclear the REP8 bit in the first cel preamble word\n");
  fprintf (
      stderr,
      "The number of bits per pixel must be one of:  1, 2, 4, 6, 8 or 16.\n");
  fprintf (stderr, "The LINEAR bit will be cleared if the number of "
                   "bits/pixel is less than 8.\n");
  fprintf (stderr, "The second preamble word is only generated for totally "
                   "literal sprites.\n");
  fprintf (stderr, "Numeric values are in the same format as C constants.\n");
  fprintf (
      stderr,
      "It is possible to have a packed background sprite that also has\n");
  fprintf (stderr, "transparency within it.  To do this, set the transparent "
                   "color (-t option)\n");
  fprintf (stderr, "and then set the BGND bit (-g option) afterwards on the "
                   "command line.\n");
  exit (1);
}
