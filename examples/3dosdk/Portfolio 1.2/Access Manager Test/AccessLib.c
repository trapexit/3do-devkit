 ...
 ******************************************************/
Item	GetShellItem(int32	screenItem) {
	int32	res;
	...
	if (shellItem <= 0) {		// if shellItem is positive, then we already got it
	
		// locate access port and access task
		if ( (accessItem = FindNamedItem(MKNODEID(KERNELNODE,MSGPORTNODE),"access")) < 0 )
			{
	...
	screenSize = screen->scr_TempBitmap->bm_Width*screen->scr_TempBitmap->bm_Height*2;
	res = ControlMem((void *)screen->scr_TempBitmap->bm_Buffer,screenSize,MEMC_OKWRITE,shellItem);
	if (res < 0)
		{
		printf("Error: Allowing shell task access to our screen memory failed.\n");
		return res;
		}
	return shellItem;
}
 ...
 *			the shell task
 ******************************************************/
int32	AllocateMsgAndReply(void) {
 ...
 *			the shell task
 ******************************************************/
int32 AccessInit(Item	screenItem) {
	int32	res;
 ...
 ******************************************************/
void	DeallocateMsgAndReply(void) {
	DeleteItem(msgItem);
	DeleteItem(portItem);
}
 ...
 *		Deallocates Reply and Message Items
 ******************************************************/
void	AccessCleanup(void) {
	int32	res;
	
	res = ControlMem((void *)screen->scr_TempBitmap->bm_Buffer,screenSize,MEMC_NOWRITE,shellItem);
	if (res < 0)
		{
		printf("Error: Removing shell task access to our screen memory failed.\n");
		}
	DeallocateMsgAndReply();
}
 ...
 ******************************************************/
void SetColorTags(int32	*tagnum) {
 ...
 ******************************************************/
int32	GetLoadFileName(char *title, char *fName, Item screenItem) {
	int32	res, tagnum = 0;
	...
	targs[tagnum++].ta_Arg = (void *)ACCREQ_LOAD;
 ...
 *		be deleted.
 ******************************************************/
int32	GetDeleteFileName(char *title, char *fName, Item screenItem) {
	int32	res, tagnum = 0;
	...
	// set up tag args for the call to access
	targs[tagnum].ta_Tag = ACCTAG_REQUEST;
	targs[tagnum++].ta_Arg = (void *)ACCREQ_DELETE;
	targs[tagnum].ta_Tag = ACCTAG_SCREEN;
	targs[tagnum++].ta_Arg = (void *)screenItem;
	targs[tagnum].ta_Tag = ACCTAG_TITLE;
	targs[tagnum++].ta_Arg = (void *)title;
 ...
 ******************************************************/
int32	EasyDeleteFile(char *fName) {
	int32 			err;
 ...
 ******************************************************/
int32	GetSaveFileName(char *title, char *fName, Item screenItem) {
	int32	res, tagnum = 0;
	...
	// set up tag args for the call to access
	targs[tagnum].ta_Tag = ACCTAG_REQUEST;
	targs[tagnum++].ta_Arg = (void *)ACCREQ_SAVE;
	targs[tagnum].ta_Tag = ACCTAG_SCREEN;
	targs[tagnum++].ta_Arg = (void *)screenItem;
	targs[tagnum].ta_Tag = ACCTAG_TITLE;
	targs[tagnum++].ta_Arg = (void *)title;
 ...
 ******************************************************/
int32	StdWriteFile(char	*fName, char *buf, int32 bufSize) {
	int32 			err;
	Item 			fd, req;
	IOReq 			*reqp;
	IOInfo 			params;
 ...
 ******************************************************/
int32	OKDialog(char *title, char *text, Item screenItem) {
	int32	res, tagnum = 0;
	...
	// set up tag args for the call to access
	targs[tagnum].ta_Tag = ACCTAG_REQUEST;
	targs[tagnum++].ta_Arg = (void *)ACCREQ_OK;
	targs[tagnum].ta_Tag = ACCTAG_SCREEN;
	targs[tagnum++].ta_Arg = (void *)screenItem;
	targs[tagnum].ta_Tag = ACCTAG_TITLE;
	targs[tagnum++].ta_Arg = (void *)title;
 ...
 ******************************************************/
int32	OKCancelDialog(char	*title, char *text, Item screenItem, Boolean *OK) {
	int32	res, tagnum = 0;
	...
	// set up tag args for the call to access
	targs[tagnum].ta_Tag = ACCTAG_REQUEST;
	targs[tagnum++].ta_Arg = (void *)ACCREQ_OKCANCEL;
	targs[tagnum].ta_Tag = ACCTAG_SCREEN;
	targs[tagnum++].ta_Arg = (void *)screenItem;
	targs[tagnum].ta_Tag = ACCTAG_TITLE;
	targs[tagnum++].ta_Arg = (void *)title;
 ...
 *		Puts up a dialog using the access mgr
 ******************************************************/
int32	DialogOneButton(char *title, char *btnText, char *text, Item screenItem) {
	int32	res, tagnum = 0;
	...
	// set up tag args for the call to access
	targs[tagnum].ta_Tag = ACCTAG_REQUEST;
	targs[tagnum++].ta_Arg = (void *)ACCREQ_ONEBUTTON;
	targs[tagnum].ta_Tag = ACCTAG_SCREEN;
	targs[tagnum++].ta_Arg = (void *)screenItem;
	targs[tagnum].ta_Tag = ACCTAG_TITLE;
	targs[tagnum++].ta_Arg = (void *)title;
 ...
 ******************************************************/
int32	DialogTwoButton(char *title, char *btn1Text, char *btn2Text, char *text, Item screenItem, Boolean *firstButton) {
	int32	res, tagnum = 0;
	...
	// set up tag args for the call to access
	targs[tagnum].ta_Tag = ACCTAG_REQUEST;
	targs[tagnum++].ta_Arg = (void *)ACCREQ_TWOBUTTON;
	targs[tagnum].ta_Tag = ACCTAG_SCREEN;
	targs[tagnum++].ta_Arg = (void *)screenItem;
	targs[tagnum].ta_Tag = ACCTAG_TITLE;
	targs[tagnum++].ta_Arg = (void *)title;
 ...
 ******************************************************/
int32 Answer(int32 dialogStyle, Item screenItem, char *title, char *text,... ) {
	char	*btn1Text;
	char	*btn2Text;
	Boolean	firstButton;
	va_list	ap;
	...
	switch (dialogStyle) {
		...
		case TWO_BUTTON_DIALOG:
			...
			result = DialogTwoButton(title, btn1Text, btn2Text, text, screenItem, &firstButton);
			if (result != 0) return result;
			else if (firstButton) return 0;
			else return 1;
			break;
