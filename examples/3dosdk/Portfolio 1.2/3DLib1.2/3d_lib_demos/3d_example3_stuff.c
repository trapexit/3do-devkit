...
init_worlds ()
{
  ...
  SetModelCCB (&World_cube2, Ccbs[0],
               SHARE_CCB); /* set all faces to the same image */
  SetFaceCCB (&World_cube2, 0, 1, Ccbs[1],
              SHARE_CCB); /* set face 1 to another image */
  ...
  SetObjectLocation (&World_camera1, Convert32_F16 (8), Convert32_F16 (8),
                     -Convert32_F16 (2));
}
...
move_things (void)
{
  ...
  frac16 tempf16;
  static frac16 p3_delta_y
      = (1 << 14); /* delta y for pyramid 3's vertical position */
  ...
  if (p3_delta_y > 0) /* we're going up */
    {
      if (p3y > p3_max_y)
        p3_delta_y = -p3_delta_y; /* start going down */
    }
  else
    {
      if (p3y < 0)
        p3_delta_y = -p3_delta_y; /* start going up */
    }
