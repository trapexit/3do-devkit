/*
	...
	Change History (most recent first):
		 ...
		 <1>	 7/8/93		CDE		Derived from TaskDemon.c
...
/***** Joystick control *****/
#define		JOYALL  (JOYSTART | JOYUP | JOYDOWN | JOYLEFT | JOYRIGHT | JOYFIREA | JOYFIREB | JOYFIREC)
...
/* Straight from Kernighan & Ritchie (typed by Mike); Added explicit cast of j in for loop */
void reverse (char s[])
{
	int c,i,j;
...
/* HINTS:
	...
	¥ Incorporate your tool name, your name, additional options, all of the category's call names
*/
{
		kprintf ( "\n#################################################################\n");
...
/* Show the option settings for this session */
{
		...
		if (gUseLogFile) kprintf( "-out: Use log file: %s.\n",gLogFile); else kprintf("Do not use a log file.\n");
		
		kprintf ( "#################################################################\n");
}

bool	GetParams(int theArgCount, char** theArgList)
...
/* other parameters */
	...
	while ( !done )
	{
		paramLen = strlen( theArgList[paramIndx] );	// get the length of the current param
...
void InspectNode( char *aName, NodeP aNode )
{
#ifdef FOR_MAC
	pipeData("%s:\n", aName);
	pipeData("{\n");
...
void InspectItemNode( char *aName, ItemNodeP aItemNode )
{
	char aString[64];
	char a2ndString[64];
	...
	sprintf(aString,"%s: V%i.%i",aName,aItemNode->n_Version,aItemNode->n_Revision);
...
void HandleItem( uint8 ftype, uint8 ntype, char *aName )
{
	Item		aItem;
