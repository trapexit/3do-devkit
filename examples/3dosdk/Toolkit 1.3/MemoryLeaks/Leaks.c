/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <3>	 10/12/93	ec		Added ptr size to
   Leaks_Dump.  Added function Leaks_MemBlockSize, was returning incorrect size
   because it included leaks header fields.
                 <3>	 10/7/93	ec		a few more comments,
   removed compiler warnings.
                 <2>	 9/29/93	EC		couple of little syntax
   problems <1>	 9/28/93	EC		first checked in 9/27/93
   ec		first writing
 ...
 *table. Use can 'call' Leaks_Dump and end up here via the ÒLeaks_DumpÓ macro
 */
void
      ...
      while (temp != NULL)
        {
          ...
          printf ("\t0x%08lX\t\t%p\t\t%04ld\t\t%ld\t%s\n", temp->memSignature,
                  temp->address, ptrSize, temp->lineNum, temp->fileName);
          temp = temp->link;
        }
    }
}
...
/*
 * Leaks_Malloc
 *	Allocate a block, add it to our tracking list if we're currently
 *tracking.
 */
void *
Leaks_Malloc (uint32 bytesNeeded, uint32 memType, int32 lineNum,
              char *fileName)
{
  MemHeader *memBlock;
  void *address;
  ...
  if ((memBlock = (MemHeader *)InternalNewPtr (
           bytesNeeded + sizeof (MemHeader), memType))
      == NULL)
    {
...
/*
 * Leaks_FindMemHeader
 *	Search through our linked list of memory blocks for the specified
 *block.
 */
static MemHeader *
Leaks_FindMemHeader (void *address, MemHeader **prevHeader)
{
  ...
  for (previous = NULL, current = gMemList; current;
  ...
  if ((memBlock = Leaks_FindMemHeader (address, &prevHeader)) == NULL)
    {
      ...
      if (gTrackAllocs != 0)
        {
          ERR (("Attempt to get size of un-accounted for memory from %s, line "
                "%ld\n",
                fileName, lineNum));
    ...
    {
      // return the size of the block, minus our header as that is the size
      // they
