/*
	...
	Change History (most recent first):
		 ...
		 <4>	 9/28/93	ec		Added prefix 'Internal' to names of NewPtr/FreePtr/Malloc/Free
		 							 routines to integrate the leaks code.  NO CODE CHANGES NEED
									 RESULT FROM THESE CHANGES - macros now changes old name to
									 'Internal...' names when compile-time flag CHECK_FOR_MEMORY_LEAKS
									 is 0.  If the flag is true, macros change names to redirect through 
									 routines in Leaks.c which allocate some additional memory which 
									 make it easier to track down memory leaks.
		 <3>	  9/6/93	JAY		fix the fix. The masking acutally makes checking for
									gPageSizeTable being NULL. What is important was making sure the
									cur table pointer is not nil.
		 <2>	 8/25/93	JAY		fix a bug in VRAMTableSearch. The routine needs to consider the
									case of gPageSizeTable being NULL.
		 <1>	 8/20/93	JAY		first checked in

  		 <9>	 7/27/93	ian 	Added special handling for page-aligned VRAM.
  									Added simple detection for writing beyond the
  									boundaries of an allocated block.
 ...
 *	this routine is also known as 'InternalNewPtr', via a macro in umemory.h.
 *----------------------------------------------------------------------------*/
 ...
 *	this routine is also known as 'InternalFreePtr' via a macro in umemory.h.
 *----------------------------------------------------------------------------*/
 ...
 *	this routine is also known as 'InternalGetPtrSize' via a macro in umemory.h.
 *----------------------------------------------------------------------------*/
...
void * InternalNewPtr(uint32 size, uint32 memtype)
{
	return InternalMalloc(size, memtype);
}

void *InternalFreePtr(void *blk)
{
