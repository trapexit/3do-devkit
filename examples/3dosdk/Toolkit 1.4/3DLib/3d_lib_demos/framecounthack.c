...
/* Timer initialization */
   timerDevice = OpenItem(FindNamedItem(MKNODEID(KERNELNODE,DEVICENODE), timername),0);
...
collecttimedata(Item bitmap,int32 display)
{
   ...
   struct timeval tv;
     ...
     if(display)
     {
		...
		sprintf(buff, "fps %4s",  Frac16str(fps,buff2,25L));
    ...
    {
//printf("bb framecount2=%ld, gtime=%ld, prevtime=%ld\n",framecount2,Gtime,prevTime);
         PutNumXY(bitmap,20, 240-50, buff); 
    }
}
...
/*
	sprintf(buff, "frame : %d  deltime : %d fps %d", framecount, Gtime-prevTime, 
		framecount/(Gtime-prevTime));
*/
