...
#define DEFAULT_DISPCTRL                                                      \
  (VDL_DISPCTRL | VDL_HINTEN | VDL_VINTEN | VDL_BLSB_BLUE | VDL_HSUB_ZERO     \
   | VDL_VSUB_ZERO | VDL_WINBLSB_BLUE | VDL_WINHSUB_ZERO | VDL_WINVSUB_ZERO)
 ...
 */
#define MakeCCBRelative(field, linkobject)                                    \
  ((int32)(linkobject) - (int32)(field)-4)

#define MakeRGB15(r, g, b)                                                    \
  (((uint32)(r) << 10) | ((uint32)(g) << 5) | (uint32)(b))
#define MakeRGB15Pair(r, g, b) (MakeRGB15 (r, g, b) * 0x00010001)

#define MakeCLUTColorEntry(index, r, g, b)                                    \
  ((((uint32)(index) << 24) | VDL_FULLRGB | ((uint32)(r) << 16)               \
    | ((uint32)(g) << 8) | ((uint32)(b))))
#define MakeCLUTRedEntry(index, r)                                            \
  ((((uint32)(index) << 24) | VDL_REDONLY | ((uint32)(r) << 16)))
#define MakeCLUTGreenEntry(index, g)                                          \
  ((((uint32)(index) << 24) | VDL_GREENONLY | ((uint32)(g) << 8)))
#define MakeCLUTBlueEntry(index, b)                                           \
  ((((uint32)(index) << 24) | VDL_BLUEONLY | ((uint32)(b) << 0)))
#define MakeCLUTBackgroundEntry(r, g, b)                                      \
  ((VDL_DISPCTRL | VDL_BACKGROUND | ((uint32)(r) << 16) | ((uint32)(g) << 8)  \
    | ((uint32)(b))))
...
#define DI_END 0
/* uint32 - numeric type of display for passing into Screen and VDL creation
 * calls */
#define DI_TYPE 1
...
/* Additional tag values above DI_PRIVATE may appear but are reserved for
 * system use */
...
typedef struct VDL
{
  ...
  int32
      vdl_DisplayType; /* The type of display that this VDL was created for */
...
#define GRAFERR_BADBITMAPSPEC                                                 \
  MAKEGERR (ER_SEVERE, ER_C_NSTND, GRAFERR_BASE + 16)
#define GRAFERR_INTERNALERROR                                                 \
  MAKEGERR (ER_SEVERE, ER_C_NSTND, GRAFERR_BASE + 17)
...
#define GRAFERR_NOWRITEACCESS                                                 \
  MAKEGERR (ER_SEVERE, ER_C_NSTND, GRAFERR_BASE + 21)
#define GRAFERR_BADDISPLAYTYPE                                                \
  MAKEGERR (ER_SEVERE, ER_C_NSTND, GRAFERR_BASE + 22)
...
#define DeleteVDL(x) DeleteItem (x)
  ...
  Err SetVRAMPages (Item ioreq, void *dest, int32 val, int32 numpages,
                    int32 mask);
  Err CopyVRAMPages (Item ioreq, void *dest, void *src, uint32 numpages,
                     uint32 mask);
  Err CloneVRAMPages (Item ioreq, void *dest, void *src, uint32 numpages,
                      uint32 mask);
  Err SetVRAMPagesDefer (Item ioreq, void *dest, int32 val, int32 numpages,
                         int32 mask);
  Err CopyVRAMPagesDefer (Item ioreq, void *dest, void *src, uint32 numpages,
                          uint32 mask);
  Err CloneVRAMPagesDefer (Item ioreq, void *dest, void *src, uint32 numpages,
                           uint32 mask);
