...
Boolean fixedFlag, halfBlueFlag, BGColorFlag, YCCFlag, z24BitFlag, sortBypassFlag, totalFlag;
...
char *argv[];
{
  ...
  if (  (instring[0] != 'P') || (instring[1] != '6')  ) {
    fprintf (stderr, "Error - file does not seem to be in ppm binary format\n");
    exit (1);
  }
  ...
  if ( rgbmax != 255  && rgbmax != 127 ) {    /* some ppm's compress color space for color reduction */
    fprintf (stderr, "Error - bad rgbmax value (%d)\n", rgbmax);
    exit (1);
  } else if (totalFlag) fprintf (stderr, "rgbmax = %d\n", rgbmax);
  
  position = ftell(fin);
  
  if (fixedFlag) {
    if (totalFlag) fprintf (stderr, "building color table\n");
    if (!sortBypassFlag) {
      initpops();
      for (i=0; i<height; i++) {
	doline (i, 1);
  ...
  if (opffFlag) {
    ...
    imgChunk.pixelorder = (literal16Flag) ? 0:1;	/* 1 means sherrie LRForm */
    imgChunk.pixelorder = (z24BitFlag) ? 3:imgChunk.pixelorder;	/* 3 means 24 bit mode */
    imgChunk.version = 0;
  ...
  for (i=0; i<outheight; i++) {
	...
	for (j=0; j<outwidth; j++) {
	  if ((putc (outarray[j*4], fout) < 0) || (putc (outarray[j*4+1], fout) < 0)) {
	    fprintf (stderr, "Error writing to output file\n");
	    exit (1);
	  }
	}
	for (j=0; j<outwidth; j++) {
	  if ((putc (outarray[j*4+2], fout) < 0) || (putc (outarray[j*4+3], fout) < 0)) {
	    fprintf (stderr, "Error writing to output file\n");
	    exit (1);
	  }
	}
  ...
  if (opffFlag & !sortBypassFlag) {
    ...
    pCLUTs[2]	= (fixedFlag) ? 1:height;	/* one clut for each scanline unless fixed (-f) */
  ...
  if (totalFlag) {
    if (totalcolors<HASHLIMIT) {
      ...
      fprintf (stderr, "total number of colors in image exceeds limit: %ld\n", (long)HASHLIMIT);
    }
  }
  exit (0);
}
...
char *option;
{
  ...
  if ((*option != '-') && (*option != '+')) {
	...
	fprintf (stderr, "Error - could not open clut file %s for output\n", option);
	exit (1);
      }
    return;
    }
	...
	fprintf (stderr, "Error - could not open bitmap file %s for output\n", option);
	exit (1);
      }
	...
	fprintf (stderr, "Illegal number of colors or illegal minimum color\n");
	exit (1);
      }
      return;
    case 'e':
...
usage ()
{
  ...
  fprintf (stderr, "\t-d\tuse dynamic color lookup table (reload CLUT on each display line) \n");
  fprintf (stderr, "\t-b\tuse half the blue entries (assume least bit is h-position)\n");
  fprintf (stderr, "\t-v\tprint total of number of unique colors generated in output image\n");
  fprintf (stderr, "\t-c\toutput a 16-bit, non-LR-form image for processing by makecel\n");
  fprintf (stderr, "\t-z\toutput a 24-bit image, stored as 32 bits/pixel for static display with custom CLUT\n");
  fprintf (stderr, "\t-m<n>\tuse at most <n> entries in color table (default: 32)\n");
  fprintf (stderr, "\t-e<n>\tstart using color tables entries at index <n> (default: 0)\n");
  fprintf (stderr, "\t-t<n>\tspecify value (hex 555-RGB) of transparent color in input\n");
  fprintf (stderr, "\t-s0\tuse evenly distributed color space (also sets fixed CLUT)\n");
  fprintf (stderr, "\t-s1\tuse most popular values for CLUT selections\n");
  fprintf (stderr, "\t-s2\tdivide color space into (approximately) equal population regions\n");
  fprintf (stderr, "\t-s3\tdivide color axes into tightest fitting boxes (split longest spans)\n");
  fprintf (stderr, "\t-s4\tsame as s3 except split spans with largest populations\n");
  fprintf (stderr, "\t-s5\tsame as s3 except split largest population*span length\n");
  //fprintf (stderr, "\"-c\" option sets \"-s0\" option, so specify other \"-s\" option(s) after \"-c\"\n");
  //fprintf (stderr, "to override.  The \"-s0\" option assumes the \"-f\" (fixed color table) option, so\n");
...
int line, pass;
{
  ...
  for (i=0; i<width; i++) {
    ...
    if ((!fixedFlag) || pass) {
      if ((popy[y]++)==0) ycount++;
      if ((popcr[cr]++)==0) crcount++;
      if ((popcb[cb]++)==0) cbcount++;
    }
  }
  
  if (!fixedFlag) sortcolors ();
...
*/
      ...
      if (BGColorFlag &&
	  ycc_array[i][0]==backred &&
	  ycc_array[i][1]==backgreen &&
	  ycc_array[i][2]==backblue) {
	m = 0;
      }
  ...
  if (!fixedFlag) writeclutline (line);
}
...
sortcolors ()
{
  ...
  maxcount = cbcount;
  if (halfBlueFlag) maxcount = cbcount*2 - 1 + (mincolor&1);
  if (ycount > maxcount) maxcount = ycount;
  if (crcount > maxcount) maxcount = crcount;
  /* print a warning if more than 32 different Y, Cr or Cb values needed */
  if (maxcolorflag==1) {
    fprintf (stderr, "%d colors exceeded on line(s) in image\n", maxcolors);
    maxcolorflag = 3;
  }
  if (maxcount > maxcolors) maxcount = maxcolors;
...
int line;
{
  ...
  if ((!fout2) && (!opffFlag)) return;	/* return if we're not going to write a clut file */
  
  mflag = 0;				/* if using color 0 need to use color 32 as well */
  if (mincolor==0) mflag=1;
  filler = 0;
  if ((maxcount+mflag)&1) filler = 1;
...
//   Tests to create VDL like those produced by PhotoShop PlugIns
  ...
  for (i=0; i<maxcount; i++) {
    j = ((i+mincolor)<<24) + (yindex[i]<<16) + (crindex[i]<<8) + (cbindex[i]);
    if (halfBlueFlag) {
      j = ((i+mincolor)<<24) + (yindex[i]<<16) + (crindex[i]<<8) + (cbindex[i>>1]);
    }
...
void
evenspacing (pop, index, maxcount)
long *pop, *index, *maxcount;
{
  int i;
...
void
boxsort (pop, index, maxcount)
long *pop, *index, *maxcount;
{
  int i, j;
  struct {int top; int bot; int weight} box[MAXCOLORS];
  int next;
  
  next = 1;
  box[0].top = 0;
  box[0].bot = 255;
  while (!pop[box[0].top]) box[0].top++;
  while (!pop[box[0].bot]) box[0].bot--;
  ...
  while (next<*maxcount) {
    ...
    box[j].bot = i-1;
    while (!pop[box[next].top]) box[next].top++;
    while (!pop[box[j].bot]) box[j].bot--;
...
void
boxpopsort (pop, index, maxcount)
long *pop, *index, *maxcount;
{
  int i, j;
  struct {int top; int bot; long weight} box[MAXCOLORS];
  ...
  box[0].bot = 255;
  box[0].weight = width;
  if (fixedFlag) box[0].weight *= height;
  while (!pop[box[0].top]) box[0].top++;
  while (!pop[box[0].bot]) box[0].bot--;
  while (next<*maxcount) {
    j = 0;
    ...
    for (i=box[j].top; i<box[j].bot; i++) {
      if (popup+pop[i] >= popdown-pop[i]) break;
      popup += pop[i];
      popdown -= pop[i];
    }
...
void
boxpoplengthsort (pop, index, maxcount)
long *pop, *index, *maxcount;
{
  int i, j;
  struct {int top; int bot; long weight} box[MAXCOLORS];
  ...
  while (next<*maxcount) {
    ...
    for (i=1; i<next; i++) {
      /* if (box[i].weight > box[j].weight) j=i; */
      if (box[i].weight*(box[i].bot-box[i].top) > box[j].weight*(box[j].bot-box[j].top)) {
	j=i;
      }
    }
    if (!box[j].weight) break;
...
*/
    index[i] = (box[i].top+box[i].bot)>>1;
    /* printf ("%d : %d %d %d %d\n", i, box[i].top, box[i].bot, box[i].weight, j); */
  }
}
...
FILE *stream;
{
  ...
  for (i=0; i<count-1;) {
    ...
    if (j<0) break;
    string[i++] = j;
    if (j == ASCII_CR || j== ASCII_LF || j== ASCII_SPACE) break;
  }
...
char *s;
{
  if (*s=='$') return strtol(s+1, NULL, 16);
...
initCLUT ()
{
  ...
  pCLUTs = malloc(MAXCLUTS*(sizeof(VDL_REC)+32)+12);	/* + 12 for chunk ID and size */
#endif
}
...
uchar r, g, b;
{
  ulong i, j;
  long k;
  if (totalcolors >= HASHLIMIT) return;

  i = (((ulong)r*(ulong)33329+(ulong)g)*(ulong)33349+(ulong)b) % (ulong)HASHLIMIT;
  j = 1; k = 1;
