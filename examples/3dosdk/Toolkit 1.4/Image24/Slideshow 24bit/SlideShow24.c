/*
	...
	Change History (most recent first):
		 ...
		 <7>	2/3/94		DSM		Updated for 24 bit display modes
...
Item			systemVDL[kScreenCount];		// Items for System's default VDL
Item			simpleVDL[kScreenCount];						// Items for simple VDL's (with CLUT Bypass)
uint32 *		simpleVDLBuffer;
...
int32 			PALmode = 0;
int32			myScreenWidth = kScreenWidthNTSC;		// Screen Width in current RUNTINE environment
int32			myScreenHeight = kScreenHeightNTSC;		// Screen Height in current RUNTINE environment
...
/*	Use these for CreateItem replacement for Submit VDL */
TagArg VDLTags[] = 
	{
	CREATEVDL_TAG_VDLTYPE,		(void*) VDLTYPE_FULL,
...
DONE: 
	...
	// we don't REALLY want to fade to black, but we need the pause so RJ's menu program doesn't
...
ParseScriptFile(char *scriptFileName)
{
	...
	printf("Found %s, %d bytes long (%x)...\n", scriptFileName, fileSize, &fileSize);
			...
			*scanChar = '\0';
			...
			else if ( fileName[0] == '-' )
			{
				if ( (fileName[1] == 'a') || (fileName[1] == 'A') )
				{
					...
					if (gImageVisTime == 0)  gImageVisTime = 180;
...
ParseArgs(int32 argc, char **argv)
{
	...
	if (argc < 2)
	{
		if ( ParseScriptFile(PALmode? "imagelistPAL": "imagelist") != noErr )
			return FALSE;
		haveFileName = TRUE;
	}
		...
		else
		{
			switch (cmdLineParam[1]) 
			{
				...
				case 'a':
					...
					if ( gImageVisTime== 0)  gImageVisTime = 180;
					printf("Auto running mode, each image is shown for %d seconds.\n", gImageVisTime/60);
					break;
...
void printVDL_ENTRY( VDLEntry* vdle)
	{
...
LoadScreen(char *fileName, int32 whichBuffer)
{
	...
	if ( (loadfile24(imageFileName, gBackPic[whichBuffer], maxImageSize,
	            MEMTYPE_ANY,  &rawVDLPtr,&picHeader[whichBuffer], myScreenWidth, myScreenHeight)) < 0 ) 
	{
		printf ("LoadImage failed\n");
		return FALSE;
	}
	bpp = picHeader[whichBuffer].bitsperpixel;
    if (bpp <= 16) picSize[whichBuffer] = minImageSize;
	else picSize[whichBuffer] = maxImageSize;
	...
	if ( (rawVDLPtr != NULL) && (rawVDLPtr->chunk_size) ) 
	{
		...
		VDLTags[0].ta_Arg = (void*) VDLTYPE_FULL;
		VDLTags[1].ta_Arg = PALmode ? (void*) DI_TYPE_PAL2 : (void*) DI_TYPE_NTSC;
		VDLTags[2].ta_Arg = (void*) (vdllines * sizeof(VDL_REC) / 4);
		VDLTags[3].ta_Arg = (void*) myScreenHeight;
		VDLTags[4].ta_Arg = (void*) gVDLPtrArray;
		
		gVDLItems[whichBuffer] = CreateVDL(VDLTags);
	
//		gVDLItems[whichBuffer] = SubmitVDL(gVDLPtrArray, ((vdllines * sizeof(VDL_REC)) / 4), VDLTYPE_FULL);
		if ( gVDLItems[whichBuffer] < 0 ) 
		{
			printf( "Error: CreateVDL() failed with error code %d\n", 
		...
		{
		// gVDLItems[whichBuffer] = systemVDL[whichBuffer];
		gVDLItems[whichBuffer] = simpleVDL[whichBuffer];
		}
	
	oldVDL = SetVDL(gScreenCntx.sc_Screens[whichBuffer], gVDLItems[whichBuffer]);
	if ( oldVDL < 0 ) 
	{
	...
	// unless it is one of the four global vdls we want to keep around
	oldVDLptr = (VDL *)LookupItem(oldVDL);
	if (    (oldVDL != systemVDL[0]) && (oldVDL != systemVDL[1])
	     && (oldVDL != simpleVDL[0]) && (oldVDL != simpleVDL[1])) DeleteVDL(oldVDL);
...
DoJoystick( void )
{
	...
	if ( joyBits & ControlRightShift ) 
	{
		if (VAVG[curbuf])
			...
			else printf(" Failed to Disable Vertical Averaging\n");
			}
		else
			{
			VAVG[curbuf] = TRUE;
			...
			else printf(" Failed to Enable Vertical Averaging\n");
			}
		goto DONE;
	}
	...
	if ( joyBits & ControlLeftShift ) 
	{
		if (HAVG[curbuf])
			...
			else printf(" Failed to Disable Horizontal Averaging\n");
			}
		else
			{
			HAVG[curbuf] = TRUE;
			...
			else printf(" Failed to Enable Horizontal Averaging\n");
			}
		goto DONE;
	}
...
ShowPrevImage(void)
{
	...
	if ( !gPrevBuffGood )
		{
		while (  LoadScreen(gFileNames[gCurrentImage], curbuf ) == FALSE)
			{
			gCurrentImage--;
			if ( gCurrentImage < 0 )
			gCurrentImage = gImageCount - 1;
...
uint32	bmWidths[kScreenCount]; 
uint32	bmHeights[kScreenCount]; 
ubyte	*bitMapBuffs[kScreenCount];
TagArg screenTags24[] = 
{
	CSG_TAG_DISPLAYHEIGHT,			(void*)kScreenHeight,
	CSG_TAG_SCREENCOUNT,			(void*)kScreenCount,
	CSG_TAG_SCREENHEIGHT,			(void*)kScreenHeight,
	CSG_TAG_BITMAPCOUNT,			(void*)1,
	CSG_TAG_BITMAPWIDTH_ARRAY,		(void*)bmWidths,
	CSG_TAG_BITMAPHEIGHT_ARRAY,		(void*)bmHeights,
	CSG_TAG_BITMAPBUF_ARRAY,		(void*)bitMapBuffs,
...
CreateScreens(void) 
{
	...
	// Determine if we are in PAL or NTSC LAND
	Err = QueryGraphics(QUERYGRAF_TAG_FIELDFREQ, &FieldFreq);
	if (Err < 0 )
		{
		PrintfSysErr(Err);
		return Err;
		}
	if (FieldFreq == NTSCFieldFreq)
		{
		PALmode = 0;
		myScreenWidth = kScreenWidthNTSC;
		myScreenHeight = kScreenHeightNTSC;
		printf(" NTSC mode detected\n");
		}
	
	if (FieldFreq == PALFieldFreq)
		{
		PALmode = 1;
		myScreenWidth = kScreenWidthPAL;
		myScreenHeight = kScreenHeightPAL;
		printf(" PAL mode detected \n");
		}
	for ( screenNdx = 0; screenNdx < kScreenCount; screenNdx++ ) 
		{
		bmWidths[screenNdx] = myScreenWidth; 
		bmHeights[screenNdx] = myScreenHeight;
		}
	screenTags24[0].ta_Arg = (void*)myScreenHeight;
	screenTags24[2].ta_Arg = (void*)myScreenHeight;
	
	
	//	Allocate VDL memory: 2 lines of 40 longs each
	//	One 40 line VDL is for the odd-field display,
	//	And one is for the even.  They are full color tables
	//	Because the hardware reloads the CLUTs with default ramps
	//	After each field.  For 24bit, this will cause  bad display.
	vdl1 = (int32 *)ALLOCMEM ((2 * 40 * sizeof(int32)), MEMTYPE_ANY);
	vdl2 = vdl1 + (40 * sizeof(int32));
	if ( (!vdl1) || (!vdl2) ) 
	{
	...
	gScreenCntx.sc_nFrameBufferPages = (myScreenWidth * myScreenHeight * 2
						+ VRAMPageSize - 1) / VRAMPageSize;
	gScreenCntx.sc_nScreens = 2;
	gScreenCntx.sc_nFrameByteCount = gScreenCntx.sc_nFrameBufferPages 
										* VRAMPageSize;
	gScreenCntx.sc_curScreen = 0;

	//	Allocate memory for the screen bitmap
	buffPtr = (ubyte *)ALLOCMEM((int)(2 * gScreenCntx.sc_nFrameBufferPages * VRAMPageSize),
	...
	bitMapBuffs[1] = buffPtr + (gScreenCntx.sc_nFrameBufferPages * VRAMPageSize);
	screenTags24[7].ta_Arg = (void*)GETBANKBITS(GrafBase->gf_ZeroPage);
	screenGroupItem = CreateScreenGroup( &(gScreenCntx.sc_Screens[0]), screenTags24 );
	...
	for ( screenNdx = 0; screenNdx < kScreenCount; screenNdx++ ) 
	{
		...
		gScreenCntx.sc_BitmapItems[screenNdx] = screen->scr_TempBitmap->bm.n_Item;
	...
	for ( screenNdx = 0; screenNdx < kScreenCount; screenNdx++ ) 
	{
		...
		gVDLItems[screenNdx] = SubmitVDL(gVDLPtrArray, (kVDLSize / 4), VDLTYPE_FULL);
		...
		// got the 'real' vdl, now make it current
		 errorCode = SetVDL(gScreenCntx.sc_Screens[screenNdx], gVDLItems[screenNdx]);
		if ( errorCode < 0 ) 
			{
		 	printf( "Error: SetVDL() failed with error code %d\n", errorCode );
	...
	screenGroupItem = CreateScreenGroup(&(gScreenCntx.sc_Screens[2]), screenTags24);
	if ( screenGroupItem < 0 ) 
	{
	...
	// So...  after all of that we have a single, double height screen.  now call out
	//  and get a simple vdl for this screen.  we want to point each to the start of
	//  the single large bitmap we allocated
	InitVDL480(vdl1, gScreenCntx.sc_Bitmaps[2], 0, PALmode);
	InitVDL480(vdl2, gScreenCntx.sc_Bitmaps[3], 1, PALmode);
	...
	VDLTags[0].ta_Arg = (void*) VDLTYPE_FULL;
	VDLTags[1].ta_Arg = PALmode ? (void*) DI_TYPE_PAL2 : (void*) DI_TYPE_NTSC;
	VDLTags[2].ta_Arg = (void*) 40;
	VDLTags[3].ta_Arg = (void*) myScreenHeight;
	VDLTags[4].ta_Arg = (void*) vdl1;
	
	gVDLItems[2] = CreateVDL(VDLTags);
//	gVDLItems[2] = SubmitVDL((VDLEntry *)vdl1, 30 * 4, VDLTYPE_FULL);
	
	
	if ( gVDLItems[2] < 0 ) 
	{
		printf("Error: CreateVDL  (%lx)\n", gVDLItems[2]);
	...
	VDLTags[2].ta_Arg = (void*) 40;
	VDLTags[3].ta_Arg = (void*) myScreenHeight;
	VDLTags[4].ta_Arg = (void*) vdl2;
	
	gVDLItems[3] = CreateVDL(VDLTags);	
//	gVDLItems[3] = SubmitVDL((VDLEntry *)vdl2, 30 * 4, VDLTYPE_FULL);
	...
	FREEMEM(vdl1, 2 * 40 * sizeof(uint32));
	return noErr;
}
...
{
	int screen;
	simpleVDLBuffer = 	(uint32 *)ALLOCMEM (sizeof(SVDL), MEMTYPE_ANY);
	if ( !simpleVDLBuffer ) 
	{
		printf("Error - unable to allocate VDL memory: CreateSimpleVDL\n");
		return -1;
	}
  
	for (screen = 0 ; screen < kScreenCount; screen++)
		{
		InitSimpleVDL((SVDL *)simpleVDLBuffer, gScreenCntx.sc_Bitmaps[screen],PALmode);
		VDLTags[0].ta_Arg = (void*) VDLTYPE_SIMPLE;
		VDLTags[1].ta_Arg = PALmode ? (void*) DI_TYPE_PAL2 : (void*) DI_TYPE_NTSC;
		VDLTags[2].ta_Arg = (void*) (sizeof(SVDL) / 4);
		VDLTags[3].ta_Arg = (void*) myScreenHeight;
		VDLTags[4].ta_Arg = (void*) simpleVDLBuffer;
	
		simpleVDL[screen] = CreateVDL(VDLTags);
//		simpleVDL[screen] = SubmitVDL((VDLEntry *)simpleVDLBuffer, ((sizeof(SVDL)) / 4), VDLTYPE_SIMPLE);
		if ( simpleVDL[screen] < 0 ) 
			{
			printf("Error in CreateVDL  (%lx)\n", simpleVDL[screen]);
			PrintfSysErr(simpleVDL[screen]);
			return -1;
			}
	}
	// We can free the memory. The system has copied it and hangs it off the VDL Item
	FREEMEM(simpleVDLBuffer, sizeof(SVDL));
	return noErr;
}
...
Init( void )
{
	...
	for ( screenNdx = 0; screenNdx < kBufferCount; screenNdx++ )
	{
		...
		*/
		...
		gBackPic[screenNdx] = (ubyte *)ALLOCMEM(maxImageSize, 
									MEMTYPE_DRAM );
		...
		memset (gBackPic[screenNdx], 0,2*(int32)(gScreenCntx.sc_nFrameByteCount) );
	}
...
void ToggleBit15(void)
	...
	uint32 n;
	n = picSize[curbuf] >> 2;
	if (picSize[curbuf] == minImageSize) p = (uint32 *) gScreenCntx.sc_Bitmaps[curbuf]->bm_Buffer;
	else p = (uint32 *) gScreenCntx.sc_Bitmaps[2]->bm_Buffer;;
	while (n--)
		*p++ ^=  0x80008000; 
	}
...
DisplayCurrentScreen( void )
{
	...
	if (prevbuf == curbuf) return TRUE;  // already showing this buffer
	if ( picSize[curbuf] == minImageSize )
	{
		
		memcpy (gScreenCntx.sc_Bitmaps[curbuf]->bm_Buffer,
		        gBackPic[curbuf],picSize[curbuf] );
		/*
	...
	else
	{
		...
		memcpy (gScreenCntx.sc_Bitmaps[2]->bm_Buffer,
		        gBackPic[curbuf],picSize[curbuf] );
...
usage (void)
{
	...
	printf (("Where <scriptFileName> is a text file with the names of the images to show\n"));
	printf (("the image files to load from the Mac.  The text file \"ImageList\" is \n"));
