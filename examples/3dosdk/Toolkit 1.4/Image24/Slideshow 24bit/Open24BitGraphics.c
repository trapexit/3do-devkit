 ...
 *
 *	Written by:		Stephen Landrum, George Mitsuoka, Neil Day, Steve Hayes and friends.
 *
 *	Copyright © 1994 The 3DO Company. All Rights Reserved.
 *
 *	History:
 *	4/29/94			jhm		Commented out #define CEL_OVER_VIDEO, so it won't come on
 *							even #if DEBUG.
 *  3/2/94			jhm		Added "#define NFW 0" so the compiler won't generate
 *							warnings while regenerating make file dependencies.
 *	2/24/94			bck		Fixed text overlay for 24 bit display.
 *	2/24/94			seh		Add text overlay.
 *	2/20/94			seh		Double buffering.
 *	2/14/94			nmd		Added 24 bit frame buffer support for single buffer
...
#define catch_nil(v,x)	if( !v ) { prt(x); goto cleanup; }
#define catch_neg(v,x)	if(v<0) { prt(x);PrintfSysErr((Item)v); goto cleanup; }
#define	dbg(x)			prt(x)
...
//			  CSG_TAG_VDLPTR_ARRAY,	(void*)0,
			  ...
			  CSG_TAG_BITMAPWIDTH_ARRAY,	(void*)bmw,
			  CSG_TAG_BITMAPHEIGHT_ARRAY,	(void*)bmh,
			  CSG_TAG_BITMAPBUF_ARRAY,	(void*)bmbufs,
			  CSG_TAG_DONE,		(void*)0,
			};
...
int32 Open24BitGraphics (ScreenContext * sc, int nScreens)
{
	...
	for (v=0; v<NVDLS; v++) {
		vdl[v] = (int32 *)AllocMem(6*40*4, MEMTYPE_DMA | MEMTYPE_VRAM | MEMTYPE_STARTPAGE);
			catch_nil(vdl[v],("failed to allocate vdl %d\n",v));
		vdlarray[v*2] = vdl[v];
		vdlarray[(v*2)+1] = vdl[v] + 120;
	}
		
	sc->sc_nFrameBufferPages = (bmw[0]*bmh[0]*4+GrafBase->gf_VRAMPageSize-1) / GrafBase->gf_VRAMPageSize;
	
	for (v=0; v<NVDLS; v++ ) {
		bmbuffer[v] = (ubyte *)ALLOCMEM((int)(sc->sc_nFrameBufferPages*GrafBase->gf_VRAMPageSize),
...
//	taScreenTags[1].ta_Arg = (void *)vdlarray;
	...
	SetVRAMPages (vramioreq, sc->sc_Bitmaps[0]->bm_Buffer, 0, sc->sc_nFrameBufferPages, ~0L);
	SetVRAMPages (vramioreq, sc->sc_Bitmaps[2]->bm_Buffer, 0, sc->sc_nFrameBufferPages, ~0L);
	...
	for (v=0; v<NVDLS; v++ ) {
		prt(("VideoBuffer[%d] $%x\n",v, (uint32)sc->sc_Bitmaps[v*2]->bm_Buffer));
	}
...
void initVDL (int32 *vdl, int32 bitmapindex)
{
	...
	vdl[0] = VDL_ENVIDDMA|VDL_LDCUR|VDL_LDPREV|(34<<VDL_LEN_SHIFT)|VDL_PREVSEL
			|(240<<VDL_LINE_SHIFT)|VDL_DISPMOD_320|VDL_480RES;
	...
	vdl[4+33] = VDL_DISPCTRL|VDL_VINTEN|VDL_HINTEN|VDL_HSUB_ZERO|VDL_BLSB_BLUE
	...
	vdl[40] = (34<<VDL_LEN_SHIFT)|(0<<VDL_LINE_SHIFT)|VDL_DISPMOD_320|VDL_480RES;
	...
	vdl[44+33] = VDL_DISPCTRL|VDL_VINTEN|VDL_HINTEN|VDL_HSUB_ZERO|VDL_BLSB_BLUE
				|(int32)((bitmapindex&1)?VDL_VSUB_ONE:VDL_VSUB_ZERO);
	...
	vdl[84+33] = VDL_DISPCTRL|VDL_VINTEN|VDL_HINTEN|VDL_HSUB_ZERO|VDL_BLSB_BLUE
				|(int32)((bitmapindex&1)?VDL_VSUB_ONE:VDL_VSUB_ZERO);
...
/*************************************************************************************
	GLGetBitmapItem
*************************************************************************************/
Item GLGetBitmapItem(char* buffer)
{
	int32 v;
...
*************************************************************************************/
void GLDisplayDoubleBuffer( int32 index )
{
	int32	ret;
	
	curindex = index;
	if (index < NBUFFERS)	{
		ret=DisplayScreen (sc->sc_Screens[index*2], sc->sc_Screens[index*2+1] );
...
void GLDisplayText( ScreenContext *sc, char *s, long num, int y )
{
	char outstr[256];
	...
	if(ccb == NULL)
	{
		...
		if ((ccb = (CCB *)
        	ALLOCMEM(sizeof(CCB),MEMTYPE_CEL | (MEMTYPE_FILL | 0x00))) == NULL)
        {
			LockedPrintf(("MoviePlayer>CreatePerformer: Cannot allocate CCB memory.\n"));
			return;
        }
        
		/* Allocate a block the size of the celData */
		if ((ccb->ccb_SourcePtr = (CelData *)
        	ALLOCMEM(byteCount,MEMTYPE_CEL | (MEMTYPE_FILL | 0x00))) == NULL)
       {
			LockedPrintf(("MoviePlayer>CreatePerformer: Cannot allocate CCB source memory.\n"));
			return;
		}
		...
		ccb->ccb_Flags = CCB_LAST | CCB_NPABS | CCB_SPABS |
			CCB_LDSIZE | CCB_LDPRS | CCB_LDPPMP | CCB_CCBPRE | /* CCB_BGND | */
			CCB_YOXY | CCB_ACW | CCB_ACCW;
        ccb->ccb_PRE0  = ((height/2-1) << PRE0_VCNT_SHIFT) + /* number of vertical data lines in sprite data -1 */
			PRE0_LINEAR +         /* use PIN for IPN (0x10) */
			PRE0_BPP_16;          /* set bits/pixel to 16 (0x6) see HDWR spec 3.6.3 */

        ccb->ccb_PRE1  = (((width * 4 / 4) - 2) << PRE1_WOFFSET10_SHIFT) +  /* offset (in words) from line start to next line start. */
			(width-1) | PRE1_LRFORM;         /* number of horizontal pixels to render -1 */
	...
	sprintf (outstr, "%s %d          ", s, num);	/* NOTE: the terminating spaces
		 * will clear out the rest of the cel, otherwise we get extra digits on the
		 * display, and the number looks huge! */
  	MoveTo (&grafCon, 0, 0);
...
Item GLGetIndBitMapItem( int32 ix )
{
