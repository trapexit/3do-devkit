...
/* The following compile switch determines whether or not larger I/O buffers
 * are used for stream I/O. This SIGNIFICANTLY improves performance, but is not
 * ANSI compliant because it uses an MPW-specific library call to set the
 * buffer size. It is recommended that this option be left ON.
 */
#define USE_BIGGER_BUFFERS 1

/* The following compile switch determines whether or not the beginning of the
 * stream data will be forced onto a stream block boundary. Setting this switch
 * results in wasting space in the stream file, but may be useful if the
 * application does a considerable amount of stream looping, whereby the header
