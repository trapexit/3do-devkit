 ...
 *into streams Does NOT write blocks, it only writes SSND chunks.
 *
 *	History:
 *	5/12/94		dtc		Version 2.2
 *						Changed the definitions of DS_MSG_HEADER
 *and SUBS_CHUNK_COMMON to require semicolon when used in context.  (For
 *readibility and compilation using ObjectMaster). 10/31/93	rdg
 *Version 2.1 Fix chunkTime to start at 1.  So if you weave multiple audio
 *						streams together which start at the same
 *time, all of the headers will be processed together before any data starts
 *playing.  This was fixed in an earlier version but the change got lost
...
main (int argc, char *argv[])
{
  ...
  FramesPTick = (myCommChunk.sampleRate / AUDIO_TICK_RATE)
                + 1; /* round up to match AudioFolio */

  BytesPTick = ((BytesPFrame * FramesPTick) / CompressionRatio);

  SampleDataSize = (gPropChunkSize
                    - ((gPropChunkSize - SSMP_CHUNK_HDR_SIZE) % BytesPTick));

  TicksPChunk = SampleDataSize / BytesPTick;

  ChunkSize = (SSMP_CHUNK_HDR_SIZE + SampleDataSize + 3)
              & ~3; /* round chunk size up to quad byte */
  ...
  hcp->sampleDesc.sampleCount
      = (SSNDChunkSize / ((myCommChunk.sampleSize / CompressionRatio) / 8));
