...
#define	PROGRAM_VERSION_STRING	"1.1d1"
...
#define CCB_POVER_SHIFT  7
	...
	printf( "%sccb_X:\t\t%-10ld\n", gCCBDetailsIndent, saccb->ccb_X );
	printf( "%sccb_Y:\t\t%-10ld\n", gCCBDetailsIndent, saccb->ccb_Y );
...
static void		DumpSAMAP( long filePosition, StreamAnimMapPtr samap )
	...
	for ( cornerNumber = 0; cornerNumber < 4; cornerNumber++ )
		...
		printf( "%s%s:\t%ld, %ld\n", gCCBDetailsIndent, 
					...
					samap->quad[cornerNumber].x, 
					samap->quad[cornerNumber].y );
		}
	}
...
static void		DumpSAPOS( long filePosition, StreamAnimPosPtr sapos )
	...
	printf( "newXValue:\t%6ld\t", sapos->newXValue );
	printf( "newYValue:\t%6ld", sapos->newYValue );
	}
