 ...
 **	 6/15/93	RDG		New Today (based on code from Jay
 *Moreland)
 **
 **************************************************************************************/
...
int
mfcreate (char *filename, long creator, long fileType, long *fileID)
{
  OSErr err;
  Str255 pFileName;
  short fRefNum;
  ...
  if ((err = Create (pFileName, vRefNum, (OSType)creator, (OSType)fileType))
      != noErr)
    return err;
...
int
mfopen (char *filename, long *fileID)
{
  OSErr err;
  Str255 pFileName;
  short fRefNum;
...
size_t
mfread (void *ptr, size_t size, size_t nmemb, long fileID)
{
...
size_t
mfwrite (const void *ptr, size_t size, size_t nmemb, long fileID)
{
  long numberOfBytes = size * nmemb;
  OSErr err;
...
int
mfseek (long fileID, long offset, short posMode)
{
  OSErr err;
...
int
mfgetpos (long fileID, long *offset)
{
  OSErr err;
