/**************************************************************************************
 *	Project:		SAudioTool -	An MPW Tool to print formatted output for .saudio
 *									header data and allow modification of the header and
 *									data chunks..
 *
...
int	main( int argc, char* argv[] )
	...
	if ( (hcp->chunkType != SAUDIO_DATA_TYPE) || (hcp->subChunkType != SAUDIO_HEADER_TYPE) )
		{
	...
	if (gNewLogicalChannel >= 0)
		...
		if (gNewLogicalChannel <= 16 )
			...
			while(true)
				...
				bytesWritten = fwrite(dcp, 1, sizeof(SAudioDataChunk), theFile);
