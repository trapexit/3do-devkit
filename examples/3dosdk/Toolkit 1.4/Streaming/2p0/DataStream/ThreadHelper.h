 ...
 *in 3B1 3/30/93		jb		New today, taken from SHayes
 *8/10/92 example code
 *
 *******************************************************************************************/
 ...
 * relative to the creator task (higher or lower than the current task). The
 * thread parent is useful to threads that wish to communicate via with the
 * parent task via: AllocSignal(), Signal(), Wait(), & FreeSignal().
 */
...
Item NewThread (void *threadProcPtr, int32 stackSize, int32 threadPriority,
                char *threadName, void **stackMemoryPtr, int32 argc,
                void *argp);
