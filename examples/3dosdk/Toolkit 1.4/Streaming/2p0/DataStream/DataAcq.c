...
static int32	DoGetData( AcqContextPtr ctx, DataAcqMsgPtr dataMsg )
	...
	if ( bp->ioreqItem == 0 )
		...
		return kDSNoErr;
#else
		kprintf( "DoGetData - DataAcq.c - AllocPoolItem( ctx->ioReqItemPoolPtr ) failed !!!" );
		return kDSNoMemErr;
#endif
		}
				...
				switch ( dataMsg->whatToDo )
					...
					case kAcqOpExit:
						...
						if ( exitRequestMsg == NULL )
							...
							kprintf( "DataAcqThread got multiple kAcqOpExit messages!\n" );
#else
						fKeepRunning = false;
#endif
						break;
