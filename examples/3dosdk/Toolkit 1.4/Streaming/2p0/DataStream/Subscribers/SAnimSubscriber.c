 ...
 *Now the X.Y positions are assumed to be fixed point values, not integers. If
 *your animations all come out at 0,0, then you must rebuild with
 *SHIFT_CCB_POSITIONS defined to get the old behavior.
 *	05/23/94	fyp		Fixed a low-memory access problem caused
 *by a nil curFramePtr at the end of GetSAnimCel. 11/23/93	jb
 *Version 1.2 Added CloseSAnimSubscriber(), a routine to deallocate resources
 *						allocated by
 *InitSAnimSubscriber(). 10/19/93	jb		Version 1.1 Switch to
 *using UMemory.c routines for memory allocation. Add code identified by
 ...
 *buffer time+duration being less than the current stream clock. 9/15/93
 *jb		Add support for APOS chunk subtype. 8/18/93		jb
 *Changed curTime in IsTimeForNextFrame() to a SIGNED value to make comparisons
 *work correctly. 6/22/93		njc		Changed subscriber
 *structure from one msgPort for every channel to a simple linked list with a
 *semaphore to arbitrate access to the list. In addition, a freeList is
 *maintained that contains chunks that are to be returned to the Data Streaming
 *Library. 5/25/93		ec		Added SAnimDuration &
 *SAnimCurrTime; 5/25/93		njc		Version 1.0a 5/19/93
 *ec		Added reset code to FlushSAnimChannel.
  ...
  while ((pChunk = GetChunk (&chunk_ID, &tempBuf, &tempSize)) != NULL)
    {
      switch (chunk_ID)
        ...
        default:
          printf ("SetupNextSAFrame --- Unexpected chunk ID: '%4.s'",
                  &chunk_ID);
          return NULL;
        }
    }
