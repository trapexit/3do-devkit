 ...
 *function names unique 3/29/94		rdg		Version 2.9.1 Add new
 *trace code. 10/26/93	rdg		Version 2.9 Distinguish between user
 *and internal muting and unmuting Get rid of envelope rate knobs in channel
 *context... tweak once at init time. 10/15/93	rdg		Version 2.8
 ...
 *******************************************************************************************/
void
StartSAudioChannel (SAudioContextPtr ctx, long channelNumber)
{
  SAudioChannelPtr chanPtr;
 ...
 *******************************************************************************************/
long
StopSAudioChannel (SAudioContextPtr ctx, long channelNumber)
{
  long status;
  SAudioChannelPtr chanPtr;
  ...
  if (IsChanEnabled (chanPtr))
    {
      ...
      CHECKRESULT ("FlushSAudioChannel() - StopSAudioChannel() failed",
                   status);
