 ...
 *memory allocation 8/9/93		jb		Version 1.4 Changed
 *DEFAULT_MAXQ to 0x4000 for Dragontail3 release as required.
 *	7/10/93		jb		Version 1.3
 *						Switch to using MakeName() routine and
 *get rid of non-reentrant hack that we had before for generating unique names.
 *Now, when we create a thread with a unique name, we use the thread's stack
 *						memory address as a postfix to the string
 *"Thread" to name it. 6/1/93		jb		Version 1.2.3 Remove
