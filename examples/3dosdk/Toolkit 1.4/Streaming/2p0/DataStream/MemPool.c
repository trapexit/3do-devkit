 ...
 *	File:			MemPool.c
 *
 *	Contains:		Pool oriented memory manager. Manages preallocated pools of
 *					fixed sized objects. Free objects are singly linked together
 *					into a free list using the "-4" offset of the objects. Objects
 *					are reused in LIFO order (add and remove from list head only)
 ...
 *	4/18/93		jb		Fix cast in CreateMemPool()
 ...
 *				numToPreallocate * (sizeOfEntry + 4) + sizeof(MemPool)
 * and links them together using the extra space. Note that entry data size is
...
/*******************************************************************************************
 * Routine to allocate an entry from an allocation pool. Returns a pointer to the
 * data portion of the entry.
 *******************************************************************************************/
...
/*******************************************************************************************
 * Routine to iterate through each _free_ member in a memory pool and call a user
 * specified function. The user function is called with a pointer to the data portion
 * of the pool entry. Handy for initialization of all members in a pool, or for whatever
 * makes sense in terms of operating on only the _free_ members in the pool.
 *******************************************************************************************/
Boolean		ForEachFreePoolMember( MemPoolPtr memPool, ForEachFreePoolMemberFuncPtr forEachFunc,
									void* argValue )
	{
	MemPoolEntryPtr	entry;
