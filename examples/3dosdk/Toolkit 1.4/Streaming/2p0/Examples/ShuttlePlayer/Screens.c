 ...
 *	File:			Screens.c
 *
 *	Contains:		routines for loading and displaying intro screens
 *
 ...
 *
 *	History:
 *	12/14/93		lla		Added animation to Kurt's Logo screen
 *	12/10/93		lla		Added 3DO Logo disolve
...
void ShowHelp (ScreenContext *gScreenContextPtr)
	{
...
char *imageFile (long selection )
	{
	switch (selection)
		{
		case 0: return	"$boot/Player_Screens/Help.img";
		case 1: return	"$boot/Player_Screens/3DO_Studio_Logo.img";
		case 2: return	"$boot/Player_Screens/3DO_Studio.anim";
		case 3: return	"$boot/Player_Screens/Menu.img";
		default: return	"Bad file name for screen image";
		}
	}
...
//------------------------------------------------------------------------------------
//
//		Go through the intro sequence and exit with the menScreen loaded
//
//------------------------------------------------------------------------------------

long ShowIntro ( ScreenContext *gScreenContextPtr )
	{
	long			status = 0;
...
//------------------------------------------------------------------------------------
	...
	imagePtr = LoadImage ( imageFile (helpImage), NULL, NULL, gScreenContextPtr );	// MEMTYPE_VRAM
	VRAMIOReq = GetVRAMIOReq ();
	status = CopyVRAMPages ( VRAMIOReq,
						gScreenContextPtr->sc_Bitmaps[gScreenContextPtr->sc_curScreen]->bm_Buffer,
						imagePtr,
...
//------------------------------------------------------------------------------------
	...
	GetJoyPad( joypadStatePtr, 1 );
			...
			nextImagePtr = LoadImage ( imageFile (Studio_Logo), NULL, NULL, gScreenContextPtr );	// MEMTYPE_VRAM
	...
	while (nFrames)
		...
		if (nFrames)
			...
			status = CopyVRAMPages ( VRAMIOReq,
								...
								nextImagePtr,
								gScreenContextPtr->sc_nFrameBufferPages, -1 );
			status = DisplayScreen (gScreenContextPtr->sc_Screens[gScreenContextPtr->sc_curScreen], 0);
			DrawScreenCels ( gScreenContextPtr->sc_Screens[gScreenContextPtr->sc_curScreen], pcVideo );
			}
		else
			{
			aVideo->cur_Frame = 0;
...
//------------------------------------------------------------------------------------
		...
		gScreenContextPtr->sc_curScreen = menuScreen;
		nextImagePtr = LoadImage ( imageFile (menuImage), NULL, NULL, gScreenContextPtr );	// MEMTYPE_VRAM
