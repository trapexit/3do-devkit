 ...
 *	10/25/93		rdg		Modify to play SSND only streams.
 *							Move connect to dataAcq into Init routine.
...
/************************************/
void			DismantleAudioPlayer( PlayerPtr ctx );
...
int32	PlaySSNDStream( char* streamFileName, PlaySSNDUserFn userFn, void* userCB, 
	...
	if ( status != 0 )
		{
		CHECK_DS_RESULT( "InitSSNDPlayerFromStreamHeader", status );
		return status;
		}
 ...
 *******************************************************************************************/
int32	InitSSNDPlayerFromStreamHeader( PlayerPtr ctx, char* streamFileName )
	{
	int32			status;
	...
	if ( status != 0 )
		{
		CHECK_DS_RESULT( "FindAndLoadStreamHeader", status );
		return status;
		}
...
void		DismantleAudioPlayer( PlayerPtr ctx )
	{
	if ( ctx->streamCBPtr )				DisposeDataStream( ctx->messageItem, ctx->streamCBPtr );

	if ( ctx->acqContext )				DisposeDataAcq( ctx->acqContext );

	if ( ctx->controlContextPtr )		DisposeCtrlSubscriber( ctx->controlContextPtr );

	if ( ctx->audioContextPtr )			DisposeSAudioSubscriber( ctx->audioContextPtr );
