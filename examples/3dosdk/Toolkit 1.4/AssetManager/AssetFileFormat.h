 ...
 *
 *	Contains:		platform independent description of the 3DO Asset File Format
 *
 * The Asset File is a simple directorized, read-only database file. We borrow from
 * the Macintosh Resource Manager in using 32-bit data types and integer id values
 * to uniquely identify arbitrary, user-defined data. The index is essentially a
 * table of types, each table entry is variable length. There is one table entry
 * per resource type, and the entry tag is the same as the resource type. The
 * table entry contains pairs of id's and file offsets for the corresponding data.
 *
 ...
 * does not pay attention to IFF chunks, but depends on the stored file position
 * data to be valid. This means that this data cannot be relocated in a stream
 * of other data.
 */
