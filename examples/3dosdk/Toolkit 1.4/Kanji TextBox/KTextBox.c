...
int32 OpenRomFile(KFont3DO *theFont, char *name)
{
	...
	theFont->romior = NULL;
  	...
  	if (romfptr) {
    	...
    	if ((theFont->romIOReqItem=CreateItem(MKNODEID(KERNELNODE,IOREQNODE),targs)) < 0) {
...
//	printf( " romBlockSize = %ld log = %ld \n", romBlockSize,  theFont->logRomBlockSize);
...
int32 ReadRom(KFont3DO *theFont, uint32 offset, char *buf, int32 len)
{
    ...
    if (((err=DoIO(theFont->romIOReqItem,&theFont->romioInfo)) < 0) || (theFont->romior->io_Error!=0)) {
	PrintfSysErr((Item)theFont->romior->io_Error); 						
      DeleteItem (theFont->romIOReqItem);
      DIAGNOSTIC (("ReadRom:  Error in DoIO offset = %ld ( %lx )\n", offset, offset));
...
int ReadFile_T(Stream *fs, Int32 size, long *buffer, long offset)
{
	long retval;
	char * cbuf;
	long ntoread, nleft;	
...
int ReadFile_E(KFont3DO *theFont, Int32 size, long *buffer, long offset)
{
	if (theFont->fsRead)
...
/*
    if( (fontDesc->header->offsetHcodetbl < 0) || (fontDesc->header->offsetZcodetbl < 0)){
		DIAGNOSTIC("Error codetbl is less than zero.\n");
		return (ER_KTextBox_BadFontFile);
    }

    if( (fontDesc->header->offsetHwidthtbl < 0) || (fontDesc->header->offsetZwidthtbl < 0)){
		DIAGNOSTIC("Error widthtbl is less than zero.\n");
		return (ER_KTextBox_BadFontFile);
    }
*/
	 ...
	 *	半角のコードテーブルと文字幅テーブルをロードする                *
	 \***********************************************************************/
		if(fontDesc->header->offsetHcodetbl != 0){		// <HPP> for our sake this is ignored
		    ...
		    if (dataPtr == NULL) {
				...
				FREEMEM(fontDesc->header, sizeof(KFontFileHeader));
				DIAGNOSTIC("Cannot allocate storage for font file font_Hinfo.codeTbl.\n");
				return (ER_KTextBox_CannotMemAlloc);
		    }
		...
		if(fontDesc->header->offsetHwidthtbl != 0){		// <HPP> for our sake this is ignored
		    ...
		    if (dataPtr == NULL) {
				...
				if (theFont->fsRead)
					CloseDiskStream(theFont->font_fs);	    		
				FREEMEM(fontDesc->header, sizeof(KFontFileHeader));
				DIAGNOSTIC("Cannot allocate storage for font file hwidthTbl.\n");
				return (ER_KTextBox_CannotMemAlloc);
		    }
		...
		if(fontDesc->header->offsetZwidthtbl != 0){		// <HPP> for our sake this is ignored
		    ...
		    if (dataPtr == NULL) {
				...
				if (theFont->fsRead)
					CloseDiskStream(theFont->font_fs);	    		
				FREEMEM(fontDesc->header, sizeof(KFontFileHeader));
				DIAGNOSTIC("Cannot allocate storage for font file zwidthTbl.\n");
				return (ER_KTextBox_CannotMemAlloc);
		    }
		...
		if( theFont->font_FullRead ) {						// <HPP 03-08-94> 
			...
			if (dataPtr == NULL) {
				...
				if (theFont->fsRead)
					CloseDiskStream(theFont->font_fs);
				FREEMEM(fontDesc->header, sizeof(KFontFileHeader));
				DIAGNOSTIC("Cannot allocate storage for font file hcharData.\n");
				return (ER_KTextBox_CannotMemAlloc);
			}
		...
		if( theFont->font_FullRead ) {						// <HPP 03-08-94> 
			...
			if (dataPtr == NULL) {
				...
				if (theFont->fsRead)
					CloseDiskStream(theFont->font_fs);	    		
				FREEMEM(fontDesc->header, sizeof(KFontFileHeader));
				DIAGNOSTIC("Cannot allocate storage for font file zcharData.\n");
				return (ER_KTextBox_CannotMemAlloc);
			}
...
Int32 KBinarySearch(KFontRec *thisFont, uint16 charVal, uint16 n)
{
    ...
    if(thisFont->codeTbl[left] == charVal) return left;
    while(left < right){
	mid = (left + right)/2;
...
/*    if (theFont->font_Flags & FONT_ASCII) {
	*lineHeight = theFont->fontHinfo.charHeight;
	while ( (*textLeft > 0) && roomLeft && (curChar->Value != '\n') ) {
	    ...
	    wordChars = 0;
	    numendingword = 0;
	    while ( (*textLeft > 0) && (curChar->Value == ' ') && (curChar->Value != '\n') ) {
		if (!startofline) {
		    wordWidth += tx_get_char_width(theFont,' ',fontAscii);
		    wordChars++;
		    numendingword++;
		}
		else (*lineStart)++;
	    ...
	    startofline = FALSE;
	    if ( curChar->Value != '\n' ) {
		while ( (*textLeft > 0) && ((wordWidth <= widthLeft) || firstWord) && (curChar->Value != ' ') && (curChar->Value != '\n') ) {
		    (*textLeft)--;
		    wordChars++;
		    wordWidth += tx_get_char_width(theFont,curChar->Value,fontAscii);
		    curChar++;
		    numendingword++;
		}
...
//	endk = charWidth/ 2;
    ...
    if (!skipChar) {
		...
		pCelData = (char *)fourBitCel->ccb_CelData + ((xDiff/2) + (celByteWidth*yDiff));
#else
		/* point to right position in celdata */
...
#if RED_HARDWARE
	/* point to right position in celdata */
	pCelData = (char *)oneBitCel->ccb_CelData + (xDiff/8) + (celByteWidth*yDiff);
#else
	/* point to right position in celdata */
...
Int32 KWrapAndDraw( KFont3DO *theFont, KTextBox *theTextBox, Int32 boxWidth, KCharRec *lineStart, KCharRec *textEnd, Int32 lineLeft, Int32 boxRowBytes,  Int32 top, Int32 bottom, CCB *theCCB)
{
	...
	do {
		...
		/* 一行目の書き出し位置がはみ出る場合はエラー*/
		    DIAGNOSTIC("The font height size is greater than the wrapRect height.\n");
		    return (ER_KTextBox_BadParameter); 
		}
		else {
...
Int32 KTextBoxMain( KFont3DO *theFont, KTextBox *theTextBox, uint8 *theText, Int32 textLen, CCB **userCCB)
{
 ...
 /*   
    if ( (cel_left < 0 ) || (cel_right > 320 ) || (cel_top < 0 ) || (cel_bottom > 240 ) ){
		DIAGNOSTIC("celRect is too large.\n");
		return (ER_KTextBox_BadParameter);
    }
 */  
