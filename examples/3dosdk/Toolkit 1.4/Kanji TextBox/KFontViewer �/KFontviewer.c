/*
	...
	Change History (most recent first):
		...
		<1+>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
...
//	InitMem(1);
	...
	gIdleTimer = InitTimer(TM_TYPE_MICROSEC + TM_MODE_DELTA);	// allocate a cursor timer
	...
	if (err == 0)
	{
		...
		if (gIdleTimer)
		{
			sprintf( string, "Load time: secs=%0d, usecs=%0d", tv.tv_sec, tv.tv_usec ); 
	...
	while (TRUE)
		...
		if (lControlvals & JOYSTART) goto DONE;
		...
		else if (lControlvals & JOYFIREA || lControlvals & JOYFIREB || lControlvals & JOYFIREC) {
...
//		CopyBlack(sc.sc_Bitmaps[sc.sc_curScreen], sc.sc_nFrameBufferPages );
		 ...
		 */
		if (textccb == NULL) {
			DIAGNOSTIC("Problem with TextBox - either bad input parameters or out of memory.");
			goto DONE;
		}
		else {
		...
		if (gIdleTimer)
		{
			sprintf( string, "chars=%0ld, secs=%0ld, usecs=%0ld", charsPerScreen, tv.tv_sec, tv.tv_usec ); 
...
{
	CopyVRAMPages( VRAMIOReq, bitmap->bm_Buffer, gBackPic, nFrameBufferPages, -1 );
 ...
 */
{
	...
	CopyVRAMPages( VRAMIOReq, sc->sc_Bitmaps[sc->sc_curScreen]->bm_Buffer, gBackPic, sc->sc_nFrameBufferPages, -1 );
