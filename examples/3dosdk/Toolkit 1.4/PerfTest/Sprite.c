/*
	...
	Change History (most recent first):
		 ...
		 <3>	 8/11/93	JAY		change calling sequence to LoadAnim
		 <2>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
...
void SetSymOrder(long nsym)
	...
	symorder = nsym;
	if (nsym <= 1) symorder = 1;
	if (nsym >= MAXSYM) symorder = 1;
...
Sprite * LoadSprite(char *name,  frac16 xvel, frac16 yvel, frac16 zvel, frac16 tvel)
	...
	if (RedSi) ccb->ccb_Flags =  (ccb->ccb_Flags& (~CCB_ACE)) | SCEFLAG;
	w = ccb->ccb_Width;
	h = ccb->ccb_Height;
	spr->nxtSprite = NULL;
	spr->anim = lanim;
...
int32 DeleteSprite()
	...
	head = SpriteList;
	spr = head->nxtSprite;
    if (spr == NULL) return(1);  /* keep at least one sprite around */
...
void  bounce(Sprite *s, long x, long y)
	{
	if (x < xmin) if (s->xvel < 0)  s->xvel = 0 - s->xvel;
	if (x > xmax) if (s->xvel > 0) s->xvel = 0 - s->xvel;
	if (y < ymin) if (s->yvel < 0) s->yvel = 0 - s->yvel;
	if (y > ymax) if (s->yvel > 0) s->yvel = 0 - s->yvel;
	}
	

void  bouncez(Sprite *s,  frac16 z)
	{
	if (zscaling == 0) return;
	if (z < zmin) if (s->zvel < 0)  s->zvel = 0 - s->zvel;
	if (z > zmax) if (s->zvel > 0) s->zvel = 0 - s->zvel;
	}
void rotate(Sprite *s, long xc, long yc, frac16 fw, frac16 fh) 
	...
	long x, y;
	theta = s->tpos + s->tvel;
	if (theta >= tmax) theta = theta - tmax;
	if (theta < tmin ) theta = theta + tmax;
	s->tpos = theta;
	wover2 = fw>>1;
	hover2 = fh>>1;
	fleft = -wover2;
...
void DrawSprites(Item bitmapItem )
	...
	if (symorder == 1)
		...
		while (s != NULL)
			...
			if( err < 0) kprintf("Error in DrawCels: %d \n", err);
			s = s->nxtSprite;
			}
		}
	else
