/*
	...
	Change History (most recent first):
		 ...
		 <3>	  4/5/93	JAY		remove /remote/ from filenames (i.e. make the pathname relative
									to initial working directory)
		 <2>	 3/23/93	JAY		change AF_TAG_LOUDNESS to AF_TAG_AMPLITUDE
		 <1>	 3/17/93	JAY		making changes for Cardinal
				 1/03/93	jb		Use mixer channel #3 for end of round & end of game sounds;
									This will share channel 3 with the "cannot drop piece" sound.
				12/30/92	jb		Add real sound calls.
...
/*********************************************************************************************
 * This routine is called by the main module at startup time to load/initialize anything 
 * that may be needed to play sound effects and background music. It returns TRUE if 
 * initialization suceeds, FALSE otherwise.
 *********************************************************************************************/
...
#endif
	// SetChannel will return FALSE if it fails, but there's no real reason to check for that
	// right now.
 ...
 * The input argument indicates how many segments are being removed by the playing of
 * the last piece. If this == 0, a simple "click" (or something) should be played.
 * Otherwise, the number could be used to play more complex sounds, for example,
 * the number could be use to play a musical chord containing N notes.
 *********************************************************************************************/
