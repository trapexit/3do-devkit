/*
        MODIFIED VERSION!!!
                This has been tweaked so that GetJoyPad() returns the raw
   joybits instead of boolean.
 ...
 *	--------	-------------
 *------------------------------------------------- 93.06.21		ec
 *Updated for eventp broker (finally...)  Remove dependency on Utils3DO.
 *	93.06.03		ec			Added GetJoyPadContinuous,
 *changed SetJoyPadContinuous to return the old setting. 93.02.26
 *ec			first writing
 *
 */
 ...
 * SetJoyPadContinuous
 *
 * set which joypad buttons may be held down continuously.  return the old
 * setting in case the user wants to restore it later.
 */
int32
...
GetJoyPad (JoyPadStatePtr joyStatePtr, int32 padNum)
{
  ...
  if ((padNum < 1) || (padNum > kDefaultCtrlPadCount))
    {
      kprintf (
          "ERROR: %ld is an invalid pad number.  Must be between 1 and %d\n",
