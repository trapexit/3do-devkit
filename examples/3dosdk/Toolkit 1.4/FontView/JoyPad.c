/*
	MODIFIED VERSION!!!  
		This has been tweaked so that GetJoyPad() returns the raw joybits
		instead of boolean.
 ...
 * Returns raw joybits.
 */
uint32 GetJoyPad(JoyPadStatePtr joyStatePtr, int32 padNum)
{
	uint32	joyBits;
	...
	{
		kprintf("ERROR: %ld is an invalid pad number.  Must be between 1 and %d\n", 
