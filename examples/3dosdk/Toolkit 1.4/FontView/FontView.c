 ...
 *
 *	Copyright:		© 1993-1994 The 3DO Company. All Rights Reserved.
 *
 *	History:
 *	05/12/94  Ian	Removed references to CelUtils functions and datatypes
 *					that are not being included in the current release.  
 *					Removed the bogus outline/shadow simulation stuff from 
 *					03/25/94 since the font/textlib now supports it directly.
 *	
 *	04/01/94  Ian	Added ability to specify a file that contains the display text.
 *
 *	03/31/94  Ian	Made magnification work properly with bogus shadow & outline.
 *
 *	03/25/94  Ian	Added bogus outline and shadow effects, mostly to test
 *					visually the concepts we want to build into TextLib.
 *
 *	03/16/94  Ian	Added printf of FontDescriptor values.  Relinked with
 *					repaired TextLib routines that properly handle attempts
 *					to render characters not present in the font file.
 *
 ...
 *	having to stop/restart the program.  The C button toggles between x1 and
 *	x2 magnification of the displayed font.  The B button toggles the
...
static void update_display(void)
{
	...
	if (bgCel && bgOn) {
		...
		SetVRAMPages(vramIOReq, sc.sc_Bitmaps[sc.sc_curScreen]->bm_Buffer, VRAMBLUE, sc.sc_nFrameBufferPages, 0xFFFFFFFF);
	}
...
static Err do_user_interaction(void)
{
	...
	for (;;) {
		...
		if (joyState.xBtn) {					// exit program
			return 0;
		}

		if (joyState.aBtn) {					// recreate font display cel
		...
		if (joyState.cBtn) {					// toggle magnification
			if (magnification == 1) {
				magnification = 2;
		...
		if (joyState.downArrow) {				// move cel up/down...
			tCelPosY += moveSpeed;
		} else if (joyState.upArrow) {
			tCelPosY -= moveSpeed;
		}

		if (joyState.rightArrow) {				// move cel left/right
			tCelPosX += moveSpeed;
...
int main(int argc, char **argv)
{
	...
	if (fontFileName == NULL) {
		printf("\nUsage: FontView fontFileName [-b bgCelFileName][-t textFileName]\n");
		return 1;
	}
