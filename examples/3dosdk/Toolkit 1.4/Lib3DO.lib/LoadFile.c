 ...
 *       04/12/94	Ian		Modified so that a file of size zero
 ...
 *caller. <3>  02/18/94	Ian		Reworked error detection logic. The old
 *way was failing to return bad status if the open worked but the read failed.
 *	<2>  09/06/93	JAY		change free to Free in two error paths
 *in LoadFile. <1>  08/20/93	JAY		first checked in
 *
 *******************************************************************************************/
...
void *
LoadFile (char *pfname, long *pfsize, uint32 memTypeBits)
{
  long rv;
  long filesize;
  long blocksize;
  ...
  if (filesize > 0)
    {
      if ((buffer = Malloc (bufsize, memTypeBits)) == NULL)
        {
          ...
          DIAGNOSE_SYSERR (rv,
                           ("Can't allocate buffer for file %s\n", pfname));
          goto ERROR_EXIT;
        }
      ...
      if ((rv = AsynchReadBlockFile (&thefile, ioreqItem, buffer, bufsize, 0))
          >= 0)
        {
