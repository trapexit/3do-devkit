/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <3>	 1/17/94	JAY		change calling
   parameters in memset call to match prototype. <2>	 8/13/93	JAY
   clean up for 1p01 release
 ...
 */
{
  ...
  if ((j = DoIO (TheIOPort, &FileInfo)) < 0)
    {
      kprintf ("ERROR:  couldn't write Mac file %s : %d ", filename, j);
      kprintf ("($%x), %d ($%x)\n", j, TheReplyPort->io_Error,
               TheReplyPort->io_Error);
      kprintf ("SysErr() = ");
    ...
    retvalue = TheReplyPort->io_Actual;
