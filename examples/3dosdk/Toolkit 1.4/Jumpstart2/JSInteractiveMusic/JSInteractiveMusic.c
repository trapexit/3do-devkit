/*
        File:		JSInteractiveMusic.c

        Contains:	Variation of JSInteractiveSound which plays background
   music

        Files used:

                $boot/JSData/Art/sky.img				-
   the background image $boot/JSData/Art/JSUFO.cel
   -	the UFO cel
                $boot/JSData/Art/Cockpit.cel			-
   the cockpit cel
                $boot/JSData/Art/Crosshairs.cel			- the
   crosshairs "cursor" cel
                $boot/JSData/Art/Laser.cel				-
   the laser beam cel
                $boot/JSData/Art/Blip.cel				-
   the radar blip cel
                $boot/JSData/Art/Options.cel			-
   the options buttons ccb
                $boot/JSData/Art/PleaseWait.cel			-
   the "Please Wait" box cel
                $boot/JSData/Sound/whirr.aiff			-
   the whirring sound effect
                $boot/JSData/Sound/zap.aiff				-
   the zap sound effect $boot/JSData/Sound/JSPlayBgndMusic.task	-
   the sound handler for music-playing
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <1>	 1/4/94		CDE		Derived from old
   Jumpstart's GameUFO.c; Replaced direct joypad access with event broker
   interface; Constrained behavior to that of JSInteractiveSound with
   music-playing by removing extraneous code
*/
...
bool
Zap (CCB *aTargetCCB, Coord X, Coord Y)
{
  static TagArg taStartArgs[] = { { AF_TAG_FREQUENCY, (void *)0x8000 },
  ...
  if (X >= 0 && Y >= 0 && (Y < ((aTargetCCB->ccb_Height << 8) / iDistance))
...
DoControlPad (void)
{
  static TagArg taStartArgs[] = { { AF_TAG_FREQUENCY, (void *)0x4000 },
  ...
  if (joyBits & ControlStart)
  ...
  else
    {
      if (joyBits & ControlA && !nLaserCount)
        {
          ...
          if (Zap (gUFO_CCB,
                   (gCrosshairsCCB->ccb_XPos >> 16)
                       + (gCrosshairsCCB->ccb_Width >> 1),
                   (gCrosshairsCCB->ccb_YPos >> 16)
                       + (gCrosshairsCCB->ccb_Height >> 1)))
            {
              Balance = ((gCrosshairsCCB->ccb_XPos
                          + (gCrosshairsCCB->ccb_Width >> 1))
                         >> 2)
                        / 160;
              ...
              gExplosionCCB->ccb_XPos
                  = gCrosshairsCCB->ccb_XPos
                    + (gCrosshairsCCB->ccb_Width << 15)
                    - (gExplosionCCB->ccb_Width * gUFO_CCB->ccb_VDY >> 1);
              gExplosionCCB->ccb_YPos
                  = gCrosshairsCCB->ccb_YPos
                    + (gCrosshairsCCB->ccb_Height << 15)
                    - (gExplosionCCB->ccb_Height * gUFO_CCB->ccb_VDY >> 1);
      ...
      if (joyBits & ControlRight
          && gCrosshairsCCB->ccb_XPos
                 < ((320 - gCrosshairsCCB->ccb_Width) << 16))
      ...
      if (joyBits & ControlDown
          && gCrosshairsCCB->ccb_YPos
                 < ((240 - gCrosshairsCCB->ccb_Height) << 16))
...
TargetAction (CCB *aTargetCCB)
{
  ...
  iTest = (aTargetCCB->ccb_YPos >> 16)
          + ((aTargetCCB->ccb_Height << 8) / iDistance);
  ...
  iTest = (aTargetCCB->ccb_XPos >> 16)
          + ((aTargetCCB->ccb_Width << 8) / iDistance);
...
InitGame (void)
{
  ...
  if ((gCockpitCCB = LoadCel ("$boot/JSData/Art/Cockpit.cel", MEMTYPE_CEL))
      == NULL)
    {
  ...
  if ((gOptionsCCB = LoadCel ("$boot/JSData/Art/Options.cel", MEMTYPE_CEL))
      == NULL)
    {
  ...
  if ((gPleaseWaitCCB
       = LoadCel ("$boot/JSData/Art/PleaseWait.cel", MEMTYPE_CEL))
      == NULL)
    {
  ...
  if ((iSoundHandler = LoadProgram ("$boot/JSData/Sound/JSPlayBgndMusic.task"))
      < 0)
    {
      DIAGNOSTIC ("Can't spawn task for SoundHandler");
      return FALSE;
    }
  SetItemPri (iSoundHandler, CURRENT_TASK_PRIORITY + 1);
  ...
  if ((gCrosshairsCCB
       = LoadCel ("$boot/JSData/Art/Crosshairs.CEL", MEMTYPE_CEL))
      == NULL)
    {
  ...
  if ((gLaserCCB = LoadCel ("$boot/JSData/Art/Laser.CEL", MEMTYPE_CEL))
      == NULL)
    {
  ...
  if ((gExplosionAnim = LoadAnim ("$boot/JSData/Art/Boom.ANIM", MEMTYPE_CEL))
      == 0)
    {
  ...
  if ((iExplosion = LoadSoundEffect ("$boot/JSData/Sound/Explosion.aiff", 1))
      < 0)
    {
