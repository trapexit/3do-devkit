/*
	File:		JSSlideShowVDL.c

	Contains:	Load and display image files which may contain custom VDLs

	Files used:
		
		$boot/JSData/imagelist			-	list of image filenames to load and display
		$boot/JSData/Art/Å.vdl			-	images listed in the above file
	...
	Change History (most recent first):
		 ...
		 <2>	 1/3/94		CDE		Derived from SlideShow.c;
		 							Replaced direct joypad access with event broker interface;
									Cleaned up includes, prototypes, changed some function types;
									Removed -r, -p options from Usage() since they were unimplemented;
		 							Added ec's fix for bug in ParseScriptFile which prevented the
		 							last file from being loaded if the line didn't end in a return.
*/
...
DONE: 
	...
	printf("\nJSSlideShowVDL has completed.  Thanks for shopping with us!\n");
...
/*
	...
	Returns noErr if all is well, non-noErr otherwise.
...
Boolean ParseArgs(int32 argc, char **argv)
{
	...
	if (argc < 2)
	{
		if ( ParseScriptFile("$boot/JSData/imagelist") != noErr )
			return FALSE;
		haveFileName = TRUE;
	}
...
Boolean LoadScreen(char *fileName, int32 whichBuffer)
{
	...
	if ( LoadImage(imageFileName, gBackPic[whichBuffer], &rawVDLPtr, &gScreenContext) == NULL ) 
	{
	...
	if ( (rawVDLPtr != NULL) && (rawVDLPtr->chunk_size) ) 
	{
		...
		MergeVDL((VDLEntry *) &(rawVDLPtr->vdl[0]), gVDLPtrArray);
	...
	gVDLItems[whichBuffer] = SubmitVDL(gVDLPtrArray, (kVDLSize / 4), VDLTYPE_FULL);
...
Init( void )
{
	...
	{
		printf("Cannot initialize the event broker\n");
		return FALSE;
	}
   
	// allocate memory for the raw VDL, initialize it.  Fail if we can't get the memory
	if ( AllocateVDL(&gVDLPtrArray, gScreenContext.sc_Bitmaps[0] ) != noErr )
		goto DONE;
	...
	for ( aScreenIndex = 0; aScreenIndex < kBufferCount; aScreenIndex++ )
	{
		...
		gVDLItems[aScreenIndex] = SubmitVDL(gVDLPtrArray, (kVDLSize / 4), VDLTYPE_FULL);
		if ( gVDLItems[aScreenIndex] < 0 ) 
		{
			printf( "Error: SubmitVDL() failed with error code %d\n", gVDLItems[0] );
			PrintfSysErr(gVDLItems[aScreenIndex]);
			goto DONE;
		}
	
		// got the 'real' vdl, now make it current
		errorCode = SetVDL(gScreenContext.sc_Screens[aScreenIndex], gVDLItems[aScreenIndex]);
		if ( errorCode < 0 ) 
		{
			printf( "Error: SetVDL() failed with error code %d\n", errorCode );
	...
	for ( aScreenIndex = 0; aScreenIndex < kBufferCount; aScreenIndex++ )
	{
		...
		{
			printf ("unable to allocate gBackPic");
			goto DONE;
		}
		// initialize it to back
		SetVRAMPages( gVRAMIOReq, gBackPic[aScreenIndex], 0, gScreenContext.sc_nFrameBufferPages, -1);
	}
...
DisplayCurrentScreen( void )
{
	...
	CopyVRAMPages( gVRAMIOReq, gScreenContext.sc_Bitmaps[currScreen]->bm_Buffer, gBackPic[currScreen], 
...
Usage (void)
{
	printf (("\nUsage:\n\tJSSlideShowVDL scriptFileName\n"));
	printf (("Where <scriptFileName> is a text file with the names of the images to show\n"));
	printf (("the image files to load from the Mac.  The text file \"$boot/JSData/ImageList\" is \n"));
...
*/
{
	...
	for ( aScreenIndex = 0; aScreenIndex < kBufferCount; aScreenIndex++ )
		FreeMem(gBackPic[aScreenIndex], (int32)(gScreenContext.sc_nFrameByteCount));
