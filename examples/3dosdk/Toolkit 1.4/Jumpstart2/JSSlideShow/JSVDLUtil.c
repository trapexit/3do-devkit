/*
	...
	Contains:	Routines to mess around with video display lists.
	...
	Change History (most recent first):
		 ...
		 <1>	 1/5/94		CDE		derived from ourVDL.c in the SlideShow
		 							example folder
*/
...
InitVDL(VDLEntry *vdlPtr, Bitmap *destBitmap)
{
	...
	for (scanLine = 0; scanLine < kScreenHeight; scanLine++) 
	{
		...
		vdlRecPtr->controlword = SetDMACtlWord(kClutColorWords - kFillerWords, 1, false);
		...
		for (colorTableEntry = 0; colorTableEntry < kCLUTWords; colorTableEntry++) 
	...
	vdlRecPtr->controlword = SetDMACtlWord(kClutColorWords - kFillerWords, 1, true);
...
#define		ODD(num)	(num & 1)
	...
	for (scanLineEntry = 0; scanLineEntry < kScreenHeight; scanLineEntry++)
	{
		...
		//  the vld length.  SubmitVDL will now fail if the total length of a vdl
		...
		// when calculating the next frame buffer address, remember that the image
 ...
 */
void 
MergeVDL(VDLEntry *rawVDLPtr, VDLEntry *activeVDLPtr)
{
	uint32		scanLineEntry;
	int32		ndx;
...
AllocateVDL(VDLEntry **newVDL, Bitmap *screenBitMap)
{
	...
	*newVDL = (VDLEntry *)ALLOCMEM(kVDLSize, 
					MEMTYPE_ANY 
					| (MEMTYPE_FILL | 0x00));
	if ( *newVDL == NULL )
	{
