/*
	...
	Change History (most recent first):
		...
		<3>		1/5/94		CDE		Removed ChangeInitialDirectory;
									Added $boot to filenames;
									Removed -r, -p options messages from Usage() since they were unimplemented;
									Cleaned up includes, prototypes, changed some function types.
		<2>		1/3/94		CDE		Replaced direct joypad access with event broker interface.
		<1>		12/21/93	CDE		Derived from SlideShow.c;
		 							removed VDL handling.
...
Boolean LoadScreen(char *fileName, int32 whichBuffer)
{
	...
	if ( LoadImage(imageFileName, gBackPic[whichBuffer], (VdlChunk **) NULL, &gScreenContext) == NULL )
	{
...
void Usage (void)
{
	...
	printf (("the image files to load from the Mac.  The text file \"$boot/JSData/basicImageList\" is \n"));
