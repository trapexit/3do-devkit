/*
	...
	Change History (most recent first):
		...
		<7+>	 8/10/93	JAY		change interface to LoadAnim
		 <7>	 7/28/93	JAY		add VRAMIOReq parameter to CopyVRAMPages() and SetVRAMPages()
									for the Dragon Tail release
		 <6>	 6/23/93	JAY		update ccb names and other name changes for Dragon1
		 <5>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
		 <4>	 3/31/93	JAY		change code to call ReadControlPad() for getting state of
									control pad. Doing this for red hardware and cardinal (3B1).
		 <3>	 3/29/93	JAY		¥ use run to launch this app. launch doesn't understand
									arguments. ¥ changed call to LoadImage to check for gBackPic
									being returned instead of 0. ¥ rewrote StartUp to match 3DOOrbit
									startup
		 <2>	 3/18/93	JAY		change lists.h to list.h for Cardinal build
 ...
 */
#define JOYCONTINUOUS  (JOYSTART | JOYUP | JOYDOWN | JOYLEFT | JOYRIGHT | JOYFIREA | JOYFIREB | JOYFIREC)
 ...
 */
{
	...
	if (opened & MATHMASK) DBUG(("MATH Opened \n"));
	...
	while (TRUE)
		...
		if (RedSi) ccb->ccb_Flags |= CCB_ACE; 
		...
		if ( retvalue < 0 ) kprintf( "DrawCels() failed, error=%ld\n", retvalue );
...
/* Returns non-FALSE if user didn't press START, else returns FALSE to quit */
{
	...
	if ( joybits & JOYFIREA ) {
		if ( joybits & JOYLEFT ) {
			...
			gKeysDown = TRUE;
			if (gFrameIncr >= 0) gFrameIncr -= 0x00001000;
			goto DONE;
		}
		else if ( joybits & JOYRIGHT ) {
			if ( gKeysDown == TRUE ) goto DONE;
			gKeysDown = TRUE;
			if (gFrameIncr < 0x00100000) gFrameIncr += 0x00001000;
			goto DONE;
		}
