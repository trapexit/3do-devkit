/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <2>	 7/28/93	JAY		add IO parameter block
   to CopyVRAMPages() and WaitVBL for Dragon Tail release. <1>	 6/25/93
   JAY		first checked in To Do:
*/
...
struct CCB EchoCCB = {
  ...
  CCB_LAST |                   /* last cel in list */
      ...
      PMODE_ONE | CCB_LDPRS |  /* Use HDDX,HDDY Perspective data */
  ...
  */
  /*	PRE0_BGND  |  */ /* no transparency */
      (((CEL_HEIGHT / 2) - PRE0_VCNT_PREFETCH) << PRE0_VCNT_SHIFT)
      |             /* height */
...
ce_Init (ColorEcho *ce)
{
  ...
  ce->ce_HalfDiagonal
      = Sqrt32 ((DISPLAY_WIDTH >> 1) * (DISPLAY_WIDTH >> 1)
  ...
  /*
  ** Copy CCB to DMA type memory in case program is loaded into non-DMAable
  *memory.
  ...
  if (ce->ce_Zoom <= ZOOMONE)
    {
      Item VBLIOReq;
      /*
      ** Draw boxes into target screen so that subsequent graphics draw over
      *the
    ...
    {
      /*
      ** Draw dots into displayed screen which will be the source for the next
      *DrawCel.
      */
      BitMap = sc->sc_BitmapItems[1 - sc->sc_curScreen];
...
ce_DrawNextScreen (ScreenContext *sc, ColorEcho *ce)
{
  ...
  if (ce->ce_XOffset > MAXOFFSET)
    ce->ce_XOffset = MAXOFFSET;
  if (ce->ce_XOffset < -MAXOFFSET)
    ce->ce_XOffset = -MAXOFFSET;
  ce->ce_YOffset += ce->ce_YVelocity;
  if (ce->ce_YOffset > MAXOFFSET)
    ce->ce_YOffset = MAXOFFSET;
  if (ce->ce_YOffset < -MAXOFFSET)
    ce->ce_YOffset = -MAXOFFSET;
  ...
  if (ce->ce_Zoom > MAXZOOM)
    ce->ce_Zoom = MAXZOOM;
  if (ce->ce_Zoom < MINZOOM)
    ce->ce_Zoom = MINZOOM;
  ...
  if (ce->ce_IfSport)
    {
      ...
      CopyVRAMPages (
          VRAMIOReq, sc->sc_Bitmaps[sc->sc_curScreen]->bm_Buffer, /* dest */
  ...
  /* Point CCB to bitmap. */
  ce->ce_CCB->ccb_SourcePtr = (CelData *)GetPixelAddress (
      sc->sc_Screens[(1 - sc->sc_curScreen)], 0, 0);
