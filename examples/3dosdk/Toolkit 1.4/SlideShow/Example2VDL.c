/*
	...
	Contains:	This file contains routines to load and examine 3DO format files.
	...
	Change History (most recent first):
		...
		<10>	 1/18/94	JAY		fix calling sequence due to stricker type checking by 1.6v2
									armcc
		 <9>	 8/20/93	JAY		intergrate changes for VDLs again
		 <8>	 8/11/93	JAY		intergrate changes for VDLs
		 <7>	 7/31/93	JAY		add IO parameters to <Set,Copy>VRAMPages() for dragon tail
									release
		 <6>	 6/24/93	JAY		Updated for 4B1: Changed AllocateVDL, changed parameters to
									SubmitVDL
		 <5>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
		 <4>	  4/5/93	JAY		remove /remote/ reference. use ChangeInitialDirectory()
		 <3>	 3/31/93	JAY		change GetJoystick() to ReadControlPad() for red hardware and
									cardinal
		 <2>	 3/18/93	JAY		changed lists.h to list.h
		 <1>	 3/18/93	JAY		first checked in
				02/18/93	ec		1.1 release. Updated for new libraries and VDL images.  Added VDL 
										diddling options.
 ...
 */
{
	...
	*animName = 0;
	...
	while (TRUE)
		...
		if ( NOT DoJoystick(ccb) ) goto DONE;
		...
		/*	If gSetPMode is true, the active PMode for this Cel will be forced
		 *	to 0. This means that whatever settings were used for PMODE-0
		 *	(opaque, translucent, etc.) will be applied to the entire Cel. If
		 *	gSetPMode is false, the Cel will use the PMODE value from its pixel
		 *	data high bit if it is Uncoded 16 bit or Coded 16 bit or Coded 6 bit.
...
StartUp( void )
{
	...
	{
		if ( AllocateVDL(&gRawVDLPtrArray[screenNdx], gScrnCntxt.sc_Bitmaps[screenNdx] ) != noErr )
			return false;
	}
...
// ******* 
	...
	if ( joybits & JOYFIREB )
	{
		...
		if ( joybits & JOYRIGHT ) 
		{
			...
			SetCEControl(gScrnCntxt.sc_BitmapItems[0], 
				(gScrnCntxt.sc_Bitmaps[0]->bm_CEControl & ~B0POS_MASK) | state,
				B0POS_MASK);
		...
		else if ( joybits & JOYLEFT ) 
		{
			...
			SetCEControl(gScrnCntxt.sc_BitmapItems[0], 
				(gScrnCntxt.sc_Bitmaps[0]->bm_CEControl & ~B15POS_MASK) | b15state,
				B15POS_MASK);
			SetCEControl( gScrnCntxt.sc_BitmapItems[1], 
				(gScrnCntxt.sc_Bitmaps[0]->bm_CEControl & ~B15POS_MASK) | b15state,
				B15POS_MASK);
	...
	if ( joybits & (JOYUP|JOYDOWN|JOYLEFT|JOYRIGHT) )
		...
		if ( joybits & JOYUP )
			...
			{
			if (++gYPos > kScreenHeight) gYPos = kScreenHeight;
			}
		if ( joybits & JOYLEFT )
			{
			gReverse = FALSE;
			...
			{
			gReverse = TRUE;
			if (++gXPos > kScreenWidth) gXPos = kScreenWidth;
			}
...
InitBackPic( char *fileName, ScreenContext *scp )
{
	...
	if ( (rawVDLPtr != NULL) && (rawVDLPtr->chunk_size) ) 
	{
		...
		for ( screenNdx = 0; screenNdx < kScreenCount; screenNdx++ )
			MergeVDL((int32 *)&(rawVDLPtr->vdl[0]), (int32 *)gRawVDLPtrArray[screenNdx]);
	...
	for ( screenNdx = 0; screenNdx < kScreenCount; screenNdx++ )
	{
		...
		gVDLItems[screenNdx] = SubmitVDL(gRawVDLPtrArray[screenNdx], (kVDLSize / 4), VDLTYPE_FULL);
		...
		errorCode = SetVDL(gScrnCntxt.sc_Screens[screenNdx], gVDLItems[screenNdx]);
		if ( errorCode < 0 ) 
		{
...
Usage(char *progName)
{
	...
	kprintf("\n");
	kprintf("\tButton A plus right/left arrow cycles 'focus' through the bits in\n");
	kprintf("\t\tthe VDL control word.\n");
	kprintf("\tButton A plus up/down cycles througth the possible values for the\n");
	kprintf("\t\tVDL control word bit in 'focus'.\n");
	kprintf("\tButton A and B AT THE SAME TIME write the VDL control word into\n");
	kprintf("\t\tthe current screen VDL.\n");
	kprintf("\n");
	kprintf("\tButton B and left arrow cycles through Cel Engine control word\n");
	kprintf("\t\t(CECONTROL) B15POS_MASK values.\n");
	kprintf("\tButton B and right arrow cycles through Cel Engine control word bit\n");
	kprintf("\t\t(CECONTROL) B0POS_PDC values.\n");
	kprintf("\n");
	kprintf("\tButton C and right/left/up/down arrows stretches the cel or anim.\n");
