/*
	...
	Change History (most recent first):
				 ...
				 5/09/94	crm		removed explicit references to GrafBase struct
				 					timing changed from VBL interrupts to audio clock
		...
		if ( (*scanChar == '\r') || (*scanChar == '\n') || (*scanChar == '\0') )
		{
			*scanChar = '\0';
			...
			else if ( fileName[0] == '-' )
			{
				...
				{
					gAutoShowFlag = TRUE;
					gImageVisTime = strtol(&fileName[3], NULL, 0) * kTicksPerSecond;
		...
		else
		{
			switch (cmdLineParam[1]) 
			{
				...
				case 'a':
					...
					kprintf("Auto running mode, each image is shown for %ld seconds.\n", gImageVisTime / kTicksPerSecond);
					break;
...
usage (void)
{
	...
	kprintf (("the image files to load from the Mac.  The text file \"ImageList\" is \n"));
