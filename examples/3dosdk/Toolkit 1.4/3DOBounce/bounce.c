/*
   ...
   Converting to new LoadAnim interface. Much faster loading.
                 <6>	 7/28/93	JAY		add VRAMIOReq parameter
   to SetVRAMPages() and CopyVRAMPages() for Dragon Tail release. <5> 6/24/93
   JAY		removing GetJoystick because it is not called and gf_JoySave
   has gone away with Dragon1
                 <4>	  4/7/93	JAY		MEMTYPE_SPRITE is now
   MEMTYPE_CEL for Cardinal2
                 <3>	  4/5/93	JAY		adding call to
   ChangeInitialDirectory()
                 <2>	 3/31/93	JAY		replace local version
   of GetJoystick() with ReadControlPad( JOYCONTINUOUS ) for real red hardware
