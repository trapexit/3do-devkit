/*
	...
	Contains:	animations with optional
				rotations
				scaling
				symmetry operations  Order of symmetry can be varied from 1 to 16.
	...
	Change History (most recent first):
		 <9>	 5/13/94	MPH		Modified PALsymanim to use the new QueryGraphics() call
		 							instead of looking at GrafBase for determining PAL or NTSC
									mode.
									Replaced references to GrafBase->VRAMPageSize with
									GetPageSize(MEMTYPE_VRAM).
		 <8>	 3/10/94	MPH		modified program to deal with PAL. Used our OpenGraphicsNTSCPAL for display.
		 <7>	 7/28/93	JAY		modify WaitVBL, SetVRAMPages and CopyVRAMPages for Dragon Tail
									release
		 <6>	 6/23/93	JAY		change FindMathFolio to OpenMathFolio
		 <5>	  4/7/93	JAY		MEMTYPE_SPRITE is now MEMTYPE_CEL for Cardinal2
		 <4>	  4/5/93	JAY		remove /remote/ from filename (i.e. make pathname relative to
									initial working directory
		 <2>	 3/18/93	JAY		change lists.h to list.h
...
/* if it doesn't work, so what? */
  ...
  prev = NULL;
	...
	spr = LoadSprite("PALvspics/testd50.cel", Convert32_F16(1),Convert32_F16(2), 0x00002000, Convert32_F16(2)); 
	spr = LoadSprite("PALvspics/testo50.cel", Convert32_F16(2),Convert32_F16(2), 0x00002800, Convert32_F16(3));  
	spr = LoadSprite("PALvspics/test3.cel", Convert32_F16(1), Convert32_F16(1), 0x00001000, Convert32_F16(1)); 
...
//  WaitVBL();
  ...
  for (;;) {
    ...
    curJoy = ReadControlPad(0 );
    if ( curJoy != 0) frames = 0; else frames++;
    ...
    if ((curJoy & JOYFIREB) ) {
		...
		if (frames >= 7200) { frames = 0; zscaling = 0; rotating = 0; }
		if (frames >= 3600) rotating = 1;
		if (frames >=  5400) zscaling = 1;
		if (frames >=  6900) {rotating = 2; zscaling =0;}
#else
		MoveSprites();
#endif
