/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                <8>		3/10/94		MPH First PALonly Release <7>
   8/15/93	JAY		change LoadAnim interface to new faster
   interface
                 <6>	 7/28/93	JAY		add IO Req parameter to
   SetVRAMPages() and CopyVRAMPages() for Dragon Tail release.
                 <5>	 6/24/93	JAY		change FindMathFolio to
   OpenMathFolio <4>	  4/7/93	JAY		MEMTYPE_SPRITE is now
   MEMTYPE_CEL for Cardinal2 <3>	  4/5/93	JAY		add
   ChangeInitialDirectory()
                 <2>	  4/3/93	JAY		Removed custom joypad
   routine. Now call ReadControlPad() in Lib3DO.lib. This was done for Real
   Red. <1>	  4/3/93	JAY		first checked in 3/31/93
   RDG		added a (gasp) cleanup routine to unload the samplers 3/25/93
   RDG		finished adding calls to sound module
...
OpenGraphicsNTSCPAL (ScreenContext *sc, uint32 nScreens)
{
  ...
  i = QueryGraphics (QUERYGRAF_TAG_FIELDFREQ, &fieldFreq);
  if (i < 0)
    {
      DIAGNOSE (("Error detecting field frequency (Error = %ld)\n", i));
      goto EXIT_MyOpenGraphics;
    }
  ...
  height = NTSC_HEIGHT;
  diType = NTSC_TYPE;
  if (fieldFreq == PAL_FREQUENCY)
    {
      width = PAL_WIDTH;
      height = PAL_HEIGHT;
      diType = PAL_TYPE;
    }
  else if (fieldFreq != NTSC_FREQUENCY)
    {
  ...
  i = GetPageSize (MEMTYPE_VRAM); //	NO! GrafBase->gf_VRAMPageSize;
  ...
  while (TRUE)
    {
      ...
      if (gBallYPos <= -40)
        ballYSpeed = 1;
      if (gBallYPos > -40 && gBallYPos <= -20)
        ballYSpeed = 2;
      if (gBallYPos > 0 && gBallYPos <= 10)
        ballYSpeed = 3;
      if (gBallYPos > 10 && gBallYPos <= 20)
        ballYSpeed = 4;
      if (gBallYPos > 20 && gBallYPos <= 40)
        ballYSpeed = 5;
      if (gBallYPos > 40 && gBallYPos <= 60)
        ballYSpeed = 6;
      if (gBallYPos > 60 && gBallYPos <= 80)
        ballYSpeed = 7;
      if (gBallYPos > 80 && gBallYPos <= 100)
        ballYSpeed = 8;
      if (gBallYPos > 100 && gBallYPos <= 120)
        ballYSpeed = 10;
      if (gBallYPos > 120 && gBallYPos <= 140)
        ballYSpeed = 12;
      if (gBallYPos > 140 && gBallYPos <= 160)
        ballYSpeed = 14;
      if (gBallYPos > 160 && gBallYPos <= 180)
        ballYSpeed = 16;
      if (gBallYPos > 180)
        ballYSpeed = 18;
              ...
              if (gCollision)
                {
                  if ((gBallXPos + 5 >= gTvXPos + (ccbTv->ccb_Width - 15))
                      ...
                      && (gBallYPos + (ccbBall->ccb_Height - 15)
                          >= gTvYPos + 5)
      ...
      /*		if( gBallXPos <= 20 )
         ballScale = -16; if( gBallXPos >  20 && gBallXPos <=  30 ) ballScale =
         -14; if( gBallXPos >  30 && gBallXPos <=  50 ) ballScale = -12; if(
         gBallXPos >  50 && gBallXPos <=  70 ) ballScale = -10; if( gBallXPos >
         70 && gBallXPos <=  90 ) ballScale = -8; if( gBallXPos >  90 &&
         gBallXPos <= 110 ) ballScale = -6; if( gBallXPos > 125 && gBallXPos <=
         125 ) ballScale = -4; if( gBallXPos > 135 && gBallXPos <= 145 )
         ballScale = -2; if( gBallXPos > 145 && gBallXPos <= 155 ) ballScale =
         0; if( gBallXPos > 155 && gBallXPos <= 165 ) ballScale = -2; if(
         gBallXPos > 165 && gBallXPos <= 180 ) ballScale = -4; if( gBallXPos >
         180 && gBallXPos <= 200 ) ballScale = -6; if( gBallXPos > 200 &&
         gBallXPos <= 220 ) ballScale = -8; if( gBallXPos > 220 && gBallXPos <=
         240 ) ballScale = -10; if( gBallXPos > 240 && gBallXPos <= 260 )
         ballScale = -12; if( gBallXPos > 260 )
         ballScale = -14;
      */
      ...
      if (gLightON)
        {
          if (gBallXPos <= 20)
            ballSwSpeed = -12;
          if (gBallXPos > 20 && gBallXPos <= 40)
            ballSwSpeed = -10;
          if (gBallXPos > 40 && gBallXPos <= 60)
            ballSwSpeed = -8;
          if (gBallXPos > 60 && gBallXPos <= 80)
            ballSwSpeed = -6;
          if (gBallXPos > 80 && gBallXPos <= 100)
            ballSwSpeed = -4;
          if (gBallXPos > 100 && gBallXPos <= 120)
            ballSwSpeed = -3;
          if (gBallXPos > 120 && gBallXPos <= 140)
            ballSwSpeed = -2;
          if (gBallXPos > 140 && gBallXPos <= 160)
            ballSwSpeed = -1;

          if (gBallXPos > 160 && gBallXPos <= 180)
            ballSwSpeed = 1;
          if (gBallXPos > 180 && gBallXPos <= 200)
            ballSwSpeed = 2;
          if (gBallXPos > 200 && gBallXPos <= 220)
            ballSwSpeed = 3;
          if (gBallXPos > 220 && gBallXPos <= 240)
            ballSwSpeed = 4;
          if (gBallXPos > 240 && gBallXPos <= 260)
            ballSwSpeed = 6;
          if (gBallXPos > 260 && gBallXPos <= 280)
            ballSwSpeed = 8;
          if (gBallXPos > 280 && gBallXPos <= 300)
            ballSwSpeed = 10;
          if (gBallXPos > 300)
            ballSwSpeed = 12;
          ...
          if (aCorner[2].pt_Y >= 170)
            aCorner[2].pt_Y = 180;
          if (aCorner[3].pt_Y >= 170)
            aCorner[3].pt_Y = 180;

          if (aCorner[0].pt_X <= 40)
            aCorner[0].pt_X = 40;
          if (aCorner[1].pt_X <= 40)
            aCorner[1].pt_X = 40;
          if (aCorner[2].pt_Y <= -40)
            aCorner[2].pt_X = -40;
          if (aCorner[3].pt_Y <= -40)
            aCorner[3].pt_X = -40;
          if (aCorner[0].pt_X >= 280)
            aCorner[0].pt_X = 280;
          if (aCorner[1].pt_X >= 280)
            aCorner[1].pt_X = 280;
          if (aCorner[2].pt_Y >= 280)
            aCorner[2].pt_X = 280;
          if (aCorner[3].pt_Y >= 280)
            aCorner[3].pt_X = 280;
        }
      ...
      if (gGlobeYPos <= 10)
        globeYSpeed = 1;
      if (gGlobeYPos > 10 && gGlobeYPos <= 35)
        globeYSpeed = 2;
      if (gGlobeYPos > 35 && gGlobeYPos <= 60)
        globeYSpeed = 3;
      if (gGlobeYPos > 60 && gGlobeYPos <= 80)
        globeYSpeed = 4;
      if (gGlobeYPos > 80 && gGlobeYPos <= 100)
        globeYSpeed = 5;
      if (gGlobeYPos > 100 && gGlobeYPos <= 120)
        globeYSpeed = 6;
      if (gGlobeYPos > 120 && gGlobeYPos <= 140)
        globeYSpeed = 7;
      if (gGlobeYPos > 140 && gGlobeYPos <= 160)
        globeYSpeed = 9;
      if (gGlobeYPos > 160 && gGlobeYPos <= 180)
        globeYSpeed = 11;
      if (gGlobeYPos > 180)
        globeYSpeed = 13;
      ...
      if (gBouncing)
        {
          if (globeUpDir)
            {
              ...
              if (gCollision)
                {
                  if ((gGlobeXPos + (ccbGlobe->ccb_Height - 5)
                       >= gBallXPos + 15)
                      && (gGlobeXPos + 5
                          <= gBallXPos + (ccbBall->ccb_Width - 15))
                      && (gGlobeYPos + 5
                          >= gBallYPos + (ccbBall->ccb_Height - 15))
                      && (gGlobeYPos + 5
                          <= gBallYPos + (ccbBall->ccb_Height - 5)))
                    {
                      globeUpDir = FALSE;
                      ballUpDir = TRUE;
              ...
              if (gCollision)
                {
                  if ((gGlobeXPos + (ccbGlobe->ccb_Height - 5)
                      ...
                      && (gGlobeXPos + 5
                          <= gBallXPos + (ccbBall->ccb_Width - 15))
                      && (gGlobeYPos + (ccbBall->ccb_Height - 5)
                          >= gBallYPos + 5)
                      && (gGlobeYPos + (ccbBall->ccb_Height - 5)
                          <= gBallYPos + 15))
                    {
          ...
          if (globeLftDir)
              ...
              if (gCollision)
                {
                  if ((gGlobeXPos + 5 >= gBallXPos + (ccbBall->ccb_Width - 15))
                      && (gGlobeXPos + 5
                          <= gBallXPos + (ccbBall->ccb_Width - 5))
                      && (gGlobeYPos + (ccbGlobe->ccb_Height - 15)
                          >= gBallYPos + 5)
                      && (gGlobeYPos + 15
                          <= gBallYPos + (ccbBall->ccb_Height - 5)))
                    {
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if ((gGlobeXPos + (ccbGlobe->ccb_Width - 5)
                       <= gBallXPos + 15)
                      && (gGlobeXPos + (ccbGlobe->ccb_Width - 5)
                          >= gBallXPos + 5)
                      && (gGlobeYPos + (ccbGlobe->ccb_Height - 15)
                          >= gBallYPos + 5)
                      && (gGlobeYPos + 15
      ...
      if (gLightON)
        {
          if (gGlobeXPos <= 20)
            globeSwSpeed = -12;
          if (gGlobeXPos > 20 && gGlobeXPos <= 40)
            globeSwSpeed = -10;
          if (gGlobeXPos > 40 && gGlobeXPos <= 60)
            globeSwSpeed = -8;
          if (gGlobeXPos > 60 && gGlobeXPos <= 80)
            globeSwSpeed = -6;
          if (gGlobeXPos > 80 && gGlobeXPos <= 100)
            globeSwSpeed = -4;
          if (gGlobeXPos > 100 && gGlobeXPos <= 120)
            globeSwSpeed = -3;
          if (gGlobeXPos > 120 && gGlobeXPos <= 140)
            globeSwSpeed = -2;
          if (gGlobeXPos > 140 && gGlobeXPos <= 160)
            globeSwSpeed = -1;

          if (gGlobeXPos > 160 && gGlobeXPos <= 180)
            globeSwSpeed = 1;
          if (gGlobeXPos > 180 && gGlobeXPos <= 200)
            globeSwSpeed = 2;
          if (gGlobeXPos > 200 && gGlobeXPos <= 220)
            globeSwSpeed = 3;
          if (gGlobeXPos > 220 && gGlobeXPos <= 240)
            globeSwSpeed = 4;
          if (gGlobeXPos > 240 && gGlobeXPos <= 260)
            globeSwSpeed = 6;
          if (gGlobeXPos > 260 && gGlobeXPos <= 280)
            globeSwSpeed = 8;
          if (gGlobeXPos > 280 && gGlobeXPos <= 300)
            globeSwSpeed = 10;
          if (gGlobeXPos > 300)
            globeSwSpeed = 12;
      ...
      if (gTvYPos <= 60)
        tvYSpeed = 1;
      if (gTvYPos > 60 && gTvYPos <= 75)
        tvYSpeed = 2;
      if (gTvYPos > 75 && gTvYPos <= 90)
        tvYSpeed = 3;
      if (gTvYPos > 90 && gTvYPos <= 105)
        tvYSpeed = 4;
      if (gTvYPos > 105 && gTvYPos <= 120)
        tvYSpeed = 5;
      if (gTvYPos > 120 && gTvYPos <= 135)
        tvYSpeed = 6;
      if (gTvYPos > 135 && gTvYPos <= 150)
        tvYSpeed = 7;
      if (gTvYPos > 150 && gTvYPos <= 165)
        tvYSpeed = 9;
      if (gTvYPos > 165 && gTvYPos <= 180)
        tvYSpeed = 11;
      if (gTvYPos > 180)
        tvYSpeed = 13;
              ...
              if (gCollision)
                {
                  if ((gTvXPos + (ccbTv->ccb_Height - 5) >= gCubeXPos + 15)
                      ...
                      && (gTvYPos + 5
                          >= gCubeYPos + (ccbCube->ccb_Height - 15))
                      && (gTvYPos + 5
                          <= gCubeYPos + (ccbCube->ccb_Height - 5)))
                    {
                      tvUpDir = FALSE;
                      cubeUpDir = TRUE;
      ...
      if (gLightON)
        {
          if (gTvXPos <= 20)
            tvSwSpeed = -12;
          if (gTvXPos > 20 && gTvXPos <= 40)
            tvSwSpeed = -10;
          if (gTvXPos > 40 && gTvXPos <= 60)
            tvSwSpeed = -8;
          if (gTvXPos > 60 && gTvXPos <= 80)
            tvSwSpeed = -6;
          if (gTvXPos > 80 && gTvXPos <= 100)
            tvSwSpeed = -4;
          if (gTvXPos > 100 && gTvXPos <= 120)
            tvSwSpeed = -3;
          if (gTvXPos > 120 && gTvXPos <= 140)
            tvSwSpeed = -2;
          if (gTvXPos > 140 && gTvXPos <= 160)
            tvSwSpeed = -1;

          if (gTvXPos > 160 && gTvXPos <= 180)
            tvSwSpeed = 1;
          if (gTvXPos > 180 && gTvXPos <= 200)
            tvSwSpeed = 2;
          if (gTvXPos > 200 && gTvXPos <= 220)
            tvSwSpeed = 3;
          if (gTvXPos > 220 && gTvXPos <= 240)
            tvSwSpeed = 4;
          if (gTvXPos > 240 && gTvXPos <= 260)
            tvSwSpeed = 6;
          if (gTvXPos > 260 && gTvXPos <= 280)
            tvSwSpeed = 8;
          if (gTvXPos > 280 && gTvXPos <= 300)
            tvSwSpeed = 10;
          if (gTvXPos > 300)
            tvSwSpeed = 12;
          ...
          if (aCorner[2].pt_Y >= 190)
            aCorner[2].pt_Y = 190;
          if (aCorner[3].pt_Y >= 190)
            aCorner[3].pt_Y = 190;
      ...
      if (gCubeYPos <= 10)
        cubeYSpeed = 1;
      if (gCubeYPos > 10 && gCubeYPos <= 35)
        cubeYSpeed = 2;
      if (gCubeYPos > 35 && gCubeYPos <= 60)
        cubeYSpeed = 4;
      if (gCubeYPos > 60 && gCubeYPos <= 80)
        cubeYSpeed = 6;
      if (gCubeYPos > 80 && gCubeYPos <= 100)
        cubeYSpeed = 8;
      if (gCubeYPos > 100 && gCubeYPos <= 120)
        cubeYSpeed = 10;
      if (gCubeYPos > 120 && gCubeYPos <= 140)
        cubeYSpeed = 12;
      if (gCubeYPos > 140 && gCubeYPos <= 160)
        cubeYSpeed = 14;
      if (gCubeYPos > 160 && gCubeYPos <= 180)
        cubeYSpeed = 16;
      if (gCubeYPos > 180)
        cubeYSpeed = 18;
          ...
          if (cubeUpDir)
            {
              ...
              if (gCollision)
                {
                  if ((gCubeXPos + (ccbCube->ccb_Height - 5)
                       >= gGlobeXPos + 15)
                      && (gCubeXPos + 5
                          <= gGlobeXPos + (ccbGlobe->ccb_Width - 15))
                      && (gCubeYPos + 5
                          >= gGlobeYPos + (ccbGlobe->ccb_Height - 15))
                      && (gCubeYPos + 5
                          <= gGlobeYPos + (ccbGlobe->ccb_Height - 5)))
                    {
                      cubeUpDir = FALSE;
                      globeUpDir = TRUE;
              ...
              if (gCollision)
                {
                  if ((gCubeXPos + (ccbCube->ccb_Height - 5)
                      ...
                      && (gCubeXPos + 5
                          <= gGlobeXPos + (ccbGlobe->ccb_Width - 15))
                      && (gCubeYPos + (ccbGlobe->ccb_Height - 5)
                          >= gGlobeYPos + 5)
                      && (gCubeYPos + (ccbGlobe->ccb_Height - 5)
                          <= gGlobeYPos + 15))
                    {
      ...
      if (gLightON)
        {
          if (gCubeXPos <= 20)
            cubeSwSpeed = -12;
          if (gCubeXPos > 20 && gCubeXPos <= 40)
            cubeSwSpeed = -10;
          if (gCubeXPos > 40 && gCubeXPos <= 60)
            cubeSwSpeed = -8;
          if (gCubeXPos > 60 && gCubeXPos <= 80)
            cubeSwSpeed = -6;
          if (gCubeXPos > 80 && gCubeXPos <= 100)
            cubeSwSpeed = -4;
          if (gCubeXPos > 100 && gCubeXPos <= 120)
            cubeSwSpeed = -3;
          if (gCubeXPos > 120 && gCubeXPos <= 140)
            cubeSwSpeed = -2;
          if (gCubeXPos > 140 && gCubeXPos <= 160)
            cubeSwSpeed = -1;

          if (gCubeXPos > 160 && gCubeXPos <= 180)
            cubeSwSpeed = 1;
          if (gCubeXPos > 180 && gCubeXPos <= 200)
            cubeSwSpeed = 2;
          if (gCubeXPos > 200 && gCubeXPos <= 220)
            cubeSwSpeed = 3;
          if (gCubeXPos > 220 && gCubeXPos <= 240)
            cubeSwSpeed = 4;
          if (gCubeXPos > 240 && gCubeXPos <= 260)
            cubeSwSpeed = 6;
          if (gCubeXPos > 260 && gCubeXPos <= 280)
            cubeSwSpeed = 8;
          if (gCubeXPos > 280 && gCubeXPos <= 300)
            cubeSwSpeed = 10;
          if (gCubeXPos > 300)
            cubeSwSpeed = 12;
      ...
      //@@@@@@@@@@@@@@@@@@@@@
      if (gLightON)
        CopyVRAMPages (VRAMIOReq, sc->sc_Bitmaps[gScreenSelect]->bm_Buffer,
                       gWallOnPict, sc->sc_nFrameBufferPages, -1);
      else
        CopyVRAMPages (VRAMIOReq, sc->sc_Bitmaps[gScreenSelect]->bm_Buffer,
                       gWallOffPict, sc->sc_nFrameBufferPages, -1);
      ...
      if (gBouncing)
        {
          if (gLightON)
            {
              retvalue
                  = DrawCels (sc->sc_BitmapItems[gScreenSelect], ccbBallS);
              retvalue
                  = DrawCels (sc->sc_BitmapItems[gScreenSelect], ccbBallR);
          ...
          if (gLightON)
            {
              retvalue
                  = DrawCels (sc->sc_BitmapItems[gScreenSelect], ccbCubeS);
              retvalue
                  = DrawCels (sc->sc_BitmapItems[gScreenSelect], ccbCubeR);
          ...
          if (gLightON)
            {
              retvalue
                  = DrawCels (sc->sc_BitmapItems[gScreenSelect], ccbGlobeS);
              retvalue
                  = DrawCels (sc->sc_BitmapItems[gScreenSelect], ccbGlobeR);
              retvalue
                  = DrawCels (sc->sc_BitmapItems[gScreenSelect], ccbGlobe);
            }
          else
            {
              retvalue
          ...
          if (!globeLftDir)
            {
              if (gLightON)
                {
                  retvalue = DrawCels (sc->sc_BitmapItems[gScreenSelect],
                                       ccbGlobeS);
                  retvalue = DrawCels (sc->sc_BitmapItems[gScreenSelect],
                                       ccbGlobeR);
                  retvalue
