...
|||	jsintmusicthread - Demonstrates starting background music as a thread
(a
**/
...
void
JSPlayBgndMusic (void)
/*
        This function is identical to the main function in jsplaybgndmusic.c,
   with the full soundfile pathname specified and the return statement omitted.
*/
{
  int32 Result;
  ...
  /*
              Play the stereo sample in an endless loop (the "JSData" directory
     is expected to be present in the executable's directory)
      */
...
*/
{
  ...
  i = (PROJECTED (gUFOCel->ccb_Width) >> 1)
      + ConvertF16_32 (gUFOCel->ccb_XPos);
  i = ((i - 160) << 1) / 21;
  dist = gDistance / 307;
  if (i < 0)
    {
...
bool
Zap (CCB *aTarget, Coord X, Coord Y)
/*
...
*/
{
  ...
  return ((X >= 0) && (X < PROJECTED (aTarget->ccb_Width)) && (Y >= 0)
          && (Y < PROJECTED (aTarget->ccb_Height)));
}

#define kContinuousMask                                                       \
  (ControlA | ControlB | ControlC | ControlRight | ControlLeft | ControlDown  \
   | ControlUp)
      ...
      if (controlBits & ControlStart)
        {
          ...
          {
            TagArg taStartArgs[] = { { AF_TAG_FREQUENCY, (void *)0x8000 },
...
*/
{
  ...
  DoControlPad (
      1, &controlBits,
      (ControlA | ControlRight | ControlLeft | ControlDown | ControlUp));
      ...
      if (controlBits & ControlA && !gLaserCount)
        {
          ...
          if (Zap (gUFOCel,
            ...
            /*
                    Make the explosion sound and animation emanate from the
               point of impact
            */
            {
              int32 Balance;
              ...
              Balance = ((gCrosshairsCel->ccb_XPos
                        ...
                        / 160;
              Volume
                  = Convert32_F16 ((UFO_MAXDISTANCE - gDistance) >> 1) / 3000;
              ehSetChannelLevels (
              ...
              gExplosionCel->ccb_XPos
                  = gCrosshairsCel->ccb_XPos
                    + Convert32_F16 (gCrosshairsCel->ccb_Width >> 1)
                    - ((gExplosionCel->ccb_Width >> 1) * gUFOCel->ccb_VDY);
              gExplosionCel->ccb_YPos
                  = gCrosshairsCel->ccb_YPos
                    + Convert32_F16 (gCrosshairsCel->ccb_Height >> 1)
                    - ((gExplosionCel->ccb_Height >> 1) * gUFOCel->ccb_VDY);
...
void
TargetAction (CCB *aTargetCCB)
/*
        Animate the UFO: If it's still exploding, play the explosion's next
   frame, otherwise display the UFO (after a nominal post-explosion delay).
*/
{
  ...
  iTest = ConvertF16_32 (aTargetCCB->ccb_XPos)
          + PROJECTED (aTargetCCB->ccb_Width);
...

  {
    ...
    iTest -= ConvertF16_32 (aTargetCCB->ccb_XPos);
    Balance
        = ((aTargetCCB->ccb_XPos + (Convert32_F16 (iTest) >> 1)) >> 2) / 160;
    Volume
        = Convert32_F16 ((UFO_MAXDISTANCE - gDistance) >> 1) / UFO_MAXDISTANCE;
...
*/
{
  ...
  if ((gBackgroundImage = (ubyte *)LoadImage ("jsdata/art/sky.imag", NULL,
                                              NULL, gScreenContext))
      == NULL)
    {
  ...
  if ((gPleaseWaitCel = LoadCel ("jsdata/art/pleasewait.cel", MEMTYPE_CEL))
      == NULL)
    {
  ...
  if ((gBackgroundMusic
       = CreateThread ("JSPlayBgndMusic", CURRENT_TASK_PRIORITY + 1,
                       JSPlayBgndMusic, 4000))
      < 0)
    {
  ...
  if ((gCrosshairsCel = LoadCel ("jsdata/art/crosshairs.cel", MEMTYPE_CEL))
      == NULL)
    {
  ...
  */
  ...
  /*
      ...
      */
  if (ehLoadSoundEffect (&gWhirrEffect, gMixerInfo, "jsdata/sound/whirr.aiff",
                         0)
      < 0)
    {
  ...
  if (ehLoadSoundEffect (&gZapEffect, gMixerInfo, "jsdata/sound/zap.aiff", 2)
      < 0)
    {
  ...
  gCrosshairsCel->ccb_XPos
      = Convert32_F16 ((DISPLAY_WIDTH >> 1) - gCrosshairsCel->ccb_Width);
  gCrosshairsCel->ccb_YPos
      = Convert32_F16 ((DISPLAY_HEIGHT >> 1) - gCrosshairsCel->ccb_Height);
