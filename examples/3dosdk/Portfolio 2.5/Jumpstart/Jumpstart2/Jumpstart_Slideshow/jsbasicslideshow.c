...
|||	jsbasicslideshow - Demonstrates a basic slideshow (a Jumpstart
program).
|||
|||	  Synopsis
|||
...
|||
|||	    * Respond to control pad input: Right and down arrows mean show
next image
|||	      in list Left and up arrows mean show previous image in list Start
button
|||	      means quit the program B button means show the other buffer C
button means
...
#if DEBUG
  image = LoadImage (filename, gImage[whichBuffer], (VdlChunk **)NULL,
                     gScreenContext);
  if (image == NULL)
#else
  if (LoadImage (filename, gImage[whichBuffer], (VdlChunk **)NULL,
                 gScreenContext)
      == NULL)
#endif
    {
    ...
    for (imageIndex = 0; imageIndex < NUM_BUFFERS; imageIndex++)
      {
        ...
        SetVRAMPages (gVRAMIOReq, gImage[imageIndex], 0,
