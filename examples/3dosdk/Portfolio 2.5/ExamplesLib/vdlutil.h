...
#define CLUT_COLOR_WORDS ((sizeof (VDL_REC) / sizeof (uint32)) - CONTROL_WORDS)
  ...
  typedef struct
  {
    ...
    unsigned int
        captureSlipStream : 1; /* [   22] SlipStream capture enabled */
    ...
    unsigned int scanLines : 9;     /* [ 8- 0] # of scan lines to wait before
                                       loading next VDL */
  } DMAControlWord;
  ...
  typedef struct
  {
    ...
    unsigned int controlWordType : 1; /* [   30] set to one for a display
                                         control word */
    unsigned int
        backgroundCLUT : 1; /* [   29] set background color registers */
    ...
    unsigned int
        backgroundDetector : 1;       /* [   22] enable background detector */
    unsigned int swapWindowPenHV : 1; /* [   21] swap H and V bits in pen
                                         numbers in window */
    unsigned int windowVSource : 2;   /* [20-19] selects source for V
                                         sub-positions in window pixels */
    unsigned int windowHSource : 2;   /* [18-17] selects source for H
                                         sub-position in window pixels */
    ...
    unsigned int windowHorizontalInterp : 1; /* [   13] enable horizontal
                                                interpolation in window */
    unsigned int
        bypassLSBIsRandom : 1; /* [   12] enable random number generator for
                                  low 3 of CLUT bypass */
    unsigned int windowMSBReplicate : 1; /* [   11] */
    unsigned int swapPenVH : 1; /* [   10] swap H and V bits in pen number */
    unsigned int
        VSource : 2; /* [ 9- 8] selects source of vertical sub-position */
    unsigned int
        HSource : 2; /* [ 7- 6] selects source of horizontal sub-position */
    unsigned int blueLSBSource : 2; /* [ 5- 4] selects source of blue pen
                                       number's LSB */
    unsigned int
        verticalInterpolation : 1; /* [    3] enable vertical interpolation */
    unsigned int horizontalInterpolation : 1; /* [    2] enable horizontal
                                                 interpolation */
    unsigned int colorsOnly : 1; /* [    1] reserved; set to zero */
    unsigned int
        noVerticalInterpLine : 1; /* [    0] suppress vertical interpolation
                                     for this line only */
  } VDLDisplayCtlWord;

  int32 SetDisplayCtlWord (Boolean Hinterp, Boolean Vinterp,
                           Boolean clutBypass);
  int32 SetDMACtlWord (int32 clutColorWords, int32 scanLineCount,
                       Boolean lastScanLine, int32 displayMode);
  void InitVDL (VDLEntry *vdlPtr, Bitmap *destBitmap, int32 displayMode);
  void MergeVDL (VDL_REC *rawVDLPtr, VDL_REC *activeVDLPtr, uint32 vdlLines,
                 int32 displayMode);
  ...
  int32 AllocateVDL24 (VDLEntry **newVDL, Bitmap *screenBitMap,
                       int32 displayMode);
  void MergeVDL24 (VDL_REC *rawVDLPtr, VDL_REC *activeVDLPtr, uint32 vdlLines,
                   int32 displayMode);
