...
|||	    sound effects and assemble their player apparati but not
immediately
...
|||	    sample is encountered, it connects the left output to the mixer
rather
|||	    than returning an error (a diagnostic message is printed to the
debug
|||	    terminal, however).
|||
|||	  Associated Files
...
Err _ehAcquireGainKnob (Item mixer, int32 channel, TOutput leftRight,
                        Item *knob);
void _ehGetGainKnobName (char *knobName, TOutput leftRight, int32 channel);
void _ehReleaseKnob (Item *knob);
void _ehSplitVolume (int32 volume, int32 balance, int32 *leftLevel,
                     int32 *rightLevel);

/*** Effects-handling helper functions: ***/

void
_ehGetGainKnobName (char *knobName, TOutput leftRight, int32 channel)
{
...
Item
_ehSetupMixer (const char *mixerName)
/*
...
void
_ehDisposeSample (Item sample, Item samplePlayer)
{
...
Err
_ehAcquireGainKnob (Item mixer, int32 channel, TOutput leftRight, Item *knob)
{
  Err status;
...
void
_ehSplitVolume (int32 volume, int32 balance, int32 *leftLevel,
                int32 *rightLevel)
/*
...
|||	  Description
|||
|||	    This function disposes or releases the resources referenced in a
MixerInfo, and
...
|||	  Caveats
|||
|||		The MixerInfo should have been initialized via a prior call to
ehNewMixerInfo.
|||		After calling this function, the application should no longer
use the passed-in pointer.
|||
|||	  See Also
|||
|||	    ehNewMixerInfo()
...
**/
void
ehDisposeMixerInfo (pTMixerInfo pMixerInfo)
{
  if (pMixerInfo != NULL)
    {
...
|||	  Synopsis
|||
|||	    Err ehNewMixerInfo( pTMixerInfo *pNewMixerInfo, int32 channelsUsed,
const char *mixerName )
|||
|||	  Description
|||
|||	    This function creates a MixerInfo struct and initializes a mixer
for playing
|||		subsequently-loaded samples.
|||
|||	  Arguments
|||
|||	    pNewMixerInfo	A pointer to the variable referencing the newly
allocated
|||	                    MixerInfo struct. If the allocation fails, the
variable is
|||	                    set to NULL.
|||
|||	    channelsUsed	number of mixer inputs the application intends
to use
|||
...
|||	  Return Value
|||
|||	    Zero if all operations were successful, otherwise an error code (a
negative
|||	    value).
|||
|||
...
|||	  Caveats
|||
|||		Remember to call ehDisposeMixerInfo when you're through with
the
|||		MixerInfo.
|||
|||
...
**/
Err
ehNewMixerInfo (pTMixerInfo *pNewMixerInfo, int32 channelsUsed,
                const char *mixerName)
{
  Err status = -1;

  pTMixerInfo pMixerInfo = (pTMixerInfo)AllocMem (
      sizeof (TMixerInfo), MEMTYPE_ANY | MEMTYPE_FILL | 0x00);
  if (pMixerInfo == NULL)
    {
      status = EHNOMEM_ERR;
...
/**
|||	ehNewSampleInfo - Create a SampleInfo and initialize it with safe
values.
|||
|||	  Synopsis
|||
...
|||	  Description
|||
|||	    This function allocates memory for a SampleInfo struct and
initializes it
|||		so that it can be associated with a sound effect, and later
safely disposed.
|||
|||	  Arguments
|||
...
|||	  Return Value
|||
|||	    A pointer to the newly allocated SampleInfo struct.  If the
allocation fails,
|||		NULL is returned.
|||
|||
|||	  Implementation
|||
|||	    Call this function if you want to allocate a SampleInfo, but don't
need to
|||		immediately set up its player or connect the player to the
output instrument (mixer).
|||
|||
|||	  Caveats
|||
|||		If you want to fully initialize a player for a sample and
connect it to an output
|||		instrument, use the convenience routine ehLoadSoundEffect.
|||
|||		Remember to call ehDisposeSampleInfo when you're through with
the
|||		SampleInfo.
|||
|||		Remember to call ehNewMixerInfo so you can play sound effects
through
...
**/
pTSampleInfo
ehNewSampleInfo (void)
{
  pTSampleInfo pSampleInfo = (pTSampleInfo)AllocMem (
      sizeof (TSampleInfo), MEMTYPE_ANY | MEMTYPE_FILL | 0x00);
  if (pSampleInfo != NULL)
    {
...
/**
|||	ehDisposeSampleInfo - Dispose of resources allocated by
ehNewSampleInfo.
|||
|||	  Synopsis
|||
...
|||	  Description
|||
|||	    This function disposes or release resources referenced in a
SampleInfo struct,
...
|||	  Caveats
|||
|||		The SampleInfo must be allocated by a call to ehNewSampleInfo
(or
...
**/
void
ehDisposeSampleInfo (pTSampleInfo pSampleInfo)
{
  if (pSampleInfo != NULL)
    {
...
|||	  Synopsis
|||
|||	    Err ehSetupSamplePlayer( pTSampleInfo pSampleInfo, const char
*sampleName )
|||
|||	  Description
|||
|||	    This function prepares all resources required to play a sample, but
does
|||		not connect the sample player to the output instrument (mixer).
The
|||		SampleInfo struct is filled in with references to the
resources.
|||
...
|||		a negative number if an error occurred (for example, the sample
was
|||		not found).
|||
|||
|||	  Implementation
|||
|||	    Call this function if you want to prepare a player for a sample,
but
|||		don't need to immediately connect it to an output instrument
(mixer).
|||
|||
|||	  Caveats
...
**/
Err
ehSetupSamplePlayer (pTSampleInfo pSampleInfo, const char *sampleName)
{
  Item aSample;
...
/**
|||	ehDisconnectSamplePlayer - Disconnect a sample player from an output
instrument (mixer).
|||
|||	  Synopsis
|||
|||	    Err ehDisconnectSamplePlayer( pTMixerInfo pMixerInfo, pTSampleInfo
pSampleInfo )
|||
|||	  Description
|||
|||	    This function disconnects the sample player instrument referenced
in the SampleInfo from the output
|||		instrument (mixer) referenced in the MixerInfo.  Another player
instrument can then be connected
|||	    to the mixer in the same channel.
|||
|||	  Arguments
|||
|||	    pMixerInfo		pointer to information about the output
instrument (mixer).
|||
|||		pSampleInfo		pointer to information about the
sample.
|||
|||	  Return Value
|||
|||	    Returns 0 if the sample player instrument was successfully
disconnected, or
...
|||	  Implementation
|||
|||	    Call this function when you want to disconnect a sample player from
an output instrument.
|||
|||
|||	  Caveats
|||
|||		The SampleInfo should be connected to the output instrument by
a call to
|||	    ehConnectSamplePlayer (or the convenience function
ehLoadSoundEffect).
|||
...
**/
Err
ehDisconnectSamplePlayer (pTMixerInfo pMixerInfo, pTSampleInfo pSampleInfo)
{
  Err status = -1;
  int32 numOutputs = 0;
  ...
  {
    TagArg getInfoTags[] = { { AF_TAG_CHANNELS, 0 }, { TAG_END, 0 } };
  ...
  switch (numOutputs)
    {
    case 1:
      DisconnectInstruments (pSampleInfo->si_Player, "Output", mixer,
                             gChannelName[channel]);
      break;

    case 2:
      DisconnectInstruments (pSampleInfo->si_Player, "LeftOutput", mixer,
                             gChannelName[channel]);
      break;
...
/**
|||	ehConnectSamplePlayer - Connect a sample player to an output instrument
(mixer).
|||
|||	  Synopsis
|||
|||	    Err ehConnectSamplePlayer( pTMixerInfo pMixerInfo, pTSampleInfo
pSampleInfo, int32 channel )
|||
|||	  Description
|||
|||	    This function connects the sample player instrument referenced in
the SampleInfo to the output
|||		instrument (mixer) referenced in the MixerInfo.  The player
instrument is then ready to be started.
|||
|||		If the sample is stereo, only the left player output is
...
sample.
|||
|||		channel			number of the mixer channel to use for
playing the sample.
|||
|||	  Return Value
|||
...
|||	  Implementation
|||
|||	    Call this function when you want to connect a sample player to an
output instrument.
|||
|||
|||	  Caveats
...
**/
Err
ehConnectSamplePlayer (pTMixerInfo pMixerInfo, pTSampleInfo pSampleInfo,
                       int32 channel)
{
  Err status = -1;
  int32 numOutputs = 0;
  ...
  switch (numOutputs)
    ...
    case 1:
      ...
      status = ConnectInstruments (pSampleInfo->si_Player, "Output", mixer,
                                   gChannelName[channel]);
      break;

    case 2:
      PRT (("Stereo sample encountered: Connecting Left Output to "
            "mixer\n\tIgnoring right channel\n"));
      status = ConnectInstruments (pSampleInfo->si_Player, "LeftOutput", mixer,
                                   gChannelName[channel]);
      break;
  ...
  status = _ehAcquireGainKnob (mixer, channel, kLeftOutput,
                               &pSampleInfo->si_LeftGainKnob);
  if (status < 0)
    goto CLEANUP;

  status = _ehAcquireGainKnob (mixer, channel, kRightOutput,
                               &pSampleInfo->si_RightGainKnob);
  if (status < 0)
    goto CLEANUP;
...
|||	  Synopsis
|||
|||	    void ehSetChannelLevels(pTMixerInfo pMixerInfo, Item leftKnob, Item
rightKnob, int32 volume, int32 balance)
|||
|||	  Description
|||
|||	    This function determines and sets the knob values for the mixer
referenced by a MixerInfo
|||		based on the desired volume and balance values, and the number
of players connected to the
|||		mixer.
|||
|||
|||	  Arguments
...
instrument (mixer).
|||
|||		leftKnob		item for the left gain knob of the
mixer.
|||
|||		leftKnob		item for the right gain knob of the
mixer.
|||
|||		volume			loudness to distribute over the left
and right mixer outputs.
|||						The legal 3DO range of 0 ..
MAXDSPAMPLITUDE should be observed.
|||
|||		balance			value which specifies how much of the
volume is distributed to the left and right
|||						mixer outputs.  It should be in
the range 0 .. kMaxBalance.
|||
|||	  Return Value
|||
|||	    [none]
...
|||	  Implementation
|||
|||	    Call this function to adjust the volume of the left and right mixer
outputs.
|||
|||
|||	  Caveats
|||
|||		The passed-in knob items are most conveniently obtained from a
SampleInfo allocated via
|||		ehNewSampleInfo (or the convenience function
ehLoadSoundEffect).
|||
|||		The MixerInfo must be allocated by a call to ehNewMixerInfo.
|||
...
**/
void
ehSetChannelLevels (pTMixerInfo pMixerInfo, Item leftKnob, Item rightKnob,
                    int32 volume, int32 balance)
{
  int32 leftLevel;
  int32 rightLevel;

  volume /= pMixerInfo->mi_ChannelsUsed; /* this channel's contribution to
                                            overall volume */
...
|||	  Synopsis
|||
|||	    Err ehLoadSoundEffect( pTSampleInfo *pNewSampleInfo, pTMixerInfo
pMixerInfo, const char *sampleName, int32 channel )
|||
|||	  Description
|||
|||	    This function loads and prepares a sample for playback as a sound
effect.
|||
|||
|||	  Arguments
|||
|||	    pNewSampleInfo  Pointer to the allocated SampleInfo structure, or
NULL if an error
|||		                occurred (for example, if the sample was not
found)
|||
|||	    pMixerInfo		pointer to information about the output
instrument (mixer).
|||
|||		sampleName		filename of the sample to load and
prepare.
|||
|||		channel			Mixer output channel through which the
sound effect is to be played
|||
|||	  Return Value
|||
|||	    Returns 0 if the sound effect was successfully prepared for
playback, or
|||		a negative number if an error occurred.
|||
|||
|||	  Implementation
|||
|||	    Call this convenience function to load and prepare to play a sound
effect.
|||
|||
|||	  See Also
...
**/
Err
ehLoadSoundEffect (pTSampleInfo *pNewSampleInfo, pTMixerInfo pMixerInfo,
                   const char *sampleName, int32 channel)
{
  Err status = -1;
