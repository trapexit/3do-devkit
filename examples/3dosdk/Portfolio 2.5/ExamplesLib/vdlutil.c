...
void
internalInitVDL (VDLEntry *vdlPtr, Bitmap *destBitmap, int32 displayMode,
                 Boolean bypassClut)
/*
 ...
 */
{
  ...
  for (scanLine = 0; scanLine < screenHeight; scanLine++)
    {
       ...
       * amount */
      /*  of time available for the hardware to process it during horiz blank.
       * In */
      ...
      vdlRecPtr->controlword = SetDMACtlWord (CLUT_COLOR_WORDS - FILLER_WORDS,
                                              1, false, displayMode);
      ...
      for (colorTableEntry = 0; colorTableEntry < CLUT_WORDS;
           colorTableEntry++)
  ...
  vdlRecPtr->controlword
      = SetDMACtlWord (CLUT_COLOR_WORDS - FILLER_WORDS, 1, true, displayMode);
...
void
InitVDL (VDLEntry *vdlPtr, Bitmap *destBitmap, int32 displayMode)
{
  internalInitVDL (vdlPtr, destBitmap, displayMode, false);
}

void
InitVDL24 (VDLEntry *vdlPtr, Bitmap *destBitmap, int32 displayMode)
{
 ...
 */
void
internalMergeVDL (VDL_REC *rawVDLPtr, VDL_REC *activeVDLPtr, uint32 vdlLines,
                  int32 displayMode, Boolean bypassClut)
{
  ...
  for (scanLineEntry = 0; scanLineEntry < vdlLines; scanLineEntry++)
    {
      ...
      if (vdlLines == 1)
        activeVDLRecPtr->controlword = SetDMACtlWord (
            CLUT_COLOR_WORDS - FILLER_WORDS, screenHeight, false, displayMode);
      ...
      if (scanLineEntry)
        ...
        activeVDLRecPtr->displayControl
            = SetDisplayCtlWord (true, true, bypassClut);
...
void
MergeVDL (VDL_REC *rawVDLPtr, VDL_REC *activeVDLPtr, uint32 vdlLines,
          int32 displayMode)
{
...
void
MergeVDL24 (VDL_REC *rawVDLPtr, VDL_REC *activeVDLPtr, uint32 vdlLines,
            int32 displayMode)
{
...
*/
int32
internalAllocateVDL (VDLEntry **newVDL, Bitmap *screenBitMap,
                     int32 displayMode, Boolean bypassClut)
{
...
int32
AllocateVDL (VDLEntry **newVDL, Bitmap *screenBitMap, int32 displayMode)
{
  return internalAllocateVDL (newVDL, screenBitMap, displayMode, false);
}

int32
AllocateVDL24 (VDLEntry **newVDL, Bitmap *screenBitMap, int32 displayMode)
{
