...
|||	  Description
|||
|||	    This program displays a list of programs on the screen, lets the
user pick
|||	    one, and runs it. After the selected program completes, the list of
|||	    programs is displayed again and the user can make another
selection.
|||
|||	    When the list is on screen, the user can press the up and down
arrows on
...
|||	    programs to display on screen. Every line in this file defines a
program,
|||	    using the following format:
|||
|||	    ["] label ["] cmd-line label the string that represents this
program in
|||	    the screen display. Surround this string with quotes (") if it
contains
|||	    any spaces. cmd-line used as a command-line. Whenever the user
selects the
|||	    entry, this command-line is executed as is.
|||
|||	    Within the program list, empty lines are ignored. A semicolon
indicates
|||	    the start of a comment. Any text following a semicolon is ignored
until
|||	    the end of the line. Linefeed or a carriage return indicate the end
of a
|||	    line.
|||
|||	  Arguments
|||
|||	    program list                 Name of a file containing a list of
programs
|||	                                 to run. If this argument is not
supplied,
|||	                                 the default name of menu.script is
assumed.
|||
|||	  Associated Files
|||
 ...
 */
static uint32
RenderMenu (ProgramList *programList, Bitmap *bitmap, int32 oldSelection,
            int32 newSelection)
{
  ...
  totalWidth = 0;
  totalHeight = BORDER_HEIGHT * 2 + TextHeight (BANNER_1)
                + TextHeight (BANNER_2) + BANNER_SPACING;
  numVisible = 0;
  ...
  if (oldSelection < 0) /* tells us to draw everything */
    {
      ...
      totalHeight
          -= TextHeight (BANNER_1) + TextHeight (BANNER_2) + BANNER_SPACING;
      ...
      DrawLine (left + 2, top + totalHeight - 2, left + totalWidth - 1,
                top + totalHeight - 2);
      DrawLine (left + 1, top + totalHeight - 1, left + totalWidth - 1,
                top + totalHeight - 1);
      DrawLine (left + totalWidth - 2, top + 2, left + totalWidth - 2,
                top + totalHeight - 1);
      DrawLine (left + totalWidth - 1, top + 1, left + totalWidth - 1,
                top + totalHeight - 1);
    }
  else
    {
  ...
  SCANLIST (&programList->pl_Programs, node, ProgramNode)
  {
    ...
    if ((oldSelection < 0) || (i == oldSelection) || (i == newSelection))
      {
        if (i == newSelection)
          {
            SetFgColor (HIGHLIGHT_PANEL_COLOR);
            DrawBox (innerLeft, top, innerLeft + innerWidth - 1,
                     top + labelHeight - 1);
 ...
 */
static int32
EventLoop (ProgramList *programList, Bitmap *bitmap, int32 selection)
{
  Err err;
 ...
 */
static void
SetupLoop (ProgramList *programList)
{
      ...
      if (vramIOReq >= 0)
        {
          screenGroupItem = CreateScreenGroupVA (
              &screenItem, CSG_TAG_SCREENCOUNT, 1, CSG_TAG_DISPLAYTYPE,
              DI_TYPE_DEFAULT, CSG_TAG_DONE);
