...
static bool
AddProgramEntry (ProgramList *programList, char *ptr)
{
  ProgramNode *node;
  bool quoted;
  char *startLabel;
...
Err
LoadProgramList (const char *scriptName, ProgramList *programList)
{
  Stream *stream;
  void *buffer;
  char *ptr;
 ...
 */
void
UnloadProgramList (ProgramList *programList)
{
  ProgramNode *node;
...
Err
RunProgram (ProgramList *programList, uint32 programNum)
{
  Item task;
  ProgramNode *node;
  Err result;
  ...
  node = (ProgramNode *)FindNodeFromHead (&programList->pl_Programs,
                                          programNum);
  if (node)
    {
