...
|||	  Description
|||
|||	    This program tests the juggler using non-musical events and
software-based
|||	    "timing." It constructs two synthetic sequences based on the
TestData data
|||	    structure, and creates a collection based on these two sequences.
It then
|||	    "plays" the collection via a simple print function, processed at
the time
...
int32
UserInterpFunc (Jugglee *Self, TestEvent *te)
{
  PRT (("TestEvent: Self = 0x%x, Time = %d, Data = %d\n", Self,
        te->te_TimeStamp, te->te_Data));
  return 0;
}

TestEvent TestData[] = { { 5, 567 }, { 7, 910 }, { 12, 400 } };
