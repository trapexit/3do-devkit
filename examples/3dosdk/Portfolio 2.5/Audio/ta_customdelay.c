...
|||	  Description
|||
|||	    Demonstrates how to create and use a delay line to get real-time
echo
|||	    effects in your program. Loads the specified AIFF file and plays it
into a
|||	    delay line. By tweaking the knobs on the output mixer, you can
control the
|||	    mix of delay sound versus original sound, and the speed at which
the echo
|||	    will die down.
|||
|||	  Arguments
...
|||
|||	    delay ticks                  Amount of time, in audio clock ticks,
to
|||	                                 hold each note. Defaults to 240 ticks.
|||
|||	  Caveats
|||
|||	    SleepAudioTicks() is an inefficient way to delay a program, and it
is used
...
main (int argc, char *argv[])
{
  ...
  TraceAudio (0);
  Result = InitCustomDelay (&DelayCon, DELAY_FRAMES * sizeof (int16),
                            DELAY_FRAMES - 100);
  ...
  SampleName = (argc > 1)
                   ? argv[1]
                   : "$samples/PitchedL/SynthBass/BassSynth.C3LM44k.aiff";
  ...
  Result = ConnectInstruments (SamplerIns, "Output", DelayCon.dlc_MixerIns,
                               "Input0");
...
*/
  ...
  Result = StartInstrumentVA (Instrument, AF_TAG_VELOCITY, Velocity,
...
/*********************************************************************/
int32
InitCustomDelay (DelayContext *dlc, int32 DelaySize, int32 DelayFrames)
{
  int32 Result = 0;
  Item Att;
...
#define IF_LOOP (TRUE)
  ...
  */
  ...
  Result = ConnectInstruments (dlc->dlc_TapIns, "Output", dlc->dlc_MixerIns,
                               "Input1");
  CHECKRESULT (Result, "ConnectInstruments");
...
int32
TermCustomDelay (DelayContext *dlc)
{
