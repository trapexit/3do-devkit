...
|||	  Description
|||
|||	    This demonstration program lets you move a cel (a cloud) around the
screen
|||	    with a joystick and leave an image of it on the screen when you
press the
|||	    trigger. You can also distort the cel and clear the screen. The
program
...
|||
|||	    The connection/configuration/disconnection routines can be used
as-is in
|||	    final code.
|||
|||	    The data interaction/processing is less efficient than it could be
for the
|||	    sake of modularity, comprehensibility, and flexibility. If you
unpack the
|||	    BS_NiceWaitEvent() and BS_ProcessXxxData() routines and perform
that
|||	    processing in a more direct way performance will improve. You
should also
|||	    eliminate any debouncing checks that you don't use.
|||
|||	    Joystick controls Stickmove in x and yZ zoom in/out U/D/L/R
switchdeform
|||	    cel Triggerleave copy of cel on screen Aclear screen B recalibrate
stick
|||	    (to recalibrate, hit B with stick in neutral position and move the
stick
|||	    and z throttle to the limits of travel) Cquit RightShiftswitch to
mouse
|||
|||	    Mouse controls LeftButtonleave copy of cel on screen
MiddleButtonclear
|||	    screen Movementmove cel in x and y
|||
|||	    Pad 1 controls (used with mouse): RightButtonswitch to joystick D
Pad
|||	    distort cel C Buttonquit RightShifttoggle mouse edge conditions
between
|||	    bound & wrap
|||
|||	    Pad 2 controls (only if there are 2 pads and no mouse): D Padmove
cel A
|||	    Buttonleave copy of cel on screen B Buttonclear screen C
Buttonswitch to
...
{
  CopyVRAMPages (VRAMIOReq, dest, src, mySc->sc_nFrameBufferPages, -1);
}
...
int
updateCel (int32 x, int32 y, int32 z, uint32 buttonBits, int32 mx, int32 my,
           uint32 mBits, uint32 cpBits, Point quad[4], int *fireF, int *clearF)
{
  ...
  if (gMouseF)
    {
      ...
      quad[0].pt_X = mx + 0;
      quad[0].pt_Y = my + 0;
      quad[1].pt_X = mx + BS_ZRES / 2 + dx;
      quad[1].pt_Y = my + 0;
      quad[2].pt_X = mx + BS_ZRES / 2 + dx;
      quad[2].pt_Y = my + BS_ZRES / 2 + dy;
      quad[3].pt_X = mx + 0;
      quad[3].pt_Y = my + BS_ZRES / 2 + dy;
          ...
          if (mBits & ControlC)
            {
              if (BS_GetPeripheralCount (BS_STICK))
                ...
                printf (
                    "peripheral example:  no joystick to give control to.\n");
            }
        }
      else
        {
...
main (int argc, char **argv)
{
  ...
  printf (
      "        and move the stick and z throttle to the limits of travel\n");
  printf ("     C: quit\n");
  ...
  printf (
      "     RightShift:  toggle mouse edge conditions between bound & wrap\n");
          ...
          if (eCode)
            {
              ...
              if (!gMouseF
                  && !((BS_GetStickRawButtons (sticks[0]) & Stick4Way)))
                {
              ...
              if (gMouseF)
                {
                  ...
                  BS_ProcessCPadData (pads[0], ControlC | ControlRightShift,
                                      0);
              ...
              /* positions */
              cont = updateCel (
                  BS_GetStickX (sticks[0]), BS_GetStickY (sticks[0]),
                  BS_GetStickZ (sticks[0]), BS_GetStickButtons (sticks[0]),
                  (gFakeF ? BS_GetCPadX (pads[1]) : BS_GetMouseX (mice[0])),
                  (gFakeF ? BS_GetCPadY (pads[1]) : BS_GetMouseY (mice[0])),
                  (gFakeF ? BS_GetCPadButtons (pads[1])
                          : BS_GetMouseButtons (mice[0])),
                  BS_GetCPadButtons (pads[0]), quad, &fireF, &clearF);
            }
