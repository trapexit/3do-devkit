...
ReallocFrames (ANIM *pAnim, int32 newFrameCount, uint32 memTypeBits)
{
  ...
  newFrames = (AnimFrame *)AllocMem (
      memNeeded, MEMTYPE_TRACKSIZE | (memTypeBits | MEMTYPE_FILL | 0x00));
...
ParseAnim (void *inBuf, int32 inBufSize, uint32 memTypeBits)
{
  ...
  if (NULL
      == (pAnim = (ANIM *)AllocMem (
              sizeof (*pAnim),
              MEMTYPE_TRACKSIZE | (memTypeBits | MEMTYPE_FILL | 0x00))))
    {
...
ERROR_EXIT:
  ...
  UnloadAnim (
      pAnim); /* cleanly undoes anything we've (even partially) done. */
