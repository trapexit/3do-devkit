...
create_timer_control (TimerControl **retval)
{
  ...
  if ((tctl = (TimerControl *)AllocMem (sizeof (*tctl),
...
trequest_new_timer (MsgValueTypes *mv)
{
  ...
  return (
      TimerHandle)tctl; /* cast tctl ptr to int32 opaque handle for client */
...
handle_requestmsg (MSEventData *msevent, void *userContext)
{
  ...
  switch (mv[0].msgid)
    ...
    case TSREQUEST_MSGID_SHUTDOWN:
      return msevent->msgItem; /* reply will be done from
                                  timer_services_thread() routine	   */
