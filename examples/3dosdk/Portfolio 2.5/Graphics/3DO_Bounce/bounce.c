 ...
 * upward */
#define UP_COLLISION(_thisCcb, _thisXPos, _thisYPos, _otherCcb, _otherXPos,   \
                     _otherYPos)                                              \
...
/* condition which must be met for a collision while this object is moving
 * downward */
#define DOWN_COLLISION(_thisCcb, _thisXPos, _thisYPos, _otherCcb, _otherXPos, \
                       _otherYPos)                                            \
  (HORZ_OVERLAP (_thisCcb, _thisXPos, _otherCcb, _otherXPos)                  \
 ...
 * the left */
#define LEFT_COLLISION(_thisCcb, _thisXPos, _thisYPos, _otherCcb, _otherXPos, \
                       _otherYPos)                                            \
...
/* condition which must be met for a collision while this object is moving to
 * the right */
#define RIGHT_COLLISION(_thisCcb, _thisXPos, _thisYPos, _otherCcb,            \
                        _otherXPos, _otherYPos)                               \
...
void
SetQuadFromPosAndSize (Point *aQuad, int32 xPos, int32 yPos, int32 width,
                       int32 height)
{
...
void
SetShadowQuad (Point *aQuad, int32 xPos, int32 ySpeed)
/*
...
*/
{
  ...
  gVRAMIOReq
      = CreateVRAMIOReq (); /* Obtain an IOReq for all SPORT operations */
  if (gVRAMIOReq < 0)
    {
  ...
  gScreenContext
      = (ScreenContext *)AllocMem (sizeof (ScreenContext), MEMTYPE_ANY);
  ...
  if ((gDisplayItem = CreateBasicDisplay (gScreenContext, gDisplayType, 2))
      < 0)
    {
...
main (int argc, char **argv)
{
  ...
  */
  ...
  gGlobeFrameIncr -= 0x00009000; /* tweak the frame increment to obtain a
                                    satisfactory animation rate  */
  ...
  while (TRUE)
    {
          ...
          if (ballUpDir)
            {
              ...
              if (gCollision)
                {
                  if (UP_COLLISION (gBallCcb, xPos, yPos, gTvCcb, gTvXPos,
                                    gTvYPos))
                    {
                      ballUpDir = FALSE;
                      tvUpDir = TRUE;
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (DOWN_COLLISION (gBallCcb, xPos, yPos, gTvCcb, gTvXPos,
                                      gTvYPos))
                    {
                      ballUpDir = TRUE;
                      tvUpDir = FALSE;
          ...
          if (ballLftDir)
            {
              ...
              if (gCollision)
                {
                  if (LEFT_COLLISION (gBallCcb, xPos, yPos, gTvCcb, gTvXPos,
                                      gTvYPos))
                    {
                      ballLftDir = FALSE;
                      tvLftDir = TRUE;
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (RIGHT_COLLISION (gBallCcb, xPos, yPos, gTvCcb, gTvXPos,
                                       gTvYPos))
                    {
                      ballLftDir = TRUE;
                      tvLftDir = FALSE;
      ...
      SetQuadFromPosAndSize (aQuad, xPos, yPos, gBallCcb->ccb_Width,
                             gBallCcb->ccb_Height);
      ...
      if (gLightOn)
        {
          ...
          ConstrainQuad (aQuad, BALLSHADOW_XMIN, BALLSHADOW_XMAX,
                         BALLSHADOW_YMIN, BALLSHADOW_YMAX);
        }
      ...
      if (gBouncing)
          ...
          if (globeUpDir)
            {
              ...
              if (gCollision)
                {
                  if (UP_COLLISION (gGlobeCcb, gGlobeXPos, gGlobeYPos,
                                    gBallCcb, gBallXPos, gBallYPos))
                    {
                      globeUpDir = FALSE;
                      ballUpDir = TRUE;
#if ENABLE_SOUND
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (DOWN_COLLISION (gGlobeCcb, gGlobeXPos, gGlobeYPos,
                                      gBallCcb, gBallXPos, gBallYPos))
                    {
                      globeUpDir = TRUE;
                      ballUpDir = FALSE;
          ...
          if (globeLftDir)
            {
              ...
              if (gCollision)
                {
                  if (LEFT_COLLISION (gGlobeCcb, gGlobeXPos, gGlobeYPos,
                                      gBallCcb, gBallXPos, gBallYPos))
                    {
                      globeLftDir = FALSE;
                      ballLftDir = TRUE;
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (RIGHT_COLLISION (gGlobeCcb, gGlobeXPos, gGlobeYPos,
                                       gBallCcb, gBallXPos, gBallYPos))
                    {
                      globeLftDir = TRUE;
                      ballLftDir = FALSE;
        ...
        {
          if ((globeLftDir)
              && ((gGlobeXPos -= GLOBE_XSPEED) <= GLOBE_NOBOUNCE_XMIN))
            globeLftDir = FALSE;
          if ((!globeLftDir)
      ...
      SetQuadFromPosAndSize (aQuad, gGlobeXPos, gGlobeYPos,
                             gGlobeCcb->ccb_Width, gGlobeCcb->ccb_Height);
      SpecialConstrainQuad (aQuad, MAX_RIGHT);
      ...
      if (gLightOn)
        {
          ...
          ConstrainQuad (aQuad, GLOBESHADOW_XMIN, GLOBESHADOW_XMAX,
                         GLOBESHADOW_YMIN, GLOBESHADOW_YMAX);
        }
          ...
          if (tvUpDir)
            {
              ...
              if (gCollision)
                {
                  if (UP_COLLISION (gTvCcb, gTvXPos, gTvYPos, gCubeCcb,
                                    gCubeXPos, gCubeYPos))
                    {
                      tvUpDir = FALSE;
                      cubeUpDir = TRUE;
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (DOWN_COLLISION (gTvCcb, gTvXPos, gTvYPos, gCubeCcb,
                                      gCubeXPos, gCubeYPos))
                    {
                      tvUpDir = TRUE;
                      cubeUpDir = FALSE;
          ...
          if (tvLftDir)
            {
              ...
              if (gCollision)
                {
                  if (LEFT_COLLISION (gTvCcb, gTvXPos, gTvYPos, gCubeCcb,
                                      gCubeXPos, gCubeYPos))
                    {
                      tvLftDir = FALSE;
                      cubeLftDir = TRUE;
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (RIGHT_COLLISION (gTvCcb, gTvXPos, gTvYPos, gCubeCcb,
                                       gCubeXPos, gCubeYPos))
                    {
                      tvLftDir = TRUE;
                      cubeLftDir = FALSE;
      ...
      SetQuadFromPosAndSize (aQuad, gTvXPos, gTvYPos, gTvCcb->ccb_Width,
                             gTvCcb->ccb_Height);
      SpecialConstrainQuad (aQuad, MAX_RIGHT);
      MapCel (gTvCcb, aQuad);
      ...
      if (gLightOn)
        {
          ...
          ConstrainQuad (aQuad, TVSHADOW_XMIN, TVSHADOW_XMAX, TVSHADOW_YMIN,
                         TVSHADOW_YMAX);
        }
          ...
          if (cubeUpDir)
            {
              ...
              if (gCollision)
                {
                  if (UP_COLLISION (gCubeCcb, gCubeXPos, gCubeYPos, gGlobeCcb,
                                    gGlobeXPos, gGlobeYPos))
                    {
                      cubeUpDir = FALSE;
                      globeUpDir = TRUE;
#if ENABLE_SOUND
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (DOWN_COLLISION (gCubeCcb, gCubeXPos, gCubeYPos,
                                      gGlobeCcb, gGlobeXPos, gGlobeYPos))
                    {
                      cubeUpDir = TRUE;
                      globeUpDir = FALSE;
#if ENABLE_SOUND
          ...
          if (cubeLftDir)
            {
              ...
              if (gCollision)
                {
                  if (LEFT_COLLISION (gCubeCcb, gCubeXPos, gCubeYPos,
                                      gGlobeCcb, gGlobeXPos, gGlobeYPos))
                    {
                      cubeLftDir = FALSE;
                      globeLftDir = TRUE;
          ...
          else
            {
              ...
              if (gCollision)
                {
                  if (RIGHT_COLLISION (gCubeCcb, gCubeXPos, gCubeYPos,
                                       gGlobeCcb, gGlobeXPos, gGlobeYPos))
                    {
                      cubeLftDir = TRUE;
                      globeLftDir = FALSE;
      ...
      SetQuadFromPosAndSize (aQuad, gCubeXPos, gCubeYPos, gCubeCcb->ccb_Width,
                             gCubeCcb->ccb_Height);
      ...
      if (gLightOn)
        {
          ...
          ConstrainQuad (aQuad, CUBESHADOW_XMIN, CUBESHADOW_XMAX,
                         CUBESHADOW_YMIN, CUBESHADOW_YMAX);
        }
              ...
              */
      {
        ...
        CopyVRAMPages (gVRAMIOReq,
                       ...
                       whichPict, gScreenContext->sc_nFrameBufferPages, -1);
      }
        ...
        */
        {
          ...
          gTvFrameIncr -= 0x00006000; /* tweak the frame increment to obtain a
                                         satisfactory animation rate */
        }
      goto DONE;
    }
...
*/
{
  ...
  SetVRAMPages (gVRAMIOReq, gWallOffPict, 0,
                gScreenContext->sc_nFrameBufferPages, -1);

  GetArtFilename (WALL_OFF_IMAGE, filename);
  if (LoadImage (filename, gWallOffPict, (VdlChunk **)NULL, gScreenContext)
      == 0)
    {
  ...
  SetVRAMPages (gVRAMIOReq, gWallOnPict, 0,
                gScreenContext->sc_nFrameBufferPages, -1);

  GetArtFilename (WALL_ON_IMAGE, filename);
  if (LoadImage (filename, gWallOnPict, (VdlChunk **)NULL, gScreenContext)
      == 0)
    {
