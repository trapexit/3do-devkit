...
ce_Init (ColorEcho *ce)
{
  ...
  /*
  ** Copy CCB to CEL type memory in case program is loaded into non-DMAable
  *memory.
  ** Currently all memory is accessable by Cel DMA but future machines may
  *extend
  ...
  if (ce->ce_IfSport)
    {
      t = CreateVRAMIOReq ();
      CopyVRAMPages (
          t, sc->sc_Bitmaps[sc->sc_curScreen]->bm_Buffer,    /* dest */
