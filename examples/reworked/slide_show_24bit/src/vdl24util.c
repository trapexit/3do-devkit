...
*/
{
  ...
  vdl[0]
      = VDL_ENVIDDMA /* enable video DMA */
        | VDL_LDCUR  /* load address from current-bitmap-buffer-address word
                        (see below) */
        | VDL_LDPREV /* load address from previous-bitmap-buffer-address word
                        (see below) */
        | (34 << VDL_LEN_SHIFT) /* number of control words in this VDL entry --
                                   we use 34 */
        | VDL_PREVSEL /* add modulo ammount to previous-bitmap-buffer-address
                         register; this uses the value specified by
                         VDL_DISPMOD_nnn (see below) */
        | (GetScreenHeight (displayMode)
           << VDL_LINE_SHIFT) /* persistence for VDL entry */
  ...
  vdl[4 + 32]
      = VDL_DISPCTRL
        | VDL_BACKGROUND; /* These or'd values mean this is a background-value
                                                  word.  The background value
                             is set to the component values specified in the
                             remainder of the word, namely zero. */
