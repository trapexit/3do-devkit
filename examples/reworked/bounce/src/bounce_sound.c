...
/* Initialize the mixer and the program's sounds */
{
  ...
  */
  ...
  if (ehLoadSoundEffect (&Effect[BALL_GLOBE], gMixerInfo, filename, BALL_GLOBE)
      < 0)
    goto DONE;
  ...
  if (ehLoadSoundEffect (&Effect[CUBE_GLOBE], gMixerInfo, filename, CUBE_GLOBE)
      < 0)
    goto DONE;

  GetSoundFilename (BALL_FLOOR_SND, filename);
  if (ehLoadSoundEffect (&Effect[BALL_FLOOR], gMixerInfo, filename, BALL_FLOOR)
      < 0)
    goto DONE;

  GetSoundFilename (TV_FLOOR_SND, filename);
  if (ehLoadSoundEffect (&Effect[TV_FLOOR], gMixerInfo, filename, TV_FLOOR)
      < 0)
    goto DONE;

  GetSoundFilename (CUBE_FLOOR_SND, filename);
  if (ehLoadSoundEffect (&Effect[CUBE_FLOOR], gMixerInfo, filename, CUBE_FLOOR)
      < 0)
    goto DONE;

  GetSoundFilename (GLOBE_FLOOR_SND, filename);
  if (ehLoadSoundEffect (&Effect[GLOBE_FLOOR], gMixerInfo, filename,
                         GLOBE_FLOOR)
      < 0)
    goto DONE;

  ehSetChannelLevels (gMixerInfo, Effect[BALL_TV]->si_LeftGainKnob,
                      Effect[BALL_TV]->si_RightGainKnob, BALL_TV_GAIN,
                      kEqualBalance);
  ehSetChannelLevels (gMixerInfo, Effect[BALL_GLOBE]->si_LeftGainKnob,
                      Effect[BALL_GLOBE]->si_RightGainKnob, BALL_GLOBE_GAIN,
                      kEqualBalance);
  ehSetChannelLevels (gMixerInfo, Effect[TV_CUBE]->si_LeftGainKnob,
                      Effect[TV_CUBE]->si_RightGainKnob, TV_CUBE_GAIN,
                      kEqualBalance);
  ehSetChannelLevels (gMixerInfo, Effect[CUBE_GLOBE]->si_LeftGainKnob,
                      Effect[CUBE_GLOBE]->si_RightGainKnob, CUBE_GLOBE_GAIN,
                      kEqualBalance);
  ehSetChannelLevels (gMixerInfo, Effect[BALL_FLOOR]->si_LeftGainKnob,
                      Effect[BALL_FLOOR]->si_RightGainKnob, BALL_FLOOR_GAIN,
                      kEqualBalance);
  ehSetChannelLevels (gMixerInfo, Effect[TV_FLOOR]->si_LeftGainKnob,
                      Effect[TV_FLOOR]->si_RightGainKnob, TV_FLOOR_GAIN,
                      kEqualBalance);
  ehSetChannelLevels (gMixerInfo, Effect[CUBE_FLOOR]->si_LeftGainKnob,
                      Effect[CUBE_FLOOR]->si_RightGainKnob, CUBE_FLOOR_GAIN,
                      kEqualBalance);
  ehSetChannelLevels (gMixerInfo, Effect[GLOBE_FLOOR]->si_LeftGainKnob,
                      Effect[GLOBE_FLOOR]->si_RightGainKnob, GLOBE_FLOOR_GAIN,
                      kEqualBalance);
