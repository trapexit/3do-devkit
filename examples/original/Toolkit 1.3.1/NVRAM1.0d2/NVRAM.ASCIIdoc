...
The 3DO Interactive Multiplayera has at least 32 KB of non-volatile RAM (NVRAM) available. Title developers can use this facility for persistent storage of small pieces of data. Practical uses might be to provide a Osave gameO feature, to save user preference settings, or to store title configuration information.
...
*	Steps a title should go though when the user wants to save a titleOs state
...
ItOs important to manage NVRAM properly. If NVRAM is full, some titles deal inappropriately with the situation:
...
*	If a title attempts to write to NVRAM but fails because NVRAM is full, the title should prompt the user to delete information currently in NVRAM. This can be done using Access, discussed in the 3DO Portfolio ProgrammerOs Guide. Some guidelines are discussed in the next section, OInteracting with the End-User.O


INTERACTING WITH THE END-USER

Every title should go through the same set of steps when a user wants to save a titleOs status. This section lists the steps in sequence. Commented sample code for parts of the sequence is provided as part of 3DO Toolkit 1.3.1.
...
*	ElseNif the user cancelsNreturn to Save Title display and jump back to 
step 1. 
...
Portfolio supports NVRAM access through file system calls. Your title can use I/O requests to work with an open NVRAM file as a device, as it would any other file.  Chapter 24, OUsing the File SystemO of the 0194.1 edition of the 3DO Portfolio ProgrammerOs Guide includes NVRAM-specific information. The documentation describes a relatively low-level access method. For high-level access, the file system provides calls to open, close and delete a file. You can use the routines below to create (and allocate) blocks, and to read from, write to, get the block size of a file, and set the end of a file.
...
*	The second part of the file name is used to uniquely identify the file to the user. This is flexible depending on the purpose of the file. For example, if the file is a game and users are prompted for their name as part of the game, the userOs name or the name of the game level could be part of the file name. 
...
Like any other volume, you must format NVRAM properly before the file system can access it. If youOre working from the Debuggerr, type the following commandNfollowed by the Enter keyNin the Debugger Terminal window:

format RAM 3 0 NVRAM

After issuing this command, reset the target. You should see the O/NVRAMO volume mount during file system startup. Thereafter, you can refer to this volume as O/NVRAMO. You will not need to perform this format ritual again, since the contents of NVRAM are preserved across restarts and shutdowns. On a production multiplayer NVRAM is pre-formatted.
If youOre working from a CD-ROM image, the volume is formatted automatically. 

Working With NVRAM Files From the Debugger
You can work with files in NVRAM the same way youOd work with other files. Copy them using the copy command. To view them, type into the Terminal window: 
$c/ls nvram
YouOll see a list of all files in NVRAM. To see a more detailed list, type: 
$c/lmdump RAM 3 0
