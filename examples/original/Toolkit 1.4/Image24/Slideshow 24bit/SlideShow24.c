...
int32 PALmode = 0;
int32 myScreenWidth
    = kScreenWidthNTSC; // Screen Width in current RUNTINE environment
int32 myScreenHeight
    = kScreenHeightNTSC; // Screen Height in current RUNTINE environment
...
TagArg VDLTags[] = { CREATEVDL_TAG_VDLTYPE,     (void *)VDLTYPE_FULL,
...
LoadScreen (char *fileName, int32 whichBuffer)
{
  ...
  if ((rawVDLPtr != NULL) && (rawVDLPtr->chunk_size))
    {
      ...
      VDLTags[1].ta_Arg
          = PALmode ? (void *)DI_TYPE_PAL2 : (void *)DI_TYPE_NTSC;
      ...
      //		gVDLItems[whichBuffer] = SubmitVDL(gVDLPtrArray,
      //((vdllines * sizeof(VDL_REC)) / 4), VDLTYPE_FULL);
  ...
  if ((oldVDL != systemVDL[0]) && (oldVDL != systemVDL[1])
      && (oldVDL != simpleVDL[0]) && (oldVDL != simpleVDL[1]))
    DeleteVDL (oldVDL);
...
CreateScreens (void)
{
  ...
  gScreenCntx.sc_nFrameBufferPages
      = (myScreenWidth * myScreenHeight * 2 + VRAMPageSize - 1) / VRAMPageSize;
  gScreenCntx.sc_nScreens = 2;
  gScreenCntx.sc_nFrameByteCount
      = gScreenCntx.sc_nFrameBufferPages * VRAMPageSize;
  gScreenCntx.sc_curScreen = 0;

  //	Allocate memory for the screen bitmap
  buffPtr = (ubyte *)ALLOCMEM (
      (int)(2 * gScreenCntx.sc_nFrameBufferPages * VRAMPageSize),
...
CreateSimpleVDL (void)
{
  ...
  for (screen = 0; screen < kScreenCount; screen++)
    {
      InitSimpleVDL ((SVDL *)simpleVDLBuffer, gScreenCntx.sc_Bitmaps[screen],
                     PALmode);
      ...
      //		simpleVDL[screen] = SubmitVDL((VDLEntry
      //*)simpleVDLBuffer, ((sizeof(SVDL)) / 4), VDLTYPE_SIMPLE);
  ...
  // We can free the memory. The system has copied it and hangs it off the VDL
  // Item
...
ToggleBit15 (void)
{
  ...
  if (picSize[curbuf] == minImageSize)
    p = (uint32 *)gScreenCntx.sc_Bitmaps[curbuf]->bm_Buffer;
  else
    p = (uint32 *)gScreenCntx.sc_Bitmaps[2]->bm_Buffer;
  ;
  while (n--)
    *p++ ^= 0x80008000;
}
