 ...
 *Version 1.3 Change command line numeric parsing to accept either decimal or
 *hexadecimal inputs. 9/12/93		rdg		Version 1.2 Spin
 *cursor. Use bigger I/O buffers for speed. Fix bug in initial chunk size
 *computation.  It was not quadbyte aligning the proper variable. Fix another
 *bug in chunk size computation.  Since the first chunk written uses a special
 *larger header than the subsequent data chunks, the data size needs to be
 *						increased for all non-first-chunks to
 *make up for the difference in size between the headers.  YUK! Initalize
 *variables in main(). Return 1 on error instead of 0 and delete partial
 *output.
 *	6/16/93		jb		Update to use Neil's join subscriber
 *definitions. 6/16/93		jb		New today from WriteTestStream
 *source
 *
 *******************************************************************************************/
...
usage (void)
{
  ...
  printf ("	%s <size>			size of a data chunk in BYTES "
          "(16384)\n",
          CHUNK_SIZE_STRING);
  printf (
      "	%s <time>			timestamp interval per chunk (1)\n",
