/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <3>	  8/5/93	JAY		changing dsp/ to
   $audio/dsp/ for DragonTail3
                 <2>	  4/5/93	JAY		removing /remote/ from
   filename (i.e. making path relative to initial working directory) <1>
   4/3/93	JAY		first checked in 4/3/93	JAY
   xxx put comment here xxx
                                 3/31/93	RDG		Added
   TermBounceSound and did last check before release submission 3/30/93 RDG
   Took out redundant code in DoObjectCollision- since I'm not using the
   orientation data for the object collisions, there's no need to check for
   it... 3/25/93	RDG	 	Update for Cardinal 3/12/93	RDG
   new
...
TagArg SamplerTags[] = { { AF_TAG_PITCH, 0 }, { 0, 0 } };

static char *LeftNames[]
    = { "LeftGain0", "LeftGain1", "LeftGain2", "LeftGain3",
        "LeftGain4", "LeftGain5", "LeftGain6", "LeftGain7" };

static char *RightNames[]
    = { "RightGain0", "RightGain1", "RightGain2", "RightGain3",
        "RightGain4", "RightGain5", "RightGain6", "RightGain7" };

static char *ChannelNames[] = { "Input0", "Input1", "Input2", "Input3",
...
InitBounceSound (void)
{
  ...
  CHECKITEM (Samples[BALL_GLOBE], "_InitBounceSound",
             "Couldnt load BALL_GLOBE snd");
  ...
  CHECKITEM (Samples[CUBE_GLOBE], "_InitBounceSound",
             "Couldnt load CUBE_GLOBE snd");
  Samples[BALL_FLOOR] = LoadSample (BALL_FLOOR_SND);
  CHECKITEM (Samples[BALL_FLOOR], "_InitBounceSound",
             "Couldnt load BALL_FLOOR snd");
  Samples[TV_FLOOR] = LoadSample (TV_FLOOR_SND);
  CHECKITEM (Samples[TV_FLOOR], "_InitBounceSound",
             "Couldnt load TV_FLOOR snd");
  Samples[CUBE_FLOOR] = LoadSample (CUBE_FLOOR_SND);
  CHECKITEM (Samples[CUBE_FLOOR], "_InitBounceSound",
             "Couldnt load CUBE_FLOOR snd");
  Samples[GLOBE_FLOOR] = LoadSample (GLOBE_FLOOR_SND);
  CHECKITEM (Samples[GLOBE_FLOOR], "_InitBounceSound",
             "Couldnt load GLOBE_FLOOR snd");
  ...
  for (i = 0; i < NUMSAMPLERS; i++)
    {
      ...
      Result
          = ConnectInstruments (Samplers[i], "Output", Mixer, ChannelNames[i]);
      CHECKRESULT (Result, "_InitBounceSounds",
                   "Couldn't connect a sampler to mixer");
    }
...
void
PanMixerChannel (int32 ChannelNumber, int32 MaxAmp, int32 Pan)
{
  ...
  */
  ...
  LeftGain
      = ((MaxAmp * (WINDOW_WIDTH - (Pan - LEFT_WALL_POS))) / WINDOW_WIDTH);
