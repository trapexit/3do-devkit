...
#define SFP_OWN_BUFFERS                                                       \
  (0x00001) /* True if the Buffer memory is owned by this system. */
#define SFP_NO_DIRECTOUT                                                      \
  (0x00002) /* If true, no automatic connection to directout.dsp */
#define SFP_IO_PENDING                                                        \
  (0x00004) /* Set if an IO request has been sent and not received. */
...
extern "C"
{
  ...
  SoundFilePlayer *CreateSoundFilePlayer (int32 NumBuffers, int32 BufSize,
                                          void *Buffers[]);
  SoundFilePlayer *OpenSoundFile (char *FileName, int32 NumBuffers,
                                  int32 BufSize);
  ...
  int32 ServiceSoundFile (SoundFilePlayer *sfp, int32 SignalIn,
                          int32 *SignalNeeded);
