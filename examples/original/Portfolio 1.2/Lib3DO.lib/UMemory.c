 ...
 *
 *	  09/13/93	ian		Added additional error checking for
 *common mistakes when DEBUG is on.  Reduced VRAM page table size to 32 entries
 *(from 64) on the assumption that nobody is ever gonna try to allocate more
 *						than half the available pages as
 *individual page-aligned units, and if they want to, they can always call
 *InitMalloc() to ask for the number of allocation units they need. Fixed
 *IS_ALIGNED_ALLOCATION() macro; parens were nested incorrectly, giving
 *potential false indication of a page- aligned allocation request. 07/27/93
 *ian 	Added special handling for page-aligned VRAM. Moved AvailMem() routines
 *out to UMemoryDebug.c. 05/17/93	akm 	Adding rudimentary memory leak
 *support 05/13/93	akm 	Changed abort() to Debug(); 05/13/93	akm
 *Added eric's changes from JPIDemo integration 05/13/93	akm Fixed
 *header problems for old mem check routines 05/13/93	akm 	Split shared
 *memory routines into seperate file. Added Phil's AvailMem code. 05/11/93
 *akm 	Changed sharedmemory structures to work around compiler bug 05/07/93
 *akm 	Added new implementations for shared memory routines 05/03/93	akm
 *Added new wrappers for ALLOCMEM and FREEMEM. Added new getptrsize routine.
...
#define IS_ALIGNED_ALLOCATION(typebits)                                       \
  (((typebits) & (MEMTYPE_VRAM | MEMTYPE_STARTPAGE))                          \
...
VRAMTableSearch (void *addr)
{
  ...
  if (gPageSizeTable == NULL)
    {
      DIAGNOSE (
          ("(internal) gPageSizeTable is NULL in VRAMTableSearch(%p)\n", addr))
      return NULL;
    }
#endif
...
void *
Malloc (uint32 size, uint32 typebits)
{
  uint32 *newBlock;
  ...
  if (size == 0)
    ...
    {
      DIAGNOSE (
          ("Invalid size value in Malloc(%ld, %08lx)\n", size, typebits));
      return NULL;
    }
  ...
  if ((typebits & 0xFFFFFF80) == 0xFFFFFF80)
    {
      DIAGNOSE (
          ("Warning: All typebits set in Malloc(%ld, %08lx)...\n"
...
void *
NewPtr (uint32 size, uint32 memtype)
{
