/*
   ...
   background image
                $boot/JSData/Art/boom.anim	-	animation to play when
   A button is pressed
   ...
   Jumpstart's BasicUFO.c; Added event broker interface for joypad triggering
   of explosion
*/
...
Coord
CalcCenterCoord (int32 aCoord, int32 aDimension)
{
...
void
CenterCCB (CCB *aCCB, Coord aXCenter, Coord aYCenter)
{
  int32 aWidth;
  int32 aHeight;
  int32 aXPos;
...
Boolean
DoControlPad (void)
/* Returns FALSE if user pressed START to quit, else returns TRUE, indicating
  ...
  if (nBoomCount)
    {
      ...
      if (nBoomCount)
      ...
      else
        {
          ...
          nPostBoomDelay = gExplosionAnim->num_Frames
      ...
      while (DoControlPad ())
        {
          ...
          /*
                  ...
                  DisplayScreen automatically waits for the next vertical
             blank, then sets up the hardware to use the specified screen as
             the display buffer. Once set up properly, the hardware will show
                  this buffer on the TV automatically every frame. You only
                  have to call DisplayScreen when you want to change which
             block of memory the hardware is displaying on the TV.
          */
          DisplayScreen (
