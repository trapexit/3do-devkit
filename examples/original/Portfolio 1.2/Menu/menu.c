/*
   ...
   3DO Company.
        ...
        Change History (most recent first):
                 ...
                 <8>	 7/30/93	JAY		added IO parameter
   block for SetVRAMPages() and CopyVRAMPages(). Set IOInfo blocks to zero
   before assigning values. Modified MySportIO. <5>	 6/25/93	dsm
   Changed Hilighting mechanism to positional since I couldn't get the font_CCB
   control to work. <4>	  4/7/93	JAY		MEMTYPE_SPRITE is now
...
OpenMenuDisplay ()
{
  ...
  ScreenPageCount
      = (bigsize + GrafBase->gf_VRAMPageSize - 1) / GrafBase->gf_VRAMPageSize;
...
/* Uses the SPORT to clear to zero the two bitmaps */
{
  ...
  inf[0].ioi_Recv.iob_Buffer
      = (void *)((ulong)bitmap0->bm_Buffer & (-GrafBase->gf_VRAMPageSize));
  inf[0].ioi_Send.iob_Len = (int)(GrafBase->gf_VRAMPageSize);
  inf[0].ioi_Send.iob_Buffer = GrafBase->gf_ZeroPage;

  /* Structure copy */
  inf[1] = inf[0];
  inf[1].ioi_Recv.iob_Buffer
      = (void *)((ulong)bitmap1->bm_Buffer & (-GrafBase->gf_VRAMPageSize));
...
/* Uses the SPORT to clear to zero the bitmap */
{
  ...
  inf.ioi_Recv.iob_Buffer
      = (void *)((ulong)bitmap->bm_Buffer & (-GrafBase->gf_VRAMPageSize));
