...
#define ControlAll                                                            \
  (ControlStart | ControlUp | ControlDown | ControlLeft | ControlRight        \
...
/*------------------------------------------------------------------------
 * drawText()
 *	Calls the system DrawText8() routine to display simple text on the
 *screen.
 *----------------------------------------------------------------------*/
...
main (int argc, char *argv[])
{
  ...
  CopyBlack (gScreenContext->sc_Bitmaps[gScreenContext->sc_curScreen],
  ...
  if (err == 0)
    {
      ...
      if (gIdleTimer)
        {
          sprintf (string, "Load time: secs=%0d, usecs=%0d", tv.tv_Seconds,
                   tv.tv_Microseconds);
          row += 30;
          drawText (25, row, string);
        }
  ...
  Text2_ptr = (uint8 *)"¦±²³'µÔÕOÂ±²³'µ--*,¹o"¼½¾?ÀÁÂÃAAÆÇÈEEËÌIÎÏDÑÒOÔÕOxØÙUÛU"
                       "ÝEßEÞ¢£¤!Ì§Ì"ÌÌaÌ"*not *­*(r)À-ÁAÞ-A";
  ...
  while (TRUE)
    {
      ...
      if (buttons & ControlRight)
      ...
      else if (buttons & (ControlA | ControlB | ControlC))
        {
          ...
          if (buttons & ControlC)
            {
              /*------------------------------------------------------------------------
               * The C button displays a random text color on a random
               *background color.
               *----------------------------------------------------------------------*/
      ...
      if (textccb == NULL)
        {
          PRT (("Problem with TextBox - either bad input parameters or out of "
                "memory."));
          goto DONE;
        }
      else
...
{
  CopyVRAMPages (gVRAMIOeq, bitmap->bm_Buffer, gBackPic, nFrameBufferPages,
                 ~0);
}
...
InitBackPic (ScreenContext *sc, char *filename)
{
  ...
  CopyVRAMPages (gVRAMIOeq, sc->sc_Bitmaps[sc->sc_curScreen]->bm_Buffer,
                 gBackPic, sc->sc_nFrameBufferPages, ~0);
