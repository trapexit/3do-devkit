...
/* set the current, raw values (not really needed, but here for the sake of
 * form) */
...
#define BS_InitMouseData(a)                                                   \
  memset (a, 0, sizeof (BS_MouseData));                                       \
  BS_SetMouseMinX (a, -1);                                                    \
  BS_SetMouseMaxX (a, BS_XRES);                                               \
  BS_SetMouseMinY (a, -1);                                                    \
  BS_SetMouseMaxY (a, BS_YRES);                                               \
  BS_SetMouseXThresh (a, BS_XRES << 2);                                       \
  BS_SetMouseYThresh (a, BS_YRES << 2);                                       \
