...
|||	  Description
|||
|||	    Demonstrates simple interaction with the joystick using data
filtering and
|||	    calibration. Lets you move a cel (a cloud) around the screen with
the
|||	    joystick and leaves a still of it on screen when you press the
trigger.
|||
|||	    Joytick controls Stick move in x and y Zzoom in/out U/D/L/R
switchdistort
|||	    cel Triggerleave copy of cel on screen Aclear screen Brecalibrate
stick
...
|||	  Caveats
|||
|||	    This is a simplified version of the broker shell example that just
does
...
int
updateCel (int32 x, int32 y, int32 z, uint32 buttonBits, Point quad[4],
           int *fireF, int *clearF)
{
...
handlePortChange (void)
{
   ...
   */
  /*  */
  /* you can also reconfigure your watching (i.e. call BS_WatchJoyStick(1)
   * after */
