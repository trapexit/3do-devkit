...
BS_CountPeripherals (void)
{
  ...
  if ((Err)qMsg->msg_Result < 0)
    {
      printf (
          "BS_CountPeripherals(): Event broker could not accept request\n");
  ...
  if (podlist->pdl_Header.ebh_Flavor != EB_DescribePodsReply)
    {
      printf (
          "BS_CountPeripherals():  message data was wrong flavor!(0x%x,%d)\n",
          ...
          switch (frame->ef_EventNumber)
            ...
            case EVENTNUM_ControlButtonUpdate:
              if (!cpdp || !cpdp[(podI = (frame->ef_GenericPosition - 1))])
                break;
              changedF |= BS_CPAD;
              BS_SetCPadRawButtons (
                  cpdp[podI], ((ControlPadEventData *)(frame->ef_EventData))
                                  ->cped_ButtonBits);
          ...
          if (eventItem == msgItem)
            {
              ...
              if ((int32)event->msg_Result < 0)
                {
                  printf ("BS_NiceWaitEvent: says broker refused "
...
BS_ConnectEventBroker (void)
{
  ...
  if ((Err)(msgPtr->msg_Result) < 0)
    {
      printf (
          "BS_ConnectBroker(): Event broker could not accept configuration\n");
...
BS_WatchJoyStick (int debounceAllF)
{
  ...
  if ((Err)msgPtr->msg_Result < 0)
    {
      printf (
          "BS_WatchJoyStick(): Event broker could not accept configuration\n");
      return 0;
    }
...
BS_WatchMouse (int debounceAllF)
{
  ...
  if ((Err)msgPtr->msg_Result < 0)
    {
      printf (
          "BS_WatchMouse(): Event broker could not accept configuration\n");
      return 0;
    }
  ...
  if (debounceAllF >= 0)
    {
      if (debounceAllF)
        ...
        config.cr_TriggerMask[0]
            |= gunFlags | EVENTBIT0_LightGunButtonReleased;
    }
...
BS_DisconnectBroker (void)
{
  ...
  if (SendMsg (msgPortItem, msgItem, &config, sizeof (config)))
    {
      printf (
          "BS_DisconnectBroker(): Problem sending message to event broker\n");
      return 0;
    }
  ...
  if ((Err)msgPtr->msg_Result < 0)
    {
      printf ("BS_DisconnectBroker(): Event broker could not accept "
              "configuration\n");
      return 0;
    }
