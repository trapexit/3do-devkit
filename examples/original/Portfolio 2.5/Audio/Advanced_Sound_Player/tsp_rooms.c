...
|||	tsp_rooms - Room-sensitive soundtrack example using advanced sound
player.
|||
|||	  Synopsis
|||
...
|||	    gRoom.
|||
|||	    The parent task, tsp_rooms, gets control port events and updates
gRoom as
|||	    follows:
|||
|||	    * A Enter room A.
...
|||
|||	    The soundtrack thread, Soundtrack, uses the advanced sound player
to play
|||	    a unique sound file for each room. When the main task changes
rooms, the
|||	    soundtrack thread adapts the soundtrack to the change in room at a
|||	    musically convenient location.
|||
|||	    The way this is done is that each room's soundfile is designed so
it can
|||	    play in a loop. In addition to this, the end of each sound file,
and
|||	    additional optional marked locations within the sound file, must be
|||	    musically sensible locations to transition to the beginning of any
of the
|||	    other room's sound files. When playback reaches the next marked
position
|||	    in, or the end of, the current room's sound, a default decision
function
|||	    is called to check to see if the main task changed room. If the
room is
|||	    still the same, then the current sound continues to play, or loops
if at
|||	    the end.
|||
|||	    If the room has changed, the SPPlayer is instructed to begin
playing the
|||	    sound for the new room.
|||
|||	    Note that the soundtrack adapts to room changes by the main task
instead
|||	    of making a sharp transition as soon as the room changes. It is
possible
|||	    for the main task to make several room changes before the
soundtrack
|||	    thread discovers that the room has changed at all. This results in
smooth
|||	    soundtrack transitions that occur at seemingly random locations,
which is
|||	    far less likely to become annoying to a game player than a
soundtrack that
|||	    predictably changes the instant the player crosses a given
threshold.
|||
|||	  Associated Files
|||
...
/* macros */
#define MakeUErr(svr, class, err)                                             \
  MakeErr (ER_USER, 0, svr, ER_E_USER, class, err)
...
main (int argc, char *argv[])
{
...
#ifdef MEMDEBUG
  if (CreateMemDebug (MEMDEBUGF_ALLOC_PATTERNS | MEMDEBUGF_FREE_PATTERNS
  ...
  if ((errcode = InitEventUtility (1, 0, LC_ISFOCUSED)) < 0)
    goto clean;
  ...
  if ((errcode = soundtrackthread = CreateThreadTagsVA (
           "Soundtrack", SOUNDTRACK_PRIORITY, (void (*) ())SoundtrackMain,
           SOUNDTRACK_STACK, CREATETASK_TAG_MSGFROMCHILD,
           replyport, /* Instruct thread to send
  ...
  for (;;)
    {
      ...
      if ((errcode = GetControlPad (1, TRUE, &cped)) < 0)
        goto clean;
...
Err SoundtrackDecisionFunction (SPAction *, SoundtrackDecisionData *,
                                SPSound *, const char *markername);
...
PlaySoundtrack (void)
{
  ...
  if ((errcode = samplerins = LoadInstrument ("fixedmonosample.dsp", 0, 100))
      < 0)
    goto clean;
  if ((errcode = outputins = LoadInstrument ("directout.dsp", 0, 100)) < 0)
    goto clean;
  ...
  if ((errcode = StartInstrument (outputins, NULL)) < 0)
    goto clean;

  /* Set up player */
  if ((errcode
       = spCreatePlayer (&player, samplerins, BUF_COUNT, BUF_SIZE, NULL))
      < 0)
    goto clean;

  if ((errcode = spSetDefaultDecisionFunction (
           player, (SPDecisionFunction)SoundtrackDecisionFunction,
           &decisiondata))
      < 0)
    goto clean;
    ...
    for (room = 0; room < ROOM_NROOMS; room++)
      {
        ...
        if ((errcode = spAddSoundFile (&roomsound, player, roomfilename)) < 0)
          {
        ...
        /* Set it to loop */
        if ((errcode = spLoopSound (roomsound)) < 0)
          goto clean;
  ...
  */
  ...
  /* start playing */
  if ((errcode = spStartPlayingVA (player, AF_TAG_AMPLITUDE, 0x7fff, TAG_END))
      < 0)
    goto clean;
    ...
    while (spGetPlayerStatus (player) & SP_STATUS_F_BUFFER_ACTIVE)
      {
        ...
        if ((errcode = spService (player, sigs)) < 0)
          goto clean;
      }
  }
...
*/
Err
SoundtrackDecisionFunction (SPAction *action,
                            SoundtrackDecisionData *decisiondata,
  ...
  if (decisiondata->stdd_CurRoom != newroom)
    {
      ...
      errcode = IsRoom (newroom)
                    ? spSetBranchAction (action,
                                         decisiondata->stdd_Sounds[newroom],
                                         SP_MARKER_NAME_BEGIN)
...
Item
CreateThreadTagsVA (const char *name, uint8 pri, void (*code) (),
                    int32 stacksize, uint32 tag1, ...)
{
...
Item
CreateThreadTags (const char *name, uint8 pri, void (*code) (),
                  int32 stacksize, const TagArg *tags)
{
  ...
  /* create thread item */
  if ((errcode = thread
       = CreateItemVA (MKNODEID (KERNELNODE, TASKNODE),
                       name ? TAG_ITEM_NAME : TAG_NOP, name, TAG_ITEM_PRI, pri,
                       CREATETASK_TAG_PC, code, CREATETASK_TAG_STACKSIZE,
                       stacksize, CREATETASK_TAG_SP, stackbase + stacksize,
                       CREATETASK_TAG_ALLOCDTHREADSP, TRUE, TAG_JUMP, tags))
      < 0)
    goto clean;
