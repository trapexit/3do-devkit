...
#define PIXC_8BPP_BLEND                                                       \
  (((PPMPC_MF_8 | PPMPC_SF_8) << 16)                                          \
   | (PPMPC_1S_CFBD | PPMPC_MS_PIN | PPMPC_MF_8 | PPMPC_SF_8 | PPMPC_2S_PDC))

#define PIXC_4BPP_BLEND                                                       \
  (((PPMPC_1S_CFBD | PPMPC_MS_PDC_MFONLY | PPMPC_2S_CFBD | PPMPC_AV_SF2_2)    \
    << 16)                                                                    \
...
alloc_text_celdata (CCB *pCel, int32 w, int32 h, int32 bpp)
{
  ...
  newCelData = (CelData *)AllocMem (
      newCelDataSize, MEMTYPE_TRACKSIZE | MEMTYPE_CEL | MEMTYPE_FILL | 0);
...
#else
  pCel = (CCB *)AllocMem (sizeof (CCB) + PLUTSIZE, MEMTYPE_TRACKSIZE
                                                       | MEMTYPE_CEL
                                                       | MEMTYPE_FILL | 0x00);
#endif
  ...
  for (colorIndex = 0; colorIndex < nColors; ++colorIndex)
    {
      ...
      if (colorIndex == 2 && (tCel->tc_formatFlags & TC_INTERNAL_TWOCOLOR))
        {
          fgColor = tCel->tc_fgColor[0]; /* for 2-color fonts
                                            (outlined/shadowed) */
          bgColor = tCel->tc_fgColor[1]; /* color 2 is anti-a between colors 0
                                            & 1. */
        }
      else
        {
          ...
          if (theChar == '\t')
            {
              dstX += offset_to_tabstop (
                  tCel,
                  dstX); /* might not work with right/center justify??? */
            }
          else
            {
               ...
               int32 height)
{
  ...
  tCel
      = (TextCel *)AllocMem (sizeof (TextCel), MEMTYPE_ANY | MEMTYPE_FILL | 0);
  if (tCel == NULL)
    {
...
EraseTextInCel (TextCel *tCel)
{
  memset (tCel->tc_CCB->ccb_SourcePtr, 0,
          GetMemTrackSize (tCel->tc_CCB->ccb_SourcePtr));
           ...
           CCB *ccb)
{
  ...
  ccb->ccb_PIXC = 0x1F001F80; /* pmode0 = (PDC+CFB)/1, pmode1 = (PDC+0)/1
                                 (replace mode) */
  DrawCels (bitmapItem, ccb);
}
