...
**  the library that creates cels can do so using this system so that
*DeleteCel()
...
**  three magical longwords immediately ahead of the CCB in memory.  (The
*easiest
...
**  callback function that does the freeing.  The 3rd longword is either a
*pointer
**  to the last cel in a group of logically related cels treated as a single
*entity,
**  or it is some arbitrary data that means something to a callback deletor
*routine.
**  The flavor of magic number used to validate the CCB determines the
*interpretation
...
**  to call back when DeleteCel() is called for that cel.  The callback
*function
...
**  CCBs, if non-NULL.  This gets a bit tricky; it's all involved with
*supporting
...
**  arises when the application links such an entity into a larger list of
*cels,
...
*mini-
**  list of related cels in the larger list, the DeleteCelList() routine would
*be
**  walking NextPtr links through cels that have already been freed.  The
*memory
...
**  allow DeleteCelList() to work correctly, a creator that returns a mini-list
*of
**  related cels can set the creatorData field to point to the last cel in the
**  mini-list.  The internal_delete_cel() routine uses this to advance to the
*end
**  of the mini-list, so that DeleteCelList() effectively jumps over the
*mini-list
**  after it is deleted, and continues processing with any other cels linked
*into
**  the larger list created by the application.  When CALLBACK style deletion
*is
**  used, the creatorData field is already used for callback data.  In this
*case,
**  the callback routine is responsible for returning a pointer to the next CCB
**  following the CCB(s) it deleted.  If the callback returns NULL, that
*indicates
**  that no CCBs were linked into the list beyond the ones the callback
*deleted.
**  NOTE that there is a very important subtle distinction here:  For
*non-callback
**  deletion, the creatorData points to the last CCB in the mini-list (or is
*NULL);
**  the internal_delete_cel() function calculates the next larger-list pointer
*by
**  following NextPtr from the last CCB pointed to by creatorData.  For
*callback
**  deletion, the following of NextPtr is done by the callback routine; the
**  callback's return value is not a pointer to the last CCB it deleted, it is
*a
**  pointer to the next CCB in the larger list, or NULL if there is no next
*CCB.
**
                ...
                void *creatorData)
{
  ...
  mcel = (DeleteCelMagic *)AllocMem (sizeof (DeleteCelMagic) + extraBytes,
                                     MEMTYPE_TRACKSIZE | MEMTYPE_CEL
                                         | MEMTYPE_FILL | 0);
