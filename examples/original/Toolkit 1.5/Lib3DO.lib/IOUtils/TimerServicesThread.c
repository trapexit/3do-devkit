 ...
 *				  - Converted from the old MsgportMulti dispatcher to
 *the new MSEvents dispatcher.
 *				  - Changed IVDS-specific error	codes to 3DO
 *standard codes.
 *				  - Removed	the Sleep and GetTime functionality,
 *since that's now covered more efficiently by the TimerUtils package.
 *				  - Values expressed in microseconds passed in by
 *clients can now be larger than 1 million.  We internally convert them to some
 *number of seconds plus the remainder in usecs; the seconds resulting from
 *conversion get added to the number of seconds specified by the client.  But
 *all this happens within our private data structures, client values are not
 *changed.
 *
 *	03/26/94  Ian	Busy day:
 *				  - Did a variety of cleanups -- more error checking
 *where "Can't fail" assumptions were being made, the DLL link field is now at
 *the start of the TimerControl structure, etc.
 *				  - It looks like the OS gang hasn't given us an
 *OwnerData field in the message structure for backlinks, so 'plan b' has been
 *implemented: use the item name as the backlink. -	Implemented 'AtTime'
 *functionality.
 *				  - Rewrote all Heartbeat-related logic to prevent
 *'drift' in the heartbeat frequency.  The old logic treated a heartbeat as a
 *series of Delay requests, and the frequency of events would drift based on
 *how long it took the owner to process each event and reply back to us (our
 *own overhead added a bit too, so even signal-based notifications would
 *drift). Now we treat heartbeats as a series of AtTime requests, with the time
 *calculated by adding the delta to the last event time.
 *				  - Speeded up the GetTime processing by
 *pre-allocating needed items and structures.  This is mostly because we call
 *this a lot internally now to process heartbeat timers.
 *
 *	03/17/94  Ian	Implemented pooled allocation of TimerControl
 *structures and their attached resources.  This was not so much for
 *					performance as for resource tracking.  We now
 *report any unreleased timercontrols at shutdown time and release them anyway.
 *Since the resource tracking code is 90% of what's needed for pooled
 *allocation, we got that for free.
 *
 *	03/15/94  Ian	Fixed a bug caused by the OS gang fixing a bug of
 *theirs. GetThisMsg() now returns an error if the receiver has received the
 *message, so we can no longer consider a failure of GetThisMsg() to be a
 *problem.  Ideally, we'd consider anything but a "receiver already has the
 *message" status to be a serious error, but GetThisMsg() returns a generic -1
 *status instead of a specific error code. (07/15/94: oops, my mistake; it
 *seems -1 is the specific error code meaning receiver has the msg.  strange
 *but true.)
 *
 *	02/15/94  RLG	Added support for the VBL timer as well as USEC.
 *					The VBL support includes Heartbeats, AtTime,
 *and Delays, but not GetTime.  (GetTime for a VBL is [NOT] best done by just
 *retrieving GrafBase->gf_VBLNumber.)
 *
...
static void
delete_timer_control (TimerControl *tctl)
{
  Err err;
...
static Err
create_timer_control (TimerControl **retval)
{
  Err err;
  TimerControl *tctl;
  char namebuf[12];

  if ((tctl = (TimerControl *)Malloc (sizeof (*tctl),
                                      MEMTYPE_ANY | MEMTYPE_FILL | 0x00))
      == NULL)
    {
      err = NOMEM;
      DIAGNOSE_SYSERR (err,
                       ("Can't create internal timer control structure\n"));
      goto ERROR_EXIT;
    }
...
timer_services_cleanup (void)
{
  ...
  for (cur = (TimerControl *)FirstNode (&tctl_inuse_list);
       IsNode (&tctl_inuse_list, cur); cur = next)
    {
      next = (TimerControl *)NextNode (cur);
      ownerTaskPtr = (Task *)LookupItem (cur->ownerTask);
      ownerTaskName
          = (ownerTaskPtr) ? ownerTaskPtr->t.n_Name : "task has died";
  ...
  for (cur = (TimerControl *)FirstNode (&tctl_free_list);
       IsNode (&tctl_free_list, cur); cur = next)
    {
      next = (TimerControl *)NextNode (cur);
      delete_timer_control (cur);
    }
 ...
 *	 - 	Open timer device.
 *	 -	Create the msgports we use to service requests from other
 *tasks. Called immediately upon thread startup.
 ****************************************************************************/

static void
timer_services_init (void)
{
  ...
  if ((mseHandle = SetupMSEvents (ms_events, ArrayElements (ms_events), 0))
      < 0)
    {
  ...
  if ((readtimer_ioreqptr = (IOReq *)LookupItem (readtimer_ioreq)) == NULL)
    {
      ...
      DIAGNOSE_SYSERR (startup_status,
                       ("LookupItem() failed for readtimer_ioreq\n"));
      goto ERROR_EXIT;
    }
...
static void
release_timer_control (TimerControl *tctl)
{
 ...
 *	Grab a timer control structure from the pool of free resources, or
 *create a new one if the pool is empty.
 ****************************************************************************/

static Err
acquire_timer_control (TimerControl **retval)
{
  Err err;
  TimerControl *tctl;
...
static Err
io_readtime (struct timeval *tv, uint8 unit)
{
  Err err;
  ...
  if (tctl->flags & TSIFLAG_IO_ACTIVE)
    {
      ...
      if ((err = WaitPort (timer_io_msgport, tctl->ioreqptr->io_MsgItem)) < 0)
        {
          DIAGNOSE_SYSERR (
              err, ("Failed to get pending timer ioreq completion msg\n"));
          goto ERROR_EXIT;
        }
    }
...
static Err
io_start (TimerControl *tctl, StartAction start_action)
{
  Err err;
  IOInfo ioinfo;
...
/*****************************************************************************
 * trequest_verify()
 *	Walk the list of in-use timers verifying that the owning tasks still
 *live.
 ****************************************************************************/

static Err
trequest_verify (void)
{
  TimerControl *cur;
  TimerControl *next;
  Task *ownerTaskPtr;
  ...
  for (cur = (TimerControl *)FirstNode (&tctl_inuse_list);
       IsNode (&tctl_inuse_list, cur); cur = next)
    {
      ...
      if (ownerTaskPtr == NULL)
        {
          DIAGNOSE (("Task %08lx died without releasing timer handle %08p; "
                     "now released\n",
                     cur->ownerTask, cur));
          io_cancel (cur);
...
static TimerHandle
trequest_new_timer (MsgValueTypes *mv)
{
  Err err;
  TimerRequest *trqst;
...
static Err
trequest_cancel_timer (MsgValueTypes *mv, ReleaseAction release_tctl)
{
  Err err;
  TimerRequest *trqst;
  TimerControl *tctl;
  ...
  if (tctl->ownerTask != trqst->ownerTask)
    {
      err = NOTOWNER;
      DIAGNOSE (("Cannot cancel timer, task %08lx is not owner of "
                 "TimerControl handle %08p (owner is task %08lx)\n",
...
static Err
trequest_reset_timer (MsgValueTypes *mv)
{
  Err err;
  TimerRequest *trqst;
  TimerControl *tctl;
  ...
  if (tctl->ownerTask != trqst->ownerTask)
    {
      err = NOTOWNER;
      DIAGNOSE (("Cannot reset timer, task %08lx is not owner of TimerControl "
                 "handle %08p (owner is task %08lx)\n",
                 trqst->ownerTask, tctl, tctl->ownerTask));
      goto ERROR_EXIT;
    }
...
static Err
trequest_change_userdata (MsgValueTypes *mv)
{
  Err err;
  TimerRequest *trqst;
  TimerControl *tctl;
  ...
  if (tctl->ownerTask != trqst->ownerTask)
    {
      err = NOTOWNER;
      DIAGNOSE (("Cannot change timer, task %08lx is not owner of "
                 "TimerControl handle %08p (owner is task %08lx)\n",
                 trqst->ownerTask, tctl, tctl->ownerTask));
      goto ERROR_EXIT;
    }
...
static int32
handle_requestmsg (MSEventData *msevent, void *userContext)
{
  Err err;
  MsgValueTypes *mv;
  ...
  switch (mv[0].msgid)
    ...
    case TSREQUEST_MSGID_SHUTDOWN:
      return msevent
          ->msgItem; // reply will be done from timer_services_thread() routine
    ...
    default:
      err = NOSUPPORT;
      DIAGNOSE (("Unknown message type %ld received by timer services request "
                 "handler\n",
                 mv[0].msgid));
      break;
    }
...
static int32
handle_iomsg (MSEventData *msevent, void *userContext)
{
  Err err;
  Item ioreq;
  IOReq *ior;
  ...
  if (tctl->flags & TSFLAG_SIGNAL)
    ...
    {
      if (tctl->flags & TSIFLAG_MSG_OUTSTANDING)
        {          // can't send if client hasn't
          err = 0; // replied to outstanding msg.
        }
      else
        {
          tctl->flags |= TSIFLAG_MSG_OUTSTANDING;
          err = SendSmallMsg (tctl->notify.msgport, tctl->notifymsg,
                              tctl->userdata1, tctl->userdata2);
        }
    }
  ...
  if (err < 0)
    {
      DIAGNOSE_SYSERR (err,
                       ("Can't send timer event notification to task %08lx\n",
                        tctl->ownerTask));
      goto ERROR_EXIT;
    }
...
ERROR_EXIT:
  ...
  if (err < 0)
    {
      DIAGNOSE_SYSERR (err,
                       ("TimerControl %08p released due to error\n", tctl));
      release_timer_control (tctl);
    }
 ...
 *	Handle a client's reply to an event notification message we sent
 *earlier.
 *
 *	A wee bit of ugliness:  the message data structure doesn't have an
 *OwnerData field we can use to backlink the message to our owning
 *TimerControl.  So we encoded the TimerControl pointer as an ascii string and
 *stored it in the message item name field when we created the message.  Now we
 *convert the name back to a pointer, and because we're paranoid we do a couple
 *sanity checks on it before trying to use it as a pointer.
 *
...
static int32
handle_notifymsg_reply (MSEventData *msevent, void *userContext)
{
  TimerControl *tctl;
  ...
  if ((int32)tctl <= 0 || tctl->notifymsg != msevent->msgItem)
    {
      DIAGNOSE (("Invalid timercontrol backlink, a TimerControl has been "
                 "orphaned\n"));
      return 0;
    }
#endif
 ...
 *	request msg handler to return the request msg item number to the
 *DispatchMSEvents routine, which then returns the value to us, and since it
 *isn't negative (ie, not an error) it means we're supposed to shut down.  We
 *do all our cleanups, then reply to the shutdown request message, which
 *provides a synchronous shutdown from the requestor's point of view.  If we do
 *get an error status from DispatchMSEvents we ignore it and try to keep
 *running.
 ****************************************************************************/

static void
timer_services_thread (void)
{
  Item shutdownMsg;
  ...
  do
    {
      if ((shutdownMsg = DispatchMSEvents (mseHandle, NULL, 0)) < 0)
        {
          DIAGNOSE ((
              "DispatchMSEvents() failed; timer_services_thread continues\n"));
        }
    }
  while (shutdownMsg <= 0);
...
Err
TimerServicesStartup (int32 delta_priority)
{
  Err err;
  ...
  err = CreateThread ("TimerServicesThread",
                      ...
                      timer_services_thread, TIMER_THREAD_STACKSIZE);
