 ...
 *	File:		CloneCel.c
 *
 *	Contains:	Routine to create a new cel that's a clone of an
 existing cel.
 *
 ...
 *	07/23/94  Ian	Bugfix:  we were trying to copy the PLUT even when the
 *					source PLUTPtr was NULL.
 ...
 *
 *	CloneCel() allocates a new CCB and copies the specified source CCB
 fields *	into it.  It sets the CCB_LAST flag on and NULLs the
 ccb_NextPtr field *	in the new CCB.  Other modifications to the new CCB
 depend on the options *	specified in the call.
 ...
 *
 *	The COPY_PLUT option will allocate a new PLUT, store the new PLUT
 pointer *	into the new CCB, and copy the contents of the source cel's
 PLUT into the
 *	new PLUT.  It is safe to specify this option for an uncoded cel.  If
 the *	source cel's PlutPtr field is NULL, no attempt is made to copy PLUT
 data.
 *
 *	The COPY_PIXELS option will allocate a new pixel data buffer, store
 that *	buffer's pointer into the new CCB, and copy the contents of the source
  ...
  if (options & CLONECEL_COPY_PIXELS)
    {
      ...
      if ((dataBuf = Malloc (allocExtra, MEMTYPE_CEL)) == NULL)
        {
