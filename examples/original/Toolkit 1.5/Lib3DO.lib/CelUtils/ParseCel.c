 ...
 *					buffer, we now link the CCBs together (in the
 *order they were found in the buffer) and return a pointer to the first CCB
 *found.  (IE, we now return a list of one or more cels.)  This is useful for
 *handling anti-aliased cels, among other things, as if they were a single cel.
 ...
 *cel; the CCB will contain pointers to the pixels and (optional) PLUT.  If
 ...
 *	a characteristic signature, since they are just a cel file with two
 *CCBs in them really) we reverse the order of the CCB links so that the alpha
 *	channel comes first, then the data (since that's the way to draw them).
 *
 ****************************************************************************/

CCB *
ParseCel (void *inBuf, int32 inBufSize)
{
  uint32 chunk_ID;
  int32 tempSize;
  int numCCBs;
          ...
          if (lastCCB != NULL)
            {
              if (lastCCB->ccb_SourcePtr == NULL)
                {
                  DIAGNOSE (("Found a new CCB without finding a PDAT for the "
                             "prior CCB\n"));
                  goto ERROR_EXIT;
                }
  ...
  if (numCCBs == 2 && numPLUTs == 1
      ...
      && (CEL_PRE0WORD (lastCCB) & 0x07) == 3)
    { // check for  4-bit pixels
