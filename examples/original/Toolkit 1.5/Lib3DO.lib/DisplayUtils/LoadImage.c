 ...
 *
 *	History:
 *	07/25/94  Ian	Removed remaining references to GrafBase structure.
 *  05/12/94  ian	Removed reference to GrafBase->gf_ZeroPage.  We now use
 *					sc->sc_Bitmaps[0].bm_Buffer in the
 *GETBANKBITS() call. 08/06/93  jml	Updated LoadImage with new SetVRamPages
 *call for new OS 07/25/93  ian	Now uses new faster LoadFile() block I/O
 *instead of filestream I/O. Also, it now makes fewer assumptions about the
 *validity and content of the data.
 *
 ...
 *
 *	Loads an image from a 3DO file.  The pixels are loaded into a VRAM
 *buffer which is allocated herein if not pre-allocated by the caller.  If the
 *	file contains a VDL chunk, and the rawVDLPtr parameter is non-NULL, the
 *	VDL chunk is loaded into an allocated DRAM buffer, and a pointer to
 *that buffer is returned into *rawVDLPtr.  If the file contains an IMAG
 *control chunk, the values in it are used, otherwise the pixel data is assumed
 *to be a full screenful (320x240x16bit).  The pixel destination buffer is
...
void *
LoadImage (char *name, ubyte *dest, VdlChunk **rawVDLPtr, ScreenContext *sc)
{
  int32 filebufsize;
  uint32 chunk_ID;
  int32 tempSize;
  ...
  destbufsize = sc->sc_nFrameByteCount;
  ...
  if (pVDL != NULL)
    {
      if (rawVDLPtr == NULL)
        {
          DIAGNOSE (
              ("VDL chunk found in file %s, no way to return it, ignored\n",
               name));
        }
      else
        {
          ...
          if (vdlbuf == NULL)
            {
              DIAGNOSE (
                  ("Not enough memory to return copy of VDL for file %s\n",
                   name));
              goto ERROR_EXIT;
            }
  ...
  if (tempSize > destbufsize)
  ...
  else if (tempSize < destbufsize)
    {
      ...
      SetVRAMPages (VRAMIOReq, dest, 0, sc->sc_nFrameBufferPages, -1);
...
ERROR_EXIT:
  ...
  if (dest
      && we_allocated_dest) // destbufsize is non-zero if we allocated dest
    Free (dest);
