...
void
printmatrix (char *text, mat33f16 Matrix)
{
  printf ("%s:\n | 0x%lx  ,  0x%lx  , 0x%lx |\n", text, Matrix[0][0],
 ...
 **************************************************************************/
CCB *
CreateColorCCB (int32 r, int32 g, int32 b)
{
  static CCB *blankccb;
...
ClipCel (CCB *ccb, int32 x, int32 y, int32 w, int32 h)
{
  ...
  /*** get width of source data, note that I don't bother with shifting it,
   *merely get the
  ...
  if ((bpp == PRE0_BPP_8) || (bpp == PRE0_BPP_16))
    woffset
        = ((ccb->ccb_PRE1 & PRE1_WOFFSET10_MASK) >> PRE1_WOFFSET10_SHIFT) + 2;
  else
    woffset
        = ((ccb->ccb_PRE1 & PRE1_WOFFSET8_MASK) >> PRE1_WOFFSET8_SHIFT) + 2;
  ...
  /*** slip in the proper VCNT value, by masking out the source ccb vcnt and
   *ORing in
   *** the new one. ***/
  ...
  if (lrform)
    {
      /*** this decodes the address for an LRFORM pixel. Since each x value is
       *in effect
       *** 1 word, there is no down conversion needed for that. The y value is
       *divided
       *** in half since each line of data is actually two lines. This gives us
       *the
       *** address of the word we are in, then set SKIPX if this is an odd
       *line, to skip
       *** the first pixel in the word and go directly to the next one. ***/

      subccb->ccb_SourcePtr
          = (CelData *)(&(*ccb->ccb_SourcePtr)[x] + (woffset * (y / 2)));

      if (y % 2)
        subccb->ccb_PRE0 = ((subccb->ccb_PRE0 & (~PRE0_SKIPX_MASK)))
                           | (1 << PRE0_SKIPX_SHIFT);
    }
  else
    {
      switch (bpp)
        ...
        case PRE0_BPP_16:
          subccb->ccb_SourcePtr
              = (CelData *)(&(*ccb->ccb_SourcePtr)[x / 2] + woffset * y);
          skipx = x % 2;
          break;

        case PRE0_BPP_8:
          subccb->ccb_SourcePtr
              = (CelData *)(&(*ccb->ccb_SourcePtr)[x / 4] + woffset * y);
          skipx = x % 4;
          break;

        case PRE0_BPP_6:
          subccb->ccb_SourcePtr
              = (CelData *)(&(*ccb->ccb_SourcePtr)[x / 5] + woffset * y);
          skipx = x % 5;
          break;

        case PRE0_BPP_4:
          subccb->ccb_SourcePtr
              = (CelData *)(&(*ccb->ccb_SourcePtr)[x / 8] + woffset * y);
          skipx = x % 8;
          break;

        case PRE0_BPP_2:
          subccb->ccb_SourcePtr
              = (CelData *)(&(*ccb->ccb_SourcePtr)[x / 16] + woffset * y);
          skipx = x % 16;
          break;

        case PRE0_BPP_1:
          subccb->ccb_SourcePtr
              = (CelData *)(&(*ccb->ccb_SourcePtr)[x / 32] + woffset * y);
          skipx = x % 32;
          break;
        }
...
vec3f16 point2;
{
  ...
  if ((fx & 0xffff0000) | (fy & 0xffff0000)
      | (fz & 0xffff0000)) /* Check for overflow condition */
    {
 ...
 **************************************************************************/
void
NormalizeVector (vec3f16 dumbVector)
{
  frac16 length;

  ENTER ("NormalizeVector");

  length = VectorLength (dumbVector);

  if (length != 0)
    {
      dumbVector[X_COORD] = DivSF16 ((dumbVector[X_COORD]), length);
      dumbVector[Y_COORD] = DivSF16 ((dumbVector[Y_COORD]), length);
      dumbVector[Z_COORD] = DivSF16 ((dumbVector[Z_COORD]), length);
    }
 ...
 **************************************************************************/
void
RotateQuad2D (Point srcquad[4], Point dstquad[4], int32 cx, int32 cy,
              frac16 angle)
{
  Point tmpquad[4];
...
ScrollCel (CCB *ccb, CCB *srcccb, int32 x, int32 y, int32 w, int32 h)
{
  ...
  if (srcccb->ccb_Flags & CCB_PACKED)
    return;
  ...
  if ((bpp == PRE0_BPP_8) || (bpp == PRE0_BPP_16))
    woffset
        = ((srcccb->ccb_PRE1 & PRE1_WOFFSET10_MASK) >> PRE1_WOFFSET10_SHIFT)
          + 2;
  else
    woffset
        = ((srcccb->ccb_PRE1 & PRE1_WOFFSET8_MASK) >> PRE1_WOFFSET8_SHIFT) + 2;
  ...
  if (lrform)
    {
      /*** LRFORM data must always start on a word boundry and an even line. So
       *round odd
      ...
      ccb->ccb_SourcePtr
          = (CelData *)(&(*srcccb->ccb_SourcePtr)[x] + (woffset * (y / 2)));
    }
  else
    {
      switch (bpp)
        ...
        case PRE0_BPP_16:
          ccb->ccb_SourcePtr
              = (CelData *)(&(*srcccb->ccb_SourcePtr)[x / 2] + woffset * y);
          skipx = x % 2;
          break;

        case PRE0_BPP_8:
          ccb->ccb_SourcePtr
              = (CelData *)(&(*srcccb->ccb_SourcePtr)[x / 4] + woffset * y);
          skipx = x % 4;
          break;

        case PRE0_BPP_6:
          ccb->ccb_SourcePtr
              = (CelData *)(&(*srcccb->ccb_SourcePtr)[(x / 16) * 3]
                            + woffset * y);
          skipx = x % 16;
          break;
        ...
        case PRE0_BPP_4:
          ccb->ccb_SourcePtr
              = (CelData *)(&(*srcccb->ccb_SourcePtr)[x / 8] + woffset * y);
          skipx = x % 8;
          break;

        case PRE0_BPP_2:
          ccb->ccb_SourcePtr
              = (CelData *)(&(*srcccb->ccb_SourcePtr)[x / 16] + woffset * y);
          skipx = x % 16;
          break;

        case PRE0_BPP_1:
          ccb->ccb_SourcePtr
              = (CelData *)(&(*srcccb->ccb_SourcePtr)[x / 32] + woffset * y);
          skipx = x % 16;
          break;
        } // end switch

      ccb->ccb_PRE0 = ((ccb->ccb_PRE0 & (~PRE0_SKIPX_MASK)))
                      | (skipx << PRE0_SKIPX_SHIFT);

      /*** the skipx value MUST be added to the width, to maintain proper
       *width. Otherwise
