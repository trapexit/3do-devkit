...
#define PrintRuns                                                             \
  ((0) && (runCount >= FirstRunPrint) && (runCount <= LastRunPrint))
...
#define _3D0toPixel(v)                                                        \
  (((CLUT[clamp ((v), PixelMin, PixelMax)] + dither ()) << 8) & PixelMask)
...
#define Predictor(x, y, field)                                                \
  (pixelAverage = (x == 1)                                                    \
                      ? (sourceDisplay[(y)][0].field)                         \
                      : (pixelAverage + sourceDisplay[(y)][(x)-1].field) / 2)
...
#ifdef _3DOPredictor
//#define Predictor(x,y,field)
//clamp((sourceDisplay[(y)][(x)-1].field+sourceDisplay[(y)-1][(x)].field)/2,
//PixelMin, PixelMax)
#define Predictor(x, y, field)                                                \
  ((sourceDisplay[(y)][(x)-1].field + sourceDisplay[(y)-1][(x)].field) / 2)
#else
#define Predictor(x, y, field)                                                \
  clamp (sourceDisplay[(y)][(x)-1].field + sourceDisplay[(y)-1][(x)].field    \
             - sourceDisplay[(y)-1][(x)-1].field,                             \
         PixelMin, PixelMax)
#endif
#endif
#endif
...
#define PatMapSize                                                            \
  256 /* Should be first power of 2 greater than validPatterns */
...
extern char unsigned
    red_Q_Table[PixelRange]; /* Index by value + PixelMidValue */
extern char unsigned
    green_Q_Table[PixelRange]; /* Index by value + PixelMidValue */
extern char unsigned
    blue_Q_Table[PixelRange]; /* Index by value + PixelMidValue */

extern unsigned char
    accumPat[PatMapSize]; // index by accumerror to get packed error index
extern unsigned char
    accumMap[PatMapSize]; // index by accumerror to get packed error index
extern int accumFrequency[PatMapSize]; // use count -- sorted in reverse order
                                       // by buildErrorCommon
