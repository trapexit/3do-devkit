 ...
 *& GOMARKER_BWD_POS_RELATIVE for fast fwd & bwd 3/2/94			lla
 *Added block skipping for fast fwd & bwd 12/10/93		lla
 *Added fast forward and reverse 12/01/93		lla		New
 *today
 *
 *******************************************************************************************/
...
//to cease and immediately return from 		the PlayEZQStream() function.
//
//------------------------------------------------------------------------------------
//************************************************************************************
//
  ...
  if (joypadStatePtr->xBtn)
    {
      ...
      //------------------------------------------------------------------------------------
      else
        {
          ...
          // SOPT_FLUSH ); CHECK_DS_RESULT( "DSStopStream: PlayerFunction",
          // status ); fRunStream = false;
      ...
      if (joypadStatePtr->rightArrow)
        {
          ...
          if (reqMsg)
            {
              /* WSD status = DSSetSkipMode( reqMsg->msgItem,
                                                       reqMsg,
                                                       ctx->streamCBPtr,
                                                       skipBlocks,
