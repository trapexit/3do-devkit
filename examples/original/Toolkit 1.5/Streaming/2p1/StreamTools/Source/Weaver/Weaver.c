 ...
 *						Fix - Bug#3433: Make sure that at least
 *one file name was specified 10/03/94			Bug #3306: Range check
 *IOBufferSize, max range should not exceed 65535. 7/12/94	dtc
 *Version 2.3d2 added support for ADPCM and 22K uncompressed DSP instruments
 *	 7/12/94	dtc		Version 2.3d1
 *						Updated PROGRAM_VERSION_STRING
 *to 2.3. 7/12/94	dtc		Version 2.3 Changed subspart and
 *haltpart to subscriberChunk and streamerChunk. 06/30/94	DLD
 *Added command WriteHaltChunk. Syntax: WriteHaltChunk <streamtime>
 *<SubscriberCode> This will output a Halt chunk keyed to the specified
 *						subscriber after all the chunks for a
 *given stream time. The chunk sent to the subscriber at halt time will have
 *						the subtype HALT.
 *  06/30/94	DLD		Added code to zero the chunkFlags when
 *RELATIVE_BRANCHING is enabled in the StopChunk and GotoChunk writing code.
...
main (int argc, char *argv[])
{
  ...
  if (NULL == gWSParamBlk.InStreamList)
    {
      fprintf (stderr,
               "### Error: at least one chunk file must be specified\n");
      status = EXIT_FAILURE;
      goto BAILOUT;
    }
  ...
  switch (gParseState)
    ...
    case kHaltSubscriber:
       ...
       */
      {
        ...
        CurHaltPtr->DataChunk.subscriberChunk.time
            = CurHaltPtr->DataChunk.streamerChunk.time;
        CurHaltPtr->Next = nil;
       ...
       */
      else if (strcmp (*argv, IOBUFSIZE_FLAG_STRING) == 0)
        {
          ...
          if (gIOBufferSize > 0 || gIOBufferSize < 65536)
            {
              fprintf (stderr,
                       "I/O buffersize must be between 1 and 65,535.\n\n");
              return false;
...
Usage (char *commandNamePtr)
{
  ...
  fprintf (
      stderr,
      "		%s <size>	I/O buffer size (0<size<65536, Def: 32k)\n",
