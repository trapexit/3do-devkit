...
NAME
      MovieToStream_Shuttle --	MPW tool to convert Cinepak compressed
QuickTime movie to a 3DO DataStream chunk file.

SYNOPSIS
      MovieToStream_Shuttle [flags] [movieFile(s)E]
...
HISTORY:
        10/10/94	dtc		Version 2.0b1
                                                Fix - Removed -f (frame rate)
command line option. 10/05/94	dtc		Version 2.0b1 Fix - Bug #2467:
MovieToStream crashes when converting PAL dimension 384X288. Also added
CLEAN_UP code to properly dispose of resources in DumpMovie. 10/02/94
Bug #2661: Check if QuickTime is installed. Bug #3297: Perform error checking
of blocksize (must be 2K bytes alignment). Update lastMarkerTime whenever a new
marker is written. Before exiting the program, delete output file(s) if an
error is encountered in DumpMovie. 08/17/94	dtc		Version 2.0
                                                Replaced Length with StrLength
(Length no longer works with MPW 3.4a4). 07/06/94	dtc
Version 1.9 Uncommented #include <TextUtils.h> in order to work with the new
interfaces. 05/13/94	dtc		Version 1.8 Integrated Craig
Weisenfluh's fix to allow user to specify the channel number: -c <channel>
                                                Added -o option to allow the
user to specify the output filename: -o <outputfilename> Changed usage
printout: print an option per line. Integrated the following Lynn Ackler's fix:
                                                        Added key chunk flag to
the chunkFlags, although redundant with the subChunk type, it does allow the
                                                        flexibility to have
other chunks types be "key" Added chunkFlags to the chunk header struct Added
key frame and diff frame subchunk types Add -s option to set the start time for
marker offsets. Necessary when conjoining numerous clips. Fixed a problem that
caused output of MovieToStream to be erroneous if the output file already
exists.  This was fixed by resetting the file position to the beginning and by
resetting the logical end of file if the file already exists. 03/21/94	MPH
Version 1.7 Integrated Lynn Ackler's fix for audio timebase slip due to
improper handling of time conversion in CvtTo3DOTime. 12/2/93		jb
Version 1.6 Integrated fyp's changes from 1.4p: fyp		Output markers
on key frame as per command line arg Marker rate (sec). Modified as follows: jb
Switch to using StdIO.h I/O for writing marker file. Close marker output file
on exit from DumpMovie() routine. Only write marker output file if "-m" option
is selected. 11/16/93	jb		Version 1.5 Fix truncation of returned
time value in CvtTo3DOTime() that prevented correct time in moview longer than
                                                273 seconds (4.55 minutes).
Routine used to truncate the upper 16 bits of its result. Split Usage() out as
a routine instead of strewing it all over main() as cut and paste. Change to
using EXIT_FAILURE & EXIT_SUCCESS result codes as return values from main().
        9/15/93		jb		Version 1.4
                                                Add "-v" command line switch
and the VFPRINTF macro to turn off chatty printf's scattered throughout the
DumpMovie() routine. 9/12/93		rdg		Version 1.3 Spin
cursor. 6/27/93		nc		Version 1.1 Disable sound processing
for now because it doesn't work yet... 5/27/93		jb
Version 1.0
------------------------------------------------------------------------------*/
...
// long		gFrameRate			= 30;			/* default frame
// rate == 30fps */
...
TimeValue
CvtToSoundTime (TimeValue mediaTime, StreamSoundHeaderPtr pSSndH,
...
TimeValue
CvtToMediaTime (TimeValue soundTime, StreamSoundHeaderPtr pSSndH,
                FilmHeaderPtr pFilmH)
{
   ...
   */
  conversionValue
      = (extended)(Fix2X (pSSndH->sampleRate) / Fix2X (pFilmH->scale << 16));
 ...
 *	timer, and the audio timer is the common timebase for the Data
 *Streamer.
 */
TimeValue
CvtTo3DOTime (TimeValue timeIn, Fixed scale)
{
  extended conversionValue, theResult;
  ...
  conversionValue = ((extended)(Fix2X (11025 << 16) / Fix2X (46 << 16)))
                    / ((extended)Fix2X (scale));
 ...
 */
long
CalcNumSamples (TimeValue soundChunkTime, long remainingBlockSize,
                StreamSoundHeaderPtr pSSndH)
{
  ...
  remainingBlockSize
      = (remainingBlockSize & 0xFFFFFFFC) - sizeof (StreamSoundChunk);
  soundChunkTime &= 0xFFFFFFFC;

  /* Determine the number of sound samples that will fit in the remaining
   * space. */
  remBlkTime
      = remainingBlockSize / (pSSndH->sampleSize >> 3) / pSSndH->numChannels;
  soundSampleTime
      = soundChunkTime / (pSSndH->sampleSize >> 3) / pSSndH->numChannels;
...
DumpMovie (Movie theMovie, Str255 filmFileName, Str255 keyFileName)
{
  ...
  for (n = 1; n <= GetMovieTrackCount (theMovie); n++)
    {
      ...
      m = GetTrackMedia (t);
      GetMediaHandlerDescription (m, &mediaType, originalName,
      ...
      if (mediaType == VideoMediaType)
        {
          ...
          if ((*IDesc)->cType != 'cvid')
            {
              fprintf (stderr, "\t\t#### This track is not compressed with "
                               "Compact Video.\n");
              fprintf (stderr,
                       "\t\t#### Recompress the movie and try again.\n");
              goto CLEAN_UP;
            }
        }
  ...
  for (n = 0; n < VideoMediaCount; n++)
    {
      ...
      if (movieTimeScale != GetMediaTimeScale (VideoMedia[n]))
        {
          fprintf (stderr,
                   "\t ###Movie time scale: %ld does not match track %d time "
                   "scale: %ld\n",
                   GetMovieTimeScale (theMovie), n,
                   GetMediaTimeScale (VideoMedia[n]));
          goto CLEAN_UP;
        }
    }
  ...
  if (VideoMedia[0])
    remainingBlockSize -= sizeof (FilmHeader);
  if (SoundMedia[0])
    remainingBlockSize -= sizeof (StreamSoundHeader);
  ...
  while (currentVideoTime < movieDuration)
    {
      ...
      if (SoundMedia[0] && soundLeft)
        {
           ...
           */
          soundTimeInMediaUnits
              = CvtToMediaTime (currentSoundTime, &SSndH, &FilmH);
          mediaTimePlusOneSec = currentVideoTime + SecsToMediaTime (1, &FilmH);
          soundChunkTime = CvtToSoundTime (
              mediaTimePlusOneSec - soundTimeInMediaUnits, &SSndH, &FilmH);
          ...
          if (mediaTimePlusOneSec >= soundTimeInMediaUnits)
            {
              ...
              VFPRINTF (stderr, "soundChunkTime = %d  %h\n", soundChunkTime,
                        soundChunkTime);
              numReqSamples = CalcNumSamples (soundChunkTime,
                                              remainingBlockSize, &SSndH);

              VFPRINTF (stderr, "CalcNumSamples returned numReqSamples = %d\n",
                        numReqSamples);

              Oops = GetMediaSample (SoundMedia[0], sounddata, 0, &size,
                                     currentSoundTime, &sampleTime, 0, 0, 0,
                                     numReqSamples, &numberOfSamples,
                                     &sampleFlags);
              if (Oops)
                {
                  fprintf (stderr,
                           "### %d Occurred during GetMediaSample for sound\n",
                           Oops);
                  goto CLEAN_UP;
                }
              ...
              if (size > gPhysBlockSize)
                {
                  fprintf (stderr,
                           "### Error : Sample size (%d) is greater than the "
                           "physical blocksize (%d).\n",
                           size, gPhysBlockSize);
                  fprintf (stderr, "### Error : Increase the blocksize and "
                                   "re-run the program.\n\n");
                  Oops = maxSizeToGrowTooSmall;
                  goto CLEAN_UP;
                }

              /* If the number of samples in the request is less than the
               * number requested, this is the last bufer of sound so clear the
               * flag.
               */
              if (numberOfSamples < numReqSamples)
                {
                  soundLeft = false;
                  size = (size + 3)
                         & 0xFFFFFFFC; /* round up to nearest longword */
                }

              VFPRINTF (stderr,
                        "currentSoundTime %3d, sampleTime %6d, Size %5d\n",
              ...
              // dtc				SSndC.time
              // = CvtTo3DOTime( currentSoundTime, SSndH.sampleRate );
              ...
              if (Oops = FSWrite (FilmFile, &writeSize, &SSndC))
                {
                  fprintf (stderr,
                           "### %d Occurred while writing StreamSoundChunk to "
                           "FilmFile\n",
                           Oops);
                  goto CLEAN_UP;
                }
              ...
              if (Oops = FSWrite (FilmFile, &size, *sounddata))
                {
                  fprintf (
                      stderr,
                      "### %d Occurred while writing sounddata to FilmFile\n",
                      Oops);
                  goto CLEAN_UP;
                }
      ...
      /* No sound so bump the soundChunkTime by one second to keep the video
       * writing.
       */
      else
        {
      ...
      do
        {
          ...
          for (x = VideoMediaCount - 1; x >= 0; x--)
            {
              if (currentVideoTime >= VideoMediaTrackOffset[x])
                break;
            }
          ...
          Oops = GetMediaSample (m, sampledata, 0, &size, mediatime,
                                 &sampleTime, &durationPerSample, 0, 0, 1, 0,
                                 &sampleFlags);
          if (Oops)
            {
          ...
          if (size > gPhysBlockSize)
            {
              fprintf (stderr,
                       ...
                       "physical blocksize (%d).\n",
                       size, gPhysBlockSize);
              fprintf (stderr, "### Error : Increase the blocksize and re-run "
                               "the program.\n\n");
              Oops = maxSizeToGrowTooSmall;
              goto CLEAN_UP;
            }
          ...
          // dtc			FilmF.time			= CvtTo3DOTime( sampleTime,
          // (Fixed)(mediaTimeScale << 16) );
          ...
          if (Oops = FSWrite (FilmFile, &writeSize, &FilmF))
            {
              fprintf (stderr,
                       "### %d Occurred while writing Film Frame Chunk to "
                       "FilmFile\n",
                       Oops);
              goto CLEAN_UP;
            }
          ...
          if (Oops = FSWrite (FilmFile, &writeSize, AlignedData))
            {
              fprintf (stderr,
                       "### %d Occurred while writing Film Frame Data to "
                       "FilmFile\n",
                       Oops);
              goto CLEAN_UP;
            }
...
main (int argc, char *argv[])
{
  ...
  for (parms = 1; parms < argc; parms++)
    {
      // not an option? must be a file name.  move it down so all file names
      // are sequential
      if (*argv[parms] != '-')
        {
          argv[++files] = argv[parms];
        }
      else if ((tolower (*(argv[parms] + 1)) == 'd')
               && (strlen (argv[parms]) == 2))
        {
          // bump to the next param & grab the output directory, as long as we
          // have a next param
          //  of course
          ++parms;
          if (parms == argc)
      ...
      else if ((tolower (*(argv[parms] + 1)) == 'b')
               && (strlen (argv[parms]) == 2))
        {
          // bump to the next param & grab the blocking size, as long as we
          // have a next param
          //  of course
          ++parms;
          if (parms == argc)
          ...
          if ((gPhysBlockSize % 2048) != 0)
            {
              fprintf (stderr,
                       "### %s -  Block size must be multiple of 2K bytes.\n",
                       argv[0]);
              return EXIT_FAILURE;
            }
        }
               ...
               && (strlen (argv[parms]) == 2))
        {
          // bump to the next param & grab the marker rate, as long as we have
          // a next param
          //  of course
          ++parms;
          if (parms == argc)
      ...
      else if ((tolower (*(argv[parms] + 1)) == 's')
               && (strlen (argv[parms]) == 2))
        {
          // bump to the next param & grab the start time, as long as we have a
          // next param
          //  of course
          ++parms;
          if (parms == argc)
      ...
      else if ((tolower (*(argv[parms] + 1)) == 'c')
               && (strlen (argv[parms]) == 2))
        {
          // bump to the next param & grab the channel, as long as we have a
          // next param
          //  of course
          ++parms;
          if (parms == argc)
          ...
          if (!UserOutputFn)
            {
              ...
              //					MakeDestPath(newFnNamePtr,
              //srcMovieName, destFilmDir, "\p.chunk");		// ccw
              MakeDestPath (newFnNamePtr, srcMovieName, destFilmDir,
                            "\p.FILM");
            }
