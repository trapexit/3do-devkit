 ...
 *						Deleted ifndef __DATASTREAM_H__.  This
 *define should be part of the DataStream.h header. 5/19/94		fyp
 *Version 2.0 Integrated dlg's changes (3.30.94).  Added DSSetBranchType()
 *routine. 3/14/94		lla		Replaced IsKeyFrame() with
 *DSGetChunkFlag() routine to enable reading chunkFlags as well as chunk cues.
 *	3/4/94		lla		Added IsKeyFrame() routine. & Added
 *DSSetSkipMode proto. 1/20/94		rdg		make C++ compatible
...
extern "C"
{
  ...
  int32 DSSetSkipMode (Item msgItem, DSRequestMsgPtr reqMsg,
                       DSStreamCBPtr streamCBPtr, unsigned long markerValue,
                       unsigned long options);
  int32 FlushAllSubscribers (DSStreamCBPtr streamCBPtr, MemPoolPtr msgPoolPtr,
                             StreamChunkPtr cp);
