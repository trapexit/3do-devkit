 ...
 *						Removed if-else RELATIVE_BRANCHING at end
 *HandleDataAcqReply that enclosed a check for unexpected replies from DataAcq.
 *						Check should be the same whether or not
 *RELATIVE_BRANCHING is defined. 09/19/94	dtc		Version 2.1b2
 *						Fix: Bug #3315. Ignore new data from
 *DataAcq until Data Acq replies to go-marker request.
 *	09/16/94	nmd		added support for portfolio style error
 *codes. 09/07/94	dtc		Version 2.1a1 Replaced printf with ERR
 *and fixed HALT_ENABLE code.  The previous version had a race condition when
 *resuming delivery of data chunks from the current buffer but the buffer was
 *already freed by _ReleaseChunk. Redesigned HALT_ENABLE code and added two new
 *functions:
 *						  - _CreateHaltChunkMsg to create the new
 *message, message port and message item,
 *						  - DeliverHaltCHunk to deliver the halt
 *chunk on a separate message port and wait for the reply before resuming data
 *							delivery.
 *						Fixed _InitializeThread ABORTOPEN code:
 *check the streamCBPtr->creatorStatus flag for error code and call
 *						_TearDownStreamCB if an error occurred
 *during initialization. Removed DISCOVER_STUPID_OS_MSG_BUG compile switch in
 *DeliverData. 7/29/94	dtc		Version 2.0.1d2 Replaced NULL with
 *"DataStreamThread" as thread name when creating DataStream thread in
 *NewDataStream. 7/1/94	dtc		Version 2.0.1d1 Bug #1622: Fixed
 *_DeleteSubscriber to correctly "slide down" the entries in subscriber table.
 *  6/20/94		dld		Added HALT_ENABLE feature.  When a halt
 *chunk is encountered the chunk embeded within the HALT chunk is sent to it's
 *subscriber and then the data streamer immediatly halts.  Data acquisition
 *						continues, but no further data messages
 *are sent to subscribers (The STOP chunk does not do this).  The streamer
 *hangs out in halt mode until there is a reply to the message embeded in the
 *						HALT chunk, and the processing continues.
 *This is the only way out of the HALT state. 5/19/94		fyp
 *Version 2.0 integrated lla and nmd as condition RELATIVE_BRANCHING 3/9/94
 *lla		Moved the DoGoMarker into HandleDSRequest (DSSetSkipMode)
 *	3/6/94		lla		Sync'd all subscribers using
 *DSClockSync () 2/28/94		nmd		Added searching for key
 *frame in DeliverData. 12/14/93	jb		Version 1.9.1 Moved
 *setting of the "branch in progress" flag and the translated branch
 *destination value to an external routine callable by data acquisition called
 *DSSetBrancDest(). Code was moved from the DoGoMarker() routine to set the
 *flag and store the target value. 12/2/93		jb
 *Version 1.9 Added argument to NewDataStream() to specify the number of
 *						subscriber messages that should be
 *allocated upon stream creation. 11/23/93	jb		Version 1.8 Add
 *the CloseDataStreaming() routine to allow for deallocation of context block
 *pool. Connect message to data acquisition now includes a pointer to the
 *stream control block of the connecting stream. 11/22/93	jb
 *Version 1.7 Add check for impossible chunk sizes, zero or negative, in the
 *DeliverData() routine. 10/21/93	rdg		Version 1.6 Modified
 *DoStopStream() to always send a stop message even if the stream is already
 *stopped so if a flush option is present, it will always be passed on to the
 *subscribers. 10/19/93	jb		Version 1.5 Switch to using UMemory.c
 *routines for memory allocation 10/4/93		jb
 *Version 1.4 Added End Of Stream notification. 8/18/93		jb
 *Version 1.3.4 Initialize repliesPending to zero in HandleDSRequest() before
 *						dispatching to a request handler. Also,
 *reply to requests immediately in that routine if processing is completed and
 *						no replies from dataacq or subscribers
 *are required. 8/16/93		jb		Version 1.3.2 Add call to
 *OpenAudioFolio() in _InitializeThread() if the compile switch
 *STARTSTREAM_SETS_THE_CLOCK is set. Add a range check on the whatToDo field
 *HandleDSRequest() to prevent queueing of totally invalid request messages.
 *	8/5/93		jb		Version 1.3.1
 *						Add EXIT_ONLY_AFTER_CLOSE_REPLIES related
 *code to cause the streamer only to exit after all replies to subscriber close
 *						messages have been received.
 *						Fix bug in DoSetChannel() that would
 *always return an error status. 8/3/93		jb		Set privatePtr
 *= NULL in DoConnect() for the disconnect message sent to previous data
 *acquisition thread. Get rid of semaphore control of message pool since
 *procedure level access to the library routines can't use the messages due to
 *resource ownership problems anyway. 7/9/93		jb
 *Version 1.3 Convert all subscriber messaging to asynchronous operation. Keep
 *						the streamer single threaded by adding a
 *request message queue that incoming streamer requests are queued upon. The
 *message at the head of this queue is the "current" request that is being
 *processed. It gets replied to when all subscribers/data acquisition threads
 *						have operated on any subsequent message
 *sends that are used to implement the request to the streamer. Added
 *HandleSubscriberReply() routine. Added _AllocSubsMsg() routine to allocate
 *subscriber messages under control of a semaphore. Get rid of
 *_CallDataAcquisition() routine as all communications with data acquisition
 *are now asynchronous. 6/25/93		jb		Version 1.2.7 Change
 *check in _ReleaseChunk() to == 0 from <= 0 so that if someone replies
 *chaotically to messages we don't get confused about the buffer. Move
 *processing of DSClockSync() to a procedure in the DataStreamLib.c file so
 *that sync can be sent immediately instead of suffering propagation delay by
...
static char *DSErrorText[] = {
  ...
  /* kDSBadBufAlignErr */
  "buffer list passed contains a non QUADBYTE aligned buffer",
  /* kDSBadChunkSizeErr */ "chunk size in stream is a non-QUADBYTE multiple",
  /* kDSInitErr */
  "the streamer has fallen, and it can't get up (internal initialization "
  "failed)",
  /* kDSClockNotValidErr */
  ...
  "End Of Stream registrant replaced by new registrant",
...
static TagArg ErrorTags[]
    = { TAG_ITEM_NAME,
        (void *)"DataStream",
        ERRTEXT_TAG_OBJID,
        (void *)((ER_LINKLIB << ERR_IDSIZE) | (ER_DATASTREAM)),
        ERRTEXT_TAG_MAXERR,
        (void *)(sizeof (DSErrorText) / sizeof (char *)),
        ERRTEXT_TAG_TABLE,
        (void *)DSErrorText,
        ERRTEXT_TAG_MAXSTR,
        (void *)MAX_ERROR_LEN,
...
/*******************************************************************************************
 * This routine links the DataStreamers private error codes into the system
 *error handler
 *******************************************************************************************/
static Err
_InstallDSErrors (void)
{
...
_TearDownStreamCB (DSStreamCBPtr streamCBPtr)
{
  ...
  if (streamCBPtr->acquirePort)
    RemoveMsgPort (streamCBPtr->acquirePort);
  if (streamCBPtr->acqReplyPort)
    RemoveMsgPort (streamCBPtr->acqReplyPort);
  if (streamCBPtr->requestPort)
    RemoveMsgPort (streamCBPtr->requestPort);
  if (streamCBPtr->subsReplyPort)
    RemoveMsgPort (streamCBPtr->subsReplyPort);

#if HALT_ENABLE
  if (streamCBPtr->halted_msg)
    FreePtr (streamCBPtr->halted_msg);
  if (streamCBPtr->haltChunkReplyPort)
    RemoveMsgPort (streamCBPtr->haltChunkReplyPort);
#endif
}
                    ...
                    Message *messagePtr)
{
   ...
   */
  else if (acqMsg->whatToDo == kAcqOpGetData)
    {
       ...
       */
      if (messagePtr->msg_Result == 0 && !streamCBPtr->fDiscardRcvdData)
        {
        ...
        {
          /* Some kind of non-successful completion occurred. This may not
           * really be an error, per se, but some kind of special completion,
           * like EOF. We may be here also due to fDiscardRcvdData (i.e., stale
           * buffers) or if this data reply is not from the current DataAcq.
           * Always return these buffers to the free pool for eventual re-use.
           */
          _FreeDataBuf (streamCBPtr, bp);

          /* If this data is stale, then discount its contents and its
           * msg_Result. */
      ...
      switch (messagePtr->msg_Result)
        ...
        default:
          ERR (("DataStreamThread - HandleDataAcqReply - kAcqOpGetData failed "
                "with msg_Result = %ld\n",
                messagePtr->msg_Result));
...
/*******************************************************************************************
 * Routine to parse the current buffer and deliver all data chunks left in a
 *buffer, if any to the appropriate subscribers. This routine is the guts of
 *DeliverData.
 *******************************************************************************************/
static int32
DeliverDataChunks (DSStreamCBPtr streamCBPtr, DSDataBufPtr bp,
                   StreamChunkPtr cp)
{
  SubscriberMsgPtr subMsg;
  int32 status;
  ...
  while (((char *)cp) < EndOfBufferPtr)
    {
       ...
       *  for some subscriber - that chunk is sent to the subscriber
       * immediately prior to halting.  When a reply is recieved for that
       * embeded chunk the data streamer exits HALT mode by resuming normal
       * execution.
       */
...
/*******************************************************************************************
 * Routine to deliver halt chunk in a buffer.  This routine waits for a replay
 *from the subscriber before resuming data delivery. [TBD] We may want to
 *revisit this design later.  Halting DataStreamer for a long period of time
 *may cause resources (i.e. buffers ) to not be freed which may be a bad thing.
 *******************************************************************************************/
#if HALT_ENABLE
static int32
DeliverHaltChunk (DSStreamCBPtr streamCBPtr, DSDataBufPtr bp,
                  StreamChunkPtr cp)
{
                 ...
                 Boolean *pFKeepRunning)
{
  ...
  switch (reqMsg->whatToDo)
    ...
    case kDSOpSetSkipMode:
      ...
      status = DoGoMarker (streamCBPtr, reqMsg);
      streamCBPtr->repliesPending
          = 0; /* NJC - can't accept a reply {lla - so he says}*/
      streamCBPtr->streamFlags |= STRM_KEYCHUNK_SEARCH;
      break;
#endif
...
#if HALT_ENABLE
static int32
_CreateHaltChunkMsg (DSStreamCBPtr streamCBPtr)
{
  ...
  streamCBPtr->halted_msg
      = (SubscriberMsgPtr)NewPtr (sizeof (SubscriberMsg), MEMTYPE_ANY);
...
ABORTOPEN:
  /* ASSUMES: streamCBPtr->creatorStatus now holds kDSNoErr (success) or a
   * negative error code. Inform our creator that we've finished with
   * initialization. We are either ready and able to accept requests for work,
   * or we failed to initialize ourselves. If the latter, we simply exit after
   * informing our creator. All resources we created are thankfully cleaned up
