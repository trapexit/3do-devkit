 ...
 *compler complaints Fixed cut and paste bug: the subscriber trace error codes
 *						were named "ProtoErrorCode" and collided
 *with ProtoSubscriber. 3/29/94		rdg		Version 1.4 Create a
 *"TraceBuffer" struct which contains the index and the data.
 *	3/25/94		rdg		Version 1.3
 *						Last implementation was fucked.  This
 *time, let every subscriber have it's own buffer.  The routines prototyped
 *here now take a pointer to a trace buffer and a file name to write the output
 *to, instead of using globals. 1/12/93		rdg		Version 1.2
 *						Update for use with multiple
 *subscribers. 12/24/93	rdg		Version 1.1 Generalize for use with
 *other subscribers. 10/14/93	rdg		Version 1.0  -  New Today
 *
 *
 * USAGE:
 *
