 ...
 *						Fixed un-terminated comment in
 *IsTimeForNextFrame. Replaced printf with ERR macro defined in Debug3DO.h. Fix
 *- Bug 2164: IsTimeForNextFrame fixed to make looping to beginning of stream
 *work. 8/17/94	dtc		Version 2.0.1d3 Fixed IsTimeForNextFrame to
 *delivery the first frame on time. 7/29/94	dtc		Version 2.0.1d2
 *						Replaced NULL with "CPakSubsSThread" as
 *thread name when creating CPakSubscriberS thread in NewCPakSubscriber. Used
 *DataStream version number of 2.0.1dx to be consistent. 2/16/94
 *lla		Added catchup strategy parameter: skip/decode difference frames
 *to make up time 2/2/94		lla		Distinguishes between
 *difference frames and key frames for ketchup 1/24/94		jb
 *Version 1.3 Added GetStepCPakCel() and DrawStepCPakToBuffer() stepframe
 *support. 11/23/93	jb		Version 1.2 Added CloseCPakSubscriber()
 *to clean up resources allocated by the InitCPakSubscriber() routine. 10/19/93
 *jb		Version 1.1 Switch to using UMemory.c routines for memory
 *allocation 10/14/93	jb		Add code identified by
...
static void _DrawCPakToBuffer (CPakContextPtr ctx, CPakRecPtr cpRecPtr,
...
Boolean
TimeToCatchUp (CPakRecPtr cpRecPtr)
{
  CinePakFramePtr curFramePtr;
  uint32 curTime;
 ...
 *******************************************************************************************/
CCB *
GetStepCPakCel (CPakContextPtr ctx, CPakRecPtr cpRecPtr)
{
 ...
 *******************************************************************************************/
static CCB *
_GetCPakCel (CPakContextPtr ctx, CPakRecPtr cpRecPtr, Boolean fStepFrame)
{
  SubsChunkDataPtr pChunkData;
      ...
      if (PollCPakChunk (ctx, cpRecPtr, &pChunkData, &subMsg))
        {
          switch (pChunkData->subChunkType)
            ...
            default:
              ERR (("GetCPakCel --- Unknown Chunk Type: '%.4s'",
  ...
  if (cpRecPtr->curFramePtr != NULL)
    {
      if (DSGetClock (cpRecPtr->streamCBPtr, &curTime) >= 0)
        {
          if (curTime > (cpRecPtr->curFramePtr->time
                         + cpRecPtr->curFramePtr->duration))
            {
              if (cpRecPtr->curSubMsg)
                {
                  DoFreeChunk (ctx, cpRecPtr->curSubMsg);
 ...
 *******************************************************************************************/
void
DrawStepCPakToBuffer (CPakContextPtr ctx, CPakRecPtr cpRecPtr, Bitmap *bitmap)
{
 ...
 *******************************************************************************************/
static void
_DrawCPakToBuffer (CPakContextPtr ctx, CPakRecPtr cpRecPtr, Bitmap *bitmap,
      ...
      if (PollCPakChunk (ctx, cpRecPtr, &pChunkData, &subMsg))
        {
          switch (pChunkData->subChunkType)
            ...
            default:
              ERR (("_DrawCPakToBuffer --- Unknown Chunk Type: '%.4s'",
                    &(pChunkData->subChunkType)));
              break;
            } // switch
          framesCaught++;
 ...
 *display the Cpak movie.  App developer beware, you can display out of the
 *frame buffer.
 *******************************************************************************************/
void
SetCpakScreen (int32 horizontalOffset, int32 verticalOffset, Boolean centered)
{
