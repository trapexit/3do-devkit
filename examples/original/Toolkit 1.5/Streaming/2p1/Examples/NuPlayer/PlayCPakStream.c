 ...
 *
 *	History:
 *	10/06/94		dtc		Use the default stream header if one
 *is not available.
 *	09/14/94		dtc		Replaced printf with ERR macro in
 *Debug3DO.h.
  ...
  for (subscriberIndex = 0;
       ...
       subscriberIndex++)
    {
      ...
      switch (subsPtr->subscriberType)
        ...
        default:
          ERR (("InitCPakPlayerFromStreamHeader() - unknown subscriber in "
                "stream header: '%.4s'\n",
