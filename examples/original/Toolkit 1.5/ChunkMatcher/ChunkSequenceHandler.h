...
protected:
  typedef void (TChunkSequenceHandler::*TChunkSequenceFunction) (
...
private:
  ...
  void
  operator= (const TChunkSequenceHandler &)
  {
  }
...
private:
  ...
  const TChunkSequence
      *chunkSequence[MAX_SEQUENCES]; // pointers to sequences handled
  TChunkSequenceFunction
      handler[MAX_SEQUENCES]; // pointers to handler functions
};
