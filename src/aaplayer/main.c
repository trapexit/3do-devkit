...
/**
   ...
   |||	    background image. Allows the user to change the animation's frame
   rate,
   |||	    and the four corners of the animation's cels, and to toggle
   displaying the
   |||	    matte cel.
   |||
   |||	  Arguments
...
Usage(void)
{
  ...
  printf("A + up or down        toggles display of matte cel\n");
  printf("A + left or right     changes anims frame rate\n");
  printf("Stop                  exits\n");
...
Initialize(ScreenContext *sc, int32 nScreens)
{
  ...
  */
  ...
  /*
    get the item number of the system timer, called "timer" and an io req
    for doing timer queries so we can adjust the frame rate of the animation.
  */
 ...
 **	    a 3DO anim or cel, drops into a main loop where it queries the
...
main(int32 argc, char *argv[])
{
  ...
  printf("Usage: %s <imagefile> <animfile>\n", argv[0]);
  if(argc != 3)
    {
      imagefile = "aaplayer_data/seafloor.imag";
      animfile = "aaplayer_data/jim.anim";
    }
  else
    {
      imagefile = argv[1];
  ...
  */
  ...
  /*
    ...
    This gets sorta tricky in this case, because we are loading an
    anti-aliased animation.  An anti-aliased animation consists of two cels
    per frame of animation. The one cel is the actual cel, with the second
    being the matte cel that is used to dimish the background and create the
    anti-aliasing effect. This means that each frame consists of two cels, one
    is the matte cel, the second is the actual frame.  This means that we have
    to handle this differently from normal animations.

    How you bring it in will determine what you do with them. If you load in a
    AA cel with LoadCel(), it will return you a pointer to the matte ccb with
    the actual ccb chained to it.  Then you can just call DrawCels()(You want
    the matte cel drawn first).

    Or, if you bring in the AA cel with LoadAnim(), then you get two ccbs in
    the animation, the first being the cel, and the second being the matte.
    Chain them together in the right order and then draw them.

    If you call LoadAnim() on an anti-aliased animation, then you get a the
    ccb's and you have to chain each pair together.
  */
  anim = LoadAnim(animfile, MEMTYPE_CEL);
  if(anim == NULL)
    {
  ...
  */
  if(anim->num_Frames > 0)
    {
      printf("%ld cels found in %s\n", anim->num_Frames, animfile);
    }
  else
    {
  ...
  */
  ...
  /*
    chain the two dummy cels together, alpha channel first so that it
    will be drawn first
  */
  ...
  */
  ...
  /*
    ...
    Given the way the animation is kept in the anti-aliased anim, we can't
    really use this field to adjust for frame rate, as we could in normal
    animations.  That's because each true frame consists of a pair of cels,
    and GetAnimCel() doesn't have enough intelligence in it to figure out how
    to return the proper cel that you are expecting.
  */
  gFrameIncr = 1 << 16;
  gMoveIncr = MOVE_DELTA << 16;

  /*
    Rather than using the frame increment to GetAnimCel(), we are going
    to be using a timer to figure out if its time to advance the animation,
    given a desired frame rate.

    Here, we figure out what time it is now, so we can figure frame
    timing differences
  */
  ...
  while(true)
    {
      ...
      */
      ...
      /*
        check to see if its time to advance to the next animation frame.
        If so, the grab the next frames
      */
      if(AdvanceFrame(pIOReq, &lastTime) > 0)
        {

          /*
            Here is a nifty trick.  Remember that ccb we allocated?  Well, we
            use it here.
            ...
            words, if we didn't reset the positioning values, then the anim
            would suddenly appear where it was a while ago.

            So what we do instead, is have a dummy ccb around, which contains
            all the positioning information in it.  Then we just shift the
            source and plut pointers of this cel to the cels in the animation.

            This way, a lot of the data for the cel is kept in one place and
            our housekeeping is made much easier
          */
...
HandleButton(uint32 button, CCB *alphaCCB)
{
  ...
  /*
    in this case, we have to adjust the positions of two cels,
    identically. Because we've linked the matte and true cels together, we can
    do all the calculations on one, and then just reset the CCB values of the
    other.
  */
 ...
 **	  Description
 **
 **	    This function queries the system microsecond timer for the current
 *time
 **
 **	  Arguments
 **
 **	    pIOReq	             pointer to the IO request to use in
 *querying the timer
 **
 **	    tv		             pointer to a timeval structure to contain
 *the current time
 **
 **	  Return Value
 **
...
GetTime(IOReq *pIOReq, struct timeval *tv)
{
  ...
  */
  ...
  /*
    set the fields appropriately.  We want to read from the microsecond
    timer. The read operation should fill in the memory pointed to by tv.
 ...
 **
 **	    This function queries the system microsecond timer for the current
 *time, and *	    compares it to the time of the previous frame.  If the
 *current time exceeds that *	    time plus the frame's duration, then the
 ...
 *querying the timer
 **
 **	    lastTime             pointer to a timeval structure that contains
 *the time at which
 **	    					 the current frame was first
 *displayed
 **
 **
 **	  Return Value
 **
 **	    The function returns an error code if an error occurs, false if it
 *is not yet time *	    to display the next frame, or true if it is time to
...
AdvanceFrame(IOReq *pIOReq, struct timeval *lastTime)
{
  ...
  /*
    ...
    If we desire a particular frame rate, then each frame's duration is
    1/frame rate, multiplied by 1,000,000 to convert from seconds to
    microseconds.
  */
