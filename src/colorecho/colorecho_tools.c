...
struct CCB EchoCCB = {
  ...
  */
  ...
  ((DISPLAY_WIDTH - PRE1_WOFFSET_PREFETCH) << PRE1_WOFFSET10_SHIFT) | PRE1_LRFORM
      |                 /* LR form cel */
...
ce_DrawNextScreen(ScreenContext *sc, ColorEcho *ce)
{
  ...
  /* Point CCB to bitmap. */
  ce->ce_CCB->ccb_SourcePtr
      = (CelData *)GetPixelAddress(sc->sc_Screens[(1 - sc->sc_curScreen)], 0, 0);
