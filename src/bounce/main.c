...
/**
   ...
   |||	  Description
   |||
   |||	    This is a small demo program where each of the elements of the 3DO
   logo
   |||	    bounce around a 3D room. The logo elements distort upon contact
   with the
   ...
   |||	  Associated Files
   |||
   |||	    bounce.c, bounce.make, bounce.h, bounce_sound.c, bounce_sound.hThe
   data
   |||	    files are:BounceFolder/Art/wallo.img     wall, light on image
   |||	    BounceFolder/Art/wallf.img     wall, light off image
   |||	    BounceFolder/Art/globe.anim    globe
   animationBounceFolder/Art/globes.cel
   |||	      globe shadow celBounceFolder/Art/tv.anim tv animation
   |||	    BounceFolder/Art/tvs.animtv shadow
   animationBounceFolder/Art/cube.celcube
   |||	    celBounceFolder/Art/cubes.cel     cube shadow
   celBounceFolder/Art/ball.cel
...
#define HORZ_OVERLAP(_thisCcb, _thisXPos, _otherCcb, _otherXPos)        \
  ((_thisXPos + THIS_INSET                                              \
    <= _otherXPos + (_otherCcb->ccb_Width - OTHER_INSET))               \
   && (_thisXPos + (_thisCcb->ccb_Width - THIS_INSET)                   \
       >= _otherXPos + OTHER_INSET))

/* Collision subtest for objects moving horizontally */
#define VERT_OVERLAP(_thisCcb, _thisYPos, _otherCcb, _otherYPos)        \
  ((_thisYPos + THIS_INSET                                              \
    <= _otherYPos + (_otherCcb->ccb_Height - OTHER_INSET))              \
   && (_thisYPos + (_thisCcb->ccb_Height - THIS_INSET)                  \
       >= _otherYPos + OTHER_INSET))

/* condition which must be met for a collision while this object is moving
 * upward */
#define UP_COLLISION(_thisCcb, _thisXPos, _thisYPos, _otherCcb, _otherXPos, _otherYPos) \
  (HORZ_OVERLAP(_thisCcb, _thisXPos, _otherCcb, _otherXPos)             \
   && BETWEEN(_otherYPos + (_otherCcb->ccb_Height - OTHER_INSET),       \
              _thisYPos + THIS_INSET,                                   \
              _otherYPos + (_otherCcb->ccb_Height - THIS_INSET)))

/* condition which must be met for a collision while this object is moving
 * downward */
#define DOWN_COLLISION(_thisCcb, _thisXPos, _thisYPos, _otherCcb, _otherXPos, _otherYPos) \
  (HORZ_OVERLAP(_thisCcb, _thisXPos, _otherCcb, _otherXPos)             \
   && BETWEEN(_otherYPos + THIS_INSET,                                  \
              _thisYPos + (_thisCcb->ccb_Height - THIS_INSET),          \
              _otherYPos + OTHER_INSET))

/* condition which must be met for a collision while this object is moving to
 * the left */
#define LEFT_COLLISION(_thisCcb, _thisXPos, _thisYPos, _otherCcb, _otherXPos, _otherYPos) \
  (BETWEEN(_otherXPos + (_otherCcb->ccb_Width - OTHER_INSET),           \
           _thisXPos + THIS_INSET,                                      \
           _otherXPos + (_otherCcb->ccb_Width - THIS_INSET))            \
...
static void
PrintSysError(const char *call, const char *detail, Err err)
{
  char errString[256];
  ...
  if(detail)
    printf("bounce: %s failed for %s: 0x%lx (%s)\n", call, detail, err, errString);
  else
    printf("bounce: %s failed: 0x%lx (%s)\n", call, err, errString);
}

static void
PrintFailure(const char *call, const char *detail)
{
  if(detail)
    printf("bounce: %s failed for %s\n", call, detail);
  else
    printf("bounce: %s failed\n", call);
}

static Err
CheckErr(const char *call, const char *detail, Err err)
{
  if(err < 0)
    PrintSysError(call, detail, err);

  return err;
}

static Err
DrawCelsChecked(Item bitmapItem, CCB *ccb, const char *detail)
{
  return CheckErr("DrawCels", detail, DrawCels(bitmapItem, ccb));
}
...
clear_screen(int color)
{
  SetVRAMPages(gVRAMIOReq,
               ...
               color,
               gScreenContext->sc_NumBitmapPages,
               0xFFFFFFFF);
}
...
ShowInstructions(int32 *screenSelect)
{
  ...
  bitmapItem = gScreenContext->sc_BitmapItems[0];
  clear_screen(0x00000000);

  gcon.gc_PenX = x;
  gcon.gc_PenY = 36;
  DrawText8(&gcon, bitmapItem, (const u8 *)"bounce");

  gcon.gc_PenX = x;
  gcon.gc_PenY = 60;
  DrawText8(&gcon, bitmapItem, (const u8 *)"Controls:");

  gcon.gc_PenX = x;
  gcon.gc_PenY += 12;
  DrawText8(&gcon, bitmapItem, (const u8 *)" A - toggle bouncing");

  gcon.gc_PenX = x;
  gcon.gc_PenY += 12;
  DrawText8(&gcon, bitmapItem, (const u8 *)" B - toggle collisions");

  gcon.gc_PenX = x;
  gcon.gc_PenY += 12;
  DrawText8(&gcon, bitmapItem, (const u8 *)" C - reset positions");

  gcon.gc_PenX = x;
  gcon.gc_PenY += 12;
  DrawText8(&gcon, bitmapItem, (const u8 *)" Arrows - rearrange objects");

  gcon.gc_PenX = x;
  gcon.gc_PenY += 12;
  DrawText8(&gcon, bitmapItem, (const u8 *)" L - toggle light");

  gcon.gc_PenX = x;
  gcon.gc_PenY += 12;
  DrawText8(&gcon, bitmapItem, (const u8 *)" X - return to menu");

  gcon.gc_PenX = x;
  gcon.gc_PenY += 12;
  DrawText8(&gcon, bitmapItem, (const u8 *)"Press any button to play");
  ...
  do
    {
      ...
      if(err < 0)
        PrintSysError("DoControlPad", NULL, err);
    }
  while(buttons == 0);
...
Err
DrawGlobe(Item currentBitmapItem)
{
  if(gLightOn)
    {
      return DrawCelsChecked(currentBitmapItem, gGlobeSCcb, "globe shadow cel");
    }
  else
    {
      return DrawCelsChecked(currentBitmapItem, gGlobeCcb, "globe cel");
    }
}
...
*/
{
  if(aQuad[kQuadTL].pt_X <= minX)
    aQuad[kQuadTL].pt_X = minX;
  if(aQuad[kQuadTR].pt_X <= minX)
    aQuad[kQuadTR].pt_X = minX;
  if(aQuad[kQuadBR].pt_Y <= minY)
    aQuad[kQuadBR].pt_Y = minY;
  if(aQuad[kQuadBL].pt_Y <= minY)
    aQuad[kQuadBL].pt_Y = minY;
  if(aQuad[kQuadTL].pt_X >= maxX)
    aQuad[kQuadTL].pt_X = maxX;
  if(aQuad[kQuadTR].pt_X >= maxX)
    aQuad[kQuadTR].pt_X = maxX;
  if(aQuad[kQuadBR].pt_Y >= maxY)
    aQuad[kQuadBR].pt_Y = maxY;
  if(aQuad[kQuadBL].pt_Y >= maxY)
    aQuad[kQuadBL].pt_Y = maxY;
}
...
*/
{
  ...
  if(gVRAMIOReq < 0)
    {
      PrintSysError("CreateVRAMIOReq", NULL, gVRAMIOReq);
      goto DONE;
    }
  ...
  if(gScreenContext == NULL)
    {
      PrintFailure("AllocMem", "ScreenContext");
      goto DONE;
    }
  ...
  if(errorCode < 0)
    {
      PrintSysError("OpenGraphicsFolio", NULL, errorCode);
      goto DONE;
    }
  ...
  if(gDisplayType < 0)
    {
      PrintSysError("GetDisplayType", NULL, gDisplayType);
      goto DONE;
    }
  ...
  if((gDisplayItem = CreateBasicDisplay(gScreenContext, gDisplayType, 2)) < 0)
    {
      PrintSysError("CreateBasicDisplay", NULL, gDisplayItem);
      goto DONE;
    }
  ...
  errorCode = InitControlPad(2);
  if(errorCode < 0)
    {
      PrintSysError("InitControlPad", NULL, errorCode);
      goto DONE;
    }

  errorCode = OpenAudioFolio();
  if(errorCode < 0)
    {
      PrintSysError("OpenAudioFolio", NULL, errorCode);
      goto DONE;
    }
...
*/
{
  ...
  CheckErr("CloseAudioFolio", NULL, CloseAudioFolio());
  ...
  if(gDisplayItem >= 0)
    CheckErr("DeleteBasicDisplay", NULL, DeleteBasicDisplay(gScreenContext));

  /* Close the graphics folio because we opened it
     to get the display type */
  CheckErr("CloseGraphicsFolio", NULL, CloseGraphicsFolio());
...
main(int argc, char **argv)
{
  ...
  /*
    variables which specify the direction in which the corresponding
    object is traveling
  */
  ...
  while(TRUE)
    {
      ...
      */
      ...
      SetQuadFromPosAndSize(
                            aQuad, xPos, yPos, gBallCcb->ccb_Width, gBallCcb->ccb_Height);
      ...
      */
      ...
      if(gGlobeYPos > 180)
        ySpeed = 13;
      else if(gGlobeYPos > 160)
        ySpeed = 11;
      else if(gGlobeYPos > 140)
        ySpeed = 9;
      else if(gGlobeYPos > 120)
        ySpeed = 7;
      else if(gGlobeYPos > 100)
        ySpeed = 6;
      else if(gGlobeYPos > 80)
        ySpeed = 5;
      else if(gGlobeYPos > 60)
        ySpeed = 4;
      else if(gGlobeYPos > 35)
        ySpeed = 3;
      else if(gGlobeYPos > 10)
        ySpeed = 2;
      else /* if ( gGlobeYPos <= 10 ) */
        ySpeed = 1;
      ...
      if(gBouncing)
        ...
        /* When the other objects aren't bouncing, the globe still is, but
         * within special limits */
        {
          if((globeLftDir) && ((gGlobeXPos -= GLOBE_XSPEED) <= GLOBE_NOBOUNCE_XMIN))
            globeLftDir = FALSE;
          if((!globeLftDir)
             && ((gGlobeXPos += GLOBE_XSPEED) >= GLOBE_NOBOUNCE_XMAX))
            globeLftDir = TRUE;
        }

      /* Prepare the next frame of the object animation */
      gGlobeCcb = GetAnimCel(gGlobeAnim, gGlobeFrameIncr);
      SetQuadFromPosAndSize(aQuad,
                            gGlobeXPos,
                            gGlobeYPos,
                            gGlobeCcb->ccb_Width,
                            gGlobeCcb->ccb_Height);
      SpecialConstrainQuad(aQuad, MAX_RIGHT);
      ...
      */
          ...
          if(tvUpDir)
            {
              ...
              if(gCollision)
                {
                  if(UP_COLLISION(
                                  gTvCcb, gTvXPos, gTvYPos, gCubeCcb, gCubeXPos, gCubeYPos))
                    {
                      tvUpDir = FALSE;
                      cubeUpDir = TRUE;
      ...
      SetQuadFromPosAndSize(
                            aQuad, gTvXPos, gTvYPos, gTvCcb->ccb_Width, gTvCcb->ccb_Height);
      SpecialConstrainQuad(aQuad, MAX_RIGHT);
      MapCel(gTvCcb, aQuad);
      ...
      if(gLightOn)
        {
          ...
          ConstrainQuad(
                        aQuad, TVSHADOW_XMIN, TVSHADOW_XMAX, TVSHADOW_YMIN, TVSHADOW_YMAX);
        }
      ...
      */
      ...
      if(gCubeYPos > 180)
        ySpeed = 18;
      else if(gCubeYPos > 160)
        ySpeed = 16;
      else if(gCubeYPos > 140)
        ySpeed = 14;
      else if(gCubeYPos > 120)
        ySpeed = 12;
      else if(gCubeYPos > 100)
        ySpeed = 10;
      else if(gCubeYPos > 80)
        ySpeed = 8;
      else if(gCubeYPos > 60)
        ySpeed = 6;
      else if(gCubeYPos > 35)
        ySpeed = 4;
      else if(gCubeYPos > 10)
        ySpeed = 2;
      else /* ( gCubeYPos <= 10 ) */
        ySpeed = 1;
      ...
      SetQuadFromPosAndSize(
                            aQuad, gCubeXPos, gCubeYPos, gCubeCcb->ccb_Width, gCubeCcb->ccb_Height);
      ...
      */
      {
        ...
        if(CheckErr(
                    "CopyVRAMPages",
                    "room background",
                    CopyVRAMPages(gVRAMIOReq,
                                  gScreenContext->sc_Bitmaps[screenSelect]->bm_Buffer,
                                  whichPict,
      ...
      if(gBouncing)
        {
          if(gLightOn)
            {
              if(DrawCelsChecked(
                                 currentBitmapItem, gBallSCcb, "bouncing light-on cel chain")
                 < 0)
                goto DONE;
            }
          else /* No light */
            {
              if(DrawCelsChecked(
                                 currentBitmapItem, gBallCcb, "bouncing light-off cel chain")
                 < 0)
                goto DONE;
            }
        }
      else /* Not bouncing */
        {
          ...
          */
          if(!globeLftDir)
            {
              if(DrawGlobe(currentBitmapItem) < 0)
                goto DONE;
            }
          ...
          if(gLightOn)
            {
              if(DrawCelsChecked(
                                 currentBitmapItem, gBallSCcb, "static light-on cel chain")
                 < 0)
                goto DONE;
            }
          else
            {
              if(DrawCelsChecked(
                                 currentBitmapItem, gBallCcb, "static light-off cel chain")
                 < 0)
                goto DONE;
            }
...
/*
  ...
  - X button means quit the program
  - A button means start bouncing if not already bouncing, otherwise
...
*/
{
  ...
  if(status < 0)
    {
      PrintSysError("DoControlPad", NULL, status);
      retValue = -1;
      goto DONE;
    }
  ...
  if(button & CONTROL_ARROWS)
    {
      if(button & ControlUp)
        {
          if(--gGlobeYPos < MIN_YPOS)
            gGlobeYPos = MIN_YPOS;
          if(++gTvYPos > MAX_YPOS)
            gTvYPos = MAX_YPOS;
          if(--gCubeYPos < MIN_YPOS)
            gCubeYPos = MIN_YPOS;
        }
      else if(button & ControlDown)
        {
          if(++gGlobeYPos > MAX_YPOS)
            gGlobeYPos = MAX_YPOS;
          if(--gTvYPos < MIN_YPOS)
            gTvYPos = MIN_YPOS;
          if(++gCubeYPos > MAX_YPOS)
            gCubeYPos = MAX_YPOS;
        }

      if(button & ControlLeft)
        {
          if(--gGlobeXPos < MIN_XPOS)
            gGlobeXPos = MIN_XPOS;
          if(--gBallXPos < MIN_XPOS)
            gBallXPos = MIN_XPOS;
          if(++gTvXPos > MAX_XPOS)
            gTvXPos = MAX_XPOS;
          if(++gCubeXPos > MAX_XPOS)
            gCubeXPos = MAX_XPOS;
        }
      else if(button & ControlRight)
        {
          if(++gGlobeXPos > MAX_XPOS)
            gGlobeXPos = MAX_XPOS;
          if(++gBallXPos > MAX_XPOS)
            gBallXPos = MAX_XPOS;
          if(--gTvXPos < MIN_XPOS)
            gTvXPos = MIN_XPOS;
          if(--gCubeXPos < MIN_XPOS)
            gCubeXPos = MIN_XPOS;
        }
    }
...
void
GetArtFilename(char *filename, char *fullPathname)
...
*/
{
  ...
  gWallOffPict
    = (ubyte *)AllocMem((int)(gScreenContext->sc_nFrameByteCount),
  ...
  if(LoadImage(filename, gWallOffPict, (VdlChunk **)NULL, gScreenContext) == 0)
    {
      PrintFailure("LoadImage", filename);
      goto DONE;
    }
  ...
  if((gGlobeAnim = LoadAnim(filename, MEMTYPE_CEL)) == 0)
    {
      PrintFailure("LoadAnim", filename);
      goto DONE;
    }
